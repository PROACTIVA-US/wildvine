...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `wildvine/plugin-sdk` alias is a prefix match.
    alias: [
      {
        find: "wildvine/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `wildvine/plugin-sdk/${subpath}`,
        replacement: path.join(repoRoot, "src", "plugin-sdk", `${subpath}.ts`),
      })),
      {
        find: "wildvine/plugin-sdk",
