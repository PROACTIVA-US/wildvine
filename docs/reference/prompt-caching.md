...
- With Anthropic API-key auth profiles, Wildvine seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Wildvine injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Wildvine exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.wildvine/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$WILDVINE_STATE_DIR/logs/cache-trace.jsonl`
...
- `WILDVINE_CACHE_TRACE=1` enables cache tracing.
- `WILDVINE_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `WILDVINE_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `WILDVINE_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `WILDVINE_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
