...
Wildvine features that can generate provider usage or paid API calls.
...
- `wildvine status --usage` and `wildvine channels list` show provider **usage windows**
...
Wildvine can pick up credentials from:
...
- `wildvine status --usage`
- `wildvine models status --json`
...
`wildvine models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
