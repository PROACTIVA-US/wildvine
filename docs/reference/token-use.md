---
summary: "How Wildvine builds prompt context and reports token usage + costs"
read_when:
...
Wildvine tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Wildvine assembles its own system prompt on every run. It includes:
...
For images, Wildvine downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Wildvine session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `wildvine status --usage` and `wildvine channels list` show
...
`cacheWrite`. If pricing is missing, Wildvine shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Wildvine can
...
Anthropic's 1M context window is currently beta-gated. Wildvine can inject the
...
Wildvine skips the `context-1m-*` beta header because Anthropic currently
