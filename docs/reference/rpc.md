...
Wildvine integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Wildvine owns lifecycle when `channels.signal.autoStart=true`.
...
- Wildvine spawns `imsg rpc` as a child process (legacy iMessage integration).
