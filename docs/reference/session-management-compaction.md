...
This document explains how Wildvine manages sessions end-to-end:
...
Wildvine is designed around a single **Gateway process** that owns session state.
...
Wildvine persists sessions in two layers:
...
- Transcripts: `~/.wildvine/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Wildvine resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Wildvine reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
wildvine sessions cleanup --dry-run
wildvine sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.wildvine/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Wildvine intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Wildvine consumes the events, but Pi decides when to compact).
...
Wildvine also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Wildvine bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Wildvine leaves it alone.
...
- `wildvine status` (CLI)
- `wildvine sessions` / `sessions --json`
...
Wildvine supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Wildvine strips/suppresses this in the delivery layer.

As of `2026.1.10`, Wildvine also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Wildvine uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Wildvine’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `wildvine status`.
