...
This page lists every configuration knob for Wildvine memory search. For
...
`wildvine.json` unless noted otherwise.
...
When `provider` is not set, Wildvine selects the first available:
...
Remote embeddings require an API key. Wildvine resolves from:
...
When sqlite-vec is unavailable, Wildvine falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.wildvine/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
