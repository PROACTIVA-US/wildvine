...
Wildvine can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `wildvine configure --section web` enables the bundled Firecrawl plugin automatically.
...
Wildvine always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
