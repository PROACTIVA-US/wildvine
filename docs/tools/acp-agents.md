---
summary: "Use ACP runtime sessions for Codex, Claude Code, Cursor, Gemini CLI, Wildvine ACP, and other harness agents"
read_when:
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Wildvine run external coding harnesses (for example Pi, Claude Code, Codex, Cursor, Copilot, Wildvine ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses) through an ACP backend plugin.

If you ask Wildvine in plain language to "run this in Codex" or "start Claude Code in a thread", Wildvine should route that request to the ACP runtime (not the native sub-agent runtime). Each ACP session spawn is tracked as a [background task](/automation/tasks).

If you want Codex or Claude Code to connect as an external MCP client directly
to existing Wildvine channel conversations, use [`wildvine mcp serve`](/cli/mcp)
instead of ACP.
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Wildvine-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Wildvine native sub-agent runtime  |
...
- Wildvine keeps owning the channel transport, auth, safety, and delivery.
...
- ACP session: the durable Codex/Claude/Gemini runtime state Wildvine routes to
...
- `/acp spawn codex --thread auto`: Wildvine may create a child thread/topic and bind the ACP session there
...
- Generic current-conversation binds use the shared Wildvine binding store and survive normal gateway restarts.

Notes:

- `--bind here` and `--thread ...` are mutually exclusive on `/acp spawn`.
- On Discord, `--bind here` binds the current channel or thread in place. `spawnAcpSessions` is only required when Wildvine needs to create a child thread for `--thread auto|here`.
- If the active channel does not expose current-conversation ACP bindings, Wildvine returns a clear unsupported message.
...
- Wildvine binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Wildvine returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning Wildvine agent id.
...
- Wildvine ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, Wildvine uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Wildvine may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Wildvine session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Wildvine sandbox.
...
If no target resolves, Wildvine returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Wildvine returns a clear unsupported-control error.
...
When Wildvine uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Wildvine `agentId` path).
...
```bash
wildvine plugins install acpx
wildvine config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
wildvine plugins install ./path/to/local/acpx-plugin
```
...
- Relative paths resolve from Wildvine workspace directory.
...
- Wildvine startup remains non-blocking while the backend health check runs.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install Wildvine globally with `npm install -g wildvine`, the acpx
...
normally and reports the missing dependency through `wildvine acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Wildvine plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Wildvine plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
wildvine config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `wildvine-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Wildvine
  plugins.
...
These ACPX harness permissions are separate from Wildvine exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
wildvine config set plugins.entries.acpx.config.permissionMode approve-all
wildvine config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Wildvine currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
