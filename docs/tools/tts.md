...
Wildvine can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Wildvine can send audio.
...
When `messages.tts.provider` is unset, Wildvine picks the first configured
...
- If `provider` is **unset**, Wildvine uses the first configured speech provider in registry auto-select order.
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Wildvine retries with MP3.
