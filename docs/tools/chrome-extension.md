---
summary: "Chrome extension: let Wildvine drive your existing Chrome tab"
read_when:
...
The Wildvine Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate wildvine-managed Chrome profile.
...
Wildvine then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Wildvine release (npm package) as static files. There is no separate “build” step.

After upgrading Wildvine:

- Re-run `wildvine browser extension install` to refresh the installed files under your Wildvine state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Wildvine ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Wildvine to control.
...
- `ON`: attached; Wildvine can drive that tab.
...
Debugging: `wildvine sandbox explain`
...
`wildvine browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `wildvine browser extension install` first to copy the extension to a stable location under your Wildvine state directory.
...
- **This is not isolated** like the dedicated wildvine-managed profile.
