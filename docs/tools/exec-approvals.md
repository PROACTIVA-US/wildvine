...
`ask: "always"` in `~/.wildvine/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `wildvine approvals get`, `wildvine approvals get --gateway`, or
`wildvine approvals get --node <id|name|ip>` to inspect the requested policy,
...
- **gateway host** → `wildvine` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Wildvine also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.wildvine/exec-approvals.sock",
...
When `tools.exec.strictInlineEval=true`, Wildvine treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.wildvine/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `wildvine approvals allowlist ...`).
- `wildvine security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `wildvine doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Wildvine still rejects the `env` builtin in safe-bin
...
`~/.wildvine/exec-approvals.json` directly.

CLI: `wildvine approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
- If Wildvine cannot identify exactly one concrete local file for an interpreter/runtime command
...
After an approved async exec finishes, Wildvine sends a followup `agent` turn to the same session.
...
topics, Wildvine preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Wildvine prevents the agent from reusing
