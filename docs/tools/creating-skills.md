...
<Steps>
  ...
  <Step title="Load the skill">
    Start a new session so Wildvine picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    wildvine agent --message "give me a greeting"
    ```
...
| `metadata.wildvine.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.wildvine.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.wildvine.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `wildvine agent --message "..."` to test before sharing
...
| `~/.wildvine/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Wildvine) | Lowest     | Global                |
