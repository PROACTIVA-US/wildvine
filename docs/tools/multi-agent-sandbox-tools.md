...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `wildvine sandbox explain`.
...
Legacy `agent.*` configs are migrated by `wildvine doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=wildvine-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${WILDVINE_STATE_DIR:-$HOME/.wildvine}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
