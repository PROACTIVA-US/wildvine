---
summary: "Wildvine tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools Wildvine provides
...
Wildvine has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). Wildvine ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    Some plugins are **core** (shipped with Wildvine), others are **external**
...
These tools ship with Wildvine and are available without installing any plugins:
...
- [Vine](/tools/vine) — typed workflow runtime with resumable approvals
...
| `group:nodes`      | nodes                                                                                                     |
| `group:wildvine`   | All built-in Wildvine tools (excludes plugin tools)                                                       |
