---
summary: "Agent tool surface for Wildvine (browser, canvas, nodes, message, cron) replacing legacy `wildvine-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `wildvine-*` skills
title: "Tools"
---

# Tools (Wildvine)

Wildvine exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `wildvine-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `wildvine.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Wildvine logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:wildvine`: all built-in Wildvine tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`wildvine node run`).
...
- Requires a Brave API key (recommended: `wildvine configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Wildvine-managed browser.
...
- Quick smoke: `wildvine nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `wildvine gateway` restart in-place)
