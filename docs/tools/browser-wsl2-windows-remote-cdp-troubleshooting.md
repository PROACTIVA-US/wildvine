...
read_when:
  - Running Wildvine Gateway in WSL2 while Chrome lives on Windows
...
- Wildvine Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/PROACTIVA-US/wildvine/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Wildvine and Chrome are on the same machine
...
- Wildvine points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Wildvine is not the problem yet.
...
Fix that before touching Wildvine config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Wildvine at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Wildvine to succeed
...
```bash
wildvine browser open https://example.com --browser-profile remote
wildvine browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `wildvine browser tabs` returns the target
...
3. Wildvine config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Wildvine config or Control UI auth
