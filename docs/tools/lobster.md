...
description: Typed workflow runtime for Wildvine — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Wildvine run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Wildvine runs one Lobster tool call and gets a structured result.
...
Wildvine launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Wildvine tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Wildvine Gateway (see the [Lobster repo](https://github.com/wildvine/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Wildvine keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Wildvine tools that do.
