...
All skills-related configuration lives under `skills` in `~/.wildvine/wildvine.json`.
...
- Built-in skill roots always include `~/.wildvine/skills`, `~/.agents/skills`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.wildvine.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.wildvine/skills` → bundled skills →
