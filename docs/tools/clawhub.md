---
summary: "ClawHub guide: public registry, native Wildvine install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Wildvine skills and plugins**.

- Use native `wildvine` commands to search/install/update skills and install
...
```bash
wildvine plugins install clawhub:<package>
...
Native `wildvine` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for Wildvine skills and plugins.
...
If you want to add new capabilities to your Wildvine agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `wildvine skills install <skill-slug>`
3. Start a new Wildvine session so it picks up the new skill.
...
directory. `wildvine plugins install clawhub:...` records a normal managed
...
current working directory. If an Wildvine workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Wildvine loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.wildvine/skills` or bundled skills, workspace skills take precedence.
...
Code plugins must include the required Wildvine metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/wildvine/skills` and `~/.wildvine/skills`). This is designed to find older skill installs without extra flags.
