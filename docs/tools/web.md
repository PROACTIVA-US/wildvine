...
Wildvine also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    wildvine configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Wildvine plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of Wildvine's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, Wildvine keeps the normal managed `web_search` behavior.
...
If no `provider` is set, Wildvine checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  Wildvine resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
When you choose Grok during `wildvine onboard` or `wildvine configure --section web`,
Wildvine can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, Wildvine does not
...
<Tabs>
  <Tab title="Config file">
    Run `wildvine configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.wildvine/.env`.
...
optional structured filters. Wildvine only enables the built-in xAI `x_search`
