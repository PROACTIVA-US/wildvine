...
Wildvine supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Wildvine also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `wildvine configure --section web`. It stores the key in
`~/.wildvine/wildvine.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.wildvine/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
