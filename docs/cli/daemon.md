---
summary: "CLI reference for `wildvine daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `wildvine daemon ...` in scripts
...
`wildvine daemon ...` maps to the same service control surface as `wildvine gateway ...` service commands.

## Usage

```bash
wildvine daemon status
wildvine daemon install
wildvine daemon start
wildvine daemon stop
wildvine daemon restart
wildvine daemon uninstall
```
...
Use [`wildvine gateway`](/cli/gateway) for current docs and examples.
