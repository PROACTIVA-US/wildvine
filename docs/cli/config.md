---
summary: "CLI reference for `wildvine config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `wildvine.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `wildvine configure`).

## Examples

```bash
wildvine config file
wildvine config schema
...
wildvine config set secrets.providers.vaultfile --provider-source file --provider-path /etc/wildvine/secrets.json --provider-mode json
...
wildvine config validate
wildvine config validate --json
```

### `config schema`

Print the generated JSON schema for `wildvine.json` to stdout as plain text.
...
```bash
wildvine config schema > wildvine.schema.json
```
...
wildvine config set gateway.port 19001 --strict-json
wildvine config set channels.whatsapp.groups '["*"]' --strict-json
```

## `config set` modes

`wildvine config set` supports four assignment styles:

1. Value mode: `wildvine config set <path> <value>`
2. SecretRef builder mode:

```bash
wildvine config set channels.discord.token \
...
```bash
wildvine config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/wildvine-vault \
  --provider-arg read \
...
```bash
wildvine config set --batch-json '[
  {
...
```bash
wildvine config set --batch-file ./config-set.batch.json --dry-run
```
...
wildvine config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/wildvine/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `wildvine.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.wildvine/wildvine.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
...
- `config file`: Print the active config file path (resolved from `WILDVINE_CONFIG_PATH` or default location).
