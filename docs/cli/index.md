---
summary: "Wildvine CLI reference for `wildvine` commands, subcommands, and options"
read_when:
...
- [`vinebot`](/cli/vinebot) (legacy alias namespace)
- [`voicecall`](/cli/voicecall) (plugin; if installed)

## Global flags

- `--dev`: isolate state under `~/.wildvine-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.wildvine-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `wildvine update` (source installs only).
...
Wildvine uses a wildvine palette for CLI output.
...
Palette source of truth: `src/terminal/palette.ts` (the “wildvine palette”).

## Command tree

```
wildvine [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `wildvine voicecall`).

## Security

- `wildvine security audit` — audit config + local state for common security foot-guns.
- `wildvine security audit --deep` — best-effort live Gateway probe.
- `wildvine security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `wildvine secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `wildvine secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `wildvine secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `wildvine secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `wildvine plugins list` — discover plugins (use `--json` for machine output).
- `wildvine plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `wildvine plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `wildvine plugins marketplace list <marketplace>` — list marketplace entries before install.
- `wildvine plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `wildvine plugins doctor` — report plugin load errors.
...
- `wildvine memory status` — show index stats.
- `wildvine memory index` — reindex memory files.
- `wildvine memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.wildvine/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `wildvine config` with no
...
- `config set --dry-run`: validate assignments without writing `wildvine.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `wildvine.json`.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `wildvine health` or `wildvine status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `wildvine doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Wildvine moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
```bash
wildvine channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
wildvine channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
wildvine channels remove --channel discord --account work --delete
wildvine channels status --probe
...
Tip: use `wildvine skills search`, `wildvine skills install`, and `wildvine skills update` for ClawHub-backed skills.
...
- `wildvine message send --target +15555550123 --message "Hi"`
- `wildvine message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Wildvine may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Wildvine can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `wildvine status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Wildvine services are treated as first-class and aren't flagged as "extra".
...
wildvine logs --limit 200
wildvine logs --plain
wildvine logs --json
wildvine logs --no-color
```
...
```bash
claude setup-token
wildvine models auth setup-token --provider anthropic
...
```bash
wildvine models auth login --provider anthropic --method cli --set-default
```
...
`wildvine models` is an alias for `models status`.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `WILDVINE_GATEWAY_TOKEN` / `WILDVINE_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `WILDVINE_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`wildvine browser`](/cli/browser) and the [Browser tool](/tools/browser).
