---
summary: "CLI reference for `wildvine secrets` (reload, audit, configure, apply)"
read_when:
...
Use `wildvine secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
Scan Wildvine state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `wildvine.json` refs)
...
```bash
wildvine secrets audit
wildvine secrets audit --check
wildvine secrets audit --json
wildvine secrets audit --allow-exec
```
...
```bash
wildvine secrets configure
wildvine secrets configure --plan-out /tmp/wildvine-secrets-plan.json
wildvine secrets configure --apply --yes
wildvine secrets configure --providers-only
wildvine secrets configure --skip-provider-setup
wildvine secrets configure --agent ops
wildvine secrets configure --json
```
...
- `configure` targets secret-bearing fields in `wildvine.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Wildvine fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
wildvine secrets apply --from /tmp/wildvine-secrets-plan.json --json
```
...
- `wildvine.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.wildvine/.env` known secret keys whose values were migrated
