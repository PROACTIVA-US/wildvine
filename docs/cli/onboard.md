---
summary: "CLI reference for `wildvine onboard` (interactive onboarding)"
read_when:
...
```bash
wildvine onboard
wildvine onboard --flow quickstart
wildvine onboard --flow manual
wildvine onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`WILDVINE_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `wildvine gateway run`.
...
- Fastest first chat: `wildvine dashboard` (Control UI, no channel setup).
