---
summary: "CLI reference for `wildvine message` (send + channel actions)"
read_when:
...
```
wildvine message <subcommand> [flags]
```
...
- On cache miss, Wildvine will attempt a live directory lookup when the provider supports it.
...
- `wildvine message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Wildvine does not force a `default` account SecretRef scope
...
```
wildvine message send --channel discord \
...
```
wildvine message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
wildvine message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
wildvine message poll --channel msteams \
...
```
wildvine message react --channel slack \
...
```
wildvine message react --channel signal \
...
```
wildvine message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
wildvine message send --channel telegram --target @mychat \
