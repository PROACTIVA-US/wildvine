...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Wildvine Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`wildvine acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Wildvine channel
conversations instead of hosting an ACP harness session, use
[`wildvine mcp serve`](/cli/mcp) instead.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Wildvine gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
wildvine acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
wildvine acp --url wss://gateway-host:18789 --token-file ~/.wildvine/gateway.token

# Attach to an existing session key
wildvine acp --session agent:main:main

# Attach by label (must already exist)
wildvine acp --session-label "support inbox"

# Reset the session key before the first prompt
wildvine acp --session agent:main:main --reset-session
```
...
wildvine acp client --server-args --url wss://gateway-host:18789 --token-file ~/.wildvine/gateway.token

# Override the server command (default: wildvine)
wildvine acp client --server "node" --server-args wildvine.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Wildvine through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Wildvine Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `wildvine acp` over stdio.

Example config (persisted):

```bash
wildvine config set gateway.remote.url wss://gateway-host:18789
wildvine config set gateway.remote.token <token>
```
...
```bash
wildvine acp --session agent:main:main
wildvine acp --session agent:design:main
wildvine acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Wildvine plugin tools, enable the
...
Wildvine bot over ACP, use `acpx` with its built-in `wildvine` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx wildvine` at `wildvine acp`.
3. Target the Wildvine session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Wildvine ACP session
acpx wildvine exec "Summarize the active Wildvine session state."

# Persistent named session for follow-up turns
acpx wildvine sessions ensure --name codex-bridge
acpx wildvine -s codex-bridge --cwd /path/to/repo \
  "Ask my Wildvine work agent for recent context relevant to this repo."
```

If you want `acpx wildvine` to target a specific Gateway and session key every
time, override the `wildvine` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "wildvine": {
      "command": "env WILDVINE_HIDE_BANNER=1 WILDVINE_SUPPRESS_NOTES=1 wildvine acp --url ws://127.0.0.1:18789 --token-file ~/.wildvine/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Wildvine checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env WILDVINE_HIDE_BANNER=1 WILDVINE_SUPPRESS_NOTES=1 node wildvine.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Wildvine agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Wildvine ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`WILDVINE_GATEWAY_TOKEN`, `WILDVINE_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`WILDVINE_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `WILDVINE_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `wildvine acp client` sets `WILDVINE_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `wildvine`).
