---
summary: "Expose Wildvine channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Wildvine-backed channels
  - Running `wildvine mcp serve`
  - Managing Wildvine-saved MCP server definitions
title: "mcp"
---

# mcp

`wildvine mcp` has two jobs:

- run Wildvine as an MCP server with `wildvine mcp serve`
- manage Wildvine-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Wildvine acting as an MCP server
- `list` / `show` / `set` / `unset` is Wildvine acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`wildvine acp`](/cli/acp) when Wildvine should host a coding harness
session itself and route that runtime through ACP.

## Wildvine as an MCP server

This is the `wildvine mcp serve` path.

## When to use `serve`

Use `wildvine mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Wildvine-backed channel conversations
- you already have a local or remote Wildvine Gateway with routed sessions
- you want one MCP server that works across Wildvine's channel backends instead
  of running separate per-channel bridges

Use [`wildvine acp`](/cli/acp) instead when Wildvine should host the coding
runtime itself and keep the agent session inside Wildvine.

## How it works

`wildvine mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Wildvine Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `wildvine mcp serve`
...
conversations. A conversation appears when Wildvine already has session state
...
# Remote Gateway
wildvine mcp serve --url wss://gateway-host:18789 --token-file ~/.wildvine/gateway.token

# Remote Gateway with password auth
wildvine mcp serve --url wss://gateway-host:18789 --password-file ~/.wildvine/gateway.password

# Enable verbose bridge logs
wildvine mcp serve --verbose

# Disable Claude-specific push notifications
wildvine mcp serve --claude-channel-mode off
```
...
Wildvine equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Wildvine channel configuration
...
Wildvine ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `wildvine mcp serve`
...
## Wildvine as an MCP client registry

This is the `wildvine mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Wildvine over MCP. They manage Wildvine-owned MCP
server definitions under `mcp.servers` in Wildvine config.

Those saved definitions are for runtimes that Wildvine launches or configures
later, such as embedded Pi and other runtime adapters. Wildvine stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Wildvine config
...
Wildvine also stores a lightweight MCP server registry in config for surfaces
that want Wildvine-managed MCP definitions.

Commands:

- `wildvine mcp list`
- `wildvine mcp show [name]`
- `wildvine mcp set <name> <json>`
- `wildvine mcp unset <name>`

Examples:

```bash
wildvine mcp list
wildvine mcp show context7 --json
wildvine mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
wildvine mcp set docs '{"url":"https://mcp.example.com"}'
wildvine mcp unset context7
```
