---
summary: "CLI reference for `wildvine configure` (interactive configuration prompts)"
read_when:
...
Tip: `wildvine config` without a subcommand opens the same wizard. Use
`wildvine config get|set|unset` for non-interactive edits.

For web search, `wildvine configure --section web` lets you choose a provider
...
wildvine configure --section model --section channels
```
