...
- Publish as `wildvine/plugin-sdk` (or export from core under `wildvine/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `WildvinePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: WildvineConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: WildvineConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: WildvineConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: WildvineConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: WildvineConfig): string;
  };
};
```
...
- Introduce `wildvine/plugin-sdk`.
- Add `api.runtime` to `WildvinePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `wildvineRuntime: ">=2026.2.0"`).
