...
summary: "Architecture of Wildvine's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Wildvine
...
Image injection is prompt-local: Wildvine loads image refs from the current prompt and
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `WILDVINE_LIVE_TEST=1`)
