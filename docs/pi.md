...
This document describes how Wildvine integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Wildvine uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Wildvine directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Wildvine's local TUI mode)                                             |
...
├── pi-tools.ts                    # createWildvineCodingTools()
...
├── wildvine-tools.ts              # Wildvine-specific tools
...
2. **Custom Replacements**: Wildvine replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Wildvine Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Wildvine's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Wildvine CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Wildvine wraps this with `guardSessionManager()` for tool result safety.
...
Wildvine maintains an auth profile store with multiple API keys per provider:
...
Wildvine loads custom pi extensions for specialized behavior:
...
Wildvine also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Wildvine Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Wildvine tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.wildvine/agents/<agentId>/sessions/` (or `$WILDVINE_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-wildvine-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-wildvine-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-wildvine-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-wildvine-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
