...
service via the external `wildvine` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.wildvine.gateway`
  (or `ai.wildvine.<profile>` when using `--profile`/`WILDVINE_PROFILE`; legacy `com.wildvine.*` is supported).
