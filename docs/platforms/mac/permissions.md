...
- Same path: run the app from a fixed location (for Wildvine, `dist/Wildvine.app`).
...
```bash
sudo tccutil reset Accessibility ai.wildvine.mac
sudo tccutil reset ScreenCapture ai.wildvine.mac
...
Workaround: move files into the Wildvine workspace (`~/.wildvine/workspace`) if you want to avoid per-folder grants.
