...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `wildvine.internal.`), **or**
...
Run `wildvine devices list` again before approval.

4. Verify connection:

```bash
wildvine nodes status
wildvine gateway call node.list --params "{}"
```
...
- `WILDVINE_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Wildvine iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Wildvine builds.
...
```bash
export WILDVINE_APNS_TEAM_ID="TEAMID"
export WILDVINE_APNS_KEY_ID="KEYID"
export WILDVINE_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_wildvine-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `wildvine.internal.`) and Tailscale split DNS.
...
```bash
wildvine nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__wildvine__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__wildvine__/canvas/` and `/__wildvine__/a2ui/`.
...
```bash
wildvine nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__wildvine; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
wildvine nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `wildvine devices list` and approve manually.
