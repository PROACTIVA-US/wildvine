...
> **Note:** The Android app has not been publicly released yet. The source code is available in the [Wildvine repository](https://github.com/PROACTIVA-US/wildvine) under `apps/android`. You can build it yourself using Java 17 and the Android SDK (`./gradlew :app:assemblePlayDebug`). See [apps/android/README.md](https://github.com/PROACTIVA-US/wildvine/blob/main/apps/android/README.md) for build instructions.
...
- You can run the CLI (`wildvine`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.wildvine/wildvine.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `wildvine.internal.`) on the gateway host and publish `_wildvine-gw._tcp` records.
...
1. Create `~/.wildvine/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
wildvine nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__wildvine__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__wildvine__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__wildvine__/a2ui/`.
