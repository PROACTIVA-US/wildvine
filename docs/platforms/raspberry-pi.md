---
summary: "Wildvine on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Wildvine on a Raspberry Pi
  - Running Wildvine on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Wildvine on Raspberry Pi

## Goal

Run a persistent, always-on Wildvine Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u wildvine -f
```

## 9) Access the Wildvine Dashboard
...
```bash
ssh user@gateway-host 'wildvine dashboard --no-open'
```
...
(or `WILDVINE_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/wildvine-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/wildvine-compile-cache
mkdir -p /var/tmp/wildvine-compile-cache
export WILDVINE_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `WILDVINE_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Wildvine, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit wildvine
```

```ini
[Service]
Environment=WILDVINE_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/wildvine-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Wildvine state/cache on SSD-backed storage to avoid SD-card
...
Most Wildvine features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled wildvine

# Enable if not
sudo systemctl enable wildvine

# Start on boot
sudo systemctl start wildvine
```
...
```bash
# Check logs
journalctl -u wildvine --no-pager -n 100

# Common fix: rebuild
cd ~/wildvine  # if using hackable install
npm run build
sudo systemctl restart wildvine
```
