...
你可以在 `~/.wildvine/wildvine.json` 中覆盖此设置：
...
所有日志配置都在 `~/.wildvine/wildvine.json` 的 `logging` 下。
...
- Wildvine 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `wildvine plugins enable diagnostics-otel` 启用插件。
...
- `wildvine.tokens`（计数器，属性：`wildvine.token`、`wildvine.channel`、`wildvine.provider`、`wildvine.model`）
- `wildvine.cost.usd`（计数器，属性：`wildvine.channel`、`wildvine.provider`、`wildvine.model`）
- `wildvine.run.duration_ms`（直方图，属性：`wildvine.channel`、`wildvine.provider`、`wildvine.model`）
- `wildvine.context.tokens`（直方图，属性：`wildvine.context`、`wildvine.channel`、`wildvine.provider`、`wildvine.model`）

消息流：

- `wildvine.webhook.received`（计数器，属性：`wildvine.channel`、`wildvine.webhook`）
- `wildvine.webhook.error`（计数器，属性：`wildvine.channel`、`wildvine.webhook`）
- `wildvine.webhook.duration_ms`（直方图，属性：`wildvine.channel`、`wildvine.webhook`）
- `wildvine.message.queued`（计数器，属性：`wildvine.channel`、`wildvine.source`）
- `wildvine.message.processed`（计数器，属性：`wildvine.channel`、`wildvine.outcome`）
- `wildvine.message.duration_ms`（直方图，属性：`wildvine.channel`、`wildvine.outcome`）

队列 + 会话：

- `wildvine.queue.lane.enqueue`（计数器，属性：`wildvine.lane`）
- `wildvine.queue.lane.dequeue`（计数器，属性：`wildvine.lane`）
- `wildvine.queue.depth`（直方图，属性：`wildvine.lane` 或 `wildvine.channel=heartbeat`）
- `wildvine.queue.wait_ms`（直方图，属性：`wildvine.lane`）
- `wildvine.session.state`（计数器，属性：`wildvine.state`、`wildvine.reason`）
- `wildvine.session.stuck`（计数器，属性：`wildvine.state`）
- `wildvine.session.stuck_age_ms`（直方图，属性：`wildvine.state`）
- `wildvine.run.attempt`（计数器，属性：`wildvine.attempt`）
...
- `wildvine.webhook.error`
  - `wildvine.channel`、`wildvine.webhook`、`wildvine.chatId`、`wildvine.error`
- `wildvine.message.processed`
  - `wildvine.channel`、`wildvine.outcome`、`wildvine.chatId`、`wildvine.messageId`、`wildvine.sessionKey`、`wildvine.sessionId`、`wildvine.reason`
- `wildvine.session.stuck`
  - `wildvine.state`、`wildvine.ageMs`、`wildvine.queueDepth`、`wildvine.sessionKey`、`wildvine.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `wildvine doctor`。
