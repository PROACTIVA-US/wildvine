...
read_when:
  - 启动新的 Wildvine 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 Wildvine 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — Wildvine 个人助手（默认）

## 首次运行（推荐）

Wildvine 为智能体使用专用的工作区目录。默认：`~/.wildvine/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
如果你将此工作区视为 Wildvine 的"记忆"，请将其设为 git 仓库（最好是私有的），这样 `AGENTS.md` 和你的记忆文件就会被备份。
...
- macOS 应用管理权限（屏幕录制、通知、麦克风）并通过其内置二进制文件暴露 `wildvine` CLI。
...
- 脚本编写优先使用 `wildvine` CLI；mac 应用处理权限。
...
- 对于浏览器驱动的验证，使用带有 Wildvine 管理的 Chrome 配置文件的 `wildvine browser`（tabs/status/screenshot）。
- 对于 DOM 检查，使用 `wildvine browser eval|query|dom|snapshot`（需要机器输出时使用 `--json`/`--out`）。
- 对于交互，使用 `wildvine browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要 snapshot 引用；CSS 选择器使用 `evaluate`）。
