...
- `~/Library/Application Support/Wildvine/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `wildvine-canvas://<session>/<path>`

示例：

- `wildvine-canvas://main/` → `<canvasRoot>/main/index.html`
- `wildvine-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `wildvine-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
wildvine nodes canvas present --node <id>
wildvine nodes canvas navigate --node <id> --url "/"
wildvine nodes canvas eval --node <id> --js "document.title"
wildvine nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__wildvine__/a2ui/
```
...
wildvine nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
wildvine nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "wildvine://agent?message=Review%20this%20design";
```
