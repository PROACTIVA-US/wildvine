---
read_when:
  - 你想在 Wildvine 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Wildvine 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
wildvine plugins enable minimax-portal-auth  # 如果已加载则跳过
wildvine gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/wildvine/wildvine/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `wildvine models list` 和 `wildvine models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `wildvine configure` 并选择 **MiniMax M2.1**，或
