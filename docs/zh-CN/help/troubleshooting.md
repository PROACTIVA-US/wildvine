...
你也可以设置 `WILDVINE_VERBOSE=1` 代替标志。
...
### `docs.wildvine.bot` 显示 SSL 错误（Comcast/Xfinity）

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 阻止 `docs.wildvine.bot`。
禁用 Advanced Security 或将 `docs.wildvine.bot` 添加到允许列表，然后重试。
...
- 检查允许列表：`wildvine config get agents.defaults.models`
...
如果可以的话，包含来自 `wildvine logs --follow` 的相关日志尾部。
