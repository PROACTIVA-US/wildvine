...
**Import:** `wildvine/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `WildvinePluginConfigSchema \| () => WildvinePluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: WildvinePluginApi) => void`                               | Yes      | —                   |

- `id` must match your `wildvine.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `wildvine/plugin-sdk/core`
...
| `registerCliMetadata` | `(api: WildvinePluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: WildvinePluginApi) => void`                               | No       | —                   |
...
Wildvine loads this instead of the full entry when a channel is disabled,
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Wildvine to lazy-load the real CLI module on first invocation
...
Wildvine classifies loaded plugins by their registration behavior:
...
Use `wildvine plugins inspect <id>` to see a plugin's shape.
