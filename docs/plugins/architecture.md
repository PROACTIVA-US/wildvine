...
read_when:
  - Building or debugging native Wildvine plugins
...
This page covers the internal architecture of the Wildvine plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Wildvine. Every
native Wildvine plugin registers against one or more capability types:
...
Wildvine classifies every loaded plugin into a shape based on its actual
...
Use `wildvine plugins inspect <id>` to see a plugin's shape and capability
...
When you run `wildvine doctor` or `wildvine plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `wildvine status --all` and `wildvine plugins doctor`.

## Architecture overview

Wildvine's plugin system has four layers:

1. **Manifest + discovery**
   Wildvine finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `wildvine.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Wildvine plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Wildvine reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Wildvine
...
That split lets Wildvine validate config, explain missing/disabled plugins, and
...
normal chat actions. Wildvine keeps one shared `message` tool in core, and
...
Wildvine treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Wildvine-facing
  surfaces
...
So if Wildvine adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Wildvine has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { WildvinePluginDefinition } from "wildvine/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "wildvine/plugin-sdk/media-understanding";

const plugin: WildvinePluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Wildvine already treats image/audio/video understanding as one shared
...
If Wildvine adds a new domain later, such as video generation, use the same
...
`WildvinePluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Wildvine can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Wildvine knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `WildvinePluginApi` or
  `api.runtime`
...
Native Wildvine plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Wildvine process

Compatible bundles are safer by default because Wildvine currently treats them
...
name: `@wildvine/<id>` by default, or an approved typed suffix such as
...
Wildvine exports capabilities, not implementation convenience.
...
At startup, Wildvine does roughly this:
...
The manifest is the control-plane source of truth. Wildvine uses it to:
...
Wildvine keeps short in-process caches for:
...
- Set `WILDVINE_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `WILDVINE_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `WILDVINE_PLUGIN_DISCOVERY_CACHE_MS` and
  `WILDVINE_PLUGIN_MANIFEST_CACHE_MS`.
...
Wildvine still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Wildvine calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Wildvine tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Wildvine's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Wildvine's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Wildvine adds those
...
- If Wildvine adds a new capability such as video generation later, define the
...
- Wildvine only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Wildvine plugin runtime helpers",
    count: 5,
  },
});
```
...
const result = await api.runtime.imageGeneration.generate({
  config: api.config,
  args: { prompt: "A friendly vine mascot", size: "1024x1024" },
});
...
Use SDK subpaths instead of the monolithic `wildvine/plugin-sdk` import when
authoring plugins:

- `wildvine/plugin-sdk/plugin-entry` for plugin registration primitives.
- `wildvine/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `wildvine/plugin-sdk/channel-setup`,
  `wildvine/plugin-sdk/channel-pairing`,
  `wildvine/plugin-sdk/channel-contract`,
  `wildvine/plugin-sdk/channel-feedback`,
  `wildvine/plugin-sdk/channel-inbound`,
  `wildvine/plugin-sdk/channel-lifecycle`,
  `wildvine/plugin-sdk/channel-reply-pipeline`,
  `wildvine/plugin-sdk/command-auth`,
  `wildvine/plugin-sdk/secret-input`, and
  `wildvine/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `wildvine/plugin-sdk/channel-config-helpers`,
  `wildvine/plugin-sdk/allow-from`,
  `wildvine/plugin-sdk/channel-config-schema`,
  `wildvine/plugin-sdk/channel-policy`,
  `wildvine/plugin-sdk/approval-runtime`,
  `wildvine/plugin-sdk/config-runtime`,
  `wildvine/plugin-sdk/infra-runtime`,
  `wildvine/plugin-sdk/agent-runtime`,
  `wildvine/plugin-sdk/lazy-runtime`,
  `wildvine/plugin-sdk/reply-history`,
  `wildvine/plugin-sdk/routing`,
  `wildvine/plugin-sdk/status-helpers`,
  `wildvine/plugin-sdk/runtime-store`, and
  `wildvine/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `wildvine/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `wildvine/plugin-sdk/*` subpaths. Wildvine core/test code may use the repo
...
- Avoid the root `wildvine/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `wildvine/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `wildvine/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `wildvine/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `wildvine/plugin-sdk/channel-targets`; channel-specific
...
`wildvine/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Wildvine writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Wildvine's
...
- if both `catalog` and `discovery` are registered, Wildvine uses `catalog`
...
- Read-only command paths such as `wildvine status`, `wildvine status --all`,
  `wildvine channels status`, `wildvine channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `wildvine.extensions`:
...
Security guardrail: every `wildvine.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `wildvine plugins install` installs plugin dependencies with
...
Optional: `wildvine.setupEntry` can point at a lightweight setup-only module.
When Wildvine needs setup surfaces for a disabled channel plugin, or
...
Optional: `wildvine.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Wildvine load the
...
Channel plugins can advertise setup/discovery metadata via `wildvine.channel` and
install hints via `wildvine.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@wildvine/nextcloud-talk",
  "wildvine": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@wildvine/nextcloud-talk",
...
Wildvine can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.wildvine/mpm/plugins.json`
- `~/.wildvine/mpm/catalog.json`
- `~/.wildvine/plugins/catalog.json`

Or point `WILDVINE_PLUGIN_CATALOG_PATHS` (or `WILDVINE_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "wildvine": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { delegateCompactionToRuntime } from "wildvine/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `WildvinePluginApi` and/or `api.runtime` with the smallest useful
...
This is how Wildvine stays opinionated without becoming hardcoded to one
