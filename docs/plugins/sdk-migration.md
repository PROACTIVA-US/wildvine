...
read_when:
  - You see the WILDVINE_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the WILDVINE_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Wildvine plugin
---

# Plugin SDK Migration

Wildvine has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`wildvine/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`wildvine/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`wildvine/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "wildvine/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "wildvine/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "wildvine/extension-api";
...
```bash
WILDVINE_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 wildvine gateway run
WILDVINE_SUPPRESS_EXTENSION_API_WARNING=1 wildvine gateway run
```
