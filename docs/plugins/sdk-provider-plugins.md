...
summary: "Step-by-step guide to building a model provider plugin for Wildvine"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Wildvine
...
(LLM) to Wildvine. By the end you will have a provider with a model catalog,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so Wildvine can detect
    credentials without loading your plugin runtime. If you publish the
    provider on ClawHub, those `wildvine.compat` and `wildvine.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "wildvine/plugin-sdk/provider-auth";
    ...
    `wildvine onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "wildvine/plugin-sdk/provider-entry";
    ...
    `wildvine/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      Wildvine calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Wildvine classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # wildvine.providers metadata
├── wildvine.plugin.json      # Manifest with providerAuthEnvVars
