...
summary: "Step-by-step guide to building a messaging channel plugin for Wildvine"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Wildvine to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Wildvine to a
...
<Info>
  If you have not built any Wildvine plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Wildvine keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `wildvine/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization and transport hooks. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, `createApproverRestrictedNativeApprovalCapability`, and `createChannelNativeApprovalRuntime` from `wildvine/plugin-sdk/approval-runtime` so core owns request filtering, routing, dedupe, expiry, and gateway subscription.
...
<Steps>
    ...
    ```json package.json
    {
      ...
      "wildvine": {
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Wildvine to Acme Chat."
        }
      }
    }
    ```

    ```json wildvine.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "wildvine/plugin-sdk/core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Wildvine
  ...
  <Step title="Add a setup entry">
    ...
    Wildvine loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    Wildvine. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Wildvine.
...
├── package.json              # wildvine.channel metadata
├── wildvine.plugin.json      # Manifest with config schema
