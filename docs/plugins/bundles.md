---
summary: "Install and use Codex, Claude, and Cursor bundles as Wildvine plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Wildvine maps bundle content into native features
...
Wildvine can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Wildvine maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Wildvine plugins. Native plugins run
...
of requiring authors to rewrite them as native Wildvine plugins, Wildvine
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    wildvine plugins install ./my-bundle

    # Archive
    wildvine plugins install ./my-bundle.tgz
...
## What Wildvine maps from bundles

Not every bundle feature runs in Wildvine today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Wildvine skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | Wildvine-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal Wildvine skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Wildvine skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Wildvine hook-pack
...
- Wildvine merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Wildvine exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Wildvine uses `sse`
...
Wildvine registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Wildvine sanitizes shell override keys before applying them
...
These are recognized and shown in diagnostics, but Wildvine does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Wildvine best when they use skill roots and Wildvine-style
...
Wildvine checks for native plugin format first:

1. `wildvine.plugin.json` or valid `package.json` with `wildvine.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Wildvine uses the native path. This prevents
...
- Wildvine does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `wildvine plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Wildvine does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Wildvine hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
