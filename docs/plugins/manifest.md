...
read_when:
  - You are building an Wildvine plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (wildvine.plugin.json)

This page is for the **native Wildvine plugin manifest** only.
...
Wildvine auto-detects those bundle layouts too, but they are not validated
against the `wildvine.plugin.json` schema described here.

For compatible bundles, Wildvine currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Wildvine runtime expectations.

Every native Wildvine plugin **must** ship a `wildvine.plugin.json` file in the
**plugin root**. Wildvine uses this manifest to validate configuration
...
`wildvine.plugin.json` is the metadata Wildvine reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Wildvine can inspect without loading plugin code.                                      |
...
Wildvine reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, Wildvine falls back to `choiceId`.                                        |
...
Use `contracts` only for static capability ownership metadata that Wildvine can
...
`imageGenerationProviders` are deprecated. Use `wildvine doctor --fix` to move
...
| File                   | Use it for                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `wildvine.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs           |
| `package.json`         | npm metadata, dependency installation, and the `wildvine` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Wildvine must know it before loading plugin code, put it in `wildvine.plugin.json`
...
- The manifest is **required for native Wildvine plugins**, including local filesystem loads.
