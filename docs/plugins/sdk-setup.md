...
read_when:
  ...
  - You are defining plugin config schemas or package.json wildvine metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`wildvine.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `wildvine` field that tells the plugin system what
...
```json
{
  "name": "@myorg/wildvine-my-channel",
  "version": "1.0.0",
  "type": "module",
  "wildvine": {
    "extensions": ["./index.ts"],
...
```json wildvine-clawhub-package.json
{
  "name": "@myorg/wildvine-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "wildvine": {
    "extensions": ["./index.ts"],
...
When enabled, Wildvine loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `wildvine.plugin.json` in the package root.
Wildvine uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Wildvine",
  "configSchema": {
    "type": "object",
...
Wildvine loads when it only needs setup surfaces (onboarding, config repair,
...
**When Wildvine uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `wildvine/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Wildvine validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "wildvine/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `wildvine onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "wildvine/plugin-sdk/channel-setup";
...
helpers from `wildvine/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`wildvine/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `wildvine/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "wildvine/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/wildvine-my-channel",
...
```bash
wildvine plugins install @myorg/wildvine-my-plugin
```

Wildvine tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
wildvine plugins install clawhub:@myorg/wildvine-my-plugin   # ClawHub only
wildvine plugins install npm:@myorg/wildvine-my-plugin       # npm only
```
...
```bash
wildvine plugins search <query>
wildvine plugins install <package-name>
```

<Info>
  For npm-sourced installs, `wildvine plugins install` runs
