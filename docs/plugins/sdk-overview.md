...
read_when:
  ...
  - You want a reference for all registration methods on WildvinePluginApi
...
The `register(api)` callback receives an `WildvinePluginApi` object with these
methods:
...
- User config still wins. Wildvine merges `agents.defaults.cliBackends.<id>` over the
...
| `api.config`             | `WildvineConfig`          | Current config snapshot                                          |
...
<Warning>
  Never import your own plugin through `wildvine/plugin-sdk/<your-plugin>`
  from production code. Route internal imports through `./api.ts` or
  `./runtime-api.ts`. The SDK path is the external contract only.
</Warning>

<Warning>
  Extension production code should also avoid `wildvine/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `wildvine/plugin-sdk/speech`, `.../provider-model-shared`, or another
