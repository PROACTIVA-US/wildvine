...
summary: "Create your first Wildvine plugin in minutes"
read_when:
  - You want to create a new Wildvine plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to Wildvine
---

# Building Plugins

Plugins extend Wildvine with new capabilities: channels, model providers, speech,
image generation, web search, agent tools, or any combination.

You do not need to add your plugin to the Wildvine repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`wildvine plugins install <package-name>`. Wildvine tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Wildvine to a messaging platform (Discord, IRC, etc.)
  </Card>
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json wildvine.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to Wildvine",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    wildvine plugins install clawhub:@myorg/wildvine-my-plugin
    ```

    Wildvine also checks ClawHub before npm for bare package specs like
    `@myorg/wildvine-my-plugin`.
...
Always import from focused `wildvine/plugin-sdk/<subpath>` paths:
...
import { ... } from "wildvine/plugin-sdk";
```
...
<Check>**package.json** has correct `wildvine` metadata</Check>
<Check>**wildvine.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [wildvine/wildvine](https://github.com/PROACTIVA-US/wildvine/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Wildvine X account [@wildvine](https://x.com/wildvine) for release announcements.
