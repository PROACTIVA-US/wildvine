---
summary: "Research notes: offline memory system for Wildvine workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.wildvine/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Wildvine integration
...
Target: Wildvine-style workspace (`agents.defaults.workspace`, default `~/.wildvine/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.wildvine/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Wildvine to see every session.
...
```
~/.wildvine/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Wildvine**, but keep a separable core library.

### Why integrate into Wildvine?

- Wildvine already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `wildvine memory recall "…" --k 25 --since 30d`
  - `wildvine memory reflect --since 7d`
...
Pragmatic take for `~/.wildvine/workspace`:
