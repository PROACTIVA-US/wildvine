---
summary: "Deploy Wildvine on Render with Infrastructure-as-Code"
read_when:
  - Deploying Wildvine to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Wildvine on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/PROACTIVA-US/wildvine)
...
repository configures everything needed to run Wildvine:
...
services:
  - type: web
    ...
    envVars:
      - key: WILDVINE_GATEWAY_PORT
        value: "8080"
      - key: WILDVINE_STATE_DIR
        value: /data/.wildvine
      - key: WILDVINE_WORKSPACE_DIR
        value: /data/workspace
      - key: WILDVINE_GATEWAY_TOKEN
...
Connect using the `WILDVINE_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original Wildvine repository, Render will not auto-deploy your Wildvine. To update it, run a manual Blueprint sync from the dashboard.
...
For Wildvine, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `WILDVINE_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `WILDVINE_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `wildvine backup create` in the Render shell.
