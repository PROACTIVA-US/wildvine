---
summary: "Host Wildvine on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Wildvine on Oracle Cloud
  - Looking for free VPS hosting for Wildvine
...
Run a persistent Wildvine Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@wildvine`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=wildvine --reset` to re-authenticate.

**Gateway will not start** -- Run `wildvine doctor --non-interactive` and check logs with `journalctl --user -u wildvine-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Wildvine up to date
