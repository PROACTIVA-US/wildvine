...
The fastest way to install. It detects your OS, installs Node if needed, installs Wildvine, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://wildvine.com/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Or skip the link and use `pnpm wildvine ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:wildvine/wildvine#main
```
...
```bash
wildvine --version      # confirm the CLI is available
wildvine doctor         # check for config issues
wildvine gateway status # verify the Gateway is running
```

## Hosting and deployment

Deploy Wildvine on a cloud server or VPS:
...
If the install succeeded but `wildvine` is not found in your terminal:
