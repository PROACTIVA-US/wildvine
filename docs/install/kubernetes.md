---
summary: "Deploy Wildvine Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Wildvine on a Kubernetes cluster
  - You want to test Wildvine in a Kubernetes environment
title: "Kubernetes"
---

# Wildvine on Kubernetes

A minimal starting point for running Wildvine on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Wildvine is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/wildvine 18789:18789 -n wildvine
...
```bash
kubectl get secret wildvine-secrets -n wildvine -o jsonpath='{.data.WILDVINE_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: wildvine (configurable via WILDVINE_NAMESPACE)
├── Deployment/wildvine        # Single pod, init container + gateway
├── Service/wildvine           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/wildvine-config  # wildvine.json + AGENTS.md
└── Secret/wildvine-secrets    # Gateway token + API keys
```
...
Edit `wildvine.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret wildvine-secrets -n wildvine \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/wildvine -n wildvine
```

### Custom namespace

```bash
WILDVINE_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/wildvine/wildvine:latest # or pin to a specific version from https://github.com/PROACTIVA-US/wildvine/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # wildvine.json + AGENTS.md
