...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export WILDVINE_IMAGE="ghcr.io/wildvine/wildvine:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/PROACTIVA-US/wildvine/pkgs/container/wildvine).
  ...
  <Step title="Complete onboarding">
    ...
    `wildvine-gateway` directly. `wildvine-cli` is for commands you run after
...
docker compose run --rm --no-deps --entrypoint node wildvine-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `WILDVINE_EXTRA_MOUNTS`
or `WILDVINE_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `wildvine-cli` shares `wildvine-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d wildvine-gateway`, run onboarding
and setup-time config writes through `wildvine-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `WILDVINE_IMAGE`               | Use a remote image instead of building locally                   |
| `WILDVINE_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `WILDVINE_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `WILDVINE_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `WILDVINE_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `WILDVINE_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `WILDVINE_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `WILDVINE_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `WILDVINE_CONFIG_DIR` to `/home/node/.wildvine` and
`WILDVINE_WORKSPACE_DIR` to `/home/node/.wildvine/workspace`, so those paths
...
and rolling file logs under `/tmp/wildvine/`.
...
If you installed ClawDock from the older `scripts/shell-helpers/vinedock-helpers.sh` raw path, rerun the install command above so your local helper file tracks the new location.

Then use `vinedock-start`, `vinedock-stop`, `vinedock-dashboard`, etc. Run
`vinedock-help` for all commands.
See [ClawDock](/install/vinedock) for the full helper guide.

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export WILDVINE_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export WILDVINE_SANDBOX=1
    export WILDVINE_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm wildvine-cli gateway probe
    docker compose run -T --rm wildvine-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `wildvine-cli` uses `network_mode: "service:wildvine-gateway"` so CLI
    ...
    `no-new-privileges` on `wildvine-cli`.
  </Accordion>
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export WILDVINE_HOME_VOLUME="wildvine_home"`
    2. **Bake system deps**: `export WILDVINE_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `WILDVINE_HOME_VOLUME` or `WILDVINE_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/PROACTIVA-US/wildvine/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Wildvine runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm wildvine-cli config set gateway.mode local
    docker compose run --rm wildvine-cli config set gateway.bind lan
    docker compose run --rm wildvine-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [ClawDock](/install/vinedock) — Docker Compose community setup
- [Updating](/install/updating) — keeping Wildvine up to date
