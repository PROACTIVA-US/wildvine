...
<Steps>
  ...
  <Step title="Clone the Wildvine repository">
    ```bash
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`WILDVINE_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm wildvine-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
