...
Default label is `ai.wildvine.gateway` (or `ai.wildvine.<profile>`; legacy `com.wildvine.*` may still exist):

```bash
launchctl bootout gui/$UID/ai.wildvine.gateway
rm -f ~/Library/LaunchAgents/ai.wildvine.gateway.plist
```

If you used a profile, replace the label and plist name with `ai.wildvine.<profile>`. Remove any legacy `com.wildvine.*` plists if present.
...
If you used `https://wildvine.com/install.sh` or `install.ps1`, the CLI was installed with `npm install -g wildvine@latest`.
