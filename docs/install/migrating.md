---
summary: "Move (migrate) an Wildvine install from one machine to another"
read_when:
...
This guide moves an Wildvine gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.wildvine/` by default) and your **workspace**, you preserve:

- **Config** -- `wildvine.json` and all gateway settings
...
<Tip>
Run `wildvine status` on the old machine to confirm your state directory path.
Custom profiles use `~/.wildvine-<profile>/` or a path set via `WILDVINE_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    If you use multiple profiles (e.g. `~/.wildvine-work`), archive each separately.

  </Step>

  <Step title="Install Wildvine on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.wildvine/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `WILDVINE_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `wildvine doctor`.
  </Accordion>

  <Accordion title="Copying only wildvine.json">
