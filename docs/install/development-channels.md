...
Wildvine ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/wildvine`, override with
  `WILDVINE_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
wildvine update --tag 2026.4.1-beta.1
...
wildvine update --tag wildvine@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `wildvine update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Wildvine prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `wildvine update` would do without making changes:

```bash
wildvine update --dry-run
wildvine update --channel beta --dry-run
wildvine update --tag 2026.4.1-beta.1 --dry-run
wildvine update --dry-run --json
```
...
When you switch channels with `wildvine update`, Wildvine also syncs plugin
sources:
