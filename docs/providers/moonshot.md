...
```bash
wildvine onboard --auth-choice moonshot-api-key
```

Kimi Coding:

```bash
wildvine onboard --auth-choice kimi-code-api-key
```
...
Wildvine also maps runtime `/think` levels for Moonshot:

- `/think off` -> `thinking.type=disabled`
- any non-off thinking level -> `thinking.type=enabled`

When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Wildvine normalizes incompatible `tool_choice` values to `auto` for compatibility.
