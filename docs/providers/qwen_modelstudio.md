...
read_when:
  - You want to use Qwen (Alibaba Cloud Model Studio) with Wildvine
...
```bash
# China endpoint
wildvine onboard --auth-choice modelstudio-standard-api-key-cn

# Global/Intl endpoint
wildvine onboard --auth-choice modelstudio-standard-api-key
```
...
```bash
# China endpoint
wildvine onboard --auth-choice modelstudio-api-key-cn

# Global/Intl endpoint
wildvine onboard --auth-choice modelstudio-api-key
```
...
`~/.wildvine/.env` or via `env.shellEnv`).
