---
summary: "Use Anthropic Claude via API keys, setup-token, or Claude CLI in Wildvine"
read_when:
  - You want to use Anthropic models in Wildvine
...
In Wildvine you can authenticate with an API key or a **setup-token**.
...
# or non-interactive
wildvine onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
```
...
- Anthropic Claude 4.6 models default to `adaptive` thinking in Wildvine when no explicit thinking level is set.
...
Wildvine's shared `/fast` toggle also supports direct public Anthropic traffic, including API-key and OAuth-authenticated requests sent to `api.anthropic.com`.
...
- Wildvine only injects Anthropic service tiers for direct `api.anthropic.com` requests. If you route `anthropic/*` through a proxy or gateway, `/fast` leaves `service_tier` untouched.
...
Wildvine supports Anthropic's prompt caching feature. This is **API-only**; subscription auth does not honor cache settings.
...
When using Anthropic API Key authentication, Wildvine automatically applies `cacheRetention: "short"` (5-minute cache) for all Anthropic models. You can override this by explicitly setting `cacheRetention` in your config.
...
Wildvine includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
requests; keep it if you override provider headers (see [/gateway/configuration](/gateway/configuration)).

## 1M context window (Anthropic beta)

Anthropic's 1M context window is beta-gated. In Wildvine, enable it per model
...
Wildvine maps this to `anthropic-beta: context-1m-2025-08-07` on Anthropic
requests.
...
with subscription auth, Wildvine logs a warning and falls back to the standard
...
the Anthropic API directly. Wildvine treats it as a **CLI backend provider**
...
1. Wildvine launches `claude -p --output-format json ...` on the **gateway
   host**.
2. The first turn sends `--session-id <uuid>`.
3. Follow-up turns reuse the stored Claude session via `--resume <sessionId>`.
4. Your chat messages still go through the normal Wildvine message pipeline, but
...
- Wildvine auto-loads the bundled Anthropic plugin at gateway startup when your
...
- Normal Wildvine message/session routing
...
```bash
wildvine onboard --auth-choice anthropic-cli
```
...
- Tools are disabled on the Wildvine side for CLI backend runs.
- Text in, text out. No Wildvine streaming handoff.
...
Paste the token into Wildvine (wizard: **Anthropic token (paste setup-token)**), or run it on the gateway host:
...
wildvine onboard --auth-choice setup-token
```
...
- Generate the setup-token with `claude setup-token` and paste it, or run `wildvine models auth setup-token` on the gateway host.
...
- If the Claude CLI login lives on a different machine, use
  `wildvine models auth paste-token --provider anthropic` on the gateway host.

**No API key found for provider "anthropic"**

- Auth is **per agent**. New agents don’t inherit the main agent’s keys.
- Re-run onboarding for that agent, or paste a setup-token / API key on the
  gateway host, then verify with `wildvine models status`.

**No credentials found for profile `anthropic:default`**

- Run `wildvine models status` to see which auth profile is active.
- Re-run onboarding, or paste a setup-token / API key for that profile.

**No available auth profile (all in cooldown/unavailable)**

- Check `wildvine models status --json` for `auth.unusableProfiles`.
