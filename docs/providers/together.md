...
read_when:
  - You want to use Together AI with Wildvine
...
```bash
wildvine onboard --auth-choice together-api-key
```
...
is available to that process (for example, in `~/.wildvinebot/.env` or via
`env.shellEnv`).
