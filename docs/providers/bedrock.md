---
summary: "Use Amazon Bedrock (Converse API) models with Wildvine"
read_when:
  - You want to use Amazon Bedrock models with Wildvine
...
Wildvine can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, Wildvine can automatically discover Bedrock
...
When running Wildvine on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, Wildvine's credential detection currently only checks for environment
...
- Wildvine surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
