---
summary: "Use OpenCode Zen and Go catalogs with Wildvine"
read_when:
...
OpenCode exposes two hosted catalogs in Wildvine:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Wildvine keeps the runtime provider ids
...
wildvine onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
wildvine onboard --auth-choice opencode-go
wildvine onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
