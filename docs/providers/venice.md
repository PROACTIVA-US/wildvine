---
summary: "Use Venice AI privacy-focused models in Wildvine"
read_when:
  - You want privacy-focused inference in Wildvine
...
```bash
wildvine onboard --auth-choice venice-api-key
```
...
```bash
wildvine agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Wildvine shows all available Venice models. Pick based on your needs:
...
```bash
wildvine models set venice/kimi-k2-5
wildvine models set venice/claude-opus-4-6
```

List all available models:

```bash
wildvine models list | grep venice
```

## Configure via `wildvine configure`

1. Run `wildvine configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Wildvine automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
wildvine agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
wildvine agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
wildvine agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
wildvine agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
wildvine agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `wildvine models list` to see currently available models. Some models may be temporarily offline.
