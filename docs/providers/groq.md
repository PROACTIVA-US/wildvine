...
read_when:
  - You want to use Groq with Wildvine
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Wildvine connects
...
media-understanding provider, Wildvine uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.wildvine/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `wildvine models list | grep groq`
