---
summary: "Run Wildvine with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Wildvine against a local SGLang server
...
Wildvine can connect to SGLang using the `openai-completions` API.

Wildvine can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Wildvine will query:
