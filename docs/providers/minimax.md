---
summary: "Use MiniMax models in Wildvine"
read_when:
...
Wildvine's MiniMax provider defaults to **MiniMax M2.7**.
...
```bash
wildvine plugins enable minimax  # skip if already loaded.
...
See the MiniMax plugin package README in the Wildvine repo for details.
...
- Use `wildvine models list` and `wildvine models set minimax/MiniMax-M2.7` to switch.
...
- Running `wildvine configure` and selecting a **MiniMax** auth option, or
