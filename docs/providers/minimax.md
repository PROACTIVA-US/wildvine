---
summary: "Use MiniMax M2.1 in Wildvine"
read_when:
  - You want MiniMax models in Wildvine
...
```bash
wildvine plugins enable minimax-portal-auth  # skip if already loaded.
wildvine gateway restart  # restart if gateway is already running
wildvine onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/wildvine/wildvine/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `wildvine.json`:
...
- Use `wildvine models list` and `wildvine models set minimax/MiniMax-M2.1` to switch.
...
- Running `wildvine configure` and selecting **MiniMax M2.1**, or
