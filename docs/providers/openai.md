---
summary: "Use OpenAI via API keys or Codex subscription in Wildvine"
read_when:
  - You want to use OpenAI models in Wildvine
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Wildvine.
...
```bash
wildvine onboard --auth-choice openai-api-key
# or non-interactive
wildvine onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Wildvine forwards both through the `openai/*` Responses path.
Wildvine intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Wildvine does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Wildvine.
...
wildvine onboard --auth-choice openai-codex

# Or run OAuth directly
wildvine models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Wildvine
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Wildvine also supports:

- `openai-codex/gpt-5.3-codex-spark`

Wildvine treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Wildvine also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Wildvine uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Wildvine also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Wildvine enables it by default for
...
Wildvine, set `agents.defaults.models["<provider>/<model>"].params.serviceTier`
...
Wildvine forwards `params.serviceTier` to both direct `openai/*` Responses
...
- if you route either provider through another base URL or proxy, Wildvine leaves `service_tier` untouched

### OpenAI fast mode

Wildvine exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Wildvine maps it to OpenAI priority processing:
...
`baseUrl` on `api.openai.com`), Wildvine now auto-enables OpenAI server-side
