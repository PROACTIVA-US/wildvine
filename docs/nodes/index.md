...
`wildvine devices list` before approving.
...
- For direct shell/runtime file executions, Wildvine also best-effort binds one concrete local
...
- `wildvine node run` supports token or password auth.
- Env vars are preferred: `WILDVINE_GATEWAY_TOKEN` / `WILDVINE_GATEWAY_PASSWORD`.
...
If the node retries with changed auth details, re-run `wildvine devices list`
...
```bash
wildvine nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
wildvine nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
wildvine nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
wildvine nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On macOS, the headless node host executes `system.run` locally by default. Set
  `WILDVINE_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `WILDVINE_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
