...
```bash
wildvine nodes describe --node <idOrNameOrIp>
wildvine nodes canvas snapshot --node <idOrNameOrIp>
wildvine logs --follow
```
...
wildvine nodes status
wildvine approvals get --node <idOrNameOrIp>
wildvine approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`wildvine approvals get --node ...`), not in the gateway pairing record.
