...
wildvine nodes reject <requestId>
wildvine nodes status
wildvine nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.wildvine`):

- `~/.wildvine/nodes/paired.json`
- `~/.wildvine/nodes/pending.json`

If you override `WILDVINE_STATE_DIR`, the `nodes/` folder moves with it.
