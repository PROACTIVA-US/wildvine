...
read_when:
  - Setting up Wildvine for the first time
...
Wildvine reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.wildvine/wildvine.json`.

If the file is missing, Wildvine uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `wildvine onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    wildvine onboard       # full onboarding flow
    wildvine configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ...
    wildvine config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.wildvine/wildvine.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Wildvine only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`wildvine doctor`, `wildvine logs`, `wildvine health`, `wildvine status`)
- Run `wildvine doctor` to see exact issues
- Run `wildvine doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `wildvine.json`.
    ...
    ```bash
    wildvine config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `WILDVINE_APNS_RELAY_BASE_URL` and `WILDVINE_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `WILDVINE_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.wildvine/wildvine.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `wildvine config set` for single keys.
    </Warning>
    ...
    wildvine gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.wildvine/workspace\" } } }",
      "baseHash": "<hash>",
...
Wildvine reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.wildvine/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Wildvine runs your login shell and imports only the missing keys:
...
Env var equivalent: `WILDVINE_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${WILDVINE_GATEWAY_TOKEN}" } },
