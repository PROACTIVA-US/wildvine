...
Wildvine’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`wildvine agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-wildvine-scopes` values and restore the normal full operator defaults
...
- select agents with `model: "wildvine"`, `model: "wildvine/default"`, `model: "wildvine/<agentId>"`, or `x-wildvine-agent-id`
- use `x-wildvine-model` when you want to override the selected agent's backend model
- use `x-wildvine-session-key` for explicit session routing
- use `x-wildvine-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Wildvine reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
