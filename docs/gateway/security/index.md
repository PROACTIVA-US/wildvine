...
Wildvine is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
**On this page:** [Trust model](#scope-first-personal-assistant-security-model) | [Quick audit](#quick-check-wildvine-security-audit) | [Hardened baseline](#hardened-baseline-in-60-seconds) | [DM access model](#dm-access-model-pairing--allowlist--open--disabled) | [Configuration hardening](#configuration-hardening-examples) | [Incident response](#incident-response)

## Scope first: personal assistant security model

Wildvine security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `wildvine security audit`
...
Wildvine is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Wildvine assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.wildvine`, including `wildvine.json`), treat them as a trusted operator.
...
4. Claim is not listed in [Out of Scope](https://github.com/PROACTIVA-US/wildvine/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, Wildvine also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.wildvine/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.wildvine/credentials/<channel>-allowFrom.json` (default account)
  - `~/.wildvine/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.wildvine/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.wildvine/secrets.json`
- **Legacy OAuth import**: `~/.wildvine/credentials/oauth.json`
...
| ------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                           | critical      | Other users/processes can modify full Wildvine state                                 | filesystem perms on `~/.wildvine`                                                                    | yes      |
| `fs.config.perms_writable`                                    | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.wildvine/wildvine.json`                                                      | yes      |
...
| `tools.exec.auto_allow_skills_enabled`                        | warn          | Exec approvals trust skill bins implicitly                                           | `~/.wildvine/exec-approvals.json`                                                                    | no       |
...
`wildvine security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`wildvine security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in Wildvine config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${WILDVINE_GATEWAY_PASSWORD}
```
...
- Wildvine gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Wildvine responses.
...
Wildvine stores session transcripts on disk under `~/.wildvine/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.wildvine` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Wildvine cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
...
Wildvine can refresh the skills list mid-session:
...
- If you install plugins (`wildvine plugins install <package>`), treat it like running untrusted code:
  - The install path is the per-plugin directory under the active plugin install root.
  - Wildvine runs a built-in dangerous-code scan before install. `critical` findings block by default.
  - Wildvine uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
wildvine pairing list <channel>
wildvine pairing approve <channel> <code>
```
...
By default, Wildvine routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Wildvine has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.wildvine/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.wildvine or your logs.”

## Unsafe external content bypass flags

Wildvine includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.wildvine/wildvine.json`: `600` (user read/write only)
- `~/.wildvine`: `700` (user only)

`wildvine doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `WILDVINE_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__wildvine__/canvas/` and `/__wildvine__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run Wildvine with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_wildvine-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `WILDVINE_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `wildvine doctor --generate-gateway-token`.
...
paths, set `WILDVINE_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `WILDVINE_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `WILDVINE_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Wildvine
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Wildvine verifies the identity by resolving the
...
- On the OpenAI-compatible HTTP surface, shared-secret bearer auth restores the full default operator scopes and owner semantics for agent turns; narrower `x-wildvine-scopes` values do not reduce that shared-secret path.
...
- Wildvine will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.wildvine/` (or `$WILDVINE_STATE_DIR/`) may contain secrets or private data:

- `wildvine.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `wildvine status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@wildvine", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.wildvine`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.wildvine/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `wildvine` profile).
...
Wildvine’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Wildvine limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `wildvine gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `WILDVINE_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/wildvine/wildvine-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.wildvine/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `wildvine security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Wildvine version
...
Found a vulnerability in Wildvine? Please report responsibly:

1. Email: [security@wildvine.com](mailto:security@wildvine.com)
