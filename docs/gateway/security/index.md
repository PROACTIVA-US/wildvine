...
- Tighten local perms (`~/.wildvine` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).
...
- **Pairing allowlists**: `~/.wildvine/credentials/<channel>-allowFrom.json`
...
- If you install plugins from npm (`wildvine plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.wildvine/extensions/<pluginId>/` (or `$WILDVINE_STATE_DIR/extensions/<pluginId>/`).
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.wildvine/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
4. **Re-run audit**
   - `wildvine security audit --deep` and confirm the report is clean.

## Lessons Learned (The Hard Way)

### The `find ~` Incident

On Day 1, a friendly tester asked Wildvine to run `find ~` and share the output. Wildvine happily dumped the entire home directory structure to a group chat.
...
authentication. Wildvine verifies the identity by resolving the
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Wildvine clients can connect.
...
```mermaid
flowchart TB
    A["Owner (Peter)"] -- Full trust --> B["AI (Wildvine)"]
...
1. Email: [security@wildvine.ai](mailto:security@wildvine.ai)
