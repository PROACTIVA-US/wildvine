...
Wildvine supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `WILDVINE_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Wildvine runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Wildvine validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Wildvine QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/wildvine"],
...
- Wildvine resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__WILDVINE_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `wildvine.json` refs)
...
When reload-time activation fails after a healthy state, Wildvine enters degraded secrets state.
...
- Strict command paths (for example `wildvine memory` remote-memory paths and `wildvine qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `wildvine status`, `wildvine status --all`, `wildvine channels status`, `wildvine channels resolve`, `wildvine security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `wildvine secrets reload`.
...
```bash
wildvine secrets audit --check
wildvine secrets configure
wildvine secrets audit --check
```
...
- plaintext values at rest (`wildvine.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `wildvine.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `wildvine secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `wildvine.json` plus `auth-profiles.json` for one agent scope
...
- `wildvine secrets configure --providers-only`
- `wildvine secrets configure --skip-provider-setup`
- `wildvine secrets configure --agent <id>`
...
```bash
wildvine secrets apply --from /tmp/wildvine-secrets-plan.json
wildvine secrets apply --from /tmp/wildvine-secrets-plan.json --allow-exec
wildvine secrets apply --from /tmp/wildvine-secrets-plan.json --dry-run
wildvine secrets apply --from /tmp/wildvine-secrets-plan.json --dry-run --allow-exec
```
...
Wildvine intentionally does not write rollback backups containing historical plaintext secret values.
