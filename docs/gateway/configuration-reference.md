...
description: "Complete field-by-field reference for ~/.wildvine/wildvine.json"
---
...
- `config: true` enables `/config` (reads/writes `wildvine.json`).
...
Wildvine uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.wildvine/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `WILDVINE_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `wildvine onboard --auth-choice opencode-zen`.
