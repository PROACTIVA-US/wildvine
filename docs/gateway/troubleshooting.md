...
- `wildvine gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `wildvine doctor` reports no blocking config/service issues.
- `wildvine channels status --probe` shows connected/ready channels.
...
wildvine config get agents.defaults.models
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `wildvine config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `wildvine devices list` then `wildvine devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `wildvine configure`). If you are running Wildvine via Podman, the default config path is `~/.wildvine/wildvine.json`.
...
wildvine system heartbeat last
wildvine logs --follow
```
...
wildvine approvals get --node <idOrNameOrIp>
wildvine logs --follow
wildvine status
```
...
wildvine browser start --browser-profile wildvine
...
```bash
wildvine gateway status
wildvine config get gateway.mode
wildvine config get gateway.remote.url
wildvine config get gateway.auth.mode
```
...
```bash
wildvine config get gateway.bind
wildvine config get gateway.auth.token
wildvine gateway status
wildvine logs --follow
```
