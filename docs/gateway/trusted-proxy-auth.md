...
read_when:
  - Running Wildvine behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Wildvine
...
- You run Wildvine behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Wildvine checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Wildvine extracts the user identity from the configured header
...
- Wildvine can stay on loopback HTTP behind the proxy.
...
If Wildvine itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://wildvine.example.com
    to: http://wildvine-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
wildvine.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy wildvine:18789 {
...
location / {
    ...
    proxy_pass http://wildvine:18789;
...
Wildvine rejects ambiguous configurations where both a `gateway.auth.token` (or `WILDVINE_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
`wildvine security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Wildvine config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `wildvine security audit` and review findings
