...
- During heartbeat runs, Wildvine treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `wildvine sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Wildvine skips the heartbeat run entirely (no model call).
...
```bash
wildvine system event --text "Check for urgent follow-ups" --mode now
```
