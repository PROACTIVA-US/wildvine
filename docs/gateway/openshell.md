---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Wildvine agents"
read_when:
...
OpenShell is a managed sandbox backend for Wildvine. Instead of running Docker
containers locally, Wildvine delegates sandbox lifecycle to the `openshell` CLI,
...
- Wildvine Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Wildvine creates an OpenShell
...
- You edit files locally outside Wildvine and want those changes visible in the
...
- When the sandbox is first created, Wildvine seeds the remote workspace from
...
- Wildvine does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Wildvine after the initial seed,
the remote sandbox does **not** see those changes. Use
`wildvine sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"wildvine"`  | Sandbox source for first-time create                  |
...
1. Wildvine calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Wildvine calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `wildvine sandbox` commands
