...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/wildvine`)
...
If the page fails to load, start the Gateway first: `wildvine gateway`.
...
created. Re-run `wildvine devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `wildvine devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.wildvine/wildvine.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop wildvine`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Wildvine
...
```bash
wildvine gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Wildvine **blocks** Control UI connections without device identity.
...
```bash
WILDVINE_CONTROL_UI_BASE_PATH=/wildvine/ pnpm ui:build
```
