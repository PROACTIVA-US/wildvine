...
```bash
wildvine tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `wildvine tui --deliver`
...
- Local shell commands receive `WILDVINE_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `WILDVINE_THEME=light` before launching `wildvine tui`.
- To force the original dark palette instead, set `WILDVINE_THEME=dark`.
...
- Check the Gateway logs: `wildvine logs --follow`.
- Confirm the agent can run: `wildvine status` and `wildvine models status`.
...
- No agents in picker: check `wildvine agents list` and your routing config.
