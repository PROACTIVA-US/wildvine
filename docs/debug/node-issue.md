...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Wildvine
...
Running Wildvine via Node with `tsx` fails at startup with:

```
[wildvine] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `wildvine status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch wildvine.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node wildvine.mjs status` works on Node 25.
