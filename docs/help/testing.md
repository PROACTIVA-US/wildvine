...
Wildvine has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  ...
  - Opt back into Vitest file isolation with `WILDVINE_TEST_ISOLATE=1 pnpm test`.
  - `WILDVINE_TEST_NO_ISOLATE=0` or `WILDVINE_TEST_NO_ISOLATE=false` also force isolated runs.
- Fast-local iteration note:
  ...
  - The wrapper keeps `WILDVINE_VITEST_FS_MODULE_CACHE` enabled on supported hosts, but assigns a lane-local `WILDVINE_VITEST_FS_MODULE_CACHE_PATH` so concurrent Vitest processes do not race on one shared experimental cache directory.
  - Set `WILDVINE_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want one explicit cache location for direct single-run profiling.
- Perf-debug note:
...
- Useful overrides:
  - `WILDVINE_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `WILDVINE_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Wildvine's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `WILDVINE_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `WILDVINE_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `WILDVINE_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.wildvine`.
- Set `WILDVINE_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `WILDVINE_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `WILDVINE_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `WILDVINE_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `WILDVINE_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Optional target overrides:
  - `WILDVINE_ANDROID_NODE_ID` or `WILDVINE_ANDROID_NODE_NAME`.
  - `WILDVINE_ANDROID_GATEWAY_URL` / `WILDVINE_ANDROID_GATEWAY_TOKEN` / `WILDVINE_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `WILDVINE_LIVE_TEST=1` if invoking Vitest directly)
- Set `WILDVINE_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `WILDVINE_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.7, Grok 4)
  - `WILDVINE_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `WILDVINE_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (comma allowlist)
- How to select providers:
  - `WILDVINE_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `WILDVINE_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates “provider API is broken / key is invalid” from “gateway agent pipeline is broken”
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@wildvine" actually does)
...
- How to select models:
  ...
  - `WILDVINE_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `WILDVINE_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
- How to select providers (avoid “OpenRouter everything”):
  - `WILDVINE_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Enable:
  - `pnpm test:live` (or `WILDVINE_LIVE_TEST=1` if invoking Vitest directly)
  - `WILDVINE_LIVE_SETUP_TOKEN=1`
- Token sources (pick one):
  - Profile: `WILDVINE_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - Raw token: `WILDVINE_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
- Model override (optional):
  - `WILDVINE_LIVE_SETUP_TOKEN_MODEL=anthropic/claude-opus-4-6`

Setup example:

```bash
wildvine models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
WILDVINE_LIVE_SETUP_TOKEN=1 WILDVINE_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test pnpm test:live src/agents/anthropic.setup-token.live.test.ts
```
...
- Enable:
  - `pnpm test:live` (or `WILDVINE_LIVE_TEST=1` if invoking Vitest directly)
  - `WILDVINE_LIVE_CLI_BACKEND=1`
- Defaults:
  - Model: `claude-cli/claude-sonnet-4-6`
  - Command: `claude`
  - Args: `["-p","--output-format","json","--permission-mode","bypassPermissions"]`
- Overrides (optional):
  - `WILDVINE_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-opus-4-6"`
  - `WILDVINE_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
  - `WILDVINE_LIVE_CLI_BACKEND_COMMAND="/full/path/to/claude"`
  - `WILDVINE_LIVE_CLI_BACKEND_ARGS='["-p","--output-format","json","--permission-mode","bypassPermissions"]'`
  - `WILDVINE_LIVE_CLI_BACKEND_CLEAR_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'`
  - `WILDVINE_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `WILDVINE_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `WILDVINE_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `WILDVINE_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
- `WILDVINE_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` to keep Claude Code CLI MCP config enabled (default disables MCP config with a temporary empty file).

Example:

```bash
WILDVINE_LIVE_CLI_BACKEND=1 \
  WILDVINE_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-sonnet-4-6" \
...
- For `claude-cli`, it installs the Linux `@anthropic-ai/claude-code` package into a cached writable prefix at `WILDVINE_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/wildvine/docker-cli-tools`).
- It copies `~/.claude` into the container when available, but on machines where Claude auth is backed by `ANTHROPIC_API_KEY`, it also preserves `ANTHROPIC_API_KEY` / `ANTHROPIC_API_KEY_OLD` for the child Claude CLI via `WILDVINE_LIVE_CLI_BACKEND_PRESERVE_ENV`.
...
- Enable:
  ...
  - `WILDVINE_LIVE_ACP_BIND=1`
- Defaults:
  - ACP agent: `claude`
  - Synthetic channel: Slack DM-style conversation context
  - ACP backend: `acpx`
- Overrides:
  - `WILDVINE_LIVE_ACP_BIND_AGENT=claude`
  - `WILDVINE_LIVE_ACP_BIND_AGENT=codex`
  - `WILDVINE_LIVE_ACP_BIND_ACPX_COMMAND=/full/path/to/acpx`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `WILDVINE_LIVE_ACP_BIND_ACPX_COMMAND` is unset, the test uses the configured/bundled acpx command. If your harness auth depends on env vars from `~/.profile`, prefer a custom `acpx` command that preserves provider env.
...
- Inside Docker, the runner sets `WILDVINE_LIVE_ACP_BIND_ACPX_COMMAND=$HOME/.npm-global/bin/acpx` so acpx keeps provider env vars from the sourced profile available to the child harness CLI.
...
- Single model, direct (no gateway):
  - `WILDVINE_LIVE_MODELS="openai/gpt-5.2" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `WILDVINE_LIVE_GATEWAY_MODELS="openai/gpt-5.2" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `WILDVINE_LIVE_GATEWAY_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `WILDVINE_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `WILDVINE_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API vs Gemini CLI:
  - API: Wildvine calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: Wildvine shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
`WILDVINE_LIVE_GATEWAY_MODELS="openai/gpt-5.2,openai-codex/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `WILDVINE_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.

### Aggregators / alternate gateways

If you have keys enabled, we also support testing via:

- OpenRouter: `openrouter/...` (hundreds of models; use `wildvine models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `wildvine models list` / model selection.

- Profile store: `~/.wildvine/credentials/` (preferred; what “profile keys” means in the tests)
- Config: `~/.wildvine/wildvine.json` (or `WILDVINE_CONFIG_PATH`)
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `WILDVINE_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `WILDVINE_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `WILDVINE_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
- Optional auth behavior:
  - `WILDVINE_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
They also set `WILDVINE_SKIP_CHANNELS=1` so gateway live probes do not start
...
`WILDVINE_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Wildvine gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `wildvine/default`, then sends a
...
This lane expects a usable live model key, and `WILDVINE_PROFILE_FILE`
...
container, starts a second container that spawns `wildvine mcp serve`, then
...
- `WILDVINE_CONFIG_DIR=...` (default: `~/.wildvine`) mounted to `/home/node/.wildvine`
- `WILDVINE_WORKSPACE_DIR=...` (default: `~/.wildvine/workspace`) mounted to `/home/node/.wildvine/workspace`
- `WILDVINE_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- `WILDVINE_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/wildvine/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs under `$HOME` are mounted read-only under `/host-auth/...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs inferred from `WILDVINE_LIVE_PROVIDERS` / `WILDVINE_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `WILDVINE_DOCKER_AUTH_DIRS=all`, `WILDVINE_DOCKER_AUTH_DIRS=none`, or a comma list like `WILDVINE_DOCKER_AUTH_DIRS=.claude,.codex`
- `WILDVINE_LIVE_GATEWAY_MODELS=...` / `WILDVINE_LIVE_MODELS=...` to narrow the run
- `WILDVINE_LIVE_GATEWAY_PROVIDERS=...` / `WILDVINE_LIVE_PROVIDERS=...` to filter providers in-container
- `WILDVINE_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `WILDVINE_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `WILDVINE_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
