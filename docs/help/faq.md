...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://wildvine.com/install.sh | bash -s -- --install-method git
    ```
    ...
    [https://github.com/PROACTIVA-US/wildvine/issues](https://github.com/PROACTIVA-US/wildvine/issues)
    [https://github.com/PROACTIVA-US/wildvine/pulls](https://github.com/PROACTIVA-US/wildvine/pulls)
  ...
  <Accordion title="Recommended way to install and set up Wildvine">
    The repo recommends running from source and using onboarding:

    ```bash
    curl -fsSL https://wildvine.com/install.sh | bash
    ...
    ```bash
    git clone https://github.com/PROACTIVA-US/wildvine.git
    cd wildvine
    pnpm install
    pnpm build
  ...
  <Accordion title="How do I authenticate the dashboard (token) on localhost vs remote?">
    ...
    **Not on localhost:**
    ...
    - **Tailscale Serve** (recommended): keep bind loopback, run `wildvine gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/PROACTIVA-US/wildvine/blob/main/CHANGELOG.md](https://github.com/PROACTIVA-US/wildvine/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.wildvine.com (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.wildvine.com` via Xfinity
    Advanced Security. Disable it or allowlist `docs.wildvine.com`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/PROACTIVA-US/wildvine/tree/main/docs](https://github.com/PROACTIVA-US/wildvine/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://wildvine.com/install.ps1](https://wildvine.com/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://wildvine.com/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://wildvine.com/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://wildvine.com/install.sh | bash -s -- --install-method git --verbose
    ```
    ...
    Set-PSDebug -Trace 1
    & ([scriptblock]::Create((iwr -useb https://wildvine.com/install.ps1))) -NoOnboard
    Set-PSDebug -Trace 0
    ```
  ...
  <Accordion title="Windows install says git not found or wildvine not recognized">
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Wildvine, track/report it in:

    - [Issue #30640](https://github.com/PROACTIVA-US/wildvine/issues/30640)
  ...
  <Accordion title="How do I install Wildvine on Linux?">
  ...
  <Accordion title="How do I install Wildvine on a VPS?">
  ...
  <Accordion title="Can I ask Wildvine to update itself?">
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    ...
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    OpenAI Codex OAuth is explicitly supported for external tools like Wildvine.
  ...
  <Accordion title="How does Anthropic setup-token auth work?">
    `claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in onboarding or paste it with `wildvine models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
  </Accordion>
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    - use a **setup-token**
    - reuse a local **Claude CLI** login on the gateway host with `wildvine models auth login --provider anthropic --method cli --set-default`
...
for usage/billing and raise limits as needed.
  ...
  <Accordion title="How does Codex auth work?">
    Wildvine supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).
  </Accordion>

  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Wildvine fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Wildvine. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    ...
    1. Enable the plugin: `wildvine plugins enable google`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Wildvine needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="If I buy a Mac mini to run Wildvine, can I connect it to my MacBook Pro?">
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Onboarding accepts `@username` input and resolves it to a numeric ID, but Wildvine authorization uses numeric IDs only.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Wildvine instances?">
  ...
  <Accordion title="How important is it to run Wildvine on a dedicated machine?">
  ...
  <Accordion title="Can I run Wildvine in a VM and what are the requirements?">
...
<AccordionGroup>
  <Accordion title="What is Wildvine, in one paragraph?">
  ...
  <Accordion title="What are the top five everyday use cases for Wildvine?">
  ...
  <Accordion title="Can Wildvine help with lead gen, outreach, ads, and blogs for a SaaS?">
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    ...
    Showcase: [https://wildvine.com/showcase](https://wildvine.com/showcase)
...
<AccordionGroup>
  ...
  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.wildvine/wildvine.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.wildvine/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Wildvine treats as `<workspace>/skills` on the next session.
  </Accordion>
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `wildvine skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.com](https://clawhub.com).

    ```bash
    wildvine skills search "calendar"
    wildvine skills install <skill-slug>
  ...
  <Accordion title="Can Wildvine run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.wildvine/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Wildvine?">
    ...
    wildvine browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    configured and present, Wildvine
...
<AccordionGroup>
  <Accordion title="Is all data used with Wildvine saved locally?">
  ...
  <Accordion title="Where does Wildvine store its data?">
    ...
    | `$WILDVINE_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$WILDVINE_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$WILDVINE_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.wildvine` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Wildvine?">
...
<AccordionGroup>
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Wildvine enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `wildvine doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Wildvine.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `WILDVINE_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `wildvine configure --section web` and choose a provider.
  ...
  <Accordion title="Can the Wildvine browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Can two Wildvine instances talk to each other (local + VPS)?">
...
<AccordionGroup>
  <Accordion title="How does Wildvine load environment variables?">
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Wildvine instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Wildvine but keep it installed?">
  ...
  <Accordion title="Why does Wildvine not reply in a group?">
    Two common causes:
...
<AccordionGroup>
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `wildvine onboard` and choose `Ollama`
    ...
    - for manual switching, use `wildvine models list` and `wildvine models set ollama/<model>`
  ...
  <Accordion title="What do Wildvine, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `wildvine models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Wildvine release (or run from source `main`), then restart the gateway.
...
<AccordionGroup>
  ...
  <Accordion title='Why does wildvine gateway status say "Runtime: running" but "RPC probe: failed"?'>
  ...
  <Accordion title='Why does wildvine gateway status show "Config (cli)" and "Config (service)" different?'>
  ...
  <Accordion title="How do I run Wildvine in remote mode (client connects to a Gateway elsewhere)?">
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `wildvine devices list`
      - `wildvine devices rotate --device <id> --role operator`
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    Profiles also suffix service names (`ai.wildvine.<profile>`; legacy `com.wildvine.*`, `wildvine-gateway-<profile>.service`, `Wildvine Gateway (<profile>)`).
...
<AccordionGroup>
  ...
  <Accordion title="I closed my terminal on Windows - how do I restart Wildvine?">
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Wildvine already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: wildvine gateway restart vs wildvine gateway">
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Wildvine to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `wildvine pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `wildvine pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
