---
summary: "Symptom first troubleshooting hub for Wildvine"
read_when:
  - Wildvine is not working and you need the fastest path to a fix
...
- `wildvine status` → shows configured channels and no obvious auth errors.
- `wildvine status --all` → full report is present and shareable.
- `wildvine gateway probe` → expected gateway target is reachable (`Reachable: yes`). `RPC: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `wildvine gateway status` → `Runtime: running` and `RPC probe: ok`.
- `wildvine doctor` → no blocking config/service errors.
- `wildvine channels status --probe` → channels report `connected` or `ready`.
- `wildvine logs --follow` → steady activity, no repeating fatal errors.
...
## Plugin install fails with missing wildvine extensions

If install fails with `package.json missing wildvine.extensions`, the plugin package
is using an old shape that Wildvine no longer accepts.

Fix in the plugin package:

1. Add `wildvine.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `wildvine plugins install <package>` again.

Example:

```json
{
  "name": "@wildvine/my-plugin",
  "version": "1.2.3",
  "wildvine": {
...
```mermaid
flowchart TD
  A[Wildvine is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  <Accordion title="No replies">
    ...
    wildvine gateway status
    wildvine channels status --probe
    wildvine pairing list --channel <channel> [--account <id>]
    wildvine logs --follow
    ```
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `wildvine gateway status`
    - `RPC probe: ok`
  ...
  <Accordion title="Cron or heartbeat did not fire or did not deliver">
    ...
    wildvine status
    wildvine gateway status
    wildvine cron status
    wildvine cron list
    wildvine cron runs --id <jobId> --limit 20
    wildvine logs --follow
    ```
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    wildvine config get tools.exec.host
    wildvine config get tools.exec.security
    wildvine config get tools.exec.ask
    ...
    ```bash
    wildvine config set tools.exec.host gateway
    wildvine config set tools.exec.security full
    wildvine config set tools.exec.ask off
    wildvine gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `wildvine` starts, or `user` can see local Chrome tabs.
