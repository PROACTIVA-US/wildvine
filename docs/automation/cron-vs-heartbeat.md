...
- **Cron (isolated)** is a scheduled **background run** — creates a task record tracked in `wildvine tasks`.
...
- **Task tracking**: isolated jobs create [background task](/automation/tasks) records visible in `wildvine tasks` and `wildvine tasks audit`.
...
wildvine cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
wildvine cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
wildvine cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```

## Vine: Deterministic workflows with approvals

Vine is the workflow runtime for **multi-step tool pipelines** that need deterministic execution and explicit approvals.
...
- **Vine** defines _what steps_ happen once the run starts.

For scheduled workflows, use cron or heartbeat to trigger an agent turn that calls Vine.
For ad-hoc workflows, call Vine directly.

### Operational notes (from the code)

- Vine runs as a **local subprocess** (`vine` CLI) in tool mode and returns a **JSON envelope**.
- If the tool returns `needs_approval`, you resume with a `resumeToken` and `approve` flag.
- The tool is an **optional plugin**; enable it additively via `tools.alsoAllow: ["vine"]` (recommended).
- Vine expects the `vine` CLI to be available on `PATH`.

See [Vine](/tools/vine) for full usage and examples.
...
| [Tasks](/automation/tasks) | No task record                  | Task record (silent)     | Task record (visible in `wildvine tasks`)       |
