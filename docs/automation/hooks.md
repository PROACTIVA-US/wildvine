...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be inspected with `wildvine hooks`, while hook-pack installation and updates now go through `wildvine plugins`.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Wildvine. See [Webhook Hooks](/automation/webhook) or use `wildvine webhooks` for Gmail helper commands.

Hooks can also be bundled inside plugins; see [Plugin hooks](/plugins/architecture#provider-runtime-hooks). `wildvine hooks list` shows both standalone hooks and plugin-managed hooks.
...
If you can write a small TypeScript function, you can write a hook. Managed and bundled hooks are trusted local code. Workspace hooks are discovered automatically, but Wildvine keeps them disabled until you explicitly enable them via the CLI or config.
...
- Extend Wildvine's behavior without modifying core code

## Getting Started

### Bundled Hooks

Wildvine ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.wildvine/workspace/memory/`) when you issue `/new` or `/reset`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.wildvine/logs/commands.log`
...
```bash
wildvine hooks enable session-memory
```

Check hook status:

```bash
wildvine hooks check
```

Get detailed information:

```bash
wildvine hooks info session-memory
```

### Onboarding

During onboarding (`wildvine onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.

### Trust Boundary

Hooks run inside the Gateway process. Treat bundled hooks, managed hooks, and `hooks.internal.load.extraDirs` as trusted local code. Workspace hooks under `<workspace>/hooks/` are repo-local code, so Wildvine requires an explicit enable step before loading them.

## Hook Discovery

Hooks are automatically discovered from these directories, in order of increasing override precedence:

1. **Bundled hooks**: shipped with Wildvine; located at `<wildvine>/dist/hooks/bundled/` for npm installs (or a sibling `hooks/bundled/` for compiled binaries)
2. **Plugin hooks**: hooks bundled inside installed plugins (see [Plugin hooks](/plugins/architecture#provider-runtime-hooks))
3. **Managed hooks**: `~/.wildvine/hooks/` (user-installed, shared across workspaces; can override bundled and plugin hooks). **Extra hook directories** configured via `hooks.internal.load.extraDirs` are also treated as managed hooks and share the same override precedence.
...
Hook packs are standard npm packages that export one or more hooks via `wildvine.hooks` in
`package.json`. Install them with:

```bash
wildvine plugins install <path-or-spec>
```
...
those to a prerelease, Wildvine stops and asks you to opt in explicitly with a
...
Hook packs can ship dependencies; they will be installed under `~/.wildvine/hooks/<id>`.
Each `wildvine.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `wildvine plugins install` installs hook-pack dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.wildvine.com/automation/hooks#my-hook
metadata:
  { "wildvine": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.wildvine` object supports:
...
```typescript
{
  ...
  cfg: WildvineConfig            // Current gateway config
}
```
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Wildvine persists them.
...
Runs after the built-in install security scan and before installation continues. Wildvine fires this hook for interactive skill installs as well as plugin bundle, package, and single-file installs.

Default behavior differs by target type:

- Plugin installs fail closed on built-in scan `critical` findings and scan errors unless the operator explicitly uses `wildvine plugins install --dangerously-force-unsafe-install`.
...
- **`origin`**: Normalized install origin when available (for example `wildvine-bundled`, `wildvine-workspace`, `plugin-bundle`, `plugin-package`, or `plugin-file`)
...
```json
{
  ...
  "sourcePath": "/var/folders/.../wildvine-plugin-acme-audit/package",
  "sourcePathKind": "directory",
  "origin": "plugin-package",
  "request": {
    "kind": "plugin-npm",
    "mode": "install",
    "requestedSpecifier": "@acme/wildvine-plugin-audit@1.4.2"
  },
  "builtinScan": {
    "status": "ok",
    "scannedFiles": 12,
  ...
  "plugin": {
    "pluginId": "acme-audit",
    "contentType": "package",
    "packageName": "@acme/wildvine-plugin-audit",
...
- **Managed hooks** (`~/.wildvine/hooks/`): Shared across workspaces; can override bundled and plugin hooks

### 2. Create Directory Structure

```bash
mkdir -p ~/.wildvine/hooks/my-hook
cd ~/.wildvine/hooks/my-hook
```
...
metadata: { "wildvine": { "emoji": "🎯", "events": ["command:new"] } }
---
...
wildvine hooks list --eligible

# Verbose output (show missing requirements)
wildvine hooks list --verbose

# JSON output
wildvine hooks list --json
```
...
# JSON output
wildvine hooks info session-memory --json
```
...
# Disable a hook
wildvine hooks disable command-logger
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.wildvine/workspace`)
...
```bash
wildvine hooks enable bootstrap-extra-files
```
...
**Output**: `~/.wildvine/logs/commands.log`
...
tail -n 20 ~/.wildvine/logs/commands.log

# Pretty-print with jq
cat ~/.wildvine/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.wildvine/logs/commands.log | jq .
```

**Enable**:

```bash
wildvine hooks enable command-logger
```
...
```yaml
metadata: { "wildvine": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "wildvine": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.wildvine/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.wildvine/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.wildvine/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/PROACTIVA-US/wildvine/tree/main/src/hooks/bundled)
