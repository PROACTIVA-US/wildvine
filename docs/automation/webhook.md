...
read_when:
  ...
  - Wiring external systems into Wildvine
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Wildvine config directory (typically `~/.wildvine/hooks/transforms`).
...
- `wildvine webhooks gmail setup` writes `hooks.gmail` config for `wildvine webhooks gmail run`.
