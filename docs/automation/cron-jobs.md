...
- `sessionTarget: "isolated"` or `sessionTarget: "session:..."` creates a visible task that shows up in `wildvine tasks` with delivery notifications.
...
- Jobs persist under `~/.wildvine/cron/` so restarts don’t lose schedules.
...
- For upgrades, `wildvine doctor --fix` can normalize legacy cron store fields, including old top-level delivery hints such as `threadId`.
...
wildvine cron list
wildvine cron run <job-id>
wildvine cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.wildvine/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `wildvine cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Wildvine applies a
...
Main-session cron jobs create [background task](/automation/tasks) records with `silent` notify policy (no notifications by default). They appear in `wildvine tasks list` but do not generate delivery messages.
...
These detached runs create [background task](/automation/tasks) records visible in `wildvine tasks` and subject to task audit and maintenance.
...
If `delivery` is omitted for isolated jobs, Wildvine defaults to `announce`.
...
- CLI equivalents: `wildvine cron add --light-context ...` and `wildvine cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `wildvine cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.wildvine/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.wildvine/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Wildvine classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Wildvine archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `wildvine cron runs` and adjust retention before logs become large
...
wildvine cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
wildvine cron edit <jobId> --agent ops
wildvine cron edit <jobId> --clear-agent
```
...
```bash
wildvine cron edit <jobId> --exact
```
...
```bash
wildvine system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`wildvine system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `WILDVINE_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Wildvine applies exponential retry backoff for recurring jobs after consecutive errors:
