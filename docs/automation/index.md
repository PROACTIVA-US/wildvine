...
read_when:
  - Deciding how to automate work with Wildvine
...
Wildvine provides several automation mechanisms, each suited to different use cases. This page helps you choose the right one.
...
Older release notes and docs may mention `ClawFlow` or `wildvine flows`, but the current CLI surface in this repo is `wildvine tasks`.

See [Background Tasks](/automation/tasks) for the supported task ledger commands, plus [ClawFlow](/automation/vineflow) and [CLI: flows](/cli/flows) for compatibility notes.
