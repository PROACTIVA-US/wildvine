...
- `wildvine tasks list` shows all tasks; `wildvine tasks audit` surfaces issues.
...
wildvine tasks list --runtime acp
wildvine tasks list --status running
...
wildvine tasks notify <lookup> state_changes

# Run a health audit
wildvine tasks audit
```
...
| CLI operations         | `cli`        | `wildvine agent` commands that run through the gateway | `done_only`           |
...
| `cancelled` | Stopped by the operator via `wildvine tasks cancel`                        |
...
When a task reaches a terminal state, Wildvine notifies you. There are two delivery paths:
...
```bash
wildvine tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
wildvine tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
wildvine tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `wildvine status` when issues are detected.
...
For the full operator ledger, use the CLI: `wildvine tasks list`.

## Status integration (task pressure)

`wildvine status` includes an at-a-glance task summary:
...
```
$WILDVINE_STATE_DIR/tasks/runs.sqlite
```
...
Some older Wildvine release notes and docs referred to task management as `ClawFlow` and documented an `wildvine flows` command surface.

In the current codebase, the supported operator surface is `wildvine tasks`. See [ClawFlow](/automation/vineflow) and [CLI: flows](/cli/flows) for compatibility notes that map those older references to the current task commands.

### Tasks and cron

A cron job **definition** lives in `~/.wildvine/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
...
- [ClawFlow](/automation/vineflow) — compatibility note for older docs and release notes
