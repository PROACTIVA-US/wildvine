...
read_when:
  - You want to connect Wildvine to LINE
...
LINE connects to Wildvine via the LINE Messaging API. The plugin runs as a webhook
...
```bash
wildvine plugins install @wildvine/line
```

Local checkout (when running from a git repo):

```bash
wildvine plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Wildvine applies strict pre-auth body limits and timeout before verification.
- Wildvine processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
wildvine pairing list line
wildvine pairing approve line <CODE>
```
