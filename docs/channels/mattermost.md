---
summary: "Mattermost bot setup and Wildvine config"
read_when:
...
```bash
wildvine plugins install @wildvine/mattermost
```

Local checkout (when running from a git repo):

```bash
wildvine plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Wildvine and start the gateway.
...
Native slash commands are opt-in. When enabled, Wildvine registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Wildvine derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Wildvine.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Wildvine.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Wildvine, not `404`.
...
- Approve via:
  - `wildvine pairing list mattermost`
  - `wildvine pairing approve mattermost <CODE>`
...
Use these target formats with `wildvine message send` or cron/webhooks:
...
Wildvine resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Wildvine sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Wildvine sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Wildvine derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Wildvine run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="wildvine-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"wildvine-mattermost-interactions",
...
`wildvine message send` and cron/webhook deliveries.
