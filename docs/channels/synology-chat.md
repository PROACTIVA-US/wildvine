---
summary: "Synology Chat webhook setup and Wildvine config"
read_when:
  - Setting up Synology Chat with Wildvine
...
```bash
wildvine plugins install ./path/to/local/synology-chat-plugin
```
...
1. Install and enable the Synology Chat plugin.
   - `wildvine onboard` now shows Synology Chat in the same channel setup list as `wildvine channels add`.
   - Non-interactive setup: `wildvine channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Wildvine gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Wildvine.
   - Guided: `wildvine onboard`
   - Direct: `wildvine channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `wildvine pairing list synology-chat`
  - `wildvine pairing approve synology-chat <CODE>`
...
```bash
wildvine message send --channel synology-chat --target 123456 --text "Hello from Wildvine"
wildvine message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Wildvine now rejects duplicate exact paths
