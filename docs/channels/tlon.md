...
Tlon is a decentralized messenger built on Urbit. Wildvine connects to your Urbit ship and can
...
```bash
wildvine plugins install @wildvine/tlon
```

Local checkout (when running from a git repo):

```bash
wildvine plugins install ./path/to/local/tlon-plugin
```
...
By default, Wildvine blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `wildvine message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Wildvine replies in-thread.
