...
read_when:
  - You want to connect Wildvine to QQ
...
QQ Bot connects to Wildvine via the official QQ Bot API (WebSocket gateway). The
...
## Bundled with Wildvine

Current Wildvine installs bundle QQ Bot. You do not need a separate
`wildvine plugins install` step for normal setup.
...
```bash
wildvine channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `wildvine channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single Wildvine instance:
...
```bash
wildvine channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
