...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Wildvine to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
wildvine plugins install @wildvine/feishu
```

Local checkout (when running from a git repo):

```bash
wildvine plugins install ./extensions/feishu
```
...
If you just installed Wildvine, run the wizard:
...
2. Configuring app credentials in Wildvine
...
1. You already ran `wildvine channels add` for Feishu
2. The gateway is running (`wildvine gateway status`)
...
```bash
wildvine pairing approve feishu <CODE>
```
...
2. Run `wildvine logs --follow` and look for `chat_id`
...
2. Run `wildvine logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `wildvine gateway status`  | Show gateway status           |
| `wildvine gateway install` | Install/start gateway service |
| `wildvine gateway stop`    | Stop gateway service          |
| `wildvine gateway restart` | Restart gateway service       |
| `wildvine logs --follow`   | Tail gateway logs             |
...
4. Check logs: `wildvine logs --follow`
...
5. Ensure the gateway is running: `wildvine gateway status`
6. Check logs: `wildvine logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "wildvine-fan",
        workspace: "/home/user/wildvine-fan",
        agentDir: "/home/user/.wildvine/agents/wildvine-fan/agent",
      },
      {
        id: "wildvine-xi",
        workspace: "/home/user/wildvine-xi",
        agentDir: "/home/user/.wildvine/agents/wildvine-xi/agent",
      },
    ],
  },
  bindings: [
