...
read_when:
  - Setting up Twitch chat integration for Wildvine
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Wildvine connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
wildvine plugins install @wildvine/twitch
```

Local checkout (when running from a git repo):

```bash
wildvine plugins install ./path/to/local/twitch-plugin
```
...
4. Configure the token:
   - Env: `WILDVINE_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "wildvine", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use WILDVINE_TWITCH_ACCESS_TOKEN env var)
...
```bash
WILDVINE_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
