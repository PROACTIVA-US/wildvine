...
read_when:
  - You want Wildvine to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Wildvine to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`wildvine onboard`) and `wildvine channels add` list optional channel plugins.
...
```bash
wildvine plugins install @wildvine/nostr
```

Use a local checkout (dev workflows):

```bash
wildvine plugins install --link <path-to-local-nostr-plugin>
```
...
wildvine channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "wildvine",
        displayName: "Wildvine",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "wildvine@example.com",
        lud16: "wildvine@example.com",
      },
    },
  },
}
