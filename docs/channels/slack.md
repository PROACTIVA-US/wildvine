...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Wildvine HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `wildvine pairing approve slack <code>`.
...
- The interactive callback values are Wildvine-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Wildvine falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Wildvine is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Wildvine",
    "description": "Slack connector for Wildvine"
  },
  "features": {
    "bot_user": {
      "display_name": "Wildvine",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/wildvine",
        "description": "Send a message to Wildvine",
        "should_escape": false
      }
    ]
  },
...
Wildvine supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Wildvine falls back to normal delivery for remaining payloads.
