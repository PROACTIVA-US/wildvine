...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    Wildvine can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `wildvine doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `wildvine doctor --fix` can resolve legacy `@username` entries to IDs.
