...
read_when:
  - Setting up Zalo Personal for Wildvine
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Wildvine.
...
- Install via CLI: `wildvine plugins install @wildvine/zalouser`
- Or from a source checkout: `wildvine plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `wildvine channels login --channel zalouser`
...
```bash
wildvine directory self --channel zalouser
wildvine directory peers list --channel zalouser --query "name"
wildvine directory groups list --channel zalouser --query "work"
```
...
- `wildvine pairing list zalouser`
- `wildvine pairing approve zalouser <code>`
...
- On startup, Wildvine resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Wildvine stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Wildvine state. Example:
...
- Wildvine sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Wildvine sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `wildvine channels logout --channel zalouser && wildvine channels login --channel zalouser`
...
- The channel now runs fully in Wildvine without external CLI binaries.
