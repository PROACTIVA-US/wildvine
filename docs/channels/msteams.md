...
```bash
wildvine plugins install @wildvine/msteams
```

Local checkout (when running from a git repo):

```bash
wildvine plugins install ./path/to/local/msteams-plugin
```

If you choose Teams during setup and a git checkout is detected,
Wildvine will offer the local install path automatically.
...
3. Configure Wildvine with those credentials.
...
- Talk to Wildvine via Teams DMs, group chats, or channels.
...
- On startup, Wildvine resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.wildvine/wildvine.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Wildvine, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `wildvine-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `wildvine plugins install @wildvine/msteams`
   - From a local checkout: `wildvine plugins install ./path/to/local/msteams-plugin`
...
Wildvine exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Wildvine in Teams", full: "Wildvine in Teams" },
...
Wildvine handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Wildvine only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/WildvineShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Wildvine sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `wildvine message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.wildvine/msteams-polls.json`.
...
```bash
wildvine message send --channel msteams \
...
wildvine message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
wildvine message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
wildvine message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
wildvine message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
