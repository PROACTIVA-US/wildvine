...
Matrix is the Matrix channel plugin for Wildvine.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Plugin required

Matrix is a plugin and is not bundled with core Wildvine.

Install from npm:

```bash
wildvine plugins install @wildvine/matrix
```

Install from a local checkout:

```bash
wildvine plugins install ./path/to/local/matrix-plugin
```
...
```bash
wildvine channels add
wildvine configure --section channels
```
...
- To resolve room names before saving them, use `wildvine channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "Wildvine Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.wildvine/credentials/matrix/`.
...
Set `channels.matrix.streaming` to `"partial"` when you want Wildvine to send a single draft reply,
...
- `streaming: "off"` is the default. Wildvine waits for the final reply and sends it once.
...
- If the preview no longer fits in one Matrix event, Wildvine stops preview streaming and falls back to normal final delivery.
- Media replies still send attachments normally. If a stale preview can no longer be reused safely, Wildvine redacts it before sending the final media reply.
...
By default, Matrix messages from other configured Wildvine Matrix accounts are ignored.
...
- Wildvine still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; Wildvine treats "bot-authored" as "sent by another configured Matrix account on this Wildvine gateway".
...
```bash
wildvine matrix verify status --include-recovery-key --json
```
...
```bash
wildvine matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
wildvine matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
wildvine matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
wildvine matrix verify backup restore
```

Verbose restore diagnostics:

```bash
wildvine matrix verify backup restore --verbose
```
...
```bash
wildvine matrix verify status --account assistant
wildvine matrix verify backup restore --account assistant
wildvine matrix devices list --account assistant
```
...
Wildvine treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `wildvine matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for Wildvine to treat the device as fully verified.

### What bootstrap does

`wildvine matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, Wildvine tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
If you intentionally want to discard the current room-key backup and start a new backup baseline for future messages, use `wildvine matrix verify backup reset --yes`.
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, Wildvine can attempt a stricter repair path.
If the current device is already owner-signed, Wildvine preserves that identity instead of resetting it automatically.

Upgrading from the previous public Matrix plugin:

- Wildvine automatically reuses the same Matrix account, access token, and device identity when possible.
- Before any actionable Matrix migration changes run, Wildvine creates or reuses a recovery snapshot under `~/Backups/wildvine-migrations/`.
- If you use multiple Matrix accounts, set `channels.matrix.defaultAccount` before upgrading from the old flat-store layout so Wildvine knows which account should receive that shared legacy state.
- If the previous plugin stored a Matrix room-key backup decryption key locally, startup or `wildvine doctor --fix` will import it into the new recovery-key flow automatically.
...
- If the Matrix access token changes later for the same account, homeserver, and user, Wildvine now prefers reusing the most complete existing token-hash storage root instead of starting from an empty Matrix state directory.
- On the next gateway start, backed-up room keys are restored automatically into the new crypto store.
- If the old plugin had local-only room keys that were never backed up, Wildvine will warn clearly. Those keys cannot be exported automatically from the previous rust crypto store, so some old encrypted history may remain unavailable until recovered manually.
...
`~/.wildvine/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, Wildvine reuses the best existing
...
Wildvine currently provides that in Node by:
...
Under Wildvine's security model, the gateway host and local Wildvine state directory are already inside the trusted operator boundary, so this is primarily an operational durability concern rather than a separate remote trust boundary.

Planned improvement:

- add SecretRef support for persistent Matrix key material so recovery keys and related store-encryption secrets can be sourced from Wildvine secrets providers instead of only local files
...
```bash
wildvine matrix profile set --name "Wildvine Assistant"
wildvine matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

Add `--account <id>` when you want to target a named Matrix account explicitly.

Matrix accepts `mxc://` avatar URLs directly. When you pass an `http://` or `https://` avatar URL, Wildvine uploads it to Matrix first and stores the resolved `mxc://` URL back into `channels.matrix.avatarUrl` (or the selected account override).
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by Wildvine.
For self-verification flows, Wildvine also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, Wildvine auto-accepts the request and then waits for the SAS flow to proceed normally.
...
Wildvine does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old Wildvine-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
wildvine matrix devices list
```

Remove stale Wildvine-managed devices with:

```bash
wildvine matrix devices prune-stale
```

### Direct Room Repair

If direct-message state gets out of sync, Wildvine can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
wildvine matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
wildvine matrix direct repair --user-id @alice:example.org
```
...
- `threadBindings.spawnAcpSessions` is only required for `/acp spawn --thread auto|here`, where Wildvine needs to create or bind a child Matrix thread.
...
Ack reactions use the standard Wildvine resolution order:
...
- Matrix room history is pending-only: Wildvine buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
wildvine pairing list matrix
wildvine pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, Wildvine reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.
...
Partial shared auth defaults do not create a separate implicit default account by themselves. Wildvine only synthesizes the top-level `default` account when that default has fresh auth (`homeserver` plus `accessToken`, or `homeserver` plus `userId` and `password`); named accounts can still stay discoverable from `homeserver` plus `userId` when cached credentials satisfy auth later.
Set `defaultAccount` when you want Wildvine to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `wildvine matrix verify ...` and `wildvine matrix devices ...` when you want to override that implicit selection for one command.

## Private/LAN homeservers

By default, Wildvine blocks private/internal Matrix homeservers for SSRF protection unless you
...
Wildvine uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere Wildvine asks you for a room or user target:
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; Wildvine does not trust alias state claimed by the invited room.
