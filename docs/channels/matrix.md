...
Matrix is an open, decentralized messaging protocol. Wildvine connects as a Matrix **user**
...
```bash
wildvine plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `wildvine plugins install @wildvine/matrix`
   - From a local checkout: `wildvine plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Wildvine calls the same
     login endpoint, stores the access token in `~/.wildvine/credentials/matrix/credentials.json`,
...
- On first connection, Wildvine requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Wildvine logs a warning.
...
`~/.wildvine/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Wildvine resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
