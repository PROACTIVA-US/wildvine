...
```bash
wildvine plugins install @wildvine/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
wildvine plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Wildvine" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `wildvine pairing list nextcloud-talk`
  - `wildvine pairing approve nextcloud-talk <CODE>`
