...
- `/usage cost` in chats: local cost summary aggregated from Wildvine session logs.
- CLI: `wildvine status --usage` prints a full per-provider breakdown.
- CLI: `wildvine channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
