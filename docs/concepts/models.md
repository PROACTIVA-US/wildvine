...
Wildvine selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Wildvine can use (plus aliases).
...
- If you omit the provider, Wildvine treats the input as an alias or a model for the **default provider** (only works when there is no `/` in the model ID).
...
wildvine models set <provider/model>
wildvine models set-image <provider/model>

wildvine models aliases list
wildvine models aliases add <alias> <provider/model>
wildvine models aliases remove <alias>

wildvine models fallbacks list
wildvine models fallbacks add <provider/model>
wildvine models fallbacks remove <provider/model>
wildvine models fallbacks clear

wildvine models image-fallbacks list
wildvine models image-fallbacks add <provider/model>
wildvine models image-fallbacks remove <provider/model>
wildvine models image-fallbacks clear
```

`wildvine models` (no subcommand) is a shortcut for `models status`.
...
`wildvine models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.wildvine/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Wildvine writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Wildvine regenerates `models.json`, including command-driven paths like `wildvine agent`.
