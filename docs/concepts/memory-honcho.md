...
[Honcho](https://honcho.dev) adds AI-native memory to Wildvine. It persists
...
```bash
wildvine plugins install @honcho-ai/wildvine-honcho
wildvine honcho setup
...
Settings live under `plugins.entries["wildvine-honcho"].config`:
    ...
    entries: {
      "wildvine-honcho": {
        config: {
          ...
          workspaceId: "wildvine", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `wildvine honcho setup` detects and
...
```bash
wildvine honcho setup                        # Configure API key and migrate files
wildvine honcho status                       # Check connection status
wildvine honcho ask <question>               # Query Honcho about the user
wildvine honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/wildvine-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Wildvine integration guide](https://docs.honcho.dev/v3/guides/integrations/wildvine)
- [Memory](/concepts/memory) -- Wildvine memory overview
