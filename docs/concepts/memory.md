...
summary: "How Wildvine remembers things across sessions"
read_when:
...
Wildvine remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.wildvine/workspace`).
...
<Info>
Wildvine auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Wildvine
...
```bash
wildvine memory status          # Check index status and provider
wildvine memory search "query"  # Search from the command line
wildvine memory index --force   # Rebuild the index
```
