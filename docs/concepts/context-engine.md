...
read_when:
  - You want to understand how Wildvine assembles model context
...
A **context engine** controls how Wildvine builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Wildvine ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.wildvine/wildvine.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Wildvine plugin. Install
...
wildvine plugins install -l ./my-context-engine
```
...
Every time Wildvine runs a model prompt, the context engine participates at
...
Wildvine currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Wildvine
...
The built-in `legacy` engine preserves Wildvine's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Wildvine uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Wildvine disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Wildvine automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `wildvine/plugin-sdk/core` to use
  Wildvine's built-in compaction behavior.
...
Wildvine resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Wildvine's built-in summarization. Plugin engines can implement
...
- Use `wildvine doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Wildvine
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `wildvine plugins install -l ./my-engine` to link a
