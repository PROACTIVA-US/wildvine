---
summary: "How Wildvine manages conversation sessions"
read_when:
...
Wildvine organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `wildvine security audit`.
...
Wildvine automatically bounds session storage over time. By default, it runs
...
Preview with `wildvine sessions cleanup --dry-run`.

## Inspecting sessions

- `wildvine status` -- session store path and recent activity.
- `wildvine sessions --json` -- all sessions (filter with `--active <minutes>`).
