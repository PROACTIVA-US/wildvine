...
alongside Wildvine. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Wildvine falls back to the
...
Wildvine creates a self-contained QMD home under
`~/.wildvine/agents/<agentId>/qmd/` and manages the sidecar lifecycle
automatically -- collections, updates, and embedding runs are handled for you.

## How the sidecar works

- Wildvine creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, Wildvine retries with `qmd query`.
- If QMD fails entirely, Wildvine falls back to the builtin SQLite engine.
...
collection under `~/.wildvine/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Wildvine logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Wildvine
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Wildvine uses.
...
Wildvine's builtin symlink rules. Keep temporary monorepo checkouts under
