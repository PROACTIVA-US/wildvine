...
read_when:
  - You need detailed behavior for wildvine onboard
...
This page is the full reference for `wildvine onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.wildvine/wildvine.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `wildvine doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `wildvine pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `wildvine health`.
    - `wildvine status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.wildvine/.env` so launchd can read it.
...
- OAuth credentials: `~/.wildvine/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.wildvine/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.wildvine/credentials/oauth.json` (or `$WILDVINE_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`wildvine onboard`](/cli/onboard)
