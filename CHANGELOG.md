...
- Config/Discord: coerce safe integer numeric Discord IDs to strings during config validation, keep unsafe or precision-losing numeric snowflakes rejected, and align `wildvine doctor` repair guidance with the same fail-closed behavior. (#45125) Thanks @moliendocode.
...
- Cron/tools allowlist: add `wildvine cron --tools` for per-job tool allowlists. (#58504) Thanks @andyk-ms.
...
- Exec/cron: resolve isolated cron no-route approval dead-ends from the effective host fallback policy when trusted automation is allowed, and make `wildvine doctor` warn when `tools.exec` is broader than `~/.wildvine/exec-approvals.json` so stricter host-policy conflicts are explicit. Thanks @scoootscooob and @vincentkoc.
...
- Channels/plugins: keep bundled channel plugins loadable from legacy `channels.<id>` config even under restrictive plugin allowlists, and make `wildvine doctor` warn only on real plugin blockers instead of misleading setup guidance. (#58873) Thanks @obviyus
...
- Sandbox/browser: compare browser runtime inspection against `agents.defaults.sandbox.browser.image` so `wildvine sandbox list --browser` stops reporting healthy browser containers as image mismatches. (#58759) Thanks @sandpile.
...
- Plugin SDK: deprecate the legacy provider compat subpaths plus the older bundled provider setup and channel-runtime compatibility shims, emit migration warnings, and keep the current documented `wildvine/plugin-sdk/*` entrypoints plus local `api.ts` / `runtime-api.ts` barrels as the forward path ahead of a future major-release removal.
...
- Background tasks: add the first linear task flow control surface with `wildvine tasks list|show|cancel`, keep manual multi-task flows separate from one-task auto-sync flows, and surface doctor recovery hints for obviously orphaned or broken flow/task linkage. Thanks @mbelinky and @vincentkoc.
...
- Auth/profiles: keep the last successful config load in memory for the running process and refresh that snapshot on successful writes/reloads, so hot paths stop reparsing `wildvine.json` between watcher-driven swaps.
...
- Config/update: stop `wildvine doctor` write-backs from persisting plugin-injected channel defaults, so `wildvine update` no longer seeds config keys that later break service refresh validation. (#56834) Thanks @openperf.
...
- Gateway/OpenAI HTTP: restore default operator scopes for bearer-authenticated requests that omit `x-wildvine-scopes`, so headless `/v1/chat/completions` and session-history callers work again after the recent method-scope hardening. (#57596) Thanks @openperf.
...
- Agents/MCP: dispose bundled MCP runtimes after one-shot `wildvine agent --local` runs finish, while preserving bundled MCP state across in-run retries so local JSON runs exit cleanly without restarting stateful MCP tools mid-run.
...
- Tasks: add a small task-flow runtime substrate for authoring layers with persisted wait targets and output bags, plus bundled skills/Vine examples and richer `flows show` / `doctor` recovery hints for multi-task flow state. (#58336) Thanks @mbelinky and @vincentkoc.
- Config/legacy cleanup: stop probing obsolete alternate legacy config names and service labels during local config/service detection, while keeping the active `~/.wildvine/wildvine.json` path canonical.
- Config/runtime: pin the first successful config load in memory for the running process and refresh that snapshot on successful writes/reloads, so hot paths stop reparsing `wildvine.json` between watcher-driven swaps.
- Config/SecretRef + Control UI: harden SecretRef redaction round-trip restore, block unsafe raw fallback (force Form mode when raw is unavailable), and preflight submitted-config SecretRefs before config write RPC persistence. (#58044) Thanks @joshavant.
- Config/Telegram: migrate removed `channels.telegram.groupMentionsOnly` into `channels.telegram.groups["*"].requireMention` on load so legacy configs no longer crash at startup. (#55336) thanks @jameslcowan.
- Config/update: stop `wildvine doctor` write-backs from persisting plugin-injected channel defaults, so `wildvine update` no longer seeds config keys that later break service refresh validation. (#56834) Thanks @openperf.
- Control UI/agents: auto-load agent workspace files on initial Files panel open, and populate overview model/workspace/fallbacks from effective runtime agent metadata so defaulted models no longer show as `Not set`. (#56637) Thanks @dxsx84.
...
- LINE/ACP: add current-conversation binding and inbound binding-routing parity so `/acp spawn ... --thread here`, configured ACP bindings, and active conversation-bound ACP sessions work on LINE like the other conversation channels.
- LINE/markdown: preserve underscores inside Latin, Cyrillic, and CJK words when stripping markdown, while still removing standalone `_italic_` markers on the shared text-runtime path used by LINE and TTS. (#47465) Thanks @jackjin1997.
- LINE/status: stop `wildvine status` from warning about missing credentials when sanitized LINE snapshots are already configured, while still surfacing whether the missing field is the token or secret. (#45701) Thanks @tamaosamu.
- macOS/local gateway: stop Wildvine.app from killing healthy local gateway listeners after startup by recognizing the current `wildvine-gateway` process title and using the current `wildvine gateway` launch shape.
...
- Matrix/CLI send: start one-off Matrix send clients before outbound delivery so `wildvine message send --channel matrix` restores E2EE in encrypted rooms instead of sending plain events. (#57936) Thanks @gumadeiras.
...
- Memory/doctor: probe QMD availability from the agent workspace too, so `wildvine doctor` no longer falsely reports relative `memory.qmd.command` configs as broken while runtime search still works. Thanks @vincentkoc.
...
- Memory/QMD: pass a direct-session key on `wildvine memory search` so CLI QMD searches no longer get denied as `session=<none>` under direct-only scope defaults. (#43517) Thanks @waynecc-at and @vincentkoc.
...
- Memory/QMD: stop rewriting Han/CJK BM25 queries before `qmd search`, so Wildvine search semantics match direct QMD results for mixed and spaced Chinese queries. Thanks @vincentkoc.
- Memory/QMD: surface degraded vector status from `qmd status` so `wildvine memory status --deep` warns when semantic search is unavailable because the index still has `0` vectors. Fixes #28169. Thanks @vincentkoc.
...
- Plugins/ClawHub: sanitize temporary archive filenames for scoped package names and slash-containing skill slugs so `wildvine plugins install @scope/name` no longer fails with `ENOENT` during archive download. (#56452) Thanks @soimy.
- Plugins/CLI: add descriptor-backed lazy plugin CLI registration so Matrix can keep its CLI module lazy-loaded without dropping `wildvine matrix ...` from parse-time command registration. (#57165) Thanks @gumadeiras.
...
- Plugins/startup: block workspace `.env` from overriding `WILDVINE_BUNDLED_PLUGINS_DIR`, so bundled plugin trust roots only come from inherited runtime env or package resolution instead of repo-local dotenv files. Thanks @nexrin and @vincentkoc.
...
- Status/node-only hosts: teach `wildvine status` to handle node-only hosts on current `main` without the old mixed gateway assumptions. (#56718) Thanks @ImLukeF.
...
- ClawDock/docs: move the helper scripts to `scripts/vinedock`, publish ClawDock as a first-class docs page on the docs site, and document reinstalling local helper copies from the new raw GitHub path. (#23912) thanks @Olshansk.
...
- Media/file handling: create image temp work directories under the shared Wildvine temp root and stop widening allowed local media roots from reply or tool source paths, keeping local media access limited to configured and agent-scoped roots. (#58270, #57770)
...
- Env and filesystem hardening: block credential and gateway auth env vars from workspace `.env` files, always strip dangerous inherited host env vars such as `BROWSER` and `GIT_EDITOR`, and keep sensitive host paths and Wildvine state roots blocked even when external sandbox bind sources are allowed. (#57767, #57559, #56024)
...
- xAI/onboarding: let the bundled Grok web-search plugin offer optional `x_search` setup during `wildvine onboard` and `wildvine configure --section web`, including an x_search model picker with the shared xAI key.
...
- Podman: simplify the container setup around the current rootless user, install the launch helper under `~/.local/bin`, and document the host-CLI `wildvine --container <name> ...` workflow instead of a dedicated `wildvine` service user.
...
- CLI: add `wildvine config schema` to print the generated JSON schema for `wildvine.json`. (#54523) Thanks @kvokka.
...
- Tavily: mark outbound API requests with `X-Client-Source: wildvine` so Tavily can attribute Wildvine-originated traffic. (#55335) Thanks @lakshyaag-tavily.
...
- CLI/message send: write manual `wildvine message send` deliveries into the resolved agent session transcript again by always threading the default CLI agent through outbound mirroring. (#54187) Thanks @KevInTheCloud5617.
...
- Plugins/SDK: thread `moduleUrl` through plugin-sdk alias resolution so user-installed plugins outside the wildvine directory correctly resolve `wildvine/plugin-sdk/*` subpath imports, and gate `plugin-sdk:check-exports` in `release:check`. (#54283) Thanks @xieyongliang.
...
- Daemon/Linux: stop flagging non-gateway systemd services as duplicate gateways just because their unit files mention Wildvine, reducing false-positive doctor/log noise. (#45328) Thanks @gregretkowski.
...
- Daemon/status: surface immediate gateway close reasons from lightweight probes and prefer those concrete auth or pairing failures over generic timeouts in `wildvine daemon status`. (#56282) Thanks @mbelinky.
...
- Status/port diagnostics: treat single-process dual-stack loopback gateway listeners as healthy in `wildvine status --all`, suppressing false "port already in use" conflict warnings. (#53398) Thanks @DanWebb1949.

### Breaking

- Providers/Qwen: remove the deprecated `qwen-portal-auth` OAuth integration for `portal.qwen.ai`; migrate to Model Studio with `wildvine onboard --auth-choice modelstudio-api-key`. (#52709) Thanks @pomelo-nwu.
- Config/Doctor: drop automatic config migrations older than two months; very old legacy keys now fail validation instead of being rewritten on load or by `wildvine doctor`.
...
- CLI/containers: add `--container` and `WILDVINE_CONTAINER` to run `wildvine` commands inside a running Docker or Podman Wildvine container. (#52651) Thanks @sallyom.
...
- CLI/skills: soften missing-requirements label from "missing" to "needs setup" and surface API key setup guidance (where to get a key, CLI save command, storage path) in `wildvine skills info` output. (#53411) Thanks @BunsDev.
...
- CLI/update: preflight the target npm package `engines.node` before `wildvine update` runs a global package install, so outdated Node runtimes fail with a clear upgrade message instead of attempting an unsupported latest release.
...
- Docker/setup: avoid the pre-start `wildvine-cli` shared-network namespace loop by routing setup-time onboard/config writes through `wildvine-gateway`, so fresh Docker installs stop failing before the gateway comes up. (#53385) Thanks @amsminn.
...
- Security/path resolution: prefer non-user-writable absolute helper binaries for Wildvine CLI, ffmpeg, and OpenSSL resolution so PATH hijacks cannot replace trusted helpers with attacker-controlled executables.
...
- Telegram/photos: preflight Telegram photo dimension and aspect-ratio rules, and fall back to document sends when image metadata is invalid or unavailable so photo uploads stop failing with `PHOTO_INVALID_DIMENSIONS`. (#52545) Thanks @hnshah.
- Slack/runtime defaults: trim Slack DM reply overhead, restore Codex auto transport, and tighten Slack/web-search runtime defaults around DM preview threading, cache scoping, warning dedupe, and explicit web-search opt-in. (#53957) Thanks @vincentkoc.
- Doctor/image generation: seed migrated legacy Nano Banana Google provider config with the `/v1beta` API root and an empty model list so `wildvine doctor --fix` completes and the migrated native Google image path keeps hitting the correct endpoint. (#53757) Thanks @mahopan.
...
- Gateway/ports: parse Docker Compose-style `WILDVINE_GATEWAY_PORT` host publish values correctly without reviving the legacy `CLAWDBOT_GATEWAY_PORT` override. (#44083) Thanks @bebule.
...
- Plugins/uninstall: accept installed `clawhub:` specs and versionless ClawHub package names as uninstall targets, so `wildvine plugins uninstall clawhub:<package>` works again even when the recorded install was pinned to a version.
...
- Mistral/models: lower bundled Mistral max-token defaults to safe output budgets and teach `wildvine doctor --fix` to repair old persisted Mistral provider configs that still carry context-sized output limits, avoiding deterministic Mistral 422 rejects on fresh and existing setups. Fixes #52599. Thanks @vincentkoc.
- ClawHub/macOS auth: honor macOS auth config and XDG auth paths for saved ClawHub credentials, so `wildvine skills ...` and gateway skill browsing keep using the signed-in auth state instead of silently falling back to unauthenticated mode. Fixes #53034.
...
- Config/warnings: suppress the confusing "newer Wildvine" warning when a config written by a same-base correction release like `2026.3.23-2` is read by `2026.3.23`, while still warning for truly newer or incompatible versions.
- CLI/cron: make `wildvine cron add|edit --at ... --tz <iana>` honor the requested local wall-clock time for offset-less one-shot datetimes, including DST boundaries, and keep `--tz` rejected for `--every`. (#53224) Thanks @RolfHegr.
...
- Doctor/plugins: make `wildvine doctor --fix` remove stale `plugins.allow` and `plugins.entries` refs left behind after plugin removal. Thanks @sallyom
...
- ClawHub/skills: keep updating already-tracked legacy Unicode slugs after the ASCII-only slug hardening, so older installs do not get stuck behind `Invalid skill slug` errors during `wildvine skills update`. (#53206) Thanks @drobison00.
...
- Plugins/memory-lancedb: bootstrap LanceDB into plugin runtime state on first use when the bundled npm install does not already have it, so `plugins.slots.memory="memory-lancedb"` works again after global npm installs without moving LanceDB into Wildvine core dependencies. Fixes #26100.
...
- Doctor/WhatsApp: stop auto-enable from appending built-in channel ids like `whatsapp` to `plugins.allow`, so `wildvine doctor --fix` no longer writes schema-invalid plugin allowlist entries when repairing built-in channels. Fixes #52931. Thanks @vincentkoc.
...
- Release/install: keep previously released bundled plugins and Control UI assets in published wildvine npm installs, and fail release checks when those shipped artifacts are missing. Thanks @vincentkoc.
- WhatsApp/outbound sends: keep the active Web listener on a direct process-global symbol so split runtime chunks keep sharing the connected Baileys session and `wildvine message send --channel whatsapp` stops failing after connect. Fixes #52574. Thanks @MonkeyLeeT.
...
- Plugins/install: bare `wildvine plugins install <package>` now prefers ClawHub before npm for npm-safe names, and only falls back to npm when ClawHub does not have that package or version. Docs: https://docs.wildvine.com/tools/clawhub
- Browser/Chrome MCP: remove the legacy Chrome extension relay path, bundled extension assets, `driver: "extension"`, and `browser.relayBindHost`. Run `wildvine doctor --fix` to migrate host-local browser config to `existing-session` / `user`; Docker, headless, sandbox, and remote browser flows still use raw CDP. Docs: https://docs.wildvine.com/gateway/doctor and https://docs.wildvine.com/tools/browser (#47893) Thanks @vincentkoc.
...
- Plugins/SDK: the new public plugin SDK surface is `wildvine/plugin-sdk/*`; `wildvine/extension-api` is removed with no compatibility shim. Bundled plugins must use injected runtime for host-side operations (for example `api.runtime.agent.runEmbeddedPiAgent`) and any remaining direct imports must come from narrow `wildvine/plugin-sdk/*` subpaths instead of the monolithic SDK root. Docs: https://docs.wildvine.com/plugins/sdk-migration and https://docs.wildvine.com/plugins/sdk-overview
...
- Plugins/Matrix: add a new Matrix plugin backed by the official `matrix-js-sdk`. If you are upgrading from the previous public Matrix plugin, follow the migration guide: https://docs.wildvine.com/install/migrating-matrix Thanks @gumadeiras.
- Config/env: remove legacy `CLAWDBOT_*` and `MOLTBOT_*` compatibility env names across runtime, installers, and test tooling. Use the matching `WILDVINE_*` env names instead.
- Config/state: remove legacy `.moltbot` state-dir and `moltbot.json` auto-detection/migration fallback. If you still keep state under `~/.moltbot`, move it to `~/.wildvine` or set `WILDVINE_STATE_DIR` / `WILDVINE_CONFIG_PATH` explicitly. Docs: https://docs.wildvine.com/install/migrating and https://docs.wildvine.com/start/getting-started
...
- Discord/commands: switch native command deployment to Carbon reconcile by default so Discord restarts stop churning slash commands through Wildvine's local deploy path. (#46597) Thanks @huntharo and @thewilloftheshadow.
...
- ClawHub/install: add native `wildvine skills search|install|update` flows plus `wildvine plugins install clawhub:<package>` with tracked update metadata, gateway skill-install/update support for ClawHub-backed requests, and regression coverage/docs for the new source path.
...
- Install/update: allow package-manager installs from GitHub `main` via `wildvine update --tag main`, installer `--version main`, or direct npm/pnpm git specs. (#47630) Thanks @vincentkoc.
- Plugins/bundles: add compatible Codex, Claude, and Cursor bundle discovery/install support, map bundle skills into Wildvine skills, and apply Claude bundle `settings.json` defaults to embedded Pi with shell overrides sanitized.
- CLI/hooks: route hook-pack install and update through `wildvine plugins`, keep `wildvine hooks` focused on hook visibility and per-hook controls, and show plugin-managed hook details in CLI output.
...
- Models/MiniMax: add bundled `MiniMax-M2`, `MiniMax-M2.1`, and `MiniMax-M2.1-highspeed` catalog entries so Wildvine's provider metadata and OAuth aliases stay aligned with the current upstream Pi SDK. Thanks @vincentkoc.
...
- Agents/Pi compatibility: align Wildvine's bundled MiniMax runtime behavior with the current upstream Pi 0.61.1 release so embedded runs stay in sync with the latest published Pi SDK semantics. Thanks @vincentkoc.
...
- Plugins/testing: add a public `wildvine/plugin-sdk/testing` surface for plugin-author test helpers, and move bundled-plugin-only test bridges into private repo test helpers.
...
- Configure/startup: move outbound send-deps resolution into a lightweight helper so `wildvine configure` no longer stalls after the banner while eagerly loading channel plugins. (#46301) Thanks @scoootscooob.
...
- Control UI/logging: make browser-safe logger imports avoid eager temp-dir resolution so the bundled Control UI no longer crashes to a blank screen when logging reaches `tmp-wildvine-dir`. (#48469) Fixes #48062. Thanks @7inspire.
...
- Node/startup: remove leftover debug `console.log("node host PATH: ...")` that printed the resolved PATH on every `wildvine node run` invocation. (#46515) Fixes #46411. Thanks @ademczuk.
...
- macOS/node service startup: use `wildvine node start/stop --json` from the Mac app instead of the removed `wildvine service node ...` command shape, so current CLI installs expose the full node exec surface again. (#46843) Fixes #43171. Thanks @Br1an67.
...
- CLI/Ollama onboarding: keep the interactive model picker for explicit `wildvine onboard --auth-choice ollama` runs so setup still selects a default model without reintroducing pre-picker auto-pulls. (#49249) Thanks @BruceMacD.
...
- macOS/launch at login: stop emitting `KeepAlive` for the desktop app launch agent so Wildvine no longer relaunches immediately after a manual quit while launch at login remains enabled. (#40213) Thanks @stablegenius49.
...
- make `wildvine update status` explicitly say `up to date` when the local version already matches npm latest, while keeping the availability logic unchanged. (#51409) Thanks @dongzhenye.
- Agents/embedded transport errors: distinguish common network failures like connection refused, DNS lookup failure, and interrupted sockets from true timeouts in embedded-run user messaging and lifecycle diagnostics. (#51419) Thanks @scoootscooob.
...
- Media/security: bound remote-media error-body snippets with the same streaming caps and idle timeouts as successful downloads, so malicious HTTP error responses cannot force unbounded buffering before Wildvine throws.
...
- Zalo/plugin runtime: export `resolveClientIp` from `wildvine/plugin-sdk/zalo` so installed builds no longer crash on startup when the webhook monitor loads from the packaged extension instead of the monorepo source tree. (#46549) Thanks @No898.
...
- Browser/profiles: drop the auto-created `chrome-relay` browser profile; users who need the Chrome extension relay must now create their own profile via `wildvine browser create-profile`. (#46596) Fixes #45777. Thanks @odysseus0.
...
- Gateway/WS handshake: raise the default pre-auth handshake timeout to 10 seconds and add `WILDVINE_HANDSHAKE_TIMEOUT_MS` as a runtime override so busy local gateways stop dropping healthy CLI connections at 3 seconds. (#49262) Thanks @fuller-stack-dev.
...
- Plugins/install precedence: keep bundled plugins ahead of auto-discovered globals by default, but let an explicitly installed plugin record win its own duplicate-id tie so installed channel plugins load from `~/.wildvine/extensions` after `wildvine plugins install`. (#46722) Thanks @Takhoffman.
...
- Plugins/runtime barrels: route bundled extension runtime imports through public `wildvine/plugin-sdk/*` subpaths and block relative cross-package escapes so packaged extensions stop depending on monorepo-only relative paths. (#51939) Thanks @vincentkoc.
...
- Plugins/update: let `wildvine plugins update <npm-spec>` target tracked npm installs by dist-tag or exact version, and preserve the recorded npm spec for later id-based updates. (#49998) Thanks @huntharo.
...
- Docker/timezone override: add `WILDVINE_TZ` so `docker-setup.sh` can pin gateway and CLI containers to a chosen IANA timezone instead of inheriting the daemon default. (#34119) Thanks @Lanfei.
...
- macOS/onboarding: avoid self-restarting freshly bootstrapped launchd gateways and give new daemon installs longer to become healthy, so `wildvine onboard --install-daemon` no longer false-fails on slower Macs and fresh VM snapshots.
- Gateway/status: add `wildvine gateway status --require-rpc` and clearer Linux non-interactive daemon-install failure reporting so automation can fail hard on probe misses instead of treating a printed RPC error as green.
...
- Commands/onboarding: split static auth-choice help from the plugin-backed onboarding catalog so `wildvine onboard` registration no longer pulls provider-wizard imports just to describe `--auth-choice`. (#47545) Thanks @vincentkoc.
- Windows/gateway install: bound `schtasks` calls and fall back to the Startup-folder login item when task creation hangs, so native `wildvine gateway install` fails fast instead of wedging forever on broken Scheduled Task setups.
- Windows/gateway stop: resolve Startup-folder fallback listeners from the installed `gateway.cmd` port, so `wildvine gateway stop` now actually kills fallback-launched gateway processes before restart.
...
- CLI/startup: stop `wildvine devices list` and similar loopback gateway commands from failing during startup by isolating heavy import-time side effects from the normal CLI path. (#50212) Thanks @obviyus.
...
- Config/validation: accept documented `agents.list[].params` per-agent overrides in strict config validation so `wildvine config validate` no longer rejects runtime-supported `cacheRetention`, `temperature`, and `maxTokens` settings. (#41171) Thanks @atian8179.
...
- Security/device pairing: switch `/pair` and `wildvine qr` setup codes to short-lived bootstrap tokens so the next release no longer embeds shared gateway credentials in chat or QR pairing payloads. Thanks @lintsinghua.
...
- TUI/chat log: reuse the active assistant message component for the same streaming run so `wildvine tui` no longer renders duplicate assistant replies. (#35364) Thanks @lisitan.
...
- macOS/Reminders: add the missing `NSRemindersUsageDescription` to the bundled app so `apple-reminders` can trigger the system permission prompt from Wildvine.app. (#8559) Thanks @dinakars777.
...
- Plugins/env-scoped roots: fix plugin discovery/load caches and provenance tracking so same-process `HOME`/`WILDVINE_HOME` changes no longer reuse stale plugin state or misreport `~/...` plugins as untracked. (#44046) thanks @gumadeiras.
...
- Windows/native update: make package installs use the npm update path instead of the git path, carry portable Git into native Windows updates, and mirror the installer's Windows npm env so `wildvine update` no longer dies early on missing `git` or `node-llama-cpp` download setup.
...
- Docs/onboarding: align the legacy wizard reference and `wildvine onboard` command docs with the Ollama onboarding flow so all onboarding reference paths now document `--auth-choice ollama`, Cloud + Local mode, and non-interactive usage. (#43473) Thanks @BruceMacD.
...
- CLI/thinking help: add the missing `xhigh` level hints to `wildvine cron add`, `wildvine cron edit`, and `wildvine agent` so the help text matches the levels already accepted at runtime. (#44819) Thanks @kiki830621.
...
- Exec/child commands: mark child command environments with `WILDVINE_CLI` so subprocesses can detect when they were launched from the Wildvine CLI. (#41411) Thanks @vincentkoc.
- LLM Task/Vine: add an optional `thinking` override so workflow calls can explicitly set embedded reasoning level with shared validation for invalid values and unsupported `xhigh` modes. (#15606) Thanks @xadenryan and @ImLukeF.
...
- Windows/install: stop auto-installing `node-llama-cpp` during normal npm CLI installs so `wildvine@latest` no longer fails on Windows while building optional local-embedding dependencies.
- Windows/update: mirror the native installer environment during global npm updates, including portable Git fallback and Windows-safe npm shell settings, so `wildvine update` works again on native Windows installs.
...
- Windows/gateway install: fall back from denied Scheduled Task creation to a per-user Startup-folder login item, so native `wildvine gateway install` and `--install-daemon` keep working without an elevated PowerShell shell.
...
- CLI/tables: default shared tables to ASCII borders on legacy Windows consoles while keeping Unicode borders on modern Windows terminals, so commands like `wildvine skills` stop rendering mojibake under GBK/936 consoles. Fixes #40853. Related #41015. Thanks @ApacheBin and @vincentkoc.
...
- Security/system.run: fail closed for approval-backed interpreter/runtime commands when Wildvine cannot bind exactly one concrete local file operand, while extending best-effort direct-file binding to additional runtime forms. Thanks @tdjackey for reporting.
...
- Cron/doctor: tighten isolated cron delivery so cron jobs can no longer notify through ad hoc agent sends or fallback main-session summaries, and add `wildvine doctor --fix` migration for legacy cron storage and legacy notify/webhook delivery metadata. (#40998) Thanks @mbelinky.

## 2026.3.8

### Changes

- CLI/backup: add `wildvine backup create` and `wildvine backup verify` for local state archives, including `--only-config`, `--no-include-workspace`, manifest/payload validation, and backup guidance in destructive flows. (#40163) thanks @shichangs.
...
- CLI/install: include the short git commit hash in `wildvine --version` output when metadata is available, and keep installer version checks compatible with the decorated format. (#39712) thanks @sourman.
...
- ACP/Provenance: add optional ACP ingress provenance metadata and visible receipt injection (`wildvine acp --provenance off|meta|meta+receipt`) so Wildvine agents can retain and report ACP-origin context with session trace IDs. (#40473) thanks @mbelinky.
...
- Update/macOS launchd restart: re-enable disabled LaunchAgent services before updater bootstrap so `wildvine update` can recover from a disabled gateway service instead of leaving the restart step stuck.
...
- TUI/theme: detect light terminal backgrounds via `COLORFGBG` and pick a WCAG AA-compliant light palette, with `WILDVINE_THEME=light|dark` override for terminals without auto-detection. (#38636) Thanks @ademczuk and @vincentkoc.
...
- Podman/SELinux: auto-detect SELinux enforcing/permissive mode and add `:Z` relabel to bind mounts in `run-wildvine-podman.sh` and the Quadlet template, fixing `EACCES` on Fedora/RHEL hosts. Supports `WILDVINE_BIND_MOUNT_OPTIONS` override. (#39449) Thanks @langdon and @githubbzxs.
...
- CLI/skills tables: keep terminal table borders aligned for wide graphemes, use full reported terminal width, and switch a few ambiguous skill icons to Terminal-safe emoji so `wildvine skills` renders more consistently in Terminal.app and iTerm. Thanks @vincentkoc.
...
- Docker/Podman extension dependency baking: add `WILDVINE_EXTENSIONS` so container builds can preinstall selected bundled extension npm dependencies into the image for faster and more reproducible startup in container deployments. (#32223) Thanks @sallyom.
...
- iOS/App Store Connect release prep: align iOS bundle identifiers under `ai.wildvine.client`, refresh Watch app icons, add Fastlane metadata/screenshot automation, and support Keychain-backed ASC auth for uploads. (#38936) Thanks @ngutman.
...
- Docker/multi-stage build: restructure Dockerfile as a multi-stage build to produce a minimal runtime image without build tools, source code, or Bun; add `WILDVINE_VARIANT=slim` build arg for a bookworm-slim variant. (#38479) Thanks @sallyom.
...
- Gateway/Telegram stale-socket restart guard: only apply stale-socket restarts to channels that publish event-liveness timestamps, preventing Telegram providers from being misclassified as stale solely due to long uptime and avoiding restart/pairing storms after upgrade. (wildvine#38464)
...
- Tools/xAI native web-search collision guard: drop Wildvine `web_search` from tool registration when routing to xAI/Grok model providers (including OpenRouter `x-ai/*`) to avoid duplicate tool-name request failures against provider-native `web_search`. (#14749) Thanks @realsamrat.
...
- WhatsApp/self-chat response prefix fallback: stop forcing `"[wildvine]"` as the implicit outbound response prefix when no identity name or response prefix is configured, so blank/default prefix settings no longer inject branding text unexpectedly in self-chat flows. (#27962) Thanks @ecanmor.
...
- Gateway/remote WS break-glass hostname support: honor `WILDVINE_ALLOW_INSECURE_PRIVATE_WS=1` for `ws://` hostname URLs (not only private IP literals) across onboarding validation and runtime gateway connection checks, while still rejecting public IP literals and non-unicast IPv6 endpoints. (#36930) Thanks @manju-rn.
...
- Agents/context pruning: guard assistant thinking/text char estimation against malformed blocks (missing `thinking`/`text` strings or null entries) so pruning no longer crashes with malformed provider content. (wildvine#35146) thanks @Sid-Qin.
...
- Agents/schema cleaning: detect Venice + Grok model IDs as xAI-proxied targets so unsupported JSON Schema keywords are stripped before requests, preventing Venice/Grok `Invalid arguments` failures. (wildvine#35355) thanks @Sid-Qin.
...
- TUI/session-key canonicalization: normalize `wildvine tui --session` values to lowercase so uppercase session names no longer drop real-time streaming updates due to gateway/TUI key mismatches. (#33866, #34013) thanks @lynnzc.
...
- Plugins/startup loading: lazily initialize plugin runtime, split startup-critical plugin SDK imports into `wildvine/plugin-sdk/core` and `wildvine/plugin-sdk/telegram`, and preserve `api.runtime` reflection semantics for plugin compatibility. (#28620) thanks @hmemcpy.
- Plugins/startup performance: reduce bursty plugin discovery/manifest overhead with short in-process caches, skip importing bundled memory plugins that are disabled by slot selection, and speed legacy root `wildvine/plugin-sdk` compatibility via runtime root-alias routing while preserving backward compatibility. Thanks @gumadeiras.
...
- Gateway/password CLI hardening: add `wildvine gateway run --password-file`, warn when inline `--password` is used because it can leak via process listings, and document env/file-backed password input as the preferred startup path. Fixes #27948. Thanks @vibewrk and @vincentkoc.
...
- Google/Gemini Flash model selection: switch built-in `gemini-flash` defaults and docs/examples from the nonexistent `google/gemini-3.1-flash-preview` ID to the working `google/gemini-3-flash-preview`, while normalizing legacy Wildvine config that still uses the old Flash 3.1 alias.
- Plugins/bundled scoped-import migration: migrate bundled plugins from monolithic `wildvine/plugin-sdk` imports to scoped subpaths (or `wildvine/plugin-sdk/core`) across registration and startup-sensitive runtime files, add CI/release guardrails to prevent regressions, and keep root `wildvine/plugin-sdk` support for external/community plugins. Thanks @gumadeiras.
...
- Gateway/status self version reporting: make Gateway self version in `wildvine status` prefer runtime `VERSION` (while preserving explicit `WILDVINE_VERSION` override), preventing stale post-upgrade app version output. (#32655) thanks @liuxiaopai-ai.
...
- Mattermost/plugin SDK import policy: replace remaining monolithic `wildvine/plugin-sdk` imports in Mattermost mention-gating paths/tests with scoped subpaths (`wildvine/plugin-sdk/compat` and `wildvine/plugin-sdk/mattermost`) so `pnpm check` passes `lint:plugins:no-monolithic-plugin-sdk-entry-imports` on baseline. (#36480) Thanks @Takhoffman.
...
- Windows/Plugin install: when Wildvine runs on Windows via Bun and `npm-cli.js` is not colocated with the runtime binary, fall back to `npm.cmd`/`npx.cmd` through the existing `cmd.exe` wrapper so `wildvine plugins install` no longer fails with `spawn EINVAL`. (#38056) Thanks @0xlin2023.
...
- Agents/usage normalization: normalize missing or partial assistant usage snapshots before compaction accounting so `wildvine agent --json` no longer crashes when provider payloads omit `totalTokens` or related usage fields. (#34977) thanks @sp-hk2ldn.
...
- ACP/console silent reply suppression: filter ACP `NO_REPLY` lead fragments and silent-only finals before `wildvine agent` logging/delivery so console-backed ACP sessions no longer leak `NO`/`NO_REPLY` placeholders. (#38436) Thanks @ql-wade.
...
- Daemon/systemd fresh-install probe: check for Wildvine's managed user unit before running `systemctl --user is-enabled`, so first-time Linux installs no longer fail on generic missing-unit probe errors. (#38819) Thanks @adaHubble.
- Gateway/container lifecycle: allow `wildvine gateway stop` to SIGTERM unmanaged gateway listeners and `wildvine gateway restart` to SIGUSR1 a single unmanaged listener when no service manager is installed, so container and supervisor-based deployments are no longer blocked by `service disabled` no-op responses. Fixes #36137. Thanks @vincentkoc.
...
- CLI/Docs memory help accuracy: clarify `wildvine memory status --deep` behavior and align memory command examples/docs with the current search options. (#31803) Thanks @JasonOA888 and @Avi974.
...
- CLI/bootstrap Node version hint maintenance: replace hardcoded nvm `22` instructions in `wildvine.mjs` with `MIN_NODE_MAJOR` interpolation so future minimum-Node bumps keep startup guidance in sync automatically. (#39056) Thanks @onstash.
...
- Daemon/Windows schtasks runtime detection: use locale-invariant `Last Run Result` running codes (`0x41301`/`267009`) as the primary running signal so `wildvine node status` no longer misreports active tasks as stopped on non-English Windows locales. (#39076) Thanks @ademczuk.
...
- Podman/.env gateway bind precedence: evaluate `WILDVINE_GATEWAY_BIND` after sourcing `.env` in `run-wildvine-podman.sh` so env-file overrides are honored. (#38785) Thanks @majinyu666.
...
- Browser/config schema: accept `browser.profiles.*.driver: "wildvine"` while preserving legacy `"clawd"` compatibility in validated config. (#39374; based on #35621) Thanks @gambletan and @ingyukoh.
...
- Secrets/SecretRef coverage: expand SecretRef support across the full supported user-supplied credential surface (64 targets total), including runtime collectors, `wildvine secrets` planning/apply/audit flows, onboarding SecretInput UX, and related docs; unresolved refs now fail fast on active surfaces while inactive surfaces report non-blocking diagnostics. (#29580) Thanks @joshavant.
...
- CLI/Config validation: add `wildvine config validate` (with `--json`) to validate config files before gateway startup, and include detailed invalid-key paths in startup invalid-config errors. (#31220) thanks @Sid-Qin.
...
- Zalo Personal plugin (`@wildvine/zalouser`): rebuilt channel runtime to use native `zca-js` integration in-process, removing external CLI transport usage and keeping QR/login + send/listen flows fully inside Wildvine.
...
- Plugin runtime/STT: add `api.runtime.stt.transcribeAudioFile(...)` so extensions can transcribe local audio files through Wildvine's configured media-understanding audio providers. (#22402) Thanks @benthecarman.
...
- Zalo Personal plugin (`@wildvine/zalouser`): keep canonical DM routing while preserving legacy DM session continuity on upgrade, and preserve provider-native `g-`/`u-` target ids in outbound send and directory flows so #33992 lands without breaking existing sessions or stored targets. (#33992) Thanks @darkamenosa.
...
- Feishu/Plugin sdk compatibility: add safe webhook default fallbacks when loading Feishu monitor state so mixed-version installs no longer crash if older `wildvine/plugin-sdk` builds omit webhook default constants. (#31606)
...
- Browser/Profile defaults: prefer `wildvine` profile over `chrome` in headless/no-sandbox environments unless an explicit `defaultProfile` is configured. (#14944) Thanks @BenediktSchackenberg.
- Gateway/WS security: keep plaintext `ws://` loopback-only by default, with explicit break-glass private-network opt-in via `WILDVINE_ALLOW_INSECURE_PRIVATE_WS=1`; align onboarding/client/call validation and tests to this strict-default policy. (#28670) Thanks @dashed, @vincentkoc.
...
- CLI/Browser start timeout: honor `wildvine browser --timeout <ms> start` and stop by removing the fixed 15000ms override so slower Chrome startups can use caller-provided timeouts. (#22412, #23427) Thanks @vincentkoc.
...
- Browser/Gateway hardening: preserve env credentials for `WILDVINE_GATEWAY_URL` / `CLAWDBOT_GATEWAY_URL` while treating explicit `--url` as override-only auth, and make container browser hardening flags optional with safer defaults for Docker/LXC stability. (#31504) Thanks @vincentkoc.
...
- Gateway/OpenAI chat completions: honor `x-wildvine-message-channel` when building `agentCommand` input for `/v1/chat/completions`, preserving caller channel identity instead of forcing `webchat`. (#30462) Thanks @bmendonca3.
...
- Voice-call/Twilio external outbound: auto-register webhook-first `outbound-api` calls (initiated outside Wildvine) so media streams are accepted and call direction metadata stays accurate. (#31181) Thanks @scoootscooob.
...
- Gateway/Control UI basePath POST handling: return 405 for `POST` on exact basePath routes (for example `/wildvine`) instead of redirecting, and add end-to-end regression coverage that root-mounted webhook POST paths still pass through to plugin handlers. (#31349) Thanks @Sid-Qin.
- Browser/default profile selection: default `browser.defaultProfile` behavior now prefers `wildvine` (managed standalone CDP) when no explicit default is configured, while still auto-provisioning the `chrome` relay profile for explicit opt-in use. (#32031) Fixes #31907. Thanks @liuxiaopai-ai.
...
- Models/Codex usage labels: infer weekly secondary usage windows from reset cadence when API window seconds are ambiguously reported as 24h, so `wildvine models status` no longer mislabels weekly limits as daily. (#31938) Thanks @bmendonca3.
...
- Security audit/skills workspace hardening: add `skills.workspace.symlink_escape` warning in `wildvine security audit` when workspace `skills/**/SKILL.md` resolves outside the workspace root (for example symlink-chain drift), plus docs coverage in the security glossary.
...
- Docker/Sandbox bootstrap hardening: make `WILDVINE_SANDBOX` opt-in parsing explicit (`1|true|yes|on`), support custom Docker socket paths via `WILDVINE_DOCKER_SOCKET`, defer docker.sock exposure until sandbox prerequisites pass, and reset/roll back persisted sandbox mode to `off` when setup is skipped or partially fails to avoid stale broken sandbox state. (#29974) Thanks @jamtujest and @vincentkoc.
...
- Browser/Managed tab cap: limit loopback managed `wildvine` page tabs to 8 via best-effort cleanup after tab opens to reduce long-running renderer buildup while preserving attach-only and remote profile behavior. (#29724) Thanks @pandego.
...
- CLI/installer Node preflight: enforce Node.js `v22.12+` consistently in both `wildvine.mjs` runtime bootstrap and installer active-shell checks, with actionable nvm recovery guidance for mismatched shell PATH/defaults. (#32356) Thanks @jasonhargrove.
...
- Plugins/install diagnostics: reject legacy plugin package shapes without `wildvine.extensions` and return an explicit upgrade hint with troubleshooting docs for repackaging. (#32055) Thanks @liuxiaopai-ai.
...
- Plugins/install fallback safety: resolve bare install specs to bundled plugin ids before npm lookup (for example `diffs` -> bundled `@wildvine/diffs`), keep npm fallback limited to true package-not-found errors, and continue rejecting non-plugin npm packages that fail manifest validation. (#32096) Thanks @scoootscooob.
...
- Tests/Subagent announce: set `WILDVINE_TEST_FAST=1` before importing `subagent-announce` format suites so module-level fast-mode constants are captured deterministically on Windows CI, preventing timeout flakes in nested completion announce coverage. (#31370) Thanks @zwffff.
...
- **BREAKING:** ACP dispatch now defaults to enabled unless explicitly disabled (`acp.dispatch.enabled=false`). If you need to pause ACP turn routing while keeping `/acp` controls, set `acp.dispatch.enabled=false`. Docs: https://docs.wildvine.com/tools/acp-agents
- **BREAKING:** Plugin SDK removed `api.registerHttpHandler(...)`. Plugins must register explicit HTTP routes via `api.registerHttpRoute({ path, auth, match, handler })`, and dynamic webhook lifecycles should use `registerPluginHttpRoute(...)`.
- **BREAKING:** Zalo Personal plugin (`@wildvine/zalouser`) no longer depends on external `zca`-compatible CLI binaries (`openzca`, `zca-cli`) for runtime send/listen/login; operators should use `wildvine channels login --channel zalouser` after upgrade to refresh sessions in the new JS-native path.
...
- CLI/Config: add `wildvine config file` to print the active config file path resolved from `WILDVINE_CONFIG_PATH` or the default location. (#26256) thanks @cyb1278588254.
...
- Shell env markers: set `WILDVINE_SHELL` across shell-like runtimes (`exec`, `acp`, `acp-client`, `tui-local`) so shell startup/config rules can target Wildvine contexts consistently, and document the markers in env/exec/acp/TUI docs. Thanks @vincentkoc.
...
- Sandbox/Browser Docker: pass `WILDVINE_BROWSER_NO_SANDBOX=1` to sandbox browser containers and bump sandbox browser security hash epoch so existing containers are recreated and pick up the env on upgrade. (#29879) Thanks @Lukavyi.
...
- Gateway/Linux restart health: reduce false `wildvine gateway restart` timeouts by falling back to `ss -ltnp` when `lsof` is missing, confirming ambiguous busy-port cases via local gateway probe, and targeting the original `SUDO_USER` systemd user scope for restart commands. (#34874) Thanks @vincentkoc.
...
- Android/Play package ID: rename the Android app package to `com.wildvine.android`, including matching benchmark and Android tooling references for Play publishing. (#38712) Thanks @obviyus.
...
- Android/Onboarding + voice reliability: request per-toggle onboarding permissions, update pairing guidance to `wildvine devices list/approve`, restore assistant speech playback in mic capture flow, cancel superseded in-flight speech (mute + per-reply token rotation), and keep `talk.config` loads retryable after transient failures. (#29796) Thanks @obviyus.
...
- Docker/Compose gateway targeting: run `wildvine-cli` in the `wildvine-gateway` service network namespace, require gateway startup ordering, pin Docker setup to `gateway.mode=local`, sync `gateway.bind` from `WILDVINE_GATEWAY_BIND`, default optional `CLAUDE_*` compose vars to empty values to reduce automation warning noise, and harden `wildvine-cli` with `cap_drop` (`NET_RAW`, `NET_ADMIN`) + `no-new-privileges`. Docs now call out the shared trust boundary explicitly. (#12504) Thanks @bvanderdrift and @vincentkoc.
...
- Doctor/macOS state-dir safety: warn when Wildvine state resolves inside iCloud Drive (`~/Library/Mobile Documents/com~apple~CloudDocs/...`) or `~/Library/CloudStorage/...`, because sync-backed paths can cause slower I/O and lock/sync races. (#31004) Thanks @vincentkoc.
- Doctor/Linux state-dir safety: warn when Wildvine state resolves to an `mmcblk*` mount source (SD or eMMC), because random I/O can be slower and media wear can increase under session and credential writes. (#31033) Thanks @vincentkoc.
...
- Onboarding/Docker token parity: use `WILDVINE_GATEWAY_TOKEN` as the default gateway token in interactive and non-interactive onboarding when `--gateway-token` is not provided, so `docker-setup.sh` token env/config values stay aligned. (#22658) Fixes #22638. Thanks @Clawborn and @vincentkoc.
...
- Highlight: External Secrets Management introduces a full `wildvine secrets` workflow (`audit`, `configure`, `apply`, `reload`) with runtime snapshot activation, strict `secrets apply` target-path validation, safer migration scrubbing, ref-only auth-profile support, and dedicated docs. (#26155) Thanks @joshavant.
...
- Agents/Routing CLI: add `wildvine agents bindings`, `wildvine agents bind`, and `wildvine agents unbind` for account-scoped route management, including channel-only to account-scoped binding upgrades, role-aware binding identity handling, plugin-resolved binding account IDs, and optional account-binding prompts in `wildvine channels add`. (#27195) thanks @gumadeiras.
...
- Telegram/DM allowlist runtime inheritance: enforce `dmPolicy: "allowlist"` `allowFrom` requirements using effective account-plus-parent config across account-capable channels (Telegram, Discord, Slack, Signal, iMessage, IRC, BlueBubbles, WhatsApp), and align `wildvine doctor` checks to the same inheritance logic so DM traffic is not silently dropped after upgrades. (#27936) Thanks @widingmarcus-cyber.
...
- Config/Doctor allowlist safety: reject `dmPolicy: "allowlist"` configs with empty `allowFrom`, add Telegram account-level inheritance-aware validation, and teach `wildvine doctor --fix` to restore missing `allowFrom` entries from pairing-store files when present, preventing silent DM drops after upgrades. (#27936) Thanks @widingmarcus-cyber.
...
- Browser/Fill relay + CLI parity: accept `act.fill` fields without explicit `type` by defaulting missing/empty `type` to `text` in both browser relay route parsing and `wildvine browser fill` CLI field parsing, so relay calls no longer fail when the model omits field type metadata. Landed from contributor PR #27662. (#27296) Thanks @Uface11.
...
- CLI/Gateway `--force` in non-root Docker: recover from `lsof` permission failures (`EACCES`/`EPERM`) by falling back to `fuser` kill + probe-based port checks, so `wildvine gateway --force` works for default container `node` user flows. (#27941) Thanks @steipete.
...
- Sessions cleanup/Doctor: add `wildvine sessions cleanup --fix-missing` to prune store entries whose transcript files are missing, including doctor guidance and CLI coverage. Landed from contributor PR #27508 by @Sid-Qin. (#27422)
- Doctor/State integrity: ignore metadata-only slash routing sessions when checking recent missing transcripts so `wildvine doctor` no longer reports false-positive transcript-missing warnings for `*:slash:*` keys. (#27375) thanks @gumadeiras.
...
- CLI/Daemon status TLS probe: use `wss://` and forward local TLS certificate fingerprint for TLS-enabled gateway daemon probes so `wildvine daemon status` works with `gateway.bind=lan` + `gateway.tls.enabled=true`. (#24234) thanks @liuy.
- Podman/Default bind: change `run-wildvine-podman.sh` default gateway bind from `lan` to `loopback` and document explicit LAN opt-in with Control UI origin configuration. (#27491) thanks @robbyczgw-cla.
...
- Auth/Auth profiles: normalize `auth-profiles.json` alias fields (`mode -> type`, `apiKey -> key`) before credential validation so entries copied from `wildvine.json` auth examples are no longer silently dropped. (#26950) thanks @byungsker.
...
- Onboarding/Security: clarify onboarding security notices that Wildvine is personal-by-default (single trusted operator boundary) and shared/multi-user setups require explicit lock-down/hardening.
- Branding/Docs + Apple surfaces: replace remaining `bot.molt` launchd label, bundle-id, logging subsystem, and command examples with `ai.wildvine` across docs, iOS app surfaces, helper scripts, and CLI test fixtures.
...
- Models/Auth probes: map permanent auth failover reasons (`auth_permanent`, for example revoked keys) into probe auth status instead of `unknown`, so `wildvine models status --probe` reports actionable auth failures. (#25754) thanks @rrenamed.
...
- Auto-reply/Abort shortcuts: expand standalone stop phrases (`stop wildvine`, `stop action`, `stop run`, `stop agent`, `please stop`, and related variants), accept trailing punctuation (for example `STOP WILDVINE!!!`), add multilingual stop keywords (including ES/FR/ZH/HI/AR/JP/DE/PT/RU forms), and treat exact `do not do that` as a stop trigger while preserving strict standalone matching. (#25103) Thanks @steipete and @vincentkoc.
...
- macOS/Gateway launch: prefer an available `wildvine` binary before pnpm/node runtime fallback when resolving local gateway commands, so local startup no longer fails on hosts with broken runtime discovery. (#25512) Thanks @chilu18.
...
- CLI/Memory search: accept `--query <text>` for `wildvine memory search` (while keeping positional query support), and emit a clear error when neither form is provided. (#25904, #25857) Thanks @niceysam and @stakeswky.
- CLI/Doctor: correct stale recovery hints to use valid commands (`wildvine gateway status --deep` and `wildvine configure --section model`). (#24485) Thanks @chilu18.
...
- Security/Sandbox media: restrict sandbox media tmp-path allowances to Wildvine-managed tmp roots instead of broad host `os.tmpdir()` trust, and add outbound/channel guardrails (tmp-path lint + media-root smoke tests) to prevent regressions in local media attachment reads. Thanks @tdjackey for reporting.
- Security/Sandbox media: reject hard-linked Wildvine tmp media aliases (including symlink-to-hardlink chains) during sandbox media path resolution to prevent out-of-sandbox inode alias reads. (#25820) Thanks @bmendonca3.
...
- Sessions/Cron: harden session maintenance with `wildvine sessions cleanup`, per-agent store targeting, disk-budget controls (`session.maintenance.maxDiskBytes` / `highWaterBytes`), and safer transcript/archive cleanup + run-log retention behavior. (#24753) thanks @gumadeiras.
...
- Doctor/Memory: query gateway-side default-agent memory embedding readiness during `wildvine doctor` (instead of inferring from generic gateway health), and warn when the gateway memory probe is unavailable or not ready while keeping `wildvine configure` remediation guidance. (#22327) thanks @therk.
...
- Providers/DashScope: mark DashScope-compatible `openai-completions` endpoints as `supportsDeveloperRole=false` so Wildvine sends `system` instead of unsupported `developer` role on Qwen/DashScope APIs. (#19130) Thanks @Putzhuawa and @vincentkoc.
...
- Config/Write: apply `unsetPaths` with immutable path-copy updates so config writes never mutate caller-provided objects, and harden `wildvine config get/set/unset` path traversal by rejecting prototype-key segments and inherited-property traversal. (#24134) thanks @frankekn.
...
- **BREAKING:** browser SSRF policy now defaults to trusted-network mode (`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork=true` when unset), and canonical config uses `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` instead of `browser.ssrfPolicy.allowPrivateNetwork`. `wildvine doctor --fix` migrates the legacy key automatically.
...
- CLI/Update: add `wildvine update --dry-run` to preview channel/tag/target/restart actions without mutating config, installing, syncing plugins, or restarting.
...
- Security/CLI: redact sensitive values in `wildvine config get` output before printing config paths, preventing credential leakage to terminal output/history. (#13683) Thanks @SleuthCo.
...
- Install/Discord Voice: make the native Opus decoder optional so `wildvine` install/update no longer hard-fails when native builds fail, while keeping `opusscript` as the runtime fallback decoder for Discord voice flows. (#23737, #23733, #23703) Thanks @jeadland, @Sheetaa, and @Breakyman.
- Docker/Setup: precreate `$WILDVINE_CONFIG_DIR/identity` during `docker-setup.sh` so CLI commands that need device identity (for example `devices list`) avoid `EACCES ... /home/node/.wildvine/identity` failures on restrictive bind mounts. (#23948) Thanks @ackson-beep.
...
- Gateway/Restart: fix restart-loop edge cases by keeping `wildvine.mjs -> dist/entry.js` bootstrap detection explicit, reacquiring the gateway lock for in-process restart fallback paths, and tightening restart-loop regression coverage. (#23416) Thanks @jeffwnli.
...
- Feishu/Plugins: restore bundled Feishu SDK availability for global installs and strip `wildvine: workspace:*` from plugin `devDependencies` during plugin-version sync so npm-installed Feishu plugins do not fail dependency install. (#23611, #23645, #23603)
...
- Plugins/Discovery: ignore scanned extension backup/disabled directory patterns (for example `.backup-*`, `.bak`, `.disabled*`) and move updater backup directories under `.wildvine-install-backups`, preventing duplicate plugin-id collisions from archived copies.
- Plugins/CLI: make `wildvine plugins enable` and plugin install/link flows update allowlists via shared plugin-enable policy so enabled plugins are not left disabled by allowlist mismatch. (#23190) Thanks @downwind7clawd-ctrl.
...
- Node/macOS exec host: default headless macOS node `system.run` to local execution and only route through the companion app when `WILDVINE_NODE_EXEC_HOST=app` is explicitly set, avoiding companion-app filesystem namespace mismatches during exec. (#23547) Thanks @steipete.
...
- Installer/Smoke tests: remove legacy `WILDVINE_USE_GUM` overrides from docker install-smoke runs so tests exercise installer auto TTY detection behavior directly.
...
- Gateway/Auth: preserve `WILDVINE_GATEWAY_PASSWORD` env override precedence for remote gateway call credentials after shared resolver refactors, preventing stale configured remote passwords from overriding runtime secret rotation.
...
- Security/Audit: add `wildvine security audit` detection for open group policies that expose runtime/filesystem tools without sandbox/workspace guards (`security.exposure.open_groups_with_runtime_or_fs`).
...
- Security/Gateway: emit a startup security warning when insecure/dangerous config flags are enabled (including `gateway.controlUi.dangerouslyDisableDeviceAuth=true`) and point operators to `wildvine security audit`.
...
- Telegram/Network: default Node 22+ DNS result ordering to `ipv4first` for Telegram fetch paths and add `WILDVINE_TELEGRAM_DNS_RESULT_ORDER`/`channels.telegram.network.dnsResultOrder` overrides to reduce IPv6-path fetch failures. (#5405) Thanks @Glucksberg.
...
- Docs/Telegram: correct Node 22+ network defaults (`autoSelectFamily`, `dnsResultOrder`) and clarify Telegram setup does not use positional `wildvine channels login telegram`. (#23609) Thanks @ryanbastic.
...
- Security/Audit: add `wildvine security audit` finding `gateway.nodes.allow_commands_dangerous` for risky `gateway.nodes.allowCommands` overrides, with severity upgraded to critical on remote gateway exposure.
...
- **BREAKING:** tool-failure replies now hide raw error details by default. Wildvine still sends a failure summary, but detailed error suffixes (for example provider/runtime messages and local path fragments) now require `/verbose on` or `/verbose full`.
...
- Channels/CLI: add per-account/channel `defaultTo` outbound routing fallback so `wildvine agent --deliver` can send without explicit `--reply-to` when a default target is configured. (#16985) Thanks @KirillShchetinin.
...
- CLI/Config: preserve explicitly unset config paths in persisted JSON after writes so `wildvine config unset <path>` no longer re-introduces defaulted keys (for example `commands.ownerDisplay`) through schema normalization. (#22984) Thanks @aronchick.
- CLI: keep `wildvine -v` as a root-only version alias so subcommand `-v, --verbose` flags (for example ACP/hooks/skills) are no longer intercepted globally. (#21303) thanks @adhitShet.
...
- iOS/Watch: refresh iOS and watch app icon assets with the vine icon set to keep phone/watch branding aligned. (#21997) Thanks @mbelinky.
...
- Gateway/Pairing/CLI: align read-scope compatibility in pairing/device-token checks and add local `wildvine devices` fallback recovery for loopback `pairing required` deadlocks, with explicit fallback notice to unblock approval bootstrap flows. (#21616) Thanks @shakkernerd.
...
- macOS/Build: default release packaging to `BUNDLE_ID=ai.wildvine.mac` in `scripts/package-mac-dist.sh`, so Sparkle feed URL is retained and auto-update no longer fails with an empty appcast feed. (#19750) thanks @loganprit.
...
- Security/WildvineKit/UI: prevent injected inbound user context metadata blocks from leaking into chat history in TUI, webchat, and macOS surfaces by stripping all untrusted metadata prefixes at display boundaries. (#22142) Thanks @Mellowambience, @vincentkoc.
- Security/WildvineKit/UI: strip inbound metadata blocks from user messages in TUI rendering while preserving user-authored content. (#22345) Thanks @kansodata, @vincentkoc.
- Security/WildvineKit/UI: prevent inbound metadata leaks and reply-tag streaming artifacts in TUI rendering by stripping untrusted metadata prefixes at display boundaries. (#22346) Thanks @akramcodez, @vincentkoc.
- Security/Agents: restrict local MEDIA tool attachments to core tools and the Wildvine temp root to prevent untrusted MCP tool file exfiltration. Thanks @NucleiAv and @thewilloftheshadow.
...
- Security/Sandbox: remove default `--no-sandbox` for the browser container entrypoint, add explicit opt-in via `WILDVINE_BROWSER_NO_SANDBOX` / `CLAWDBOT_BROWSER_NO_SANDBOX`, and add security-audit checks for stale/missing sandbox browser Docker hash labels. Thanks @TerminalsandCoffee and @vincentkoc.
...
- Security/Sandbox Browser: default browser sandbox containers to a dedicated Docker network (`wildvine-sandbox-browser`), add optional CDP ingress source-range restrictions, auto-create missing dedicated networks, and warn in `wildvine security --audit` when browser sandboxing runs on bridge without source-range limits. Thanks @TerminalsandCoffee for reporting.
...
- Gateway/CLI: add paired-device hygiene flows with `device.pair.remove`, plus `wildvine devices remove` and guarded `wildvine devices clear --yes [--pending]` commands for removing paired entries and optionally rejecting pending requests. (#20057) Thanks @mbelinky.
...
- iOS/Signing: restore local auto-selected signing-team overrides during iOS project generation by wiring `.local-signing.xcconfig` into the active signing config and emitting `WILDVINE_DEVELOPMENT_TEAM` in local signing setup. (#19993) Thanks @ngutman.
...
- WildvineKit/Protocol: preserve JSON boolean literals (`true`/`false`) when bridging through `AnyCodable` so Apple client RPC params no longer re-encode booleans as `1`/`0`. Thanks @mbelinky.
...
- Browser/Relay: reuse an already-running extension relay when the relay port is occupied by another Wildvine process, while still failing on non-relay port collisions to avoid masking unrelated listeners. (#20035) Thanks @mbelinky.
- Scripts: update vinedock helper command support to include `docker-compose.extra.yml` where available. (#17094) Thanks @zerone0x.
- Vine/Config: remove Vine executable-path overrides (`vinePath`), require PATH-based execution, and add focused Windows wrapper-resolution tests to keep shell-free behavior stable.
...
- Vine/CI: fix flaky test Windows cmd shim script resolution. (#20833) Thanks @vincentkoc.
...
- Security/Gateway/Canvas: replace shared-IP fallback auth with node-scoped session capability URLs for `/__wildvine__/canvas/*` and `/__wildvine__/a2ui/*`, fail closed when trusted-proxy requests omit forwarded client headers, and add IPv6/proxy-header regression coverage. Thanks @aether-ai-agent for reporting.
...
- Security/ACP+Exec: add `wildvine acp --token-file/--password-file` secret-file support (with inline secret flag warnings), redact ACP working-directory prefixes to `~` home-relative paths, constrain exec script preflight file inspection to the effective `workdir` boundary, and add security-audit warnings when `tools.exec.host="sandbox"` is configured while sandbox mode is off.
- Security/Plugins/Hooks: enforce runtime/package path containment with realpath checks so `wildvine.extensions`, `wildvine.hooks`, and hook handler modules cannot escape their trusted roots via traversal or symlinks.
...
- Security/Plugins/Hooks: add optional `--pin` for npm plugin/hook installs, persist resolved npm metadata (`name`, `version`, `spec`, integrity, shasum, timestamp), warn/confirm on integrity drift during updates, and extend `wildvine security audit` to flag unpinned specs, missing integrity metadata, and install-record version drift.
...
- Security/Vine (Windows): for the next npm release, remove shell-based fallback when launching Vine wrappers (`.cmd`/`.bat`) and switch to explicit argv execution with wrapper entrypoint resolution, preventing command injection while preserving Windows wrapper compatibility. Thanks @allsmog for reporting.
...
- Cron/CLI: add deterministic default stagger for recurring top-of-hour cron schedules (including 6-field seconds cron), auto-migrate existing jobs to persisted `schedule.staggerMs`, and add `wildvine cron add/edit --stagger <duration>` plus `--exact` overrides for per-job timing control.
...
- Docker: add optional `WILDVINE_INSTALL_BROWSER` build arg to preinstall Chromium + Xvfb in the Docker image, avoiding runtime Playwright installs. (#18449)
...
- Gateway/Presence: centralize presence snapshot broadcasts and unify runtime version precedence (`WILDVINE_VERSION` > `WILDVINE_SERVICE_VERSION` > `npm_package_version`) so self-presence and websocket `hello-ok` report consistent versions.
...
- Models CLI: validate catalog entries in `wildvine models set`. (#18129) Thanks @carrotRakko.
...
- WildvineKit/iOS ChatUI: accept canonical session-key completion events for local pending runs and preserve message IDs across history refreshes, preventing stuck "thinking" state and message flicker after gateway replies. (#18165) Thanks @mbelinky.
...
- iOS/Signing: auto-select local Apple Development team during iOS project generation/build, prefer the canonical Wildvine team when available, and support local per-machine signing overrides without committing team IDs. (#18421) Thanks @ngutman.
...
- CLI/Pairing: make `wildvine qr --remote` prefer `gateway.remote.url` over tailscale/public URL resolution and register the `wildvine vinebot qr` legacy alias path. (#18091)
- CLI/QR: restore fail-fast validation for `wildvine qr --remote` when neither `gateway.remote.url` nor tailscale `serve`/`funnel` is configured, preventing unusable remote pairing QR flows. (#18166) Thanks @mbelinky.
...
- CLI/Doctor: ensure `wildvine doctor --fix --non-interactive --yes` exits promptly after completion so one-shot automation no longer hangs. (#18502)
- CLI/Doctor: auto-repair `dmPolicy="open"` configs missing wildcard allowlists and write channel-correct repair paths (including `channels.googlechat.dm.allowFrom`) so `wildvine doctor --fix` no longer leaves Google Chat configs invalid after attempted repair. (#18544)
...
- Gateway/Update: prevent restart crash loops after failed self-updates by restarting only on successful updates, stopping early on failed install/build steps, and running `wildvine doctor --fix` during updates to sanitize config. (#18131) Thanks @RamiNoodle733.
...
- CLI/Daemon: warn when a gateway restart sees a stale service token so users can reinstall with `wildvine gateway install --force`, and skip drift warnings for non-gateway service restarts. (#18018)
...
- CLI/Status: fix `wildvine status --all` token summaries for bot-token-only channels so Mattermost/Zalo no longer show a bot+app warning. (#18527) Thanks @echo931.
- CLI/Configure: make the `/model picker` allowlist prompt searchable with tokenized matching in `wildvine configure` so users can filter huge model lists by typing terms like `gpt-5.2 openai/`. (#19010) Thanks @bjesuiter.
- CLI/Message: preserve `--components` JSON payloads in `wildvine message send` so Discord component payloads are no longer dropped. (#18222) Thanks @saurabhchopade.
...
- Models/CLI: sync auth-profiles credentials into agent `auth.json` before registry availability checks so `wildvine models list --all` reports auth correctly for API-key/token providers, normalize provider-id aliases when bridging credentials, and skip expired token mirrors. (#18610, #18615)
...
- Security/Sessions: create new session transcript JSONL files with user-only (`0o600`) permissions and extend `wildvine security audit --fix` to remediate existing transcript file permissions.
...
- Telegram: add poll sending via `wildvine message poll` (duration seconds, silent delivery, anonymity controls). (#16209) Thanks @robbyczgw-cla.
- Slack/Discord: add `dmPolicy` + `allowFrom` config aliases for DM access control; legacy `dm.policy` + `dm.allowFrom` keys remain supported and `wildvine doctor --fix` can migrate them.
...
- CLI/Plugins: ensure `wildvine message send` exits after successful delivery across plugin-backed channels so one-shot sends do not hang. (#16491) Thanks @yinghaosang.
- CLI/Plugins: run registered plugin `gateway_stop` hooks before `wildvine message` exits (success and failure paths), so plugin-backed channels can clean up one-shot CLI resources. (#16580) Thanks @gumadeiras.
...
- TUI/Gateway: resolve local gateway target URL from `gateway.bind` mode (tailnet/lan) instead of hardcoded localhost so `wildvine tui` connects when gateway is non-loopback. (#16299) Thanks @cortexuvula.
- TUI: honor explicit `--session <key>` in `wildvine tui` even when `session.scope` is `global`, so named sessions no longer collapse into shared global history. (#16575) Thanks @cinqu.
...
- Memory/QMD: make `wildvine memory index` verify and print the active QMD index file path/size, and fail when QMD leaves a missing or zero-byte index artifact after an update. (#16775) Thanks @Shunamxiao.
...
- Media/Security: allow local media reads from Wildvine state `workspace/` and `sandboxes/` roots by default so generated workspace media can be delivered without unsafe global path bypasses. (#15541) Thanks @lanceji.
...
- Security/Hooks: restrict hook transform modules to `~/.wildvine/hooks/transforms` (prevents path traversal/escape module loads via config). Config note: `hooks.transformsDir` must now be within that directory. Thanks @akhmittra.
...
- Telegram/Security: require numeric Telegram sender IDs for allowlist authorization (reject `@username` principals), auto-resolve `@username` to IDs in `wildvine doctor --fix` (when possible), and warn in `wildvine security audit` when legacy configs contain usernames. Thanks @vincentkoc.
...
- macOS: hard-limit unkeyed `wildvine://agent` deep links and ignore `deliver` / `to` / `channel` unless a valid unattended key is provided. Thanks @Cillian-Collins.
...
- Install: add optional Podman-based setup: `setup-podman.sh` for one-time host setup (wildvine user, image, launch script, systemd quadlet), `run-wildvine-podman.sh launch` / `launch setup`; systemd Quadlet unit for wildvine user service; docs for rootless container, wildvine user (subuid/subgid), and quadlet (troubleshooting). (#16273) Thanks @DarwinsBuddy.
...
- OpenAI Codex/Auth: bridge Wildvine OAuth profiles into `pi` `auth.json` so model discovery and models-list registry resolution can use Codex OAuth credentials. (#15184) Thanks @loiie45e.
...
- CLI/Completion: route plugin-load logs to stderr and write generated completion scripts directly to stdout to avoid `source <(wildvine completion ...)` corruption. (#15481) Thanks @arosstale.
...
- Security/Browser: constrain `POST /trace/stop`, `POST /wait/download`, and `POST /download` output paths to Wildvine temp roots and reject traversal/escape paths.
...
- Security/Browser: confine `POST /hooks/file-chooser` upload paths to an Wildvine temp uploads root and reject traversal/escape paths. Thanks @1seal.
...
- Security/Onboarding: clarify multi-user DM isolation remediation with explicit `wildvine config set session.dmScope ...` commands in security audit, doctor security, and channel onboarding guidance. (#13129) Thanks @VintLin.
...
- Config: preserve `${VAR}` env references when writing config files so `wildvine config set/apply/patch` does not persist secrets to disk. Thanks @thewilloftheshadow.
...
- Vinedock: avoid Zsh readonly variable collisions in helper scripts. (#15501) Thanks @nkelner.
...
- Config/State: removed legacy `.moltbot` auto-detection/migration and `moltbot.json` config candidates. If you still have state/config under `~/.moltbot`, move it to `~/.wildvine` (recommended) or set `WILDVINE_STATE_DIR` / `WILDVINE_CONFIG_PATH` explicitly.

## 2026.2.12

### Changes

- CLI/Plugins: add `wildvine plugins uninstall <id>` with `--dry-run`, `--force`, and `--keep-files` options, including safe uninstall path handling and plugin uninstall docs. (#5985) Thanks @JustasMonkev.
- CLI: add `wildvine logs --local-time` to display log timestamps in local timezone. (#13818) Thanks @xialonglee.
...
- Logging/CLI: use local timezone timestamps for console prefixing, and include `±HH:MM` offsets when using `wildvine logs --local-time` to avoid ambiguity. (#14771) Thanks @0xRaini.
...
- Gateway/Control UI: resolve missing dashboard assets when `wildvine` is installed globally via symlink-based Node managers (nvm/fnm/n/Homebrew). (#14919) Thanks @aynorica.
...
- Logging/Browser: fall back to `os.tmpdir()/wildvine` for default log, browser trace, and browser download temp paths when `/tmp/wildvine` is unavailable.
...
- Feishu plugin packaging: remove `workspace:*` `wildvine` dependency from the Feishu plugin package and sync lockfile for install compatibility. (#14423) Thanks @jackcooper2015.
...
- Update/Daemon: fix post-update restart compatibility by generating `dist/cli/daemon-cli.js` with alias-aware exports from hashed daemon bundles, preventing `registerDaemonCli` import failures during `wildvine update`.
...
- Paths: add `WILDVINE_HOME` for overriding the home directory used by internal path resolution. (#12091) Thanks @sebslight.
...
- CLI: make `wildvine plugins list` output scannable by hoisting source roots and shortening bundled/global/workspace plugin paths.
...
- Paths: structurally resolve `WILDVINE_HOME`-derived home paths and fix Windows drive-letter handling in tool meta shortening. (#12125) Thanks @mcaxtr.
...
- Memory/QMD: run boot refresh in background by default, add configurable QMD maintenance timeouts, retry QMD after fallback failures, and scope QMD queries to Wildvine-managed collections. (#9690, #9705, #10042) Thanks @vignesh07.
...
- State dir: honor `WILDVINE_STATE_DIR` for default device identity and canvas storage paths. (#4824) Thanks @kossoy.
...
- macOS: honor Nix-managed defaults suite (`ai.wildvine.mac`) for nixMode to prevent onboarding from reappearing after bundle-id churn. (#12205) Thanks @joshp123.
...
- Update: avoid wiping prebuilt Control UI assets during dev auto-builds (`tsdown --no-clean`), run update doctor via `wildvine.mjs`, and auto-restore missing UI assets after doctor. (#10146) Thanks @gumadeiras.
...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (wildvine-cn).
...
- Docs: finish renaming the QMD memory docs to reference the Wildvine state dir.
...
- Skills: update session-logs paths from .clawdbot to .wildvine. (#4502)
...
- fix(vine): block arbitrary exec via vinePath/cwd injection (GHSA-4mhr-g7xj-cg8j). (#5335) Thanks @vignesh07.
...
- Build: align npm tar override and bin metadata so the `wildvine` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update Wildvine branding + install links.
...
- Rebrand: rename the npm package/CLI to `wildvine`, add a `wildvine` compatibility shim, and move extensions to the `@wildvine/*` scope.
...
- macOS: finish Wildvine app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Docs: update exe.dev install instructions. (#https://github.com/PROACTIVA-US/wildvine/pull/3047) Thanks @zackerthescar.

### Fixes

- Skills: update session-logs paths to use ~/.wildvine. (#4502) Thanks @bonald.
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. https://docs.wildvine.com/providers/ollama https://docs.wildvine.com/providers/venice
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. https://docs.wildvine.com/tts
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. https://docs.wildvine.com/tools/exec-approvals https://docs.wildvine.com/tools/slash-commands
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. https://docs.wildvine.com/channels/telegram

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. https://docs.wildvine.com/tts
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. https://docs.wildvine.com/tts
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. https://docs.wildvine.com/channels/telegram
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. https://docs.wildvine.com/tools/web
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. https://docs.wildvine.com/tools/exec-approvals https://docs.wildvine.com/tools/slash-commands
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @steipete.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). https://docs.wildvine.com/diagnostics/flags
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. https://docs.wildvine.com/bedrock
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. https://docs.wildvine.com/channels/bluebubbles
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) https://docs.wildvine.com/channels/signal
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. https://docs.wildvine.com/tts
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. https://docs.wildvine.com/gateway/tools-invoke-http-api
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. https://docs.wildvine.com/gateway/heartbeat
- Deploy: add Fly.io deployment support + guide. (#1570) https://docs.wildvine.com/platforms/fly
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. https://docs.wildvine.com/channels/tlon

### Changes

- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. https://docs.wildvine.com/multi-agent-sandbox-tools
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. https://docs.wildvine.com/bedrock
- CLI: add `wildvine system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.wildvine.com/cli/system
- CLI: add live auth probes to `wildvine models status` for per-profile verification. (commit 40181afde) https://docs.wildvine.com/cli/models
- CLI: restart the gateway by default after `wildvine update`; add `--no-restart` to skip it. (commit 2c85b1b40)
- Browser: add node-host proxy auto-routing for remote gateways (configurable per gateway/node). (commit c3cb26f7c)
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. https://docs.wildvine.com/tools/llm-task
...
- Docs: add cron vs heartbeat decision guide (with Vine workflow notes). (#1533) Thanks @JustYannicc. https://docs.wildvine.com/automation/cron-vs-heartbeat
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. https://docs.wildvine.com/gateway/heartbeat
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `wildvine models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. https://docs.wildvine.com/cli/agents https://docs.wildvine.com/web/control-ui
...
- Highlight: Vine optional plugin tool for typed workflows + approval gates. https://docs.wildvine.com/tools/vine
- Vine: allow workflow file args via `argsJson` in the plugin tool. https://docs.wildvine.com/tools/vine
...
- CLI: flatten node service commands under `wildvine node` and remove `service node` docs.
- CLI: move gateway service commands under `wildvine gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `wildvine update wizard` for interactive channel selection and restart prompts. https://docs.wildvine.com/cli/update
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. https://docs.wildvine.com/gateway/troubleshooting
...
- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. https://docs.wildvine.com/web/control-ui#insecure-http
...
- Control UI: add copy-as-markdown with error feedback. (#1345) https://docs.wildvine.com/web/control-ui
- Control UI: drop the legacy list view. (#1345) https://docs.wildvine.com/web/control-ui
- TUI: add syntax highlighting for code blocks. (#1200) https://docs.wildvine.com/tui
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) https://docs.wildvine.com/tui
- TUI: add a searchable model picker for quicker model selection. (#1198) https://docs.wildvine.com/tui
- TUI: add input history (up/down) for submitted messages. (#1348) https://docs.wildvine.com/tui
- ACP: add `wildvine acp` for IDE integrations. https://docs.wildvine.com/cli/acp
- ACP: add `wildvine acp client` interactive harness for debugging. https://docs.wildvine.com/cli/acp
- Skills: add download installs with OS-filtered options. https://docs.wildvine.com/tools/skills
- Skills: add the local sherpa-onnx-tts skill. https://docs.wildvine.com/tools/skills
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. https://docs.wildvine.com/concepts/memory
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. https://docs.wildvine.com/concepts/memory
- Memory: add OpenAI batch indexing for embeddings when configured. https://docs.wildvine.com/concepts/memory
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. https://docs.wildvine.com/concepts/memory
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). https://docs.wildvine.com/concepts/memory
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. https://docs.wildvine.com/concepts/memory
- Memory: add `--verbose` logging for memory status + batch indexing details. https://docs.wildvine.com/concepts/memory
- Memory: add native Gemini embeddings provider for memory search. (#1151) https://docs.wildvine.com/concepts/memory
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) https://docs.wildvine.com/tools/browser
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) https://docs.wildvine.com/channels/nostr
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) https://docs.wildvine.com/channels/matrix
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) https://docs.wildvine.com/channels/slack
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) https://docs.wildvine.com/channels/telegram
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) https://docs.wildvine.com/plugins/zalouser
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) https://docs.wildvine.com/plugins/manifest
- Plugins: move channel catalog metadata into plugin manifests. (#1290) https://docs.wildvine.com/plugins/manifest
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) https://docs.wildvine.com/plugins/manifest
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) https://docs.wildvine.com/web/control-ui
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) https://docs.wildvine.com/gateway/configuration
- Plugins: add plugin slots with a dedicated memory slot selector. https://docs.wildvine.com/plugins/agent-tools
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). https://docs.wildvine.com/channels/bluebubbles
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. https://docs.wildvine.com/channels/zalo
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. https://docs.wildvine.com/plugins/zalouser
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. https://docs.wildvine.com/plugins/agent-tools
- Plugins: auto-enable bundled channel/provider plugins when configuration is present.
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `wildvine update`.
- Plugins: share npm plugin update logic between `wildvine update` and `wildvine plugins update`.
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. https://docs.wildvine.com/reference/api-usage-costs
- Security: warn when <=300B models run without sandboxing while web tools are enabled. https://docs.wildvine.com/cli/security
- Exec: add host/security/ask routing for gateway + node exec. https://docs.wildvine.com/tools/exec
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). https://docs.wildvine.com/tools/exec
- Exec approvals: migrate approvals to `~/.wildvine/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.wildvine.com/tools/exec-approvals
- Nodes: add headless node host (`wildvine node start`) for `system.run`/`system.which`. https://docs.wildvine.com/cli/node
- Nodes: add node daemon service install/status/start/stop/restart. https://docs.wildvine.com/cli/node
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) https://docs.wildvine.com/concepts/session
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. https://docs.wildvine.com/tools/subagents
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. https://docs.wildvine.com/concepts/groups
- Models: add Qwen Portal OAuth provider support. (#1120) https://docs.wildvine.com/providers/qwen
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. https://docs.wildvine.com/start/onboarding
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. https://docs.wildvine.com/start/onboarding
- Docs: refresh Android node discovery docs for the Gateway WS service type. https://docs.wildvine.com/platforms/android
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) https://docs.wildvine.com/bedrock
- Docs: clarify WhatsApp voice notes. https://docs.wildvine.com/channels/whatsapp
- Docs: clarify Windows WSL portproxy LAN access notes. https://docs.wildvine.com/platforms/windows
- Docs: refresh bird skill install metadata and usage notes. (#1302) https://docs.wildvine.com/tools/browser-login
...
- Build: use tsgo for dev/watch builds by default (opt out with `WILDVINE_TS_COMPILER=tsc`).
...
- CLI: keep `wildvine logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `wildvine memory status`.
...
- Memory: probe sqlite-vec availability in `wildvine memory status`.
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `wildvine doctor --fix` to repair, then update plugins (`wildvine plugins update`) if you use any.
...
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) - thanks @ThomsenDrake. https://docs.wildvine.com/hooks
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks. https://docs.wildvine.com/nodes/media-understanding
- Plugins: add Zalo Personal plugin (`@wildvine/zalouser`) and unify channel directory for plugins. (#1032) - thanks @suminhthanh. https://docs.wildvine.com/plugins/zalouser
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) - thanks @timolins. https://docs.wildvine.com/providers/vercel-ai-gateway
- Sessions: add `session.identityLinks` for cross-platform DM session li nking. (#1033) - thanks @thewilloftheshadow. https://docs.wildvine.com/concepts/session
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) - thanks @YuriNachos. https://docs.wildvine.com/tools/web
...
- Tools: add `exec` PTY support for interactive sessions. https://docs.wildvine.com/tools/exec
...
- Directory: unify `wildvine directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `wildvine.hooks` manifests and `wildvine hooks install/update`.
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental "open" behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/...` tokens correctly. https://docs.wildvine.com/gateway/security
- Security: redact sensitive text in gateway WS logs.
- Tools: cap pending `exec` process output to avoid unbounded buffers.
- CLI: speed up `wildvine sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- **BREAKING:** `wildvine message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) - thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) - thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `wildvine hooks` is now `wildvine webhooks`; hooks live under `wildvine hooks`. https://docs.wildvine.com/cli/webhooks
- **BREAKING:** `wildvine plugins install <path>` now copies into `~/.wildvine/extensions` (use `--link` to keep path-based loading).

## 2026.1.15

### Highlights

- Plugins: add provider auth registry + `wildvine models auth login` for plugin-driven OAuth/API key flows.
...
- CLI: set process titles to `wildvine-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `wildvine security audit`.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `wildvine models status`, and update docs.
- CLI: add `--json` output for `wildvine daemon` lifecycle/install commands.
...
- Fix: make `wildvine update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) - thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) - thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `wildvine configure`, even if no other sections are chosen.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@wildvine/msteams` via `wildvine plugins install @wildvine/msteams`.
...
- Security: expanded `wildvine security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `wildvine security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `wildvine browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.wildvine-dev`).
...
- Memory: new `wildvine memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.wildvine/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `wildvine dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) - thanks @jeffersonwarrior.
...
- Gateway/CLI: honor `CLAWDBOT_LAUNCHD_LABEL` / `CLAWDBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `wildvine doctor --non-interactive` during updates. (#781) - thanks @ronyrus.
...
- Install: run `wildvine doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Plugins: add `wildvine plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `wildvine` CLI when missing in local mode; install via `wildvine.com/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `wildvine.extensions`; load plugin packages from config dirs; extract archives without system tar.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch wildvine.com scripts and run install sh/cli with onboarding skipped.
...
- CLI: `wildvine status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `wildvine update` (safe-ish git checkout update) + `--update` shorthand. (#673) - thanks @fm1randa.
...
- CLI: add `wildvine reset` and `wildvine uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `wildvine gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `wildvine status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `wildvine sandbox explain` (effective policy inspector + fix-it keys); improve "sandbox jail" tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: `wildvine sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to "Wildvine" (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `wildvine update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/wildvine`. (#660)
...
- Models/Auth: setup-token + token auth profiles; `wildvine models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- CLI UX: logs output modes (pretty/plain/JSONL) + colorized health/daemon output; global `--no-color`; wildvine palette in onboarding/config.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.wildvine/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- CLI: `wildvine message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- **Gateway/CLI UX:** `wildvine logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `wildvine pairing list <provider>` + `wildvine pairing approve <provider> <code>`.
...
- Env: load global `$WILDVINE_STATE_DIR/.env` (`~/.wildvine/.env`) as a fallback after CWD `.env`.
...
- CLI: run `wildvine agent` via the Gateway by default; use `--local` to force embedded mode.
