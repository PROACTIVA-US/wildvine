...
- Wildvinedock: avoid Zsh readonly variable collisions in helper scripts. (#15501) Thanks @nkelner.
...
- Signal: render mention placeholders as `@uuid`/`@phone` so mention gating and Wildvinebot targeting work. (#2013) Thanks @alexgleason.
...
- Feishu plugin packaging: remove `workspace:*` `wildvine` dependency from `extensions/feishu` and sync lockfile for install compatibility. (#14423) Thanks @jackcooper2015.
...
- Docker: add WildvineDock shell helpers for Docker workflows. (#12817) Thanks @Olshansk.
...
- macOS: honor Nix-managed defaults suite (`com.wildvine.mac`) for nixMode to prevent onboarding from reappearing after bundle-id churn. (#12205) Thanks @joshp123.
...
- Skills: update session-logs paths from .wildvinebot to .wildvine. (#4502)
...
- Docs: update exe.dev install instructions. (#https://github.com/wildvine/wildvine/pull/3047) Thanks @zackerthescar.
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. https://docs.wildvine.bot/providers/ollama https://docs.wildvine.bot/providers/venice
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. https://docs.wildvine.bot/tts
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. https://docs.wildvine.bot/tools/exec-approvals https://docs.wildvine.bot/tools/slash-commands
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. https://docs.wildvine.bot/channels/telegram

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. https://docs.wildvine.bot/tts
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. https://docs.wildvine.bot/tts
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. https://docs.wildvine.bot/channels/telegram
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. https://docs.wildvine.bot/tools/web
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. https://docs.wildvine.bot/tools/exec-approvals https://docs.wildvine.bot/tools/slash-commands
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @Glucksberg.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). https://docs.wildvine.bot/diagnostics/flags
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. https://docs.wildvine.bot/bedrock
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. https://docs.wildvine.bot/channels/bluebubbles
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) https://docs.wildvine.bot/channels/signal
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. https://docs.wildvine.bot/tts
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. https://docs.wildvine.bot/gateway/tools-invoke-http-api
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. https://docs.wildvine.bot/gateway/heartbeat
- Deploy: add Fly.io deployment support + guide. (#1570) https://docs.wildvine.bot/platforms/fly
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. https://docs.wildvine.bot/channels/tlon

### Changes

- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. https://docs.wildvine.bot/multi-agent-sandbox-tools
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. https://docs.wildvine.bot/bedrock
- CLI: add `wildvine system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.wildvine.bot/cli/system
- CLI: add live auth probes to `wildvine models status` for per-profile verification. (commit 40181afde) https://docs.wildvine.bot/cli/models
...
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. https://docs.wildvine.bot/tools/llm-task
...
- Docs: add cron vs heartbeat decision guide (with Lobster workflow notes). (#1533) Thanks @JustYannicc. https://docs.wildvine.bot/automation/cron-vs-heartbeat
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. https://docs.wildvine.bot/gateway/heartbeat
...
- Doctor: honor WILDVINE_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. https://docs.wildvine.bot/cli/agents https://docs.wildvine.bot/web/control-ui
...
- Highlight: Lobster optional plugin tool for typed workflows + approval gates. https://docs.wildvine.bot/tools/lobster
- Lobster: allow workflow file args via `argsJson` in the plugin tool. https://docs.wildvine.bot/tools/lobster
...
- CLI: add `wildvine update wizard` for interactive channel selection and restart prompts. https://docs.wildvine.bot/cli/update
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. https://docs.wildvine.bot/gateway/troubleshooting
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking

- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. https://docs.wildvine.bot/web/control-ui#insecure-http
...
- Control UI: add copy-as-markdown with error feedback. (#1345) https://docs.wildvine.bot/web/control-ui
- Control UI: drop the legacy list view. (#1345) https://docs.wildvine.bot/web/control-ui
- TUI: add syntax highlighting for code blocks. (#1200) https://docs.wildvine.bot/tui
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) https://docs.wildvine.bot/tui
- TUI: add a searchable model picker for quicker model selection. (#1198) https://docs.wildvine.bot/tui
- TUI: add input history (up/down) for submitted messages. (#1348) https://docs.wildvine.bot/tui
- ACP: add `wildvine acp` for IDE integrations. https://docs.wildvine.bot/cli/acp
- ACP: add `wildvine acp client` interactive harness for debugging. https://docs.wildvine.bot/cli/acp
- Skills: add download installs with OS-filtered options. https://docs.wildvine.bot/tools/skills
- Skills: add the local sherpa-onnx-tts skill. https://docs.wildvine.bot/tools/skills
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. https://docs.wildvine.bot/concepts/memory
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. https://docs.wildvine.bot/concepts/memory
- Memory: add OpenAI batch indexing for embeddings when configured. https://docs.wildvine.bot/concepts/memory
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. https://docs.wildvine.bot/concepts/memory
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). https://docs.wildvine.bot/concepts/memory
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. https://docs.wildvine.bot/concepts/memory
- Memory: add `--verbose` logging for memory status + batch indexing details. https://docs.wildvine.bot/concepts/memory
- Memory: add native Gemini embeddings provider for memory search. (#1151) https://docs.wildvine.bot/concepts/memory
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) https://docs.wildvine.bot/tools/browser
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) https://docs.wildvine.bot/channels/nostr
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) https://docs.wildvine.bot/channels/matrix
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) https://docs.wildvine.bot/channels/slack
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) https://docs.wildvine.bot/channels/telegram
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) https://docs.wildvine.bot/plugins/zalouser
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) https://docs.wildvine.bot/plugins/manifest
- Plugins: move channel catalog metadata into plugin manifests. (#1290) https://docs.wildvine.bot/plugins/manifest
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) https://docs.wildvine.bot/plugins/manifest
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) https://docs.wildvine.bot/web/control-ui
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) https://docs.wildvine.bot/gateway/configuration
- Plugins: add plugin slots with a dedicated memory slot selector. https://docs.wildvine.bot/plugins/agent-tools
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). https://docs.wildvine.bot/channels/bluebubbles
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. https://docs.wildvine.bot/channels/zalo
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. https://docs.wildvine.bot/plugins/zalouser
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. https://docs.wildvine.bot/plugins/agent-tools
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. https://docs.wildvine.bot/reference/api-usage-costs
- Security: warn when <=300B models run without sandboxing while web tools are enabled. https://docs.wildvine.bot/cli/security
- Exec: add host/security/ask routing for gateway + node exec. https://docs.wildvine.bot/tools/exec
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). https://docs.wildvine.bot/tools/exec
- Exec approvals: migrate approvals to `~/.wildvine/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.wildvine.bot/tools/exec-approvals
- Nodes: add headless node host (`wildvine node start`) for `system.run`/`system.which`. https://docs.wildvine.bot/cli/node
- Nodes: add node daemon service install/status/start/stop/restart. https://docs.wildvine.bot/cli/node
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) https://docs.wildvine.bot/concepts/session
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. https://docs.wildvine.bot/tools/subagents
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. https://docs.wildvine.bot/concepts/groups
- Models: add Qwen Portal OAuth provider support. (#1120) https://docs.wildvine.bot/providers/qwen
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. https://docs.wildvine.bot/start/onboarding
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. https://docs.wildvine.bot/start/onboarding
- Docs: refresh Android node discovery docs for the Gateway WS service type. https://docs.wildvine.bot/platforms/android
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) https://docs.wildvine.bot/bedrock
- Docs: clarify WhatsApp voice notes. https://docs.wildvine.bot/channels/whatsapp
- Docs: clarify Windows WSL portproxy LAN access notes. https://docs.wildvine.bot/platforms/windows
- Docs: refresh bird skill install metadata and usage notes. (#1302) https://docs.wildvine.bot/tools/browser-login
...
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) — thanks @ThomsenDrake. https://docs.wildvine.bot/hooks
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks. https://docs.wildvine.bot/nodes/media-understanding
- Plugins: add Zalo Personal plugin (`@wildvine/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.wildvine.bot/plugins/zalouser
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) — thanks @timolins. https://docs.wildvine.bot/providers/vercel-ai-gateway
- Sessions: add `session.identityLinks` for cross-platform DM session li nking. (#1033) — thanks @thewilloftheshadow. https://docs.wildvine.bot/concepts/session
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) — thanks @YuriNachos. https://docs.wildvine.bot/tools/web
...
- **BREAKING:** `wildvine hooks` is now `wildvine webhooks`; hooks live under `wildvine hooks`. https://docs.wildvine.bot/cli/webhooks
...
- Tools: add `exec` PTY support for interactive sessions. https://docs.wildvine.bot/tools/exec
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental “open” behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/…` tokens correctly. https://docs.wildvine.bot/gateway/security
...
- Daemon: share profile/state-dir resolution across service helpers and honor `WILDVINE_STATE_DIR` for Windows task scripts.
...
- Onboarding/Auth: honor `WILDVINE_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) — thanks @roshanasingh4.
...
- Gateway/CLI: honor `WILDVINE_LAUNCHD_LABEL` / `WILDVINE_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `wildvine doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- macOS: prompt to install the global `wildvine` CLI when missing in local mode; install via `wildvine.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch wildvine.ai scripts and run install sh/cli with onboarding skipped.
- Installer UX: support `WILDVINE_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Config: expand `~` in `WILDVINE_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) — thanks @pasogott.
...
- CLI: respect `WILDVINE_STATE_DIR` for node pairing + voice wake settings storage. (#664) — thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`WILDVINE_LIVE_GATEWAY_IMAGE_PROBE=1`).
...
- **Docs:** new FAQ/WildvineHub/config examples/showcase entries and clarified auth, sandbox, and systemd docs.
...
- Commands: unify /status (inline) and command auth across providers; group bypass for authorized control commands; remove Discord /wildvine slash handler.
