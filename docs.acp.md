...
This document describes how the Wildvine ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`wildvine acp` exposes an ACP agent over stdio and forwards prompts to a running
Wildvine Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Wildvine Gateway
...
- ACP client spawns `wildvine acp` and speaks ACP messages over stdio.
...
`wildvine acp` resolves the Gateway URL and auth from CLI flags or config:
