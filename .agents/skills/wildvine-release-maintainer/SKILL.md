---
name: wildvine-release-maintainer
description: Maintainer workflow for Wildvine releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# Wildvine Release Maintainer
...
- Core `wildvine` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Every Wildvine release ships the npm package and macOS app together.
- The production Sparkle feed lives at `https://raw.githubusercontent.com/wildvine/wildvine/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `wildvine` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `wildvine YYYY.M.D-beta.N`
...
```bash
  WILDVINE_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/wildvine-npm-postpublish-verify.ts <published-version>
```
...
- Always validate the Wildvine npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `wildvine/wildvine` and the real mac preflight in
  `wildvine/releases-private` for every release.
...
- Wildvine publish uses GitHub trusted publishing.
...
- `.github/workflows/macos-release.yml` in `wildvine/wildvine` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `wildvine/releases-private/.github/workflows/wildvine-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `wildvine/releases-private/.github/workflows/wildvine-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@wildvine/wildvine-release-managers` before publish continues.
- Mac publish uses
  `wildvine/releases-private/.github/workflows/wildvine-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `wildvine/wildvine`
  automatically when `WILDVINE_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for Wildvine releases.
- `@wildvine/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/wildvine-npm-release.yml` with `preflight_only=true`
   and wait for it to pass. Save that run id because the real publish requires
   it to reuse the prepared npm tarball.
10. Start `.github/workflows/macos-release.yml` in `wildvine/wildvine` and wait
...
14. Start `.github/workflows/wildvine-npm-release.yml` with the same tag for
    the real publish and pass the successful npm `preflight_run_id`.
15. Wait for `npm-release` approval from `@wildvine/wildvine-release-managers`.
16. Start
    `wildvine/releases-private/.github/workflows/wildvine-macos-publish.yml`
...
- Use `wildvine-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
