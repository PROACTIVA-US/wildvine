...
```bash
export WILDVINE_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.wildvine/wildvine.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env WILDVINE_PARALLELS_DISCORD_TOKEN \
...
- Use the `wildvine` wrapper for guest `message send/read`; `node wildvine.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/wildvine-parallels-smoke.*`
...
- host inbound nonce appears in `wildvine message read` output
