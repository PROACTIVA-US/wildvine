...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.WILDVINE_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "WILDVINE_TEST_FAST", value: process.env.WILDVINE_TEST_FAST },
    ...
    { key: "WILDVINE_STATE_DIR", value: process.env.WILDVINE_STATE_DIR },
    { key: "WILDVINE_CONFIG_PATH", value: process.env.WILDVINE_CONFIG_PATH },
    { key: "WILDVINE_GATEWAY_PORT", value: process.env.WILDVINE_GATEWAY_PORT },
    { key: "WILDVINE_BRIDGE_ENABLED", value: process.env.WILDVINE_BRIDGE_ENABLED },
    { key: "WILDVINE_BRIDGE_HOST", value: process.env.WILDVINE_BRIDGE_HOST },
    { key: "WILDVINE_BRIDGE_PORT", value: process.env.WILDVINE_BRIDGE_PORT },
    { key: "WILDVINE_CANVAS_HOST_PORT", value: process.env.WILDVINE_CANVAS_HOST_PORT },
    { key: "WILDVINE_TEST_HOME", value: process.env.WILDVINE_TEST_HOME },
    { key: "WILDVINE_AGENT_DIR", value: process.env.WILDVINE_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.WILDVINE_TEST_HOME = tempHome;
  process.env.WILDVINE_TEST_FAST = "1";
  ...
  delete process.env.WILDVINE_BRIDGE_ENABLED;
  delete process.env.WILDVINE_BRIDGE_HOST;
  delete process.env.WILDVINE_BRIDGE_PORT;
  delete process.env.WILDVINE_CANVAS_HOST_PORT;
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.WILDVINE_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".wildvine");
  const priorIsolatedHome = params.env.WILDVINE_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".wildvine")
  ) {
    realStateDir = path.join(params.realHome, ".wildvine");
  }
  const tempStateDir = path.join(params.tempHome, ".wildvine");
  fs.mkdirSync(tempStateDir, { recursive: true });

  const realConfigPath = params.env.WILDVINE_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.WILDVINE_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "wildvine.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "wildvine.json"),
...
export function installTestEnv(): { cleanup: () => void; tempHome: string } {
  const live =
    process.env.LIVE === "1" ||
    process.env.WILDVINE_LIVE_TEST === "1" ||
    process.env.WILDVINE_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.WILDVINE_LIVE_USE_REAL_HOME);
