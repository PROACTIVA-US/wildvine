...
function createTempHome(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-test-env-real-home-"));
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-wildvine.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.wildvine/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.wildvine/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".wildvine", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".wildvine", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.WILDVINE_LIVE_TEST_QUIET = "1";
    process.env.WILDVINE_CONFIG_PATH = "~/custom-wildvine.json5";
    process.env.WILDVINE_TEST_HOME = priorIsolatedHome;
    process.env.WILDVINE_STATE_DIR = path.join(priorIsolatedHome, ".wildvine");
    ...
    expect(process.env.WILDVINE_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".wildvine", "wildvine.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".wildvine", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".wildvine", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.WILDVINE_LIVE_TEST = "1";
    process.env.WILDVINE_LIVE_USE_REAL_HOME = "1";
    process.env.WILDVINE_LIVE_TEST_QUIET = "1";
