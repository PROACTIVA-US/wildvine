...
describe("loadVitestExperimentalConfig", () => {
  ...
  it("passes through the filesystem module cache path when provided", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          WILDVINE_VITEST_FS_MODULE_CACHE_PATH: "/tmp/wildvine-vitest-cache",
        },
        "linux",
      ),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
        fsModuleCachePath: "/tmp/wildvine-vitest-cache",
      },
    });
  });
  ...
  it("enables import timing output and import breakdown reporting", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          WILDVINE_VITEST_IMPORT_DURATIONS: "true",
          WILDVINE_VITEST_PRINT_IMPORT_BREAKDOWN: "1",
        },
        "linux",
      ),
    ).toEqual({
