...
const clearPlannerShardEnv = (env) => {
  const nextEnv = { ...env };
  delete nextEnv.WILDVINE_TEST_SHARDS;
  delete nextEnv.WILDVINE_TEST_SHARD_INDEX;
  delete nextEnv.WILDVINE_TEST_FORCE_THREADS;
  delete nextEnv.WILDVINE_TEST_FORCE_FORKS;
  delete nextEnv.WILDVINE_TEST_DISABLE_THREAD_EXPANSION;
  delete nextEnv.WILDVINE_TEST_SHOW_POOL_DECISION;
  delete nextEnv.WILDVINE_TEST_PROFILE;
  delete nextEnv.WILDVINE_TEST_WORKERS;
  delete nextEnv.WILDVINE_TEST_SKIP_DEFAULT;
  delete nextEnv.WILDVINE_TEST_INCLUDE_EXTENSIONS;
  delete nextEnv.WILDVINE_TEST_INCLUDE_CHANNELS;
  delete nextEnv.WILDVINE_TEST_INCLUDE_GATEWAY;
  delete nextEnv.WILDVINE_TEST_UNIT_FAST_BATCH_TARGET_MS;
  return nextEnv;
};
...
const HIGH_MEMORY_LOCAL_PLANNER_ENV = {
  RUNNER_OS: "macOS",
  WILDVINE_TEST_HOST_CPU_COUNT: "12",
  WILDVINE_TEST_HOST_MEMORY_GIB: "128",
} as const;
...
function runManifestOutputWriter(workflow: string, envOverrides: NodeJS.ProcessEnv = {}): string {
  const outputPath = path.join(os.tmpdir(), `wildvine-${workflow}-output-${Date.now()}.txt`);
  try {
...
describe("scripts/test-parallel lane planning", () => {
  ...
  it("recycles default local unit-fast runs into bounded batches", () => {
    const output = runPlannerPlan(["--plan"], {
      CI: "",
      WILDVINE_TEST_UNIT_FAST_LANES: "1",
      WILDVINE_TEST_UNIT_FAST_BATCH_TARGET_MS: "1",
    });
  ...
  it("reports capability-derived output for mid-memory local macOS hosts", () => {
    const output = runPlannerPlan(
      ...
      createLocalPlannerEnv({
        RUNNER_OS: "macOS",
        WILDVINE_TEST_HOST_CPU_COUNT: "10",
        WILDVINE_TEST_HOST_MEMORY_GIB: "64",
      }),
    );
  ...
  it("prints the planner-backed CI manifest as JSON", () => {
    const output = runPlannerPlan(["--ci-manifest"], {
      ...
      WILDVINE_CI_DOCS_ONLY: "false",
      WILDVINE_CI_DOCS_CHANGED: "false",
      WILDVINE_CI_RUN_NODE: "true",
      WILDVINE_CI_RUN_MACOS: "true",
      WILDVINE_CI_RUN_ANDROID: "false",
      WILDVINE_CI_RUN_WINDOWS: "true",
      WILDVINE_CI_RUN_SKILLS_PYTHON: "false",
      WILDVINE_CI_HAS_CHANGED_EXTENSIONS: "false",
      WILDVINE_CI_CHANGED_EXTENSIONS_MATRIX: '{"include":[]}',
    });
  ...
  it("writes install-smoke outputs in install-smoke mode", () => {
    const outputs = runManifestOutputWriter("install-smoke", {
      WILDVINE_CI_DOCS_ONLY: "false",
      WILDVINE_CI_RUN_CHANGED_SMOKE: "true",
    });
  ...
  it("passes through vitest --mode values that are not wrapper runtime overrides", () => {
    const output = runPlannerPlan(
      ...
      createLocalPlannerEnv({
        RUNNER_OS: "Linux",
        WILDVINE_TEST_HOST_CPU_COUNT: "16",
        WILDVINE_TEST_HOST_MEMORY_GIB: "128",
      }),
    );
  ...
  it("rejects explicit existing files that are not known test files", () => {
    const tempFilePath = path.join(os.tmpdir(), `wildvine-non-test-${Date.now()}.ts`);
