...
describe("write-cli-startup-metadata", () => {
  ...
  it("captures async root help bundle output", async () => {
    const distDir = createTempDir("wildvine-root-help-");
    ...
    writeFileSync(
      ...
      [
        ...
        "  process.stdout.write('Wildvine help\\n');",
        "}",
        "",
      ].join("\n"),
      "utf8",
    ...
    await expect(renderBundledRootHelpText(distDir)).resolves.toBe("Wildvine help\n");
  });

  it("writes startup metadata with populated root help text", async () => {
    const tempRoot = createTempDir("wildvine-startup-metadata-");
    ...
    writeFileSync(
      ...
      "export async function outputRootHelp() { process.stdout.write('Usage: wildvine\\n'); }\n",
      "utf8",
    );
    writeFileSync(
    ...
    expect(written.rootHelpText).toBe("Usage: wildvine\n");
  });
});
