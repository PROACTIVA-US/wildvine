...
describe("test planner", () => {
  ...
  it("uses smaller shared extension batches on constrained local hosts", () => {
    const env = {
      RUNNER_OS: "macOS",
      WILDVINE_TEST_HOST_CPU_COUNT: "8",
      WILDVINE_TEST_HOST_MEMORY_GIB: "16",
      WILDVINE_TEST_LOAD_AWARE: "0",
    };
  ...
  it("caps CI extension batch concurrency when multiple shared batches are scheduled", () => {
    const env = {
      ...
      GITHUB_ACTIONS: "true",
      RUNNER_OS: "Linux",
      WILDVINE_TEST_HOST_CPU_COUNT: "4",
      WILDVINE_TEST_HOST_MEMORY_GIB: "16",
    };
    const artifacts = createExecutionArtifacts(env);
    const plan = buildExecutionPlan(
  ...
  it("does not leak default-plan shard assignments into targeted units with the same id", () => {
    ...
    const plan = buildExecutionPlan(
      ...
      {
        env: {
          WILDVINE_TEST_SHARDS: "4",
          WILDVINE_TEST_SHARD_INDEX: "2",
          WILDVINE_TEST_LOAD_AWARE: "0",
        },
        writeTempJsonArtifact: artifacts.writeTempJsonArtifact,
      },
  ...
  it("honors the requested Windows platform when building the cache path", () => {
    expect(
      resolveVitestFsModuleCachePath({
        cwd: "/repo",
        env: {
          WILDVINE_VITEST_FS_MODULE_CACHE: "1",
        },
        platform: "win32",
        unitId: "unit-fast-1",
      }),
  ...
  it("respects an explicit cache path override", () => {
    expect(
      resolveVitestFsModuleCachePath({
        cwd: "/repo",
        env: {
          WILDVINE_VITEST_FS_MODULE_CACHE_PATH: "/tmp/custom-vitest-cache",
        },
        platform: "linux",
        unitId: "unit-fast-1",
      }),
  ...
  it("does not force a cache path when the cache is disabled", () => {
    expect(
      resolveVitestFsModuleCachePath({
        cwd: "/repo",
        env: {
          WILDVINE_VITEST_FS_MODULE_CACHE: "0",
        },
        platform: "linux",
        unitId: "unit-fast-1",
      }),
