  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.WILDVINE_HOME;
        delete env.WILDVINE_STATE_DIR;
        delete env.WILDVINE_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "wildvine.mjs");
