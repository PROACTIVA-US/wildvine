...
process.env.WILDVINE_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime Wildvine modules.
const testEnv = withIsolatedTestHome();

installProcessWarningFilter();

const REGISTRY_STATE = Symbol.for("wildvine.pluginRegistryState");
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: WildvineConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: WildvineConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
