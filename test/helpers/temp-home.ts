...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked WILDVINE_HOME.
  delete process.env.WILDVINE_HOME;
  process.env.WILDVINE_STATE_DIR = path.join(base, ".wildvine");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "wildvine-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".wildvine", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
