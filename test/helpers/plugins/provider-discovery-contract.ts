...
function installDiscoveryHooks(state: DiscoveryState) {
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("wildvine/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("wildvine/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("wildvine/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
