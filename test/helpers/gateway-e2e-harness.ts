...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `wildvine-e2e-${name}-`));
  const configDir = path.join(homeDir, ".wildvine");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          WILDVINE_SKIP_PROVIDERS: "1",
          WILDVINE_SKIP_GMAIL_WATCHER: "1",
          WILDVINE_SKIP_CRON: "1",
          WILDVINE_SKIP_BROWSER_CONTROL_SERVER: "1",
          WILDVINE_SKIP_CANVAS_HOST: "1",
          WILDVINE_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
