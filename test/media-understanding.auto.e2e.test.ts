...
import type { WildvineConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("wildvine-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("wildvine-bin-sherpa-");
      const modelDir = await makeTempDir("wildvine-sherpa-model-");
      ...
      const cfg: WildvineConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("wildvine-bin-whispercpp-");
      const modelDir = await makeTempDir("wildvine-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("wildvine-bin-gemini-");
      ...
      const cfg: WildvineConfig = { tools: { media: { image: {} } } };
