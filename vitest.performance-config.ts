...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.WILDVINE_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.WILDVINE_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.WILDVINE_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.WILDVINE_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.WILDVINE_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.WILDVINE_VITEST_PRINT_IMPORT_BREAKDOWN)) {
