 ...
 */
export type WildvinePluginConfigSchema = {
...
export type WildvinePluginToolContext = {
  config?: WildvineConfig;
  /** Active runtime-resolved config snapshot when one is available. */
  runtimeConfig?: WildvineConfig;
  workspaceDir?: string;
  agentDir?: string;
  agentId?: string;
  sessionKey?: string;
...
export type WildvinePluginToolFactory = (
  ctx: WildvinePluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type WildvinePluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type WildvinePluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<WildvineConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Wildvine uses this to expose
 ...
 *
 * Runs after Wildvine resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Wildvine auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Wildvine wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Wildvine builds its generic
...
/**
 * Provider-owned stream wrapper hook after Wildvine applies its generic
 ...
 *
 * Return `true` or `false` to override Wildvine's built-in provider cache TTL
 ...
 *
 * Runs only after Wildvine exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Wildvine should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Wildvine loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Wildvine falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Wildvine calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Wildvine performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Wildvine registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Wildvine wrappers.
   *
   ...
   *
   * Called after Wildvine resolves the raw configured credential but before the
   ...
   * hint than Wildvine's generic auth-store guidance.
   */
   ...
   * Wildvine's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Wildvine should surface a provider-specific hint for
   ...
   * merging. Wildvine deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Wildvine uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `wildvine doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `wildvine doctor` should offer.
   *
   ...
   *
   * Wildvine calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Wildvine's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
   ...
   * when Wildvine has no config-aware runtime context yet.
   *
  ...
  getConfiguredCredentialValue?: (config?: WildvineConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: WildvineConfig, value: unknown) => void;
  applySelectionConfig?: (config: WildvineConfig) => WildvineConfig;
  runSetup?: (ctx: WebSearchProviderSetupContext) => WildvineConfig | Promise<WildvineConfig>;
...
export type WildvinePluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current Wildvine configuration */
  config: WildvineConfig;
 ...
 */
export type WildvinePluginCommandDefinition = {
...
export type WildvinePluginHttpRouteAuth = "gateway" | "plugin";
export type WildvinePluginHttpRouteMatch = "exact" | "prefix";

export type WildvinePluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type WildvinePluginHttpRouteParams = {
  path: string;
  handler: WildvinePluginHttpRouteHandler;
  auth: WildvinePluginHttpRouteAuth;
  match?: WildvinePluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type WildvinePluginCliContext = {
  program: Command;
  config: WildvineConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type WildvinePluginCliRegistrar = (ctx: WildvinePluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Wildvine to keep a plugin command lazy-loaded while still
 ...
 */
export type WildvinePluginCliCommandDescriptor = {
  name: string;
  description: string;
...
export type WildvinePluginServiceContext = {
  config: WildvineConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type WildvinePluginService = {
  id: string;
  start: (ctx: WildvinePluginServiceContext) => void | Promise<void>;
  stop?: (ctx: WildvinePluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Wildvine should inject bundle MCP config for this backend.
   *
...
export type WildvinePluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type WildvinePluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: WildvinePluginApi) => void | Promise<void>;
  activate?: (api: WildvinePluginApi) => void | Promise<void>;
};

export type WildvinePluginModule =
  | WildvinePluginDefinition
  | ((api: WildvinePluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

/** Main registration API injected into native plugin entry files. */
export type WildvinePluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | WildvinePluginToolFactory,
    opts?: WildvinePluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: WildvinePluginHookOptions,
  ) => void;
  registerHttpRoute: (params: WildvinePluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: WildvinePluginChannelRegistration | ChannelPlugin) => void;
  ...
  registerCli: (
    registrar: WildvinePluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Wildvine can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: WildvinePluginCliCommandDescriptor[];
    },
  ) => void;
  registerService: (service: WildvinePluginService) => void;
   ...
   */
  registerCommand: (command: WildvinePluginCommandDefinition) => void;
...
export type PluginFormat = "wildvine" | "bundle";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * Wildvine does not await this callback before allowing or denying the tool call.
     */
...
export type PluginHookBeforeInstallPlugin = {
  /** Canonical plugin id Wildvine will install under. */
  pluginId: string;
...
export type PluginHookBeforeInstallContext = {
  ...
  /** Normalized origin of the install target (e.g. "wildvine-bundled", "plugin-package"). */
  origin?: string;
};
