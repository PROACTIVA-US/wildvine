...
import type { WildvinePluginCliCommandDescriptor } from "./types.js";
...
function resolvePluginCliLoadContext(cfg?: WildvineConfig, env?: NodeJS.ProcessEnv) {
...
async function loadPluginCliMetadataRegistry(
  ...
  return {
    ...context,
    registry: await loadWildvinePluginCliRegistry({
...
async function loadPluginCliCommandRegistry(
  ...
  const context = resolvePluginCliLoadContext(cfg, env);
  const runtimeRegistry = loadWildvinePlugins({
    config: context.config,
    workspaceDir: context.workspaceDir,
    env,
    logger: context.logger,
  ...
  try {
    const metadataRegistry = await loadWildvinePluginCliRegistry({
      config: context.config,
      workspaceDir: context.workspaceDir,
      env,
      logger: context.logger,
...
export async function getPluginCliCommandDescriptors(
  cfg?: WildvineConfig,
  env?: NodeJS.ProcessEnv,
): Promise<WildvinePluginCliCommandDescriptor[]> {
  try {
    const { registry } = await loadPluginCliMetadataRegistry(cfg, env);
    const seen = new Set<string>();
    const descriptors: WildvinePluginCliCommandDescriptor[] = [];
