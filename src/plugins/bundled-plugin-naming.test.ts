...
function readBundledPluginRecords(): BundledPluginRecord[] {
  return fs
    ...
    .flatMap((dirName) => {
      ...
      const manifestPath = path.join(rootDir, "wildvine.plugin.json");
      if (!fs.existsSync(packagePath) || !fs.existsSync(manifestPath)) {
        return [];
      }

      const manifest = readJsonFile<PluginManifestShape>(manifestPath);
      const pkg = readJsonFile<WildvinePackageShape>(packagePath);
      ...
      return [
        {
          ...
          packageName,
          manifestId,
          installNpmSpec: normalizeText(pkg.wildvine?.install?.npmSpec),
          channelId: normalizeText(pkg.wildvine?.channel?.id),
        },
      ];
    });
}
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@wildvine/${pluginId}${suffix}`);
}
  ...
  it.each([
    {
      ...
      message: `Bundled extension package names must stay anchored to the manifest id via @wildvine/<id> or an approved suffix (${ALLOWED_PACKAGE_SUFFIXES.join(", ")}). Update the plugin naming docs and this invariant before adding a new naming form.`,
    ...
    },
    {
      ...
      message:
        "Bundled extension directory names should match wildvine.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    {
      name: "keeps bundled wildvine.install.npmSpec aligned with the package name",
      message:
        "Bundled wildvine.install.npmSpec values must match the package name so install/update paths stay deterministic.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Bundled wildvine.channel.id values must match wildvine.plugin.json:id for the owning plugin.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
