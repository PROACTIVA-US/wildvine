...
const CORE_PLUGIN_ENTRY_IMPORT_RE =
  /import\s*\{[^}]*\bdefinePluginEntry\b[^}]*\}\s*from\s*"wildvine\/plugin-sdk\/core"/;
    ...
    for (const entry of readdirSync(EXTENSIONS_DIR, { withFileTypes: true })) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          wildvine?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.wildvine?.extensions) ? pkg.wildvine.extensions : [];
        if (
          extensions.some(
