...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): WildvineConfig {
  return {
    messages: {
      tts: {
        provider: "openai",
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.WILDVINE_TTS_PREFS;
      process.env.WILDVINE_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.WILDVINE_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: WildvineConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
