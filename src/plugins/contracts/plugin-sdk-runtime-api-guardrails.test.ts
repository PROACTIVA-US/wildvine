...
const RUNTIME_API_EXPORT_GUARDS: Record<string, readonly string[]> = {
  ...
  [bundledPluginFile("imessage", "runtime-api.ts")]: [
    'export { DEFAULT_ACCOUNT_ID, buildChannelConfigSchema, getChatChannelMeta, type ChannelPlugin, type WildvineConfig } from "wildvine/plugin-sdk/core";',
    'export { PAIRING_APPROVED_MESSAGE } from "wildvine/plugin-sdk/channel-status";',
    'export { buildComputedAccountStatusSnapshot, collectStatusIssuesFromLastError } from "wildvine/plugin-sdk/status-helpers";',
    'export { formatTrimmedAllowFromEntries, resolveIMessageConfigAllowFrom, resolveIMessageConfigDefaultTo } from "wildvine/plugin-sdk/channel-config-helpers";',
    'export { looksLikeIMessageTargetId, normalizeIMessageMessagingTarget } from "./src/normalize.js";',
    'export { resolveChannelMediaMaxBytes } from "wildvine/plugin-sdk/media-runtime";',
    'export { IMessageConfigSchema } from "wildvine/plugin-sdk/channel-config-schema";',
    ...
    'export type IMessageAccountConfig = Omit< NonNullable<NonNullable<RuntimeApiWildvineConfig["channels"]>["imessage"]>, "accounts" | "defaultAccount" >;',
  ...
  [bundledPluginFile("googlechat", "runtime-api.ts")]: [
    'export * from "wildvine/plugin-sdk/googlechat";',
  ],
  [bundledPluginFile("matrix", "runtime-api.ts")]: [
    ...
    'export { assertHttpUrlTargetsPrivateNetwork, closeDispatcher, createPinnedDispatcher, resolvePinnedHostnameWithPolicy, ssrfPolicyFromAllowPrivateNetwork, type LookupFn, type SsrFPolicy } from "wildvine/plugin-sdk/ssrf-runtime";',
    ...
    'export { writeJsonFileAtomically } from "wildvine/plugin-sdk/json-store";',
    'export type { ChannelDirectoryEntry, ChannelMessageActionContext, WildvineConfig, PluginRuntime, RuntimeLogger, RuntimeEnv, WizardPrompter } from "wildvine/plugin-sdk/matrix-runtime-shared";',
    'export { formatZonedTimestamp } from "wildvine/plugin-sdk/matrix-runtime-shared";',
    'export function chunkTextForOutbound(text: string, limit: number): string[] { const chunks: string[] = []; let remaining = text; while (remaining.length > limit) { const window = remaining.slice(0, limit); const splitAt = Math.max(window.lastIndexOf("\\n"), window.lastIndexOf(" ")); const breakAt = splitAt > 0 ? splitAt : limit; chunks.push(remaining.slice(0, breakAt).trimEnd()); remaining = remaining.slice(breakAt).trimStart(); } if (remaining.length > 0 || text.length === 0) { chunks.push(remaining); } return chunks; }',
  ],
  [bundledPluginFile("nextcloud-talk", "runtime-api.ts")]: [
    'export * from "wildvine/plugin-sdk/nextcloud-talk";',
  ],
  ...
  [bundledPluginFile("telegram", "runtime-api.ts")]: [
    'export type { ChannelMessageActionAdapter, ChannelPlugin, WildvineConfig, WildvinePluginApi, PluginRuntime, TelegramAccountConfig, TelegramActionConfig, TelegramNetworkConfig } from "wildvine/plugin-sdk/telegram-core";',
    'export type { TelegramApiOverride } from "./src/send.js";',
    'export type { WildvinePluginService, WildvinePluginServiceContext, PluginLogger } from "wildvine/plugin-sdk/core";',
    'export type { AcpRuntime, AcpRuntimeCapabilities, AcpRuntimeDoctorReport, AcpRuntimeEnsureInput, AcpRuntimeEvent, AcpRuntimeHandle, AcpRuntimeStatus, AcpRuntimeTurnInput, AcpRuntimeErrorCode, AcpSessionUpdateTag } from "wildvine/plugin-sdk/acp-runtime";',
    'export { AcpRuntimeError } from "wildvine/plugin-sdk/acp-runtime";',
    'export { buildTokenChannelStatusSummary, clearAccountEntryFields, DEFAULT_ACCOUNT_ID, normalizeAccountId, PAIRING_APPROVED_MESSAGE, parseTelegramTopicConversation, projectCredentialSnapshotFields, resolveConfiguredFromCredentialStatuses, resolveTelegramPollVisibility } from "wildvine/plugin-sdk/telegram-core";',
    'export { buildChannelConfigSchema, getChatChannelMeta, jsonResult, readNumberParam, readReactionParams, readStringArrayParam, readStringOrNumberParam, readStringParam, resolvePollMaxSelections, TelegramConfigSchema } from "wildvine/plugin-sdk/telegram-core";',
