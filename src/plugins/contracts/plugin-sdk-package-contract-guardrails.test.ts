...
const PLUGIN_SDK_SUBPATH_PATTERN = /wildvine\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
function packWildvineToTempDir(packDir: string): string {
...
async function readPackedRootPackageJson(archivePath: string): Promise<{
  ...
  const extractDir = mkdtempSync(join(os.tmpdir(), "wildvine-packed-root-package-json-"));
  try {
    await tar.x({
      file: archivePath,
      cwd: extractDir,
...
function buildLegacyPluginSourceAlias(): string {
  return ["wildvine", ["plugin", "source"].join("-")].join("/") + "/";
}
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references wildvine/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("mirrors matrix runtime deps needed by the bundled host graph", () => {
    ...
    const allowlist = matrixPackageJson.wildvine?.releaseChecks?.rootDependencyMirrorAllowlist;
  ...
  it("keeps matrix crypto WASM in the packed artifact manifest", async () => {
    const tempRoot = mkdtempSync(join(os.tmpdir(), "wildvine-matrix-wasm-pack-"));
    try {
      const packDir = join(tempRoot, "pack");
      mkdirSync(packDir, { recursive: true });

      const archivePath = packWildvineToTempDir(packDir);
      ...
      expect(packedPackageJson.dependencies?.["@wildvine/plugin-package-contract"]).toBeUndefined();
