...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `wildvine-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.wildvine/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without wildvine.extensions", async () => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@wildvine/nope", version: "0.0.1" }),
      "utf-8",
    );
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@wildvine/voice-call@0.0.1", "--ignore-scripts"]);
