...
export function applyProviderDefaultModel(cfg: WildvineConfig, modelRef: string): WildvineConfig {
...
function buildOpenAICompatibleSelfHostedProviderConfig(params: {
  ...
  contextWindow?: number;
  maxTokens?: number;
}): { config: WildvineConfig; modelId: string; modelRef: string; profileId: string } {
