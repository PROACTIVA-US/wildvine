...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing wildvine.extensions; update the plugin package to include wildvine.extensions (for example ["./dist/index.js"]). See https://docs.wildvine.com/help/troubleshooting#plugin-install-fails-with-missing-wildvine-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "wildvine.plugin.json",
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_WILDVINE_EXTENSIONS: "missing_wildvine_extensions",
  EMPTY_WILDVINE_EXTENSIONS: "empty_wildvine_extensions",
...
function ensureWildvineExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_WILDVINE_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json wildvine.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_WILDVINE_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    return {
      ok: false,
      error: `Bundle "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "wildvine security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureWildvineExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureWildvineExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from wildvine.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-wildvine"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json wildvine.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Wildvine >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set WILDVINE_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Wildvine >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Wildvine and retry.`,
  ...
  try {
    ...
    return {
      ok: false,
      error: `Plugin "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "wildvine security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
export async function installPluginFromFile(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `Plugin file "${pluginId}" installation blocked: code safety scan failed (${String(err)}). Run "wildvine security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
...
export async function installPluginFromNpmSpec(
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "wildvine-npm-pack-",
    spec,
    timeoutMs,
