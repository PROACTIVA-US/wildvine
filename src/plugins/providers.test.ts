...
type LoadWildvinePlugins = typeof import("./loader.js").loadWildvinePlugins;
...
const loadWildvinePluginsMock = vi.fn<LoadWildvinePlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/wildvine.plugin.json`,
  };
}
...
function getLastLoadPluginsCall(): Record<string, unknown> {
  const call = loadWildvinePluginsMock.mock.calls.at(-1)?.[0];
...
function createAutoEnabledProviderConfig() {
  const rawConfig: WildvineConfig = {
    plugins: {},
  };
  const autoEnabledConfig: WildvineConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    vi.resetModules();
    vi.doMock("./loader.js", () => ({
      loadWildvinePlugins: (...args: Parameters<LoadWildvinePlugins>) =>
        loadWildvinePluginsMock(...args),
    }));
  ...
  beforeEach(() => {
    loadWildvinePluginsMock.mockReset();
    ...
    loadWildvinePluginsMock.mockReturnValue(registry);
