...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-plugin-binding-"));
...
vi.mock("../infra/home-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    expandHomePrefix: (value: string) => {
      if (value === "~/.wildvine/plugin-binding-approvals.json") {
  ...
  it.each([
    ...
    },
    {
      ...
      existingRecord: {
        ...
        targetSessionKey: "wildvine-app-server:thread:019ce411-6322-7db2-a821-1a61c530e7d9",
        targetKind: "session" as const,
        conversation: {
          channel: "telegram",
          accountId: "default",
