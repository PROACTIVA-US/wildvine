...
type PluginLoadConfig = NonNullable<Parameters<typeof loadWildvinePlugins>[0]>["config"];
type PluginRegistry = ReturnType<typeof loadWildvinePlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "wildvine-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.WILDVINE_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "wildvine.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function writeBundledPlugin(params: {
  ...
  process.env.WILDVINE_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".wildvine", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ WILDVINE_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.WILDVINE_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadWildvinePlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          wildvine: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.WILDVINE_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadWildvinePlugins>) {
...
function useNoBundledPlugins() {
  process.env.WILDVINE_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadWildvinePlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadWildvinePlugins>[0], "cache" | "config">,
) {
  return loadWildvinePlugins({
    cache: false,
    ...options,
...
function loadBundleFixture(params: {
  ...
  const stateDir = makeTempDir();
  const bundleRoot = path.join(workspaceDir, ".wildvine", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ WILDVINE_STATE_DIR: stateDir, ...params.env }, () =>
    loadWildvinePlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadWildvinePlugins>,
  pluginId: string,
) {
  expect(
    registry.diagnostics.some(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadWildvinePlugins>;
  loadSecond: () => ReturnType<typeof loadWildvinePlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadWildvinePlugins>;
  loadVariant: () => ReturnType<typeof loadWildvinePlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const wildvineHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(wildvineHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    WILDVINE_HOME: wildvineHome,
    HOME: ignoredHome,
    WILDVINE_STATE_DIR: stateDir,
    WILDVINE_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadWildvinePlugins({
    cache: false,
    config: {
      plugins: {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.WILDVINE_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.WILDVINE_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".wildvine", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ WILDVINE_STATE_DIR: stateDir }, () =>
      loadWildvinePlugins({
        workspaceDir,
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadWildvinePlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadWildvinePlugins(options);
        const scoped = loadWildvinePlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadWildvinePlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadWildvinePlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadWildvinePlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadWildvinePlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadWildvinePlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadWildvinePlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadWildvinePlugins({
              ...
              env: {
                ...process.env,
                WILDVINE_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadWildvinePlugins({
              ...
              env: {
                ...process.env,
                WILDVINE_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadWildvinePlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                WILDVINE_HOME: undefined,
                WILDVINE_STATE_DIR: stateDir,
                WILDVINE_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadWildvinePlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(wildvineHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadWildvinePlugins(options),
          loadVariant: () =>
            loadWildvinePlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadWildvinePlugins({
      env: {
        ...
        HOME: homeDir,
        WILDVINE_HOME: undefined,
        WILDVINE_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers WILDVINE_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const wildvineHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "wildvine-home-demo",
      dir: path.join(wildvineHome, "plugins", "wildvine-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "wildvine-home-demo", register() {} };`,
    });
    ...
    const registry = loadWildvinePlugins({
      ...
      config: {
        plugins: {
          allow: ["wildvine-home-demo"],
          entries: {
            "wildvine-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/wildvine-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "wildvine-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadWildvinePlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadWildvinePlugins>) => {
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "wildvine.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "wildvine.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const broadSetupRegistry = loadWildvinePlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadWildvinePlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@wildvine/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@wildvine/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@wildvine/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@wildvine/setup-runtime-not-preferred-test",
  ...
  register(api) {
    ...
    const registry = await loadWildvinePluginCliRegistry({
      config: {
        plugins: {
          load: { paths: [plugin.file] },
          allow: ["config-cli"],
  ...
  it("uses the real channel entry in cli-metadata mode for CLI metadata capture", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@wildvine/cli-metadata-channel",
          wildvine: { extensions: ["./index.cjs"], setupEntry: "./setup-entry.cjs" },
        },
        null,
        2,
      ),
    ...
    fs.writeFileSync(
      path.join(pluginDir, "index.cjs"),
      `const { defineChannelPluginEntry } = require("wildvine/plugin-sdk/core");
require("node:fs").writeFileSync(${JSON.stringify(fullMarker)}, "loaded", "utf-8");
module.exports = {
  ...
  it("collects channel CLI metadata during full plugin loads", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@wildvine/full-cli-metadata-channel",
          wildvine: { extensions: ["./index.cjs"] },
        },
        null,
        2,
      ),
...
module.exports = {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "wildvine.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "wildvine.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ WILDVINE_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("wildvine/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ WILDVINE_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadWildvinePlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__wildvineLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("wildvine/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("wildvine/plugin-sdk");
