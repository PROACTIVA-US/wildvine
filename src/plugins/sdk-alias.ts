...
import { resolveWildvinePackageRootSync } from "../infra/wildvine-root.js";
...
function hasTrustedWildvineRootIndicator(params: {
  packageRoot: string;
  ...
  const hasWildvineBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("wildvine")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.wildvine === "string");
  const hasWildvineEntrypoint = fs.existsSync(path.join(params.packageRoot, "wildvine.mjs"));
  return hasCliEntryExport || hasWildvineBin || hasWildvineEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedWildvineRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedWildvineRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveWildvinePackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedWildvineRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveWildvinePackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedWildvineRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`wildvine/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias ? { "wildvine/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "wildvine/plugin-sdk": pluginSdkAlias } : {}),
