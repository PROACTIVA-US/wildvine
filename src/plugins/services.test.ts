...
import type { WildvinePluginService, WildvinePluginServiceContext } from "./types.js";
...
function createRegistry(services: WildvinePluginService[]) {
...
function expectServiceContext(
  ctx: WildvinePluginServiceContext,
...
function expectServiceLogger(ctx: WildvinePluginServiceContext) {
...
function expectServiceContexts(
  contexts: WildvinePluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: WildvinePluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: WildvinePluginServiceContext[] = [];
