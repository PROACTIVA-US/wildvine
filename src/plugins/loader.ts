...
import { discoverWildvinePlugins } from "./discovery.js";
...
export function resolveRuntimePluginRegistry(
  ...
  return getCompatibleActivePluginRegistry(options) ?? loadWildvinePlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: WildvinePluginDefinition;
  register?: WildvinePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as WildvinePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as WildvinePluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.WILDVINE_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverWildvinePlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverWildvinePlugins>["candidates"][number];
  right: ReturnType<typeof discoverWildvinePlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadWildvinePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadWildvinePlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  const discovery = discoverWildvinePlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into Wildvine yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: WildvinePluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as WildvinePluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
...
export async function loadWildvinePluginCliRegistry(
  options: PluginLoadOptions = {},
