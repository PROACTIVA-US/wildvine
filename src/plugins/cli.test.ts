...
const mocks = vi.hoisted(() => ({
  ...
  loadWildvinePluginCliRegistry: vi.fn(),
  loadWildvinePlugins: vi.fn(),
  applyPluginAutoEnable: vi.fn(),
}));

vi.mock("./loader.js", () => ({
  loadWildvinePluginCliRegistry: (...args: unknown[]) =>
    mocks.loadWildvinePluginCliRegistry(...args),
  loadWildvinePlugins: (...args: unknown[]) => mocks.loadWildvinePlugins(...args),
}));
...
function expectPluginLoaderConfig(config: WildvineConfig) {
  expect(mocks.loadWildvinePlugins).toHaveBeenCalledWith(
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: WildvineConfig;
  autoEnabledConfig: WildvineConfig;
}) {
...
describe("registerPluginCliCommands", () => {
  beforeEach(() => {
    ...
    mocks.loadWildvinePluginCliRegistry.mockReset();
    mocks.loadWildvinePluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadWildvinePlugins.mockReset();
    mocks.loadWildvinePlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as WildvineConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as WildvineConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    const { rawConfig, autoEnabledConfig } = createAutoEnabledCliFixture();
    mocks.applyPluginAutoEnable.mockReturnValue({ config: autoEnabledConfig, changes: [] });
    mocks.loadWildvinePluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadWildvinePluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
      }),
    );
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadWildvinePluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to awaited CLI metadata collection when runtime loading ignored async registration", async () => {
    ...
    await registerPluginCliCommands(program, {} as WildvineConfig, undefined, undefined, {
      mode: "lazy",
    });

    expect(mocks.loadWildvinePluginCliRegistry).toHaveBeenCalledTimes(1);
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as WildvineConfig, undefined, undefined, {
      mode: "lazy",
    });
