...
const loadWildvinePluginsMock = vi.fn();
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadWildvinePluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { WILDVINE_HOME: "/srv/wildvine-home" },
      },
    },
    {
