...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-wildvine",
    targetDir: params?.targetDir ?? "/tmp/opik-wildvine",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "wildvine-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/wildvine-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@wildvine/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["wildvine-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/wildvine-codex-app-server",
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-wildvine",
        spec: "@opik/opik-wildvine",
        integrity: "sha512-old",
        installPath: "/tmp/opik-wildvine",
      }),
      pluginIds: ["opik-wildvine"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-wildvine",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "wildvine-codex-app-server",
        spec: "wildvine-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/wildvine-codex-app-server",
      }),
      pluginIds: ["wildvine-codex-app-server"],
      specOverrides: {
        "wildvine-codex-app-server": "wildvine-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "wildvine-codex-app-server",
        targetDir: "/tmp/wildvine-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "wildvine-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @wildvine/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@wildvine/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @wildvine/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "wildvine-codex-app-server@beta",
        resolvedName: "wildvine-codex-app-server",
        resolvedSpec: "wildvine-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "wildvine-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "wildvine-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "wildvine-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "wildvine-codex-app-server",
      }),
      specOverrides: {
        "wildvine-codex-app-server": "wildvine-codex-app-server@beta",
      },
      expectedSpec: "wildvine-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "wildvine-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "wildvine-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@wildvine/voice-call",
      targetDir: "/tmp/wildvine-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@wildvine/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@wildvine/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@wildvine/voice-call");
    expect(result.config.plugins?.entries?.["@wildvine/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@wildvine/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@wildvine/voice-call",
      installPath: "/tmp/wildvine-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { WILDVINE_HOME: "/srv/wildvine-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/wildvine-home";
    mockBundledSources(
      createBundledSource({
