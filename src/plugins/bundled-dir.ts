...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.WILDVINE_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveWildvinePackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveWildvinePackageRootSync({ argv1: process.argv[1] }),
