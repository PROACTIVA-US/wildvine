...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "wildvine", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".wildvine", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".wildvine", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "wildvine-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".wildvine", "extensions"),
      },
    });
  });
});
