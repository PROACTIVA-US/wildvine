...
const APPROVALS_PATH = "~/.wildvine/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "wildvine-app-server:thread:",
  "wildvine-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("wildvine.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("wildvine.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Wildvine instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
