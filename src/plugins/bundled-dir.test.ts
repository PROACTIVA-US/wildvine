...
const originalBundledDir = process.env.WILDVINE_BUNDLED_PLUGINS_DIR;
...
function createWildvineRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "wildvine" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.WILDVINE_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.WILDVINE_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "wildvine-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "wildvine-bundled-dir-dist-",
        hasDistExtensions: true,
      },
      {
        expectedRelativeDir: path.join("dist", "extensions"),
    ...
    [
      ...
      {
        prefix: "wildvine-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "wildvine-bundled-dir-git-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    const repoRoot = createWildvineRoot(layout);
    expectResolvedBundledDirFromRoot({
      repoRoot,
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createWildvineRoot({
          prefix: "wildvine-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        const cwdRepoRoot = createWildvineRoot({
          prefix: "wildvine-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "wildvine.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createWildvineRoot({
          prefix: "wildvine-bundled-dir-override-",
          hasDistExtensions: true,
        });
        return {
          installedRoot,
