...
let loadWildvinePluginsMock: ReturnType<typeof vi.fn>;
...
function createWebSearchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    WILDVINE_HOME: "/tmp/wildvine-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/wildvine.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/wildvine.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectRuntimeProviderResolution(
  ...
  expect(loadWildvinePluginsMock).not.toHaveBeenCalled();
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadWildvinePluginsMock = vi
      .spyOn(loaderModule, "loadWildvinePlugins")
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: (_config: { plugins?: Record<string, unknown> }, env: NodeJS.ProcessEnv) => {
        env.WILDVINE_HOME = "/tmp/wildvine-home-b";
      },
    },
  ] as const)("$name", ({ mutate }) => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({ WILDVINE_HOME: "/tmp/wildvine-home-a" });
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        WILDVINE_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadWildvinePluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      WILDVINE_PLUGIN_DISCOVERY_CACHE_MS: "5",
      WILDVINE_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadWildvinePluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      WILDVINE_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.WILDVINE_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
