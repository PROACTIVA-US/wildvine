...
async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-marketplace-test-"));
  try {
    return await fn(dir);
  } finally {
...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("wildvine-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir(async (homeDir) => {
      const openClawHome = path.join(homeDir, "wildvine-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, WILDVINE_HOME: openClawHome },
