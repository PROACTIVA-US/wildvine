...
import { clearPluginDiscoveryCache, discoverWildvinePlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("wildvine-plugins", tempDirs);
}
...
function buildCachedDiscoveryEnv(
  ...
  return {
    ...
    WILDVINE_PLUGIN_DISCOVERY_CACHE_MS: "5000",
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverWildvinePlugins>[0],
) {
  return discoverWildvinePlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithCachedEnv(params: Parameters<typeof discoverWildvinePlugins>[0]) {
  return discoverWildvinePlugins(params);
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      wildvine: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
function writePluginManifest(params: { pluginDir: string; id: string }) {
  fs.writeFileSync(
    path.join(params.pluginDir, "wildvine.plugin.json"),
    JSON.stringify({
      id: params.id,
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverWildvinePlugins>>,
...
describe("discoverWildvinePlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".wildvine", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".wildvine", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverWildvinePlugins({
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-wildvine");
    ...
    writePluginPackageManifest({
      ...
      packageName: "@opik/opik-wildvine",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-wildvine" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "wildvine"),
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-wildvine"]);
  });
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@wildvine/voice-call",
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@wildvine/ollama-provider",
          pluginId: "ollama",
          entryPath: "src/index.ts",
        });
        return {};
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@wildvine/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@wildvine/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@wildvine/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("wildvine");
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@wildvine/escape-pack",
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@wildvine/missing-entry-pack",
