...
function listAgents(cfg: WildvineConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: WildvineConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<WildvineConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: WildvineConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: WildvineConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: WildvineConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<WildvineConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  WildvineConfig,
  {
    bindingsRef: WildvineConfig["bindings"];
    agentsRef: WildvineConfig["agents"];
    sessionRef: WildvineConfig["session"];
...
function resolveRouteCacheForConfig(cfg: WildvineConfig): Map<string, ResolvedAgentRoute> {
