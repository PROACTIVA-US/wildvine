...
const TEMP_MEDIA_PREFIX = "wildvine-media-";
...
function createGroqAudioConfig(): WildvineConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): WildvineConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): WildvineConfig {
  return {
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredWildvineTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        WILDVINE_AGENT_DIR: isolatedAgentDir,
