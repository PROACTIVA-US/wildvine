...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "wildvine", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "wildvine", version: "2.3.4" });
  ...
  it("ignores non-wildvine package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers WILDVINE_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        WILDVINE_VERSION: "9.9.9",
        WILDVINE_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale WILDVINE_VERSION for compatibility checks", () => {
    const previous = process.env.WILDVINE_VERSION;
    const previousService = process.env.WILDVINE_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.WILDVINE_VERSION = "2026.3.25";
      process.env.WILDVINE_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.WILDVINE_VERSION = previous;
      process.env.WILDVINE_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ...
        WILDVINE_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        WILDVINE_VERSION: "   ",
        WILDVINE_SERVICE_VERSION: "  2.0.0  ",
