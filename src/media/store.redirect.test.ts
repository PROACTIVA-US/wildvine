...
const HOME = path.join(os.tmpdir(), "wildvine-home-redirect");
...
describe("media store redirects", () => {
  ...
  beforeAll(async () => {
    envSnapshot = captureEnv(["WILDVINE_STATE_DIR"]);
    await fs.rm(HOME, { recursive: true, force: true });
    process.env.WILDVINE_STATE_DIR = HOME;
  });
  ...
  it("strips sensitive headers when a redirect crosses origins", async () => {
    ...
    expect(secondHeaders.get("user-agent")).toBe("Wildvine-Test/1.0");
  });
