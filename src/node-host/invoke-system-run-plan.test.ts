...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `wildvine-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "wildvine-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "wildvine-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "wildvine-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "wildvine-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "wildvine-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "wildvine-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "wildvine-inline-shell-node-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32").each(cases)("$name", (testCase) => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-approval-hardening-"));
    const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "wildvine-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-shell-option-value-"));
    try {
