...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-install-sh-"));
  const cliPath = path.join(root, "wildvine");
...
WILDVINE_BIN="$FAKE_WILDVINE_BIN"
resolve_wildvine_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_WILDVINE_BIN: cliPath,
        WILDVINE_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Wildvine 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Wildvine dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Wildvine dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-install-stdin-"));
