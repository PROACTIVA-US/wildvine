...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers WILDVINE_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        WILDVINE_HANDSHAKE_TIMEOUT_MS: "75",
        WILDVINE_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        WILDVINE_HANDSHAKE_TIMEOUT_MS: "",
        WILDVINE_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
