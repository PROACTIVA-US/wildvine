...
describe("gateway auth", () => {
  ...
  it("resolves token/password from WILDVINE gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          WILDVINE_GATEWAY_TOKEN: "env-token",
          WILDVINE_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides WILDVINE_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        WILDVINE_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
