...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-wildvine-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-wildvine-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-wildvine-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-wildvine-thread-id")?.trim() || undefined;
  ...
  const allTools = createWildvineTools({
