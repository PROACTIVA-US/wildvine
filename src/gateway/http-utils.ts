...
export const WILDVINE_MODEL_ID = "wildvine";
export const WILDVINE_DEFAULT_MODEL_ID = "wildvine/default";
...
export function resolveTrustedHttpOperatorScopes(
  ...
  const headerValue = getHeader(req, "x-wildvine-scopes");
...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    getHeader(req, "x-wildvine-agent-id")?.trim() ||
    getHeader(req, "x-wildvine-agent")?.trim() ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === WILDVINE_MODEL_ID || lowered === WILDVINE_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^wildvine[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `wildvine` or `wildvine/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-wildvine-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-wildvine-model`." };
  }
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-wildvine-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-wildvine-message-channel")) ??
