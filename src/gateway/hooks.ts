...
export function resolveHooksConfig(cfg: WildvineConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: WildvineConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-wildvine-token"] === "string"
      ? req.headers["x-wildvine-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-wildvine-idempotency-key"]) ||
