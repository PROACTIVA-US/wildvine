...
const READ_SCOPE_HEADER = { "x-wildvine-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-session-history-"));
  ...
  test("returns session history over direct REST", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(
        (
          body.messages?.[0] as {
            __wildvine?: { id?: string; seq?: number };
          }
        )?.__wildvine,
      ).toMatchObject({
        seq: 1,
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __wildvine?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __wildvine?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__wildvine?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __wildvine?: { seq?: number } }>;
        messages?: Array<{ __wildvine?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__wildvine?.seq)).toEqual([1]);
  ...
  test("streams session history updates over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(
        (
          messageEvent.data as {
            message?: { __wildvine?: { id?: string; seq?: number } };
          }
        ).message?.__wildvine,
      ).toMatchObject({
