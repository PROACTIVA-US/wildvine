...
import { ensureWildvineCliOnPath } from "../infra/path-env.js";
import {
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("wildvine doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.WILDVINE_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.WILDVINE_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "WILDVINE_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "WILDVINE_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `wildvine config set gateway.auth.mode token` and `wildvine config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  if (matrixInstallPathIssue) {
    const lines = formatPluginInstallPathIssue({
      ...
      pluginLabel: "Matrix",
      defaultInstallCommand: "wildvine plugins install @wildvine/matrix",
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachWildvineTranscriptMeta(update.message, {
