...
function loadGatewayConfig(): WildvineConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: WildvineConfig, env?: NodeJS.ProcessEnv): number {
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__WILDVINE_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
