...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("wildvine doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with WILDVINE_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with WILDVINE_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.WILDVINE_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://wildvine-gateway.ai:18789",
      onConnectError,
    });
