...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.WILDVINE_CONFIG_PATH;
  const prevDisableCache = process.env.WILDVINE_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "wildvine-plugin-http-auth-test-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.WILDVINE_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.WILDVINE_DISABLE_CONFIG_CACHE;
    } else {
      process.env.WILDVINE_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
