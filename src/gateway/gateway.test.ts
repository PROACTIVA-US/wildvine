...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".wildvine", "extensions", params.id);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "wildvine");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-gw-http-tools-home-"));
      process.env.HOME = tempHome;
      process.env.WILDVINE_STATE_DIR = path.join(tempHome, ".wildvine");
      delete process.env.WILDVINE_CONFIG_PATH;
      process.env.WILDVINE_SKIP_CHANNELS = "1";
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-wizard-home-"));
      process.env.HOME = tempHome;
