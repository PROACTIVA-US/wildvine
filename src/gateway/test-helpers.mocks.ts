...
const GATEWAY_TEST_PLUGIN_REGISTRY_STATE_KEY = Symbol.for(
  "wildvine.gatewayTestHelpers.pluginRegistryState",
);
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("wildvine.gatewayTestHelpers.configRoot");

const hoisted = vi.hoisted(() => {
  const key = Symbol.for("wildvine.gatewayTestHelpers.hoisted");
...
const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `wildvine-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.WILDVINE_CONFIG_PATH = path.join(root, "wildvine.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "wildvine.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "wildvine-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: WildvineConfig) =>
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadWildvinePlugins: () => pluginRegistryState.registry,
  };
});
