...
const { createWildvineTools } = await import("../agents/wildvine-tools.js");
...
type SessionSendTool = ReturnType<typeof createWildvineTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createWildvineTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["WILDVINE_GATEWAY_PORT", "WILDVINE_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.WILDVINE_GATEWAY_PORT = String(gatewayPort);
  process.env.WILDVINE_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("WILDVINE_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createWildvineTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
