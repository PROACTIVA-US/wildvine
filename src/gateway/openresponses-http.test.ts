...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("WILDVINE_CONFIG_PATH is required for gateway config tests");
  }
...
async function postResponses(port: number, body: unknown, headers?: Record<string, string>) {
  const res = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
    ...
    headers: {
      ...
      "x-wildvine-scopes": "operator.write",
      ...headers,
    },
...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "wildvine", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `wildvine` or `wildvine/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "wildvine", input: "hi" },
        { "x-wildvine-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "wildvine/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "wildvine/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "wildvine", input: "hi" },
        { "x-wildvine-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-wildvine-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("treats HTTP callers as non-owner regardless of requested scopes", async () => {
    ...
    const adminScopeResponse = await postResponses(
      port,
      { model: "wildvine", input: "hi" },
      { "x-wildvine-scopes": "operator.admin, operator.write" },
    );
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "wildvine", input: "hi" },
      { "x-wildvine-scopes": "operator.admin, operator.write" },
    );
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        headers: {
          ...
          "content-type": "application/json",
          "x-wildvine-scopes": "operator.approvals",
        },
        body: JSON.stringify({
          model: "wildvine",
          input: "hi",
