...
const loadWildvinePlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadWildvinePlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadWildvinePlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadWildvinePlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.wildvine.com/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", async () => {
    ...
    loadWildvinePlugins.mockReturnValue(pluginRegistry);
