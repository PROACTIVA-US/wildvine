...
const WRITE_SCOPE_HEADER = { "x-wildvine-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-wildvine-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("wildvine/default");
  ...
  it("ignores narrower declared scopes for shared-secret bearer auth", async () => {
    const res = await postEmbeddings(
      ...
      { "x-wildvine-scopes": "operator.read" },
    );
    expect(res.status).toBe(200);
  ...
  it("rejects invalid agent targets", async () => {
    ...
    expect(json.error).toEqual({
      ...
      message: "Invalid `model`. Use `wildvine` or `wildvine/<agentId>`.",
    });
  });

  it("rejects disallowed x-wildvine-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "wildvine/default",
        input: "hello",
      },
      { "x-wildvine-model": "ollama/nomic-embed-text" },
    );
    expect(res.status).toBe(400);
    const json = (await res.json()) as { error?: { type?: string; message?: string } };
    expect(json.error).toEqual({
