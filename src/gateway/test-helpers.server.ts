...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "WILDVINE_SKIP_BROWSER_CONTROL_SERVER",
  "WILDVINE_SKIP_GMAIL_WATCHER",
  "WILDVINE_SKIP_CANVAS_HOST",
  "WILDVINE_BUNDLED_PLUGINS_DIR",
  "WILDVINE_SKIP_CHANNELS",
  "WILDVINE_SKIP_PROVIDERS",
  "WILDVINE_SKIP_CRON",
  "WILDVINE_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.WILDVINE_CONFIG_PATH) {
    configPaths.add(process.env.WILDVINE_CONFIG_PATH);
  }
  if (process.env.WILDVINE_STATE_DIR) {
    configPaths.add(path.join(process.env.WILDVINE_STATE_DIR, "wildvine.json"));
  }
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  const stateDir = process.env.WILDVINE_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".wildvine-test-suite");
  ...
  tempControlUiRoot = path.join(tempHome, ".wildvine-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>wildvine-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__wildvineTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__wildvineTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["WILDVINE_GATEWAY_TOKEN"]);
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.WILDVINE_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
