...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.WILDVINE_STATE_DIR;
  previousConfigPath = process.env.WILDVINE_CONFIG_PATH;
  previousSkipBrowserControl = process.env.WILDVINE_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.WILDVINE_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.WILDVINE_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.WILDVINE_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.WILDVINE_SKIP_CHANNELS;
  previousSkipProviders = process.env.WILDVINE_SKIP_PROVIDERS;
  previousSkipCron = process.env.WILDVINE_SKIP_CRON;
  previousMinimalGateway = process.env.WILDVINE_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.WILDVINE_STATE_DIR = path.join(tempHome, ".wildvine");
...
function applyGatewaySkipEnv() {
  ...
  process.env.WILDVINE_SKIP_PROVIDERS = "1";
  process.env.WILDVINE_SKIP_CRON = "1";
  process.env.WILDVINE_TEST_MINIMAL_GATEWAY = "1";
  process.env.WILDVINE_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "wildvine-test-no-bundled-extensions")
    : "wildvine-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "wildvine-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".wildvine-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.WILDVINE_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.WILDVINE_CONFIG_PATH;
    } else {
      process.env.WILDVINE_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.WILDVINE_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.WILDVINE_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.WILDVINE_SKIP_GMAIL_WATCHER;
    } else {
      process.env.WILDVINE_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.WILDVINE_SKIP_CANVAS_HOST;
    } else {
      process.env.WILDVINE_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.WILDVINE_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.WILDVINE_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.WILDVINE_SKIP_CHANNELS;
    } else {
      process.env.WILDVINE_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.WILDVINE_SKIP_PROVIDERS;
    } else {
      process.env.WILDVINE_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.WILDVINE_SKIP_CRON;
    } else {
      process.env.WILDVINE_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.WILDVINE_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.WILDVINE_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const prev = process.env.WILDVINE_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.WILDVINE_GATEWAY_TOKEN = fallbackToken;
  }
