...
export function attachWildvineTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__wildvine && typeof record.__wildvine === "object" && !Array.isArray(record.__wildvine)
      ? (record.__wildvine as Record<string, unknown>)
      : {};
  return {
    ...record,
    __wildvine: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachWildvineTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "wildvine" && model === "delivery-mirror";
