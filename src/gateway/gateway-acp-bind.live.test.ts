...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.WILDVINE_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.WILDVINE_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.WILDVINE_LIVE_ACP_BIND_AGENT);
      const acpxCommand = process.env.WILDVINE_LIVE_ACP_BIND_ACPX_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "wildvine.json");
      ...
      process.env.WILDVINE_GATEWAY_PORT = String(port);
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.WILDVINE_GATEWAY_PORT = previous.port;
        }
