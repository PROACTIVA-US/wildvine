...
import { type WildvineConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: WildvineConfig): string[] {
...
export function listAgentsForGateway(cfg: WildvineConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: WildvineConfig): string {
...
function resolveSessionStoreAgentId(cfg: WildvineConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: WildvineConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
...
export function getSessionDefaults(cfg: WildvineConfig): GatewaySessionsDefaults {
...
export function listSessionsFromStore(params: {
  ...
  let sessions = Object.entries(store)
    ...
    .map(([key, entry]) => {
      ...
      // For agent sessions, fall back to the agent's identity name (e.g., "Seneca")
      // Check workspace IDENTITY.md first, then fall back to config identity
      const agentParsed = parseAgentSessionKey(key);
      let agentIdentityName: string | undefined;
      if (agentParsed?.agentId) {
        const workspace = resolveAgentWorkspaceDir(cfg, agentParsed.agentId);
        const workspaceIdentity = loadAgentIdentityFromWorkspace(workspace);
        agentIdentityName =
          workspaceIdentity?.name?.trim() || resolveIdentityName(cfg, agentParsed.agentId);
      }
      const displayName =
        entry?.displayName ??
        (channel
