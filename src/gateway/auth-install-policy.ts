...
type GatewayInstallAuthMode = NonNullable<NonNullable<WildvineConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: WildvineConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.WILDVINE_GATEWAY_PASSWORD?.trim() ||
