...
function cfg(input: Partial<WildvineConfig>): WildvineConfig {
  return input as WildvineConfig;
}
...
const DEFAULT_ENV = {
  WILDVINE_GATEWAY_TOKEN: "env-token",
  WILDVINE_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when WILDVINE env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when WILDVINE env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when WILDVINE env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
