...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([WILDVINE_MODEL_ID, WILDVINE_DEFAULT_MODEL_ID]);
  ids.add(`wildvine/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`wildvine/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== WILDVINE_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
