...
import { resolveWildvineAgentDir } from "../agents/agent-paths.js";
...
import { ensureWildvineModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, WildvineConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.WILDVINE_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const PROVIDERS = parseFilter(process.env.WILDVINE_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.WILDVINE_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.WILDVINE_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.WILDVINE_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["WILDVINE_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.WILDVINE_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.WILDVINE_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<WildvineConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    ...
    disableBonjour: process.env.WILDVINE_DISABLE_BONJOUR,
    logLevel: process.env.WILDVINE_LOG_LEVEL,
    agentDir: process.env.WILDVINE_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.WILDVINE_STATE_DIR,
  };
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.WILDVINE_DISABLE_BONJOUR = "1";
    process.env.WILDVINE_LOG_LEVEL = "silent";
  }
  ...
  const hostAgentDir = resolveWildvineAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-live-state-"));
  process.env.WILDVINE_STATE_DIR = tempStateDir;
  ...
  process.env.WILDVINE_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.wildvine-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveWildvineAgentDir();
  const sanitizedCfg: WildvineConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-live-"));
  const tempConfigPath = path.join(tempDir, "wildvine.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.WILDVINE_CONFIG_PATH = tempConfigPath;
  ...
  try {
        ...
        try {
          ...
          for (
            ...
            toolText = await requestGatewayAgentText({
              ...
              modelKey,
              message: strictReply
                ? "Wildvine live tool probe (local, safe): " +
    ...
    process.env.WILDVINE_CONFIG_PATH = previous.configPath;
    process.env.WILDVINE_GATEWAY_TOKEN = previous.token;
    process.env.WILDVINE_SKIP_CHANNELS = previous.skipChannels;
    process.env.WILDVINE_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.WILDVINE_SKIP_CRON = previous.skipCron;
    process.env.WILDVINE_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.WILDVINE_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.WILDVINE_LOG_LEVEL = previous.logLevel;
    process.env.WILDVINE_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.WILDVINE_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureWildvineModelsJson(cfg);
      ...
      const rawModels = process.env.WILDVINE_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via WILDVINE_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.wildvine-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
