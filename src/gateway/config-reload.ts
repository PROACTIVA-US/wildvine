...
export function resolveGatewayReloadSettings(cfg: WildvineConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: WildvineConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: WildvineConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: WildvineConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: WildvineConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: WildvineConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: WildvineConfig) => {
