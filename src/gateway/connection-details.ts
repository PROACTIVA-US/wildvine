...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: WildvineConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.WILDVINE_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.WILDVINE_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set WILDVINE_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: wildvine doctor --fix",
