...
import type { loadWildvinePlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureWildvineModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadWildvinePlugins>;
  ...
  // Tests can opt out via WILDVINE_SKIP_CHANNELS (or legacy WILDVINE_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.WILDVINE_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.WILDVINE_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (WILDVINE_SKIP_CHANNELS=1 or WILDVINE_SKIP_PROVIDERS=1)",
    );
  }
