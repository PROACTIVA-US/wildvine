...
let lastCreateWildvineToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/wildvine-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateWildvineToolsContext?.agentTo,
          agentThreadId: lastCreateWildvineToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createWildvineTools: (ctx: Record<string, unknown>) => {
      lastCreateWildvineToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateWildvineToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-wildvine-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-wildvine-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateWildvineToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateWildvineToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-wildvine-message-to": "channel:24514",
        "x-wildvine-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
