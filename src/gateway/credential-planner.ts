...
type GatewaySecretDefaults = NonNullable<WildvineConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${WILDVINE_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.WILDVINE_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.WILDVINE_GATEWAY_PASSWORD);
}
