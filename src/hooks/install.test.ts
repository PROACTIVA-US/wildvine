...
const fixtureRoot = path.join(os.tmpdir(), `wildvine-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
beforeAll(async () => {
  ...
  zipHooks.file(
    ...
    JSON.stringify({
      name: "@wildvine/zip-hooks",
      version: "0.0.1",
      wildvine: { hooks: ["./hooks/zip-hook"] },
    }),
  );
  zipHooks.file(
  ...
  const makeTarWithPackage = async (params: {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        wildvine: { hooks: [`./hooks/${params.hookName}`] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  tarHooksBuffer = await makeTarWithPackage({
    packageName: "@wildvine/tar-hooks",
    hookName: "tar-hook",
  });
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@wildvine/test-hooks@0.0.1", "--ignore-scripts"]);
