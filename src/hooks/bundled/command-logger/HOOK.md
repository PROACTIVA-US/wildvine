...
homepage: https://docs.wildvine.com/automation/hooks#command-logger
metadata:
  {
    "wildvine":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.wildvine/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.wildvine/logs/commands.log ~/.wildvine/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/wildvine`:
   ```
   /home/username/.wildvine/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
