...
This directory contains hooks that ship with Wildvine. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.wildvine/wildvine.json`:
...
2. Restart gateway: `pkill -9 -f 'wildvine.*gateway' && pnpm wildvine gateway`
3. Enable the hook: `wildvine hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.wildvine.bot/automation/hooks
