import type { WildvineConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "wildvine-bundled": {
    ...
    canOverride: ["wildvine-bundled"],
    canBeOverriddenBy: ["wildvine-managed", "wildvine-plugin"],
  },
  "wildvine-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["wildvine-bundled", "wildvine-plugin"],
    canBeOverriddenBy: ["wildvine-managed"],
  },
  "wildvine-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["wildvine-bundled", "wildvine-managed", "wildvine-plugin"],
    canBeOverriddenBy: ["wildvine-managed"],
  },
  "wildvine-workspace": {
    precedence: 40,
    ...
    canOverride: ["wildvine-workspace"],
    canBeOverriddenBy: ["wildvine-workspace"],
  },
};
...
export function resolveHookConfig(
  config: WildvineConfig | undefined,
  hookKey: string,
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "wildvine-plugin") {
    return { enabled: true };
  }
