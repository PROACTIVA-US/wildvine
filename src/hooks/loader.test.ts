...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["WILDVINE_BUNDLED_HOOKS_DIR"]);
  ...
  describe("loadInternalHooks", () => {
    ...
    const expectNoCommandHookRegistration = async (cfg: WildvineConfig) => {
    ...
    it("should treat missing hooks.internal.enabled as enabled (default-on)", async () => {
      ...
      for (const cfg of [
        {} satisfies WildvineConfig,
        { hooks: {} } satisfies WildvineConfig,
        { hooks: { internal: {} } } satisfies WildvineConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
