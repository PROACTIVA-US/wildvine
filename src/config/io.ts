...
import type { WildvineConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  wildvine config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  wildvine config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: WildvineConfig,
  pathSegments: string[],
): { changed: boolean; next: WildvineConfig } {
...
function coerceConfig(value: unknown): WildvineConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as WildvineConfig;
}
...
function stampConfigVersion(cfg: WildvineConfig): WildvineConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: WildvineConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer Wildvine (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as WildvineConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): WildvineConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as WildvineConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(
    ...
    let cfgToWrite = persistCandidate as WildvineConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as WildvineConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.WILDVINE_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.WILDVINE_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.WILDVINE_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.WILDVINE_WATCH_SESSION === "string" &&
        deps.env.WILDVINE_WATCH_SESSION.trim().length > 0
          ? deps.env.WILDVINE_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.WILDVINE_WATCH_COMMAND === "string" &&
        deps.env.WILDVINE_WATCH_COMMAND.trim().length > 0
          ? deps.env.WILDVINE_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `WILDVINE_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: WildvineConfig | null = null;
let runtimeConfigSourceSnapshot: WildvineConfig | null = null;
...
export function getRuntimeConfigSourceSnapshot(): WildvineConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: WildvineConfig;
  candidate: WildvineConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: WildvineConfig): WildvineConfig {
...
export function loadConfig(): WildvineConfig {
  ...
  // reparsing wildvine.json on hot code paths.
  setRuntimeConfigSnapshot(config);
  return runtimeConfigSnapshot ?? config;
}

export function getRuntimeConfig(): WildvineConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<WildvineConfig> {
