...
import { compareWildvineVersions } from "./version.js";
...
function warnIfConfigFromFuture(cfg: WildvineConfig, logger: Pick<typeof console, "warn">): void {
  ...
  const cmp = compareWildvineVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): WildvineConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as WildvineConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: WildvineConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.WILDVINE_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.WILDVINE_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
