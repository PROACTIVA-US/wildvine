...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ WILDVINE_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-wildvine",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-wildvine#general");
  });
  ...
  it("derives session transcripts dir from WILDVINE_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { WILDVINE_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when WILDVINE_STATE_DIR differs from stored paths", () => {
