...
import { buildWebSearchProviderConfig, withTempHome, writeWildvineConfig } from "./test-helpers.js";
import { WildvineSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = WildvineSchema.safeParse({
      $schema: "https://wildvine.com/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://wildvine.com/config.json");
    }
  });
...
describe("ui.seamColor", () => {
  ...
  it("rejects non-hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "vine" } });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = WildvineSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("rejects removed legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeWildvineConfig(home, {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeWildvineConfig(home, {
        gateway: { bind: "${WILDVINE_BIND}" },
      });

      const prev = process.env.WILDVINE_BIND;
      process.env.WILDVINE_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        ...
        if (prev === undefined) {
          delete process.env.WILDVINE_BIND;
        } else {
          process.env.WILDVINE_BIND = prev;
        }
      }
    });
  });
