...
export function parseWildvineVersion(raw: string | null | undefined): WildvineVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeWildvineVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseWildvineVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameWildvineStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseWildvineVersion(a);
  const parsedB = parseWildvineVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareWildvineVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseWildvineVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseWildvineVersion(current);
  const parsedTouched = parseWildvineVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameWildvineStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareWildvineVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: WildvineVersion): number {
