...
import { parseWildvineVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-config-"));
  try {
    await run(home);
  } finally {
...
async function expectNoNewerVersionWarning(touchedVersion: string) {
  await withTempHome(async (home) => {
    ...
    expect(logger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining("Config was last written by a newer Wildvine"),
    );
...
describe("config io paths", () => {
  it("uses ~/.wildvine/wildvine.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".wildvine", 19001);
  ...
  it("defaults to ~/.wildvine/wildvine.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".wildvine", "wildvine.json"));
    });
  });

  it("uses WILDVINE_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { WILDVINE_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".wildvine", "wildvine.json"));
    });
  });

  it("honors explicit WILDVINE_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".wildvine", 20002, "custom.json");
      const io = createIoForHome(home, { WILDVINE_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseWildvineVersion(VERSION);
    if (!parsedVersion) {
