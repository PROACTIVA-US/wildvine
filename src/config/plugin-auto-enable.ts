...
const ENV_CATALOG_PATHS = ["WILDVINE_PLUGIN_CATALOG_PATHS", "WILDVINE_MPM_CATALOG_PATHS"];
...
function collectModelRefs(cfg: WildvineConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: WildvineConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: WildvineConfig, pluginId: string): boolean {
...
function hasPluginOwnedToolConfig(cfg: WildvineConfig, pluginId: string): boolean {
  if (pluginId === "xai") {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.wildvine) || !isRecord(entry.wildvine.channel)) {
      continue;
    }
    const channel = entry.wildvine.channel;
...
function collectCandidateChannelIds(cfg: WildvineConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: WildvineConfig): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: WildvineConfig): boolean {
...
function configMayNeedPluginAutoEnable(cfg: WildvineConfig, env: NodeJS.ProcessEnv): boolean {
...
function hasBrowserToolReference(cfg: WildvineConfig): boolean {
...
function hasExplicitBrowserPluginEntry(cfg: WildvineConfig): boolean {
  return Boolean(
    cfg.plugins?.entries && Object.prototype.hasOwnProperty.call(cfg.plugins.entries, "browser"),
  );
}

function resolveBrowserAutoEnableReason(cfg: WildvineConfig): string | null {
...
function isPluginExplicitlyDisabled(cfg: WildvineConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: WildvineConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: WildvineConfig, pluginId: string): WildvineConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
