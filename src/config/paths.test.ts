...
describe("gateway port resolution", () => {
  it("prefers numeric env values over config", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19002 } }, envWith({ WILDVINE_GATEWAY_PORT: "19001" })),
    ).toBe(19001);
  });
  ...
  it("accepts Compose-style IPv4 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ WILDVINE_GATEWAY_PORT: "127.0.0.1:18789" }),
      ),
    ).toBe(18789);
  });
  ...
  it("accepts Compose-style IPv6 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ WILDVINE_GATEWAY_PORT: "[::1]:28789" }),
      ),
    ).toBe(28789);
  });
  ...
  it("falls back to config when the Compose-style suffix is invalid", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19003 } },
        envWith({ WILDVINE_GATEWAY_PORT: "127.0.0.1:not-a-port" }),
      ),
    ).toBe(19003);
  });
  ...
  it("falls back when malformed IPv6 inputs do not provide an explicit port", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19003 } }, envWith({ WILDVINE_GATEWAY_PORT: "::1" })),
    ).toBe(19003);
    expect(resolveGatewayPort({}, envWith({ WILDVINE_GATEWAY_PORT: "2001:db8::1" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });

  it("falls back to the default port when env is invalid and config is unset", () => {
    expect(resolveGatewayPort({}, envWith({ WILDVINE_GATEWAY_PORT: "127.0.0.1:not-a-port" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });
});
...
describe("state + config path candidates", () => {
  function expectWildvineHomeDefaults(env: NodeJS.ProcessEnv): void {
    const configuredHome = env.WILDVINE_HOME;
    if (!configuredHome) {
      throw new Error("WILDVINE_HOME must be set for this assertion helper");
    }
  ...
  it("uses WILDVINE_HOME for default state/config locations", () => {
    ...
    expectWildvineHomeDefaults(env);
  });
  ...
  it("orders default config candidates in a stable order", () => {
    ...
    const expected = [
      ...
      path.join(resolvedHome, ".wildvine", "clawdbot.json"),
      path.join(resolvedHome, ".clawdbot", "wildvine.json"),
  ...
  it("prefers ~/.wildvine when it exists and legacy dir is missing", async () => {
    await withTempDir({ prefix: "wildvine-state-" }, async (root) => {
  ...
  it("falls back to existing legacy state dir when ~/.wildvine is missing", async () => {
    await withTempDir({ prefix: "wildvine-state-legacy-" }, async (root) => {
  ...
  it("CONFIG_PATH prefers existing config when present", async () => {
    await withTempDir({ prefix: "wildvine-config-" }, async (root) => {
  ...
  it("respects state dir overrides when config is missing", async () => {
    await withTempDir({ prefix: "wildvine-config-override-" }, async (root) => {
      const legacyDir = path.join(root, ".wildvine");
      await fs.mkdir(legacyDir, { recursive: true });
