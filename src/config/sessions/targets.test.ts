...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): WildvineConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: WildvineConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        WILDVINE_STATE_DIR: envStateDir,
      };
      const cfg: WildvineConfig = {};
