...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("wildvine-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "wildvine-sandbox-browser-global" },
      agentBrowser: { network: "wildvine-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("wildvine-sandbox-browser-agent");
  });
