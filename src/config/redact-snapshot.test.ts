...
import type { ConfigFileSnapshot } from "./types.wildvine.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/wildvine/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/wildvine/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/wildvine/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/wildvine/nickserv-password.txt");
