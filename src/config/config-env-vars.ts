...
function collectConfigEnvVarsByTarget(cfg?: WildvineConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: WildvineConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: WildvineConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: WildvineConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.WILDVINE_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
