...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.WILDVINE_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveWildvinePackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    WILDVINE_STATE_DIR: path.join(os.tmpdir(), "wildvine-config-doc-baseline-state"),
    WILDVINE_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
