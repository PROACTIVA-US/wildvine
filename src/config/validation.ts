...
import type { WildvineConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.wildvine.com/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: WildvineConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: WildvineConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: WildvineConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = WildvineSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as WildvineConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: WildvineConfig | null | undefined;

  const ensureCompatConfig = (): WildvineConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Wildvine config schema.
