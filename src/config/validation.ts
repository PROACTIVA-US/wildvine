...
export function validateConfigObjectRaw(
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as WildvineConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as WildvineConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as WildvineConfig,
  };
}
