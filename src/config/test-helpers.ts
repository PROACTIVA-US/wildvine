...
import { resetConfigRuntimeState, type WildvineConfig } from "./config.js";

export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  resetConfigRuntimeState();
  try {
    return await withTempHomeBase(fn, { prefix: "wildvine-config-" });
  } finally {
    resetConfigRuntimeState();
  }
}

export async function writeWildvineConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.WILDVINE_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected WILDVINE_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeWildvineConfig(home, config);
