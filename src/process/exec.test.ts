...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        WILDVINE_BASE_ENV: "base",
        WILDVINE_TO_REMOVE: undefined,
      },
      env: {
        WILDVINE_TEST_ENV: "ok",
      },
    });

    expect(resolved.WILDVINE_BASE_ENV).toBe("base");
    expect(resolved.WILDVINE_TEST_ENV).toBe("ok");
    expect(resolved.WILDVINE_TO_REMOVE).toBeUndefined();
    expect(resolved.WILDVINE_CLI).toBe(WILDVINE_CLI_ENV_VALUE);
  });
