...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "wildvine doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "wildvine doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: WildvineConfig): Promise<string[]> {
...
function hasConfiguredMatrixChannel(cfg: WildvineConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: WildvineConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: WildvineConfig, env: NodeJS.ProcessEnv): boolean {
  return (
