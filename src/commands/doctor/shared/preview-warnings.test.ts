...
function manifest(id: string): PluginManifestRecord {
  return {
    ...
    rootDir: `/plugins/${id}`,
    source: `/plugins/${id}`,
    manifestPath: `/plugins/${id}/wildvine.plugin.json`,
  };
}
...
describe("doctor preview warnings", () => {
  ...
  it("includes stale plugin config warnings", () => {
    ...
    expect(warnings[0]).toContain('Run "wildvine doctor --fix"');
  ...
  it("warns but skips auto-removal when plugin discovery has errors", () => {
    ...
    expect(warnings[0]).toContain('rerun "wildvine doctor --fix"');
  });
