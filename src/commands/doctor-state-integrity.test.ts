...
type EnvSnapshot = {
  HOME?: string;
  WILDVINE_HOME?: string;
  WILDVINE_STATE_DIR?: string;
  WILDVINE_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    WILDVINE_HOME: process.env.WILDVINE_HOME,
    WILDVINE_STATE_DIR: process.env.WILDVINE_STATE_DIR,
    WILDVINE_OAUTH_DIR: process.env.WILDVINE_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: WildvineConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: WildvineConfig) {
...
async function runStateIntegrityText(cfg: WildvineConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.WILDVINE_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when WILDVINE_OAUTH_DIR is explicitly configured", async () => {
    process.env.WILDVINE_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: WildvineConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints wildvine-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: WildvineConfig = {};
    ...
    expect(text).toMatch(/wildvine sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/wildvine sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /wildvine sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
