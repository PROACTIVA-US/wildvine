...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.WILDVINE_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.WILDVINE_GATEWAY_PASSWORD ?? configuredPassword;
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<WildvineConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const baseConfig: WildvineConfig = snapshot.valid
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.WILDVINE_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.WILDVINE_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
