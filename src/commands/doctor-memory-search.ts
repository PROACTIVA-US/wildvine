 ...
 * Runs as part of `wildvine doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("wildvine config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("wildvine config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("wildvine memory status --deep")}`,
        ]
          .filter(Boolean)
          .join("\n"),
        "Memory search",
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`wildvine config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("wildvine configure --section model")}`,
        `- To disable: ${formatCliCommand("wildvine config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("wildvine memory status --deep")}`,
      ].join("\n"),
      "Memory search",
