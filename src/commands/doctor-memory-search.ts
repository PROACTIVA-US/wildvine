 ...
 * Runs as part of `wildvine doctor` — config-only, no network calls.
 */
export async function noteMemorySearchHealth(cfg: WildvineConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("wildvine config set agents.defaults.memorySearch.provider openai")}`,
          "",
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`wildvine auth add --provider ${resolved.provider}`)}`,
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("wildvine auth add --provider openai")}`,
