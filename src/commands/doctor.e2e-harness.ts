...
export const resolveWildvinePackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadWildvinePlugins: () => createEmptyPluginRegistry(),
}));
...
beforeEach(() => {
  ...
  resolveWildvinePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.WILDVINE_STATE_DIR;
  originalUpdateInProgress = process.env.WILDVINE_UPDATE_IN_PROGRESS;
  process.env.WILDVINE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-doctor-state-"));
