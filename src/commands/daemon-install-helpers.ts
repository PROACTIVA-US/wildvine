...
export async function buildGatewayInstallPlan(params: {
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      process.platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(params.env.WILDVINE_PROFILE)
        : undefined,
  ...
  //   1. ~/.wildvine/.env file vars  (lowest — user secrets / fallback keys)
  //   2. Config env vars              (wildvine.json env.vars + inline keys)
  //   3. Auth-profile env refs        (credential store → env var lookups)
  //   4. Service environment          (HOME, PATH, WILDVINE_* — highest)
  return {
    programArguments,
    workingDirectory,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("wildvine gateway install")}\` after fixing the error.`;
}
