...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "WILDVINE_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("WILDVINE_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv WILDVINE_GATEWAY_TOKEN");
    expect(message).not.toContain("WILDVINE_GATEWAY_PASSWORD");
  });
  ...
  it("treats SecretRef-backed credentials as configured", async () => {
    const noteFn = vi.fn();
    const getenv = vi.fn(async (name: string) =>
      name === "WILDVINE_GATEWAY_PASSWORD" ? "launchctl-password" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
          password: { source: "env", provider: "default", id: "WILDVINE_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        providers: {
    ...
    expect(message).toContain("WILDVINE_GATEWAY_PASSWORD");
  });
