...
const requireAgentDir = () => {
  const agentDir = process.env.WILDVINE_AGENT_DIR;
  if (!agentDir) {
    throw new Error("WILDVINE_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under WILDVINE_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-oauth-"));
    process.env.WILDVINE_STATE_DIR = tempStateDir;
    process.env.WILDVINE_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.WILDVINE_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to WILDVINE_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-minimax-"));
    process.env.WILDVINE_STATE_DIR = tempStateDir;
    process.env.WILDVINE_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.WILDVINE_AGENT_DIR;
