...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/wildvine-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/wildvine-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "wildvine-agent-" });
}
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<WildvineConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<WildvineConfig["channels"]>["telegram"]>>,
  ...
  const cfg = {
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "wildvine"),
        ...agentOverrides,
      },
      list: agentsList,
    },
...
async function withCrossAgentResumeFixture(
  run: (params: { sessionId: string; sessionKey: string; cfg: WildvineConfig }) => Promise<void>,
): Promise<void> {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as WildvineConfig },
    writeOptions: {},
