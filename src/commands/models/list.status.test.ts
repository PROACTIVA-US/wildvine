...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveWildvineAgentDir: vi.fn().mockReturnValue("/tmp/wildvine-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/wildvine-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/wildvine-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/wildvine-dev/wildvine.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveWildvineAgentDir: mocks.resolveWildvineAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/wildvine-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveWildvineAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/wildvine-dev/wildvine.json");
    expect(payload.auth.storePath).toBe("/tmp/wildvine-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/wildvine-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/wildvine-agent-custom");
