...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveWildvineAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("wildvine models auth setup-token")}\` or \`${formatCliCommand("wildvine configure")}\`.`
          : `Run \`${formatCliCommand("wildvine configure")}\` or set an API key env var.`;
