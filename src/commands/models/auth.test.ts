...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: WildvineConfig;
  let lastUpdatedConfig: WildvineConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/wildvine/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/wildvine/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/wildvine/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: WildvineConfig) => WildvineConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/wildvine/agents/main",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with wildvine configure --section web (recommended mode: cached). Docs: https://docs.wildvine.com/tools/web",
    );
  });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/wildvine/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `wildvine plugins list --json`.',
    );
  });
