...
export async function loadValidConfigOrThrow(): Promise<WildvineConfig> {
...
export async function updateConfig(
  mutator: (cfg: WildvineConfig) => WildvineConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: WildvineConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: WildvineConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("wildvine agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
