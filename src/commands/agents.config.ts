...
function resolveAgentName(cfg: WildvineConfig, agentId: string) {
...
function resolveAgentModel(cfg: WildvineConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: WildvineConfig): AgentSummary[] {
