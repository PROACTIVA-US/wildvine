...
function pruneAuthProfiles(
  cfg: WildvineConfig,
  profileIds: Set<string>,
): { next: WildvineConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("wildvine models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("wildvine models auth setup-token")} or ${formatCliCommand(
      "wildvine configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "wildvine models auth login --provider openai-codex",
    )} or ${formatCliCommand("wildvine configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("wildvine configure")}\` or \`${formatCliCommand("wildvine onboard")}\`.`;
}
