...
function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<WildvineConfig>;
}): ProviderPlugin {
  return {
    ...
    auth: [
      createProviderApiKeyAuthMethod({
        ...
        ...(params.applyConfig ? { applyConfig: () => params.applyConfig as WildvineConfig } : {}),
        wizard: {
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as WildvineConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("wildvine-auth-");
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/wildvine-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireWildvineAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
