...
const SEARCH_TOOL = "https://docs.wildvine.com/mcp.SearchWildvine";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.wildvine.com");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('wildvine docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.wildvine.com/");
      runtime.log(`Search: ${formatCliCommand('wildvine docs "your query"')}`);
    }
    return;
  }
