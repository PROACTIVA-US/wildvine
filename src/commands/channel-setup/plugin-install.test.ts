...
import { loadWildvinePlugins } from "../../plugins/loader.js";
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const cfg: WildvineConfig = { update: { channel } };
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: WildvineConfig = { plugins: { allow: ["other"] } };
    ...
    expect(result.cfg.plugins?.installs?.zalo?.spec).toBe("@wildvine/zalo");
    expect(result.cfg.plugins?.installs?.zalo?.installPath).toBe("/tmp/zalo");
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({ spec: "@wildvine/zalo" }),
    );
  });
  ...
  it("uses the catalog plugin id for local-path installs", async () => {
    ...
    const result = await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...baseEntry,
        id: "teams",
        pluginId: "@wildvine/msteams-plugin",
      },
      prompter,
      runtime,
    });
    ...
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("@wildvine/msteams-plugin");
    expect(result.cfg.plugins?.entries?.["@wildvine/msteams-plugin"]?.enabled).toBe(true);
  });
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
    const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
    const cfg: WildvineConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    resolveBundledPluginSources.mockReturnValue(
      new Map([
        [
          ...
          {
            pluginId: "zalo",
            localPath: bundledPluginRootAt("/opt/wildvine", "zalo"),
            npmSpec: "@wildvine/zalo",
          },
        ],
      ]),
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            value: "local",
            hint: bundledPluginRootAt("/opt/wildvine", "zalo"),
          }),
        ]),
      }),
    );
    ...
    resolveBundledPluginSources.mockReturnValue(
      new Map([
        [
          ...
          {
            pluginId: "whatsapp",
            localPath: bundledPluginRootAt("/opt/wildvine", "whatsapp"),
  ...
  it("clears discovery cache before reloading the setup plugin registry", () => {
    ...
    reloadChannelSetupPluginRegistry({
      cfg,
      runtime,
      workspaceDir: "/tmp/wildvine-workspace",
    });

    expect(clearPluginDiscoveryCache).toHaveBeenCalledTimes(1);
    expect(loadWildvinePlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config: cfg,
        workspaceDir: "/tmp/wildvine-workspace",
        cache: false,
    ...
    expect(clearPluginDiscoveryCache.mock.invocationCallOrder[0]).toBeLessThan(
      vi.mocked(loadWildvinePlugins).mock.invocationCallOrder[0] ?? Number.POSITIVE_INFINITY,
    );
  });
  ...
  it("scopes channel reloads when setup starts from an empty registry", () => {
    const runtime = makeRuntime();
    const cfg: WildvineConfig = {};
    getChannelPluginCatalogEntry.mockReturnValue({ pluginId: "@wildvine/telegram-plugin" });
    ...
    expect(loadWildvinePlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        workspaceDir: "/tmp/wildvine-workspace",
        cache: false,
        onlyPluginIds: ["@wildvine/telegram-plugin"],
        includeSetupOnlyChannelPlugins: true,
      }),
    );
  });
  ...
  it("scopes snapshots by plugin id when channel and plugin ids differ", () => {
    ...
    expect(loadWildvinePlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        workspaceDir: "/tmp/wildvine-workspace",
        cache: false,
        onlyPluginIds: ["@wildvine/msteams-plugin"],
        includeSetupOnlyChannelPlugins: true,
        activate: false,
      }),
    );
