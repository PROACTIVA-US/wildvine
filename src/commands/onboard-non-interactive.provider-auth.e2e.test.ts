...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.WILDVINE_GATEWAY_PASSWORD,
    customApiKey: process.env.CUSTOM_API_KEY,
    disableConfigCache: process.env.WILDVINE_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("WILDVINE_STATE_DIR", prev.stateDir);
  restoreEnvVar("WILDVINE_CONFIG_PATH", prev.configPath);
  restoreEnvVar("WILDVINE_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("WILDVINE_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("WILDVINE_SKIP_CRON", prev.skipCron);
  restoreEnvVar("WILDVINE_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("WILDVINE_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("WILDVINE_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("CUSTOM_API_KEY", prev.customApiKey);
  restoreEnvVar("WILDVINE_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.WILDVINE_DISABLE_CONFIG_CACHE = "1";
  ...
  process.env.WILDVINE_STATE_DIR = tempHome;
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("wildvine-onboard-minimax-", async ({ configPath, runtime }) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("wildvine-onboard-minimax-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("wildvine-onboard-zai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("wildvine-onboard-zai-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("wildvine-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("wildvine-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("wildvine-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("rejects vLLM auth choice in non-interactive mode", async () => {
    await withOnboardEnv("wildvine-onboard-vllm-non-interactive-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("wildvine-onboard-litellm-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("wildvine-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("wildvine-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("wildvine-onboard-together-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("wildvine-onboard-qianfan-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
