...
export function createStatusScanSharedMocks(configPathLabel: string) {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/wildvine-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = WildvineConfig>(
  overrides: T = {} as T,
): WildvineConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): WildvineConfig {
