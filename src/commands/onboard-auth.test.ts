...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under WILDVINE_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("wildvine-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-oauth-sync-"));
    ...
    process.env.WILDVINE_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-oauth-nosync-"));
    process.env.WILDVINE_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside WILDVINE_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-oauth-external-"));
    process.env.WILDVINE_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* WILDVINE_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to WILDVINE_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("wildvine-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
