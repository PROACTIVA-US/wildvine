...
beforeAll(() => {
  previousProfile = process.env.WILDVINE_PROFILE;
  process.env.WILDVINE_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.WILDVINE_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/wildvine-root.js", () => ({
  resolveWildvinePackageRoot: vi.fn().mockResolvedValue("/tmp/wildvine"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/wildvine/pnpm-lock.yaml",
      markerPath: "/tmp/wildvine/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Wildvine status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("wildvine status --all") ||
          l.includes("wildvine --profile isolated status --all") ||
          l.includes("wildvine status --all") ||
          l.includes("wildvine --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.WILDVINE_GATEWAY_TOKEN = "abcd1234";
    try {
