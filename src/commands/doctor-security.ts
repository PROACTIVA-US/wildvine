...
import type { WildvineConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: WildvineConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: WildvineConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Wildvine default (allowlist)";
  const defaultRequestedAskSource = "Wildvine default (on-miss)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("wildvine approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: WildvineConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: WildvineConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("wildvine security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.wildvine/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("wildvine approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.wildvine.com/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("wildvine configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("wildvine config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("wildvine doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("wildvine config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('wildvine config set session.dmScope "per-channel-peer"') +
