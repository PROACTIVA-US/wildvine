...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: WildvineConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: WildvineConfig) => { changed: boolean; next: WildvineConfig };
  defaultModel: string;
  overrideConfig: WildvineConfig;
  alreadyDefaultConfig: WildvineConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: WildvineConfig) => config,
      applyDefaultConfig: (config: WildvineConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: WildvineConfig = { agents: { defaults: {} } };
