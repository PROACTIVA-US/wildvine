...
const ensureWildvineModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveWildvineAgentDir = vi.fn().mockReturnValue("/tmp/wildvine-agent");
...
beforeEach(() => {
  ...
  ensureWildvineModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureWildvineModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureWildvineModelsJson).toHaveBeenCalled();
    expect(ensureWildvineModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
