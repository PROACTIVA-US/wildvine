...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".wildvine", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: WildvineConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["WILDVINE_GATEWAY_TOKEN", await getenv("WILDVINE_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["WILDVINE_GATEWAY_PASSWORD", await getenv("WILDVINE_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "WILDVINE_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.WILDVINE_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- WILDVINE_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
