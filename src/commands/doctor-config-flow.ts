...
function stripUnknownConfigKeys(config: WildvineConfig): {
  config: WildvineConfig;
  removed: string[];
} {
  const parsed = WildvineSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: WildvineConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: WildvineConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: WildvineConfig): Promise<{
  config: WildvineConfig;
  changes: string[];
}> {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".wildvine");
  const targetPath = path.join(targetDir, "wildvine.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".wildvinebot", "wildvinebot.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: WildvineConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("wildvine doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("wildvine doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("wildvine doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "wildvine doctor --fix" to remove these keys.');
    }
  }
