...
export function normalizeCompatibilityConfigValues(cfg: WildvineConfig): {
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as WildvineConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as WildvineConfig["browser"],
    };
    changes.push(
  ...
  const normalizeLegacyNanoBananaSkill = () => {
    type ModelProviderEntry = Partial<
      NonNullable<NonNullable<WildvineConfig["models"]>["providers"]>[string]
    >;
    type ModelsConfigPatch = Partial<NonNullable<WildvineConfig["models"]>>;
    ...
    if (!hasGoogleApiKey && legacyApiKey) {
      ...
      rawModels.providers = rawProviders as NonNullable<WildvineConfig["models"]>["providers"];
      next = {
        ...next,
        models: rawModels as WildvineConfig["models"],
      };
      changes.push(
  ...
  const normalizeLegacyTalkConfig = () => {
    ...
    const normalizedTalk = normalizeTalkSection(rawTalk as WildvineConfig["talk"]);
    if (!normalizedTalk) {
      return;
    }
  ...
  const normalizeLegacyMediaProviderOptions = () => {
    ...
    next = {
      ...
      tools: {
        ...next.tools,
        media: nextMedia as NonNullable<WildvineConfig["tools"]>["media"],
      },
    };
  };
  ...
  const normalizeLegacyMistralModelMaxTokens = () => {
    ...
    next = {
      ...
      models: {
        ...next.models,
        providers: nextProviders as NonNullable<WildvineConfig["models"]>["providers"],
      },
    };
  };
