...
function createPerplexityConfig(apiKey: string, enabled?: boolean): WildvineConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: WildvineConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): WildvineConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: WildvineConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: WildvineConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
