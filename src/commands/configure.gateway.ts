...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "WILDVINE_GATEWAY_TOKEN",
          placeholder: "WILDVINE_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like WILDVINE_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Wildvine will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Wildvine trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.wildvine.com/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
