    ...
    createTestRegistry([
      {
        ...
        plugin: {
          ...
          setup: {
            applyAccountConfig: ({ cfg }: { cfg: WildvineConfig }) => cfg,
          },
          setupWizard: {
            channel: "telegram",
            status: {
...
describe("setupChannels post-write hooks", () => {
  ...
  it("collects onboarding post-write hooks and runs them against the final config", async () => {
    ...
    const restore = patchChannelOnboardingAdapterForTest({
      configureInteractive,
      afterConfigWritten,
      getStatus: vi.fn(async ({ cfg }: { cfg: WildvineConfig }) => ({
        channel: "telegram",
    ...
    try {
      const cfg = await setupChannels({} as WildvineConfig, runtime, prompter, {
