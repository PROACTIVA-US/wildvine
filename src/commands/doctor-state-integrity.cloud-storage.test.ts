...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Wildvine", ".wildvine");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/wildvine-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".wildvine");
  ...
  it("anchors cloud detection to OS homedir when WILDVINE_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".wildvine");
    const originalWildvineHome = process.env.WILDVINE_HOME;
    process.env.WILDVINE_HOME = "/tmp/wildvine-home-override";
    ...
    try {
      ...
      if (originalWildvineHome === undefined) {
