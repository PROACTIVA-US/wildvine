...
function createTelegramCfg(botToken: string, enabled?: boolean): WildvineConfig {
  return {
    channels: {
      telegram: {
        botToken,
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: WildvineConfig }) =>
            ...
            credentials: [
              {
                ...
                inspect: ({ cfg }: { cfg: WildvineConfig }) => ({
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: WildvineConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: WildvineConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: WildvineConfig }) => ({
        channel: "telegram",
...
async function expectQuickstartPickerSkipsWithoutRuntime() {
  ...
  await expect(
    runSetupChannels({} as WildvineConfig, prompter, {
      quickstartDefaults: true,
    }),
  ).resolves.toEqual({} as WildvineConfig);
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as WildvineConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("./channel-setup/plugin-install.js", async (importOriginal) => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: WildvineConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as WildvineConfig, prompter, {
      quickstartDefaults: true,
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@wildvine/msteams-plugin": { enabled: true },
          },
        },
      } as WildvineConfig,
      prompter,
        ...
        if (channel === "msteams") {
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                listAccountIds: (cfg: WildvineConfig) =>
                  Object.keys(
                    (cfg.channels?.msteams as { accounts?: Record<string, unknown> } | undefined)
                      ?.accounts ?? {},
                  ),
                resolveAccount: (cfg: WildvineConfig, accountId: string) =>
                  (
                    cfg.channels?.msteams as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: WildvineConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: WildvineConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
