...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByWildvine ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByWildvine ? "installed · " : "";
  ...
  runtime.log(theme.heading("Wildvine status"));
  runtime.log("");
  ...
  if (summary.taskAudit.errors > 0) {
    runtime.log("");
    runtime.log(
      theme.muted(`Task maintenance: ${formatCliCommand("wildvine tasks maintenance --apply")}`),
    );
  }
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`wildvine devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("wildvine devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("wildvine devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("wildvine security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("wildvine security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.wildvine.com/faq");
  runtime.log("Troubleshooting: https://docs.wildvine.com/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("wildvine status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("wildvine logs --follow")}`);
  if (nodeOnlyGateway) {
    runtime.log(`  Need node service?  ${formatCliCommand("wildvine node status")}`);
  } else if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("wildvine status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("wildvine gateway probe")}`);
  }
}
