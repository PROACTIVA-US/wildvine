...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.WILDVINE_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    WILDVINE_PROFILE: profile,
    WILDVINE_GATEWAY_PORT: String(port),
    WILDVINE_LAUNCHD_LABEL: resolvedLaunchdLabel,
    WILDVINE_SYSTEMD_UNIT: systemdUnit,
    WILDVINE_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    WILDVINE_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    WILDVINE_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = env.WILDVINE_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    WILDVINE_GATEWAY_TOKEN: gatewayToken,
    WILDVINE_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    WILDVINE_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    WILDVINE_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    WILDVINE_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    WILDVINE_LOG_PREFIX: "node",
    WILDVINE_SERVICE_MARKER: NODE_SERVICE_MARKER,
    WILDVINE_SERVICE_KIND: NODE_SERVICE_KIND,
    WILDVINE_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    WILDVINE_STATE_DIR: sharedEnv.stateDir,
    WILDVINE_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.WILDVINE_STATE_DIR;
  const configPath = env.WILDVINE_CONFIG_PATH;
