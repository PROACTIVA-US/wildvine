...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.WILDVINE_GATEWAY_TOKEN).toBe("secret");
    ...
    if (process.platform === "darwin") {
      expect(env.WILDVINE_LAUNCHD_LABEL).toBe("com.wildvine.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    ...
    if (process.platform === "darwin") {
      expect(env.WILDVINE_LAUNCHD_LABEL).toBe("com.wildvine.work");
    }
  });
});
