...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.WILDVINE_GATEWAY_PORT).toBe("18789");
    expect(env.WILDVINE_GATEWAY_TOKEN).toBeUndefined();
    expect(env.WILDVINE_SERVICE_MARKER).toBe("wildvine");
    expect(env.WILDVINE_SERVICE_KIND).toBe("gateway");
    expect(typeof env.WILDVINE_SERVICE_VERSION).toBe("string");
    expect(env.WILDVINE_SYSTEMD_UNIT).toBe("wildvine-gateway.service");
    expect(env.WILDVINE_WINDOWS_TASK_NAME).toBe("Wildvine Gateway");
    if (process.platform === "darwin") {
      expect(env.WILDVINE_LAUNCHD_LABEL).toBe("ai.wildvine.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", WILDVINE_PROFILE: "work" },
      port: 18789,
    });
    expect(env.WILDVINE_SYSTEMD_UNIT).toBe("wildvine-gateway-work.service");
    expect(env.WILDVINE_WINDOWS_TASK_NAME).toBe("Wildvine Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.WILDVINE_LAUNCHD_LABEL).toBe("ai.wildvine.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through WILDVINE_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", WILDVINE_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.WILDVINE_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits WILDVINE_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
