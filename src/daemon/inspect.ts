...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "wildvine" | "wildvinebot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["wildvine", "wildvinebot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["wildvine_service_marker"];
  const kindKeys = ["wildvine_service_kind"];
...
function isWildvineGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("com.wildvine.");
}

function isWildvineGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("wildvine-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isWildvineGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("wildvine gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("wildvinebot") || lower.includes("moltbot");
}
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "wildvinebot" : "moltbot",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "wildvine" && isWildvineGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "wildvine" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "wildvine" && isWildvineGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isWildvineGatewayTaskName(name)) {
        continue;
      }
