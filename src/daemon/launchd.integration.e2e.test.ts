...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `wildvine-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      WILDVINE_LAUNCHD_LABEL: `ai.wildvine.launchd-int-${testId}`,
      WILDVINE_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
