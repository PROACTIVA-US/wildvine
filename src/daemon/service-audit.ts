...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds WILDVINE_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `wildvine gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service WILDVINE_GATEWAY_TOKEN does not match gateway.auth.token in wildvine.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.WILDVINE_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.WILDVINE_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `wildvine gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
