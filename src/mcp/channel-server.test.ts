...
import { z } from "zod";
import { createWildvineChannelMcpServer, WildvineChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createWildvineChannelMcpServer({
...
describe("wildvine channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new WildvineChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
