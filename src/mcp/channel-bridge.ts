...
export class WildvineChannelBridge {
  ...
  async start(): Promise<void> {
    ...
    const gatewayUrlOverrideSource =
      connection.urlSource === "cli --url"
        ? "cli"
        : connection.urlSource === "env WILDVINE_GATEWAY_URL"
          ? "env"
          : undefined;
    ...
    this.gateway = new GatewayClient({
      ...
      clientDisplayName: "Wildvine MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`wildvine mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `wildvine mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
