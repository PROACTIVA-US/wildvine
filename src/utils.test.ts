...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("wildvine-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("wildvine-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("wildvine-lid-a-", async (first) => {
      await withTempDir("wildvine-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.wildvine when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "wildvine-config-dir-"));
    try {
  ...
  it("expands WILDVINE_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/wildvine-home",
      WILDVINE_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/wildvine-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers WILDVINE_HOME over HOME", () => {
    vi.stubEnv("WILDVINE_HOME", "/srv/wildvine-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/wildvine-home"));
...
describe("shortenHomePath", () => {
  it("uses $WILDVINE_HOME prefix when WILDVINE_HOME is set", () => {
    vi.stubEnv("WILDVINE_HOME", "/srv/wildvine-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/wildvine-home")}/.wildvine/wildvine.json`)).toBe(
      "$WILDVINE_HOME/.wildvine/wildvine.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $WILDVINE_HOME replacement when WILDVINE_HOME is set", () => {
    vi.stubEnv("WILDVINE_HOME", "/srv/wildvine-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/wildvine-home")}/.wildvine/wildvine.json`),
    ).toBe("config: $WILDVINE_HOME/.wildvine/wildvine.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/wildvine", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "wildvine"),
    );
  });
  ...
  it("prefers WILDVINE_HOME for tilde expansion", () => {
    vi.stubEnv("WILDVINE_HOME", "/srv/wildvine-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/wildvine")).toBe(path.resolve("/srv/wildvine-home", "wildvine"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/wildvine", env)).toBe(path.resolve("/srv/wildvine-home", "wildvine"));
  });
