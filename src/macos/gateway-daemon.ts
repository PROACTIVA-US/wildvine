...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `wildvine --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.WILDVINE_GATEWAY_PORT ??
    process.env.WILDVINE_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.WILDVINE_GATEWAY_BIND ??
    process.env.WILDVINE_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
...
void main().catch((err) => {
  console.error(
    "[wildvine] Gateway daemon failed:",
