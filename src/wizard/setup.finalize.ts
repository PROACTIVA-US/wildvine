...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      try {
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.wildvine.com/gateway/health",
            "https://docs.wildvine.com/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("wildvine gateway run")}`,
          `Or rerun with: ${formatCliCommand("wildvine onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("wildvine onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.wildvine.com/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: $WILDVINE_CONFIG_PATH (default: ~/.wildvine/wildvine.json) under gateway.auth.token, or in WILDVINE_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("wildvine config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("wildvine doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("wildvine dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Wildvine."
            : "Copy/paste this URL in a browser on this machine to control Wildvine.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("wildvine dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.wildvine.com/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.wildvine.com/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("wildvine configure --section web")}`,
          "",
          "Docs: https://docs.wildvine.com/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("wildvine configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.wildvine.com/tools/web"}`,
          "Docs: https://docs.wildvine.com/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("wildvine configure --section web")}`,
          "",
          "Docs: https://docs.wildvine.com/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://wildvine.com/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Wildvine."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Wildvine.",
  );
