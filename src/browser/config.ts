import type { BrowserConfig, BrowserProfileConfig, WildvineConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_WILDVINE_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "wildvine" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_WILDVINE_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "wildvine" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_WILDVINE_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_WILDVINE_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Wildvine browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for wildvine/wildvine CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: WildvineConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_WILDVINE_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "wildvine";
