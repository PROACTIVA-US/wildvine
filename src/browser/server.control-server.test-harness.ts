...
beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          wildvine: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
export function installBrowserControlServerHooks() {
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.WILDVINE_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.WILDVINE_GATEWAY_TOKEN;
    } else {
      process.env.WILDVINE_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.WILDVINE_GATEWAY_PASSWORD;
    } else {
      process.env.WILDVINE_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
