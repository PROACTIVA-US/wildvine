...
vi.mock("./chrome.js", () => ({
  ...
  resolveWildvineUserDataDir: vi.fn(() => "/tmp/wildvine"),
...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    const before = ctx.forProfile("wildvine");
    expect(before.profile.cdpPort).toBe(18800);

    cfgProfiles.wildvine = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;

    const after = ctx.forProfile("wildvine");
