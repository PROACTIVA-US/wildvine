...
vi.mock("./chrome.js", () => ({
  resolveWildvineUserDataDir: vi.fn(() => "/tmp/wildvine-test/wildvine/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveWildvineUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "wildvine-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveWildvineUserDataDir).mockReturnValue(userDataDir);
