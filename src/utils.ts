...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".wildvine");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.WILDVINE_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$WILDVINE_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via WILDVINE_STATE_DIR.
