...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.WILDVINE_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Wildvine";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Wildvine\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.WILDVINE_MDNS_HOSTNAME?.trim() || "wildvine";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
