...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Wildvine tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideWildvine = path.join(os.tmpdir(), "outside-wildvine", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
