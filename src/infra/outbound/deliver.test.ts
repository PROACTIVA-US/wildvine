...
import { resolvePreferredWildvineTmpDir } from "../tmp-wildvine-dir.js";
...
const telegramChunkConfig: WildvineConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: WildvineConfig = {
  channels: { whatsapp: { textChunkLimit: 4000 } },
};

const expectedPreferredTmpRoot = resolvePreferredWildvineTmpDir();
...
describe("deliverOutboundPayloads", () => {
  ...
  it("scopes media local roots to the active agent workspace when agentId is provided", async () => {
    ...
    expect(
      sendOpts?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".wildvine", "workspace-work")),
      ),
    ).toBe(true);
  });

  it("includes Wildvine tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
  ...
  it("includes Wildvine tmp root in signal mediaLocalRoots", async () => {
  ...
  it("forwards audioAsVoice through generic plugin media delivery", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as WildvineConfig,
      channel: "matrix",
  ...
  it("includes Wildvine tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes Wildvine tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: WildvineConfig = { channels: { signal: { mediaMaxMb: 2 } } };
