...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["WILDVINE_STATE_DIR", "NODE_ENV", "VITEST"]);
    ...
    vi.mocked(resolveWildvinePackageRoot).mockClear();
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/wildvine/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
