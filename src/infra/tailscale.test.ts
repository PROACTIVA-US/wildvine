...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.WILDVINE_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.WILDVINE_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.WILDVINE_TEST_TAILSCALE_BINARY;
    } else {
      process.env.WILDVINE_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
