...
function createDefaultMatrixConfig(): WildvineConfig {
  return {
    channels: {
      matrix: {
...
function createOpsLegacyCryptoFixture(params: {
  ...
  const stateDir = path.join(params.home, ".wildvine");
...
describe("matrix legacy encrypted-state migration", () => {
    ...
    await withTempHome(async (home) => {
      ...
      expect(detection.warnings).toContain(
        "Legacy Matrix encrypted state detected at " +
          ...
          ', but multiple Matrix accounts are configured and channels.matrix.defaultAccount is not set. Set "channels.matrix.defaultAccount" to the intended target account before rerunning "wildvine doctor --fix" or restarting the gateway.',
      );
    });
  });
  ...
  it("warns instead of throwing when a legacy crypto path is a file", async () => {
    await withTempHome(async (home) => {
      ...
      expect(detection.plans).toHaveLength(0);
      expect(detection.warnings).toContain(
        `Legacy Matrix encrypted state path exists but is not a directory: ${path.join(stateDir, "matrix", "crypto")}. Wildvine skipped automatic crypto migration for that path.`,
      );
    });
  });
