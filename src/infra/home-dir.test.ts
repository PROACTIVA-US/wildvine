...
describe("resolveEffectiveHomeDir", () => {
  it("prefers WILDVINE_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/wildvine-home"),
    );
  });
  ...
  it("expands WILDVINE_HOME when set to ~", () => {
    const env = {
      WILDVINE_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for WILDVINE_HOME", () => {
    const result = resolveRequiredHomeDir(
      { WILDVINE_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when WILDVINE_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ WILDVINE_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    ...
    expect(value).toBe(`${path.resolve("/srv/wildvine-home")}/x`);
  });
