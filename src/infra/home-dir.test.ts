...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers WILDVINE_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for WILDVINE_HOME",
      env: { WILDVINE_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when WILDVINE_HOME is tilde-only and no fallback home exists",
      env: { WILDVINE_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores WILDVINE_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          WILDVINE_HOME: "/srv/wildvine-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { WILDVINE_HOME: "/srv/wildvine-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/wildvine-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/wildvine-home " },
      expected: "/srv/wildvine-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/wildvine-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of WILDVINE_HOME", () => {
    expect(
