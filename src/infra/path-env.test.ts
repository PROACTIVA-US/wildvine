...
import { ensureWildvineCliOnPath } from "./path-env.js";
...
describe("ensureWildvineCliOnPath", () => {
  const envKeys = [
    "PATH",
    "WILDVINE_PATH_BOOTSTRAPPED",
    "WILDVINE_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/wildvine-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "wildvine");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureWildvineCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.WILDVINE_PATH_BOOTSTRAPPED;
    delete process.env.WILDVINE_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling wildvine exists", () => {
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "wildvine");
      ...
      if (envValue === undefined) {
        ...
        process.env.WILDVINE_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/wildvine-path/case-linuxbrew");
