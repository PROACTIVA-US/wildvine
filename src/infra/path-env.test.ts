...
describe("ensureWildvineCliOnPath", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-path-"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling wildvine exists", async () => {
    const tmp = path.join(fixtureRoot, `case-${fixtureCount++}`);
    const appBinDir = path.join(tmp, "AppBin");
    await fs.mkdir(appBinDir, { recursive: true });
    const cliPath = path.join(appBinDir, "wildvine");
    await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
    await fs.chmod(cliPath, 0o755);

    const originalPath = process.env.PATH;
    const originalFlag = process.env.WILDVINE_PATH_BOOTSTRAPPED;
    ...
    try {
      ...
      if (originalFlag === undefined) {
        ...
        process.env.WILDVINE_PATH_BOOTSTRAPPED = originalFlag;
      }
    }
  });
