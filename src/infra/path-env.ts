...
function candidateBinDirs(opts: EnsureWildvinePathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `wildvine` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "wildvine");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.WILDVINE_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "wildvine"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `wildvine`
...
/**
 * Best-effort PATH bootstrap so skills that require the `wildvine` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureWildvineCliOnPath(opts: EnsureWildvinePathOpts = {}) {
  if (isTruthyEnvValue(process.env.WILDVINE_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.WILDVINE_PATH_BOOTSTRAPPED = "1";
