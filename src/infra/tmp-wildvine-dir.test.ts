...
type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredWildvineTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `wildvine-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredWildvineTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_WILDVINE_TMP_DIR) {
...
describe("resolvePreferredWildvineTmpDir", () => {
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/wildvine when /tmp/wildvine is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/wildvine when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/wildvine exists but is not writable",
    ...
    {
      name: "falls back when /tmp/wildvine is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/wildvine is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/wildvine is group/other writable",
  ...
  it("repairs existing /tmp/wildvine permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_WILDVINE_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_WILDVINE_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_WILDVINE_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/wildvine after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_WILDVINE_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_WILDVINE_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_WILDVINE_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_WILDVINE_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Wildvine temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "wildvine");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredWildvineTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_WILDVINE_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Wildvine temp dir/);
  });
});
