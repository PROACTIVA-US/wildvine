...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "wildvine.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Wildvine)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "wildvine.internal", expected: "wildvine.internal." },
    { value: "wildvine.internal.", expected: "wildvine.internal." },
    { value: "  wildvine.internal  ", expected: "wildvine.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { WILDVINE_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { WILDVINE_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("wildvine.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "wildvine.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderZoneText({
      ...
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/wildvine",
    });

    expectZoneRecords(txt, [
      `$ORIGIN wildvine.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_wildvine-gw._tcp IN PTR studio-london._wildvine-gw._tcp`,
      `studio-london._wildvine-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Wildvine)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/wildvine`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN wildvine.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._wildvine-gw._tcp IN TXT`,
        `displayName=Mac Studio (Wildvine)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("wildvine.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("wildvine.internal."),
