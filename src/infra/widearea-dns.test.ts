...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expect(txt).toContain(`$ORIGIN wildvine.internal.`);
    ...
    expect(txt).toContain(`_wildvine-gw._tcp IN PTR studio-london._wildvine-gw._tcp`);
    expect(txt).toContain(`studio-london._wildvine-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Wildvine)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/wildvine`);
  });
