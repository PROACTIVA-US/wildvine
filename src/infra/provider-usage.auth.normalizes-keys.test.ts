...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".wildvine");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".wildvine", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      WILDVINE_STATE_DIR: path.join(home, ".wildvine"),
      ...env,
    };
