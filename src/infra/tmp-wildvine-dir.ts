...
export const POSIX_WILDVINE_TMP_DIR = "/tmp/wildvine";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredWildvineTmpDirOptions = {
...
export function resolvePreferredWildvineTmpDir(
  options: ResolvePreferredWildvineTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "wildvine" : `wildvine-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[wildvine] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Wildvine temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Wildvine temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_WILDVINE_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_WILDVINE_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_WILDVINE_TMP_DIR)) {
      return POSIX_WILDVINE_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_WILDVINE_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_WILDVINE_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_WILDVINE_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_WILDVINE_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_WILDVINE_TMP_DIR;
