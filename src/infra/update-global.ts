...
const PRIMARY_PACKAGE_NAME = "wildvine";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const WILDVINE_MAIN_PACKAGE_SPEC = "github:wildvine/wildvine#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Wildvine", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.WILDVINE_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.WILDVINE_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return WILDVINE_MAIN_PACKAGE_SPEC;
  }
