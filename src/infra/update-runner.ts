...
const DEFAULT_PACKAGE_NAME = "wildvine";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["wildvine", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "wildvine.mjs");
    ...
    const doctorStep = await runStep(
      step("wildvine doctor", doctorArgv, gitRoot, { WILDVINE_UPDATE_IN_PROGRESS: "1" }),
    );
