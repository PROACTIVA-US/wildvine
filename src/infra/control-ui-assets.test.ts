...
import { resolveWildvinePackageRoot } from "./wildvine-root.js";
...
describe("control UI assets helpers", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-ui-"));
  });
  ...
  it("resolves control-ui root for dist/gateway bundle argv1", async () => {
    await withTempDir(async (tmp) => {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "wildvine" }));
  ...
  it("resolves dist control-ui index path from package root argv1", async () => {
    await withTempDir(async (tmp) => {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "wildvine" }));
      await fs.writeFile(path.join(tmp, "wildvine.mjs"), "export {};\n");
      await fs.mkdir(path.join(tmp, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(tmp, "wildvine.mjs"))).toBe(
  ...
  it("resolves control-ui root for package entrypoint argv1", async () => {
    await withTempDir(async (tmp) => {
      ...
      expect(resolveControlUiRootSync({ argv1: path.join(tmp, "wildvine.mjs") })).toBe(
        path.join(tmp, "dist", "control-ui"),
      );
    });
  });
  ...
  it("resolves dist control-ui index path from .bin argv1", async () => {
    await withTempDir(async (tmp) => {
      ...
      const pkgRoot = path.join(tmp, "node_modules", "wildvine");
      await fs.mkdir(binDir, { recursive: true });
      await fs.mkdir(path.join(pkgRoot, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(binDir, "wildvine"), "#!/usr/bin/env node\n");
      await fs.writeFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "wildvine" }));
      await fs.writeFile(path.join(pkgRoot, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(binDir, "wildvine"))).toBe(
  ...
  it("resolves via fallback when package root resolution fails but package name matches", async () => {
    await withTempDir(async (tmp) => {
      // Package named "wildvine" but resolveWildvinePackageRoot failed for other reasons
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "wildvine" }));
      await fs.writeFile(path.join(tmp, "wildvine.mjs"), "export {};\n");
      await fs.mkdir(path.join(tmp, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "dist", "control-ui", "index.html"), "<html></html>\n");
  ...
  it("returns null when package name does not match wildvine", async () => {
    await withTempDir(async (tmp) => {
  ...
  it("resolves control-ui root when argv1 is a symlink (nvm scenario)", async () => {
    await withTempDir(async (tmp) => {
      ...
      await fs.writeFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "wildvine" }));
      await fs.writeFile(path.join(realPkg, "wildvine.mjs"), "export {};\n");
      ...
      const ok = await trySymlink(
        path.join("..", "real-pkg", "wildvine.mjs"),
        path.join(bin, "wildvine"),
      );
      if (!ok) {
        return; // symlinks not supported (Windows CI)
      }

      const resolvedRoot = resolveControlUiRootSync({ argv1: path.join(bin, "wildvine") });
  ...
  it("resolves package root via symlinked argv1", async () => {
    await withTempDir(async (tmp) => {
      ...
      const packageRoot = await resolveWildvinePackageRoot({ argv1: path.join(bin, "wildvine") });
  ...
  it("resolves dist index path via symlinked argv1 (async)", async () => {
    await withTempDir(async (tmp) => {
      ...
      const indexPath = await resolveControlUiDistIndexPath(path.join(bin, "wildvine"));
