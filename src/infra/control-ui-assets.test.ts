...
vi.mock("./wildvine-root.js", () => ({
  resolveWildvinePackageRoot: vi.fn(async () => null),
  resolveWildvinePackageRootSync: vi.fn(() => null),
}));
...
let wildvineRoot: typeof import("./wildvine-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    wildvineRoot = await import("./wildvine-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/wildvine");
    const wrapperArgv1 = abs("fixtures/bin/wildvine");
  ...
  it("uses resolveWildvinePackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/wildvine");
    (
      wildvineRoot.resolveWildvinePackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/wildvine"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "wildvine" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "wildvine.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-wildvine");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/wildvine-bundle");
    (
      wildvineRoot.resolveWildvinePackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/Wildvine.app/Contents/MacOS/Wildvine");
    const bundledUiDir = abs("fixtures/Wildvine.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/wildvine-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
