...
export function triggerWildvineRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.WILDVINE_SYSTEMD_UNIT,
        process.env.WILDVINE_PROFILE,
      );
  ...
  const label =
    process.env.WILDVINE_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.WILDVINE_PROFILE);
