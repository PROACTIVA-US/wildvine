...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __wildvineDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__wildvineDiagnosticEventsState) {
    globalStore.__wildvineDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__wildvineDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: WildvineConfig): boolean {
