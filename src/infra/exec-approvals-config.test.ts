...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevWildvineHome = process.env.WILDVINE_HOME;

    try {
      process.env.WILDVINE_HOME = dir;
      const approvalsPath = path.join(dir, ".wildvine", "exec-approvals.json");
      ...
      if (prevWildvineHome === undefined) {
        delete process.env.WILDVINE_HOME;
      } else {
        process.env.WILDVINE_HOME = prevWildvineHome;
      }
    }
  });
});
