...
describe("runGatewayUpdate", () => {
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "wildvine", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "wildvine.mjs")} doctor --non-interactive`]: {
        stdout: "",
      },
    });
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g wildvine@latest",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g wildvine@beta",
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g wildvine@latest") {
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "wildvine");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.writeFile(
        path.join(pkgRoot, "package.json"),
        JSON.stringify({ name: "wildvine", version: "1.0.0" }),
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g wildvine@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "wildvine", version: "2.0.0" }),
            "utf-8",
  ...
  it("repairs UI assets when doctor run removes control-ui files", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === `${process.execPath} ${path.join(tempDir, "wildvine.mjs")} doctor --non-interactive`
      ) {
