...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/wildvine-state", "devices")).toEqual({
      dir: path.join("/tmp/wildvine-state", "devices"),
      pendingPath: path.join("/tmp/wildvine-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/wildvine-state", "devices", "paired.json"),
    });
  });
