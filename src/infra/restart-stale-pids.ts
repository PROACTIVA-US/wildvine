 ...
 * returning control to the caller (typically `triggerWildvineRestart` →
...
/**
 * Parse wildvine gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("wildvine")) {
 ...
 * Returns only PIDs that belong to wildvine gateway processes (not the current process).
 */
