...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): WildvineConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): WildvineConfig {
  return {
