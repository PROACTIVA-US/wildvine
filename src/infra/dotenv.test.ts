...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.wildvine/.env as fallback without overriding CWD .env", async () => {
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "WILDVINE_STATE_DIR=./evil-state",
            "WILDVINE_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.WILDVINE_STATE_DIR).toBe(stateDir);
        expect(process.env.WILDVINE_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "WILDVINE_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "WILDVINE_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "WILDVINE_LIVE_GEMINI_KEY=sk-gemini-live",
            "WILDVINE_LIVE_OPENAI_KEY=sk-openai-live",
            "WILDVINE_GATEWAY_TOKEN=attacker-token",
            "WILDVINE_GATEWAY_PASSWORD=attacker-password",
            "WILDVINE_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
        ...
        delete process.env.WILDVINE_LIVE_ANTHROPIC_KEY;
        delete process.env.WILDVINE_LIVE_ANTHROPIC_KEYS;
        delete process.env.WILDVINE_LIVE_GEMINI_KEY;
        delete process.env.WILDVINE_LIVE_OPENAI_KEY;
        delete process.env.WILDVINE_GATEWAY_TOKEN;
        delete process.env.WILDVINE_GATEWAY_PASSWORD;
        delete process.env.WILDVINE_GATEWAY_SECRET;
        ...
        expect(process.env.WILDVINE_LIVE_ANTHROPIC_KEY).toBeUndefined();
        expect(process.env.WILDVINE_LIVE_ANTHROPIC_KEYS).toBeUndefined();
        expect(process.env.WILDVINE_LIVE_GEMINI_KEY).toBeUndefined();
        expect(process.env.WILDVINE_LIVE_OPENAI_KEY).toBeUndefined();
        expect(process.env.WILDVINE_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.WILDVINE_GATEWAY_PASSWORD).toBeUndefined();
        expect(process.env.WILDVINE_GATEWAY_SECRET).toBeUndefined();
      });
    });
  });

  it("blocks WILDVINE_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.WILDVINE_STATE_DIR).toBeUndefined();
        expect(process.env.WILDVINE_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (WILDVINE_AGENT_DIR, WILDVINE_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, WILDVINE_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "WILDVINE_AGENT_DIR=./evil-agent",
            `WILDVINE_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "WILDVINE_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );

        delete process.env.WILDVINE_AGENT_DIR;
        delete process.env.WILDVINE_BUNDLED_PLUGINS_DIR;
        delete process.env.PI_CODING_AGENT_DIR;
        delete process.env.WILDVINE_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.WILDVINE_AGENT_DIR).toBeUndefined();
        expect(process.env.WILDVINE_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.WILDVINE_OAUTH_DIR).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "WILDVINE_BUNDLED_HOOKS_DIR=./attacker-hooks",
            "WILDVINE_BUNDLED_PLUGINS_DIR=./attacker-plugins",
            "WILDVINE_BUNDLED_SKILLS_DIR=./attacker-skills",
          ].join("\n"),
        );

        delete process.env.WILDVINE_BUNDLED_HOOKS_DIR;
        delete process.env.WILDVINE_BUNDLED_PLUGINS_DIR;
        delete process.env.WILDVINE_BUNDLED_SKILLS_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.WILDVINE_BUNDLED_HOOKS_DIR).toBeUndefined();
        expect(process.env.WILDVINE_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.WILDVINE_BUNDLED_SKILLS_DIR).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "WILDVINE_LIVE_GEMINI_KEY=sk-gemini-live",
            "WILDVINE_LIVE_OPENAI_KEY=sk-openai-live",
            "WILDVINE_GATEWAY_TOKEN=trusted-token",
            "WILDVINE_GATEWAY_PASSWORD=trusted-password",
            "WILDVINE_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_API_KEY;
        ...
        expect(process.env.WILDVINE_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.WILDVINE_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.WILDVINE_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.WILDVINE_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.WILDVINE_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.WILDVINE_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.WILDVINE_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "WILDVINE_STATE_DIR=./evil-state\n");
