...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.WILDVINE_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.WILDVINE_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.WILDVINE_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.WILDVINE_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.WILDVINE_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set WILDVINE_APNS_PRIVATE_KEY_P8 or WILDVINE_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading WILDVINE_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
