...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__wildvine_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "wildvine-root");
...
function setPackageRoot(root: string, name = "wildvine") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./wildvine-root.js").resolveWildvinePackageRootSync,
  asyncResolver: typeof import("./wildvine-root.js").resolveWildvinePackageRoot,
  opts: Parameters<typeof import("./wildvine-root.js").resolveWildvinePackageRootSync>[0],
...
describe("resolveWildvinePackageRoot", () => {
  let resolveWildvinePackageRoot: typeof import("./wildvine-root.js").resolveWildvinePackageRoot;
  let resolveWildvinePackageRootSync: typeof import("./wildvine-root.js").resolveWildvinePackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveWildvinePackageRoot, resolveWildvinePackageRootSync } =
      await import("./wildvine-root.js"));
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "wildvine");
        const pkgRoot = path.join(project, "node_modules", "wildvine");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "wildvine");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "wildvine.mjs")));
    ...
    {
      name: "falls through from a non-wildvine moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-wildvine");
    ...
    {
      name: "returns null for non-wildvine package roots",
      setup: () => {
        const pkgRoot = fx("not-wildvine");
        setPackageRoot(pkgRoot, "not-wildvine");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "wildvine.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "wildvine");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveWildvinePackageRootSync,
      resolveWildvinePackageRoot,
      opts,
      expected,
    );
  });
