...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@wildvine/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@wildvine/matrix", id: "matrix", expected: true },
    { packageName: "@wildvine/matrix", id: "@wildvine/matrix", expected: true },
    { packageName: "@wildvine/matrix", id: "signal", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@wildvine/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@wildvine__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-install-safe-"));
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-install-safe-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-install-safe-outside-"));
      try {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    const parentDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-install-safe-"));
