...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.WILDVINE_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*wildvine-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "wildvine";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "wildvine");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "wildvine-gw");
  ...
  records.push(`_wildvine-gw._tcp IN PTR ${instanceLabel}._wildvine-gw._tcp`);
  records.push(`${instanceLabel}._wildvine-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._wildvine-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; wildvine-content-hash: ${contentHash}\n${contentBody}`;
}
