...
let createWildvineCodingTools: typeof import("./pi-tools.js").createWildvineCodingTools;

describe("createWildvineTools plugin context", () => {
  beforeEach(async () => {
    resolvePluginToolsMock.mockClear();
    vi.resetModules();
    ({ createWildvineTools } = await import("./wildvine-tools.js"));
    ({ createWildvineCodingTools } = await import("./pi-tools.js"));
  });
  ...
  it("does not inject ambient thread defaults into plugin tools", async () => {
    ...
    const first = createWildvineTools({
      config: {} as never,
      agentThreadId: "111.222",
    }).find((tool) => tool.name === "plugin-thread-default");
    const second = createWildvineTools({
      config: {} as never,
  ...
  it("does not inject messageThreadId defaults for missing params objects", async () => {
    ...
    const wrapped = createWildvineTools({
      config: {} as never,
      agentThreadId: "77",
