...
const GOOGLE_LIVE_SINGLE_KEY = "WILDVINE_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "WILDVINE_LIVE_ANTHROPIC_KEY",
    listVar: "WILDVINE_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "WILDVINE_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `WILDVINE_LIVE_${base}_KEY`;
