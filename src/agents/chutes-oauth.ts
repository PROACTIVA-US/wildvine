...
export function parseOAuthCallbackInput(
  ...
  try {
    ...
    // Security: Never fabricate state for bare code input. This was a CSRF vulnerability
    // where the catch block returned { code: input, state: expectedState }, allowing
    // attackers to bypass state validation entirely by providing just an auth code.
  ...
  // Security: Validate state matches expected value to prevent CSRF attacks.
  // Without this check, an attacker could swap their auth code into victim's session.
  if (state !== expectedState) {
    return { error: "State mismatch. This may be a CSRF attack. Please restart the login flow." };
  }
  return { code, state };
}
