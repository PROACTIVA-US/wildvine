...
function resolveAttachmentLimits(config: WildvineConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".wildvine", "attachments");
  const relDir = path.posix.join(".wildvine", "attachments", attachmentId);
