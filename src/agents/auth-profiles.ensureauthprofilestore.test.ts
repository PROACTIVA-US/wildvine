...
describe("ensureAuthProfileStore", () => {
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-auth-merge-"));
    const previousAgentDir = process.env.WILDVINE_AGENT_DIR;
    ...
    try {
      ...
      process.env.WILDVINE_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.WILDVINE_AGENT_DIR = previousAgentDir;
      }
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("wildvine-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("wildvine-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-oauth-migrate-"));
    ...
    try {
      ...
      process.env.WILDVINE_STATE_DIR = root;
      process.env.WILDVINE_AGENT_DIR = agentDir;
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("wildvine-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    {
      ...
      prefix: "wildvine-nonstr-key-ref-",
    ...
    },
    {
      ...
      prefix: "wildvine-nonstr-key-num-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "wildvine-nonstr-key-dup-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "wildvine-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "wildvine-nonstr-token-ref-",
    ...
    },
    {
      ...
      prefix: "wildvine-nonstr-token-num-",
      profileId: "anthropic:default",
      profile: {
        type: "token",
        provider: "anthropic",
