 ...
 * A common mistake — caused by the similarity with the `wildvine.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (wildvine.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.WILDVINE_AUTH_STORE_READONLY === "1";
