...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-embedded-agent-"));
...
const ensureModels = (cfg: WildvineConfig) => ensureWildvineModelsJson(cfg, agentDir) as unknown;
