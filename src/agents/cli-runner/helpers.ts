...
/**
 * Cleanup suspended Wildvine CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: WildvineConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-cli-images-"));
  const paths: string[] = [];
