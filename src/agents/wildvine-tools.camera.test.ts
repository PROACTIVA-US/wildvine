...
function getNodesTool(options?: { modelHasVision?: boolean; allowMediaInvokeCommands?: boolean }) {
  ...
  const tool = createWildvineTools(toolOptions).find((candidate) => candidate.name === "nodes");
  if (!tool) {
...
beforeEach(async () => {
  ...
  vi.unstubAllGlobals();
  await loadWildvineToolsForTest();
});
...
describe("nodes camera_snap", () => {
  ...
  it("omits inline base64 image blocks when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/wildvine-camera-snap-front-.*\.jpg$/);
  });
...
describe("nodes photos_latest", () => {
  ...
  it("returns MEDIA paths and no inline images when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/wildvine-camera-snap-.*\.jpg$/);
  });
...
async function loadWildvineToolsForTest(): Promise<void> {
  vi.resetModules();
