...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: WildvineConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/wildvine" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: WildvineConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: WildvineConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: WildvineConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: WildvineConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: WildvineConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: WildvineConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: WildvineConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/wildvine");
  });

  it("uses WILDVINE_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "wildvine-home");
    vi.stubEnv("WILDVINE_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as WildvineConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".wildvine", "workspace"));
  });

  it("uses WILDVINE_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "wildvine-home");
    vi.stubEnv("WILDVINE_HOME", home);
    // Clear state dir so it falls back to WILDVINE_HOME
    vi.stubEnv("WILDVINE_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as WildvineConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".wildvine", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/wildvine-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/wildvine-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-agent-scope-"));
