...
describe("runCliAgent spawn path", () => {
  ...
  it("hydrates prompt media refs into CLI image args", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredWildvineTmpDir(), "wildvine-cli-prompt-image-"),
    );
    ...
    expect(argv[imageArgIndex + 1]).toContain("wildvine-cli-images-");
  ...
  it("appends hydrated prompt media refs to generic backend prompts", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredWildvineTmpDir(), "wildvine-cli-prompt-image-generic-"),
    );
    ...
    const appendedPath = argv.find((value) => value.includes("wildvine-cli-images-"));
  ...
  it("prefers explicit images over prompt refs", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredWildvineTmpDir(), "wildvine-cli-explicit-images-"),
    );
  ...
  it("falls back to per-agent workspace when workspaceDir is missing", async () => {
    const runCliAgent = await setupCliRunnerTestModule();
    const tempDir = await fs.mkdtemp(
      path.join(process.env.TMPDIR ?? "/tmp", "wildvine-cli-runner-"),
    );
