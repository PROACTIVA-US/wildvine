    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "wildvine.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "wildvine.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "wildvine.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("wildvine-");
  const acpxRoot = await tempDirs.make("wildvine-acpx-plugin-");
  const helperRoot = await tempDirs.make("wildvine-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("wildvine-");
  const pluginRoot = await tempDirs.make("wildvine-plugin-");
  const outsideDir = await tempDirs.make("wildvine-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("wildvine-");
    const pluginRoot = await tempDirs.make("wildvine-claude-bundle-");
