...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseWildvineManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyWildvineManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveWildvineMetadata(
  frontmatter: ParsedSkillFrontmatter,
): WildvineSkillMetadata | undefined {
  const metadataObj = resolveWildvineManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveWildvineManifestRequires(metadataObj);
  const install = resolveWildvineManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveWildvineManifestOs(metadataObj);
  return {
