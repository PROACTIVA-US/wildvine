...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["WILDVINE_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns WILDVINE_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-bundled-override-"));
    process.env.WILDVINE_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "wildvine" }));
