...
function resolveConfigBackedProviderAuth(params: { provider: string; config?: WildvineConfig }):
  | {
      apiKey: string;
      discoveryApiKey?: string;
      mode: "api_key";
...
function resolveXaiConfigFallbackAuth(params: { provider: string; config?: WildvineConfig }):
  | {
      apiKey: string;
      source: string;
      mode: "api-key";
