...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "wildvine-models-" });
}

const _MODELS_CONFIG: WildvineConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("normalizes gemini 3 ids to preview for google providers", async () => {
    await withTempHome(async () => {
      const { ensureWildvineModelsJson } = await import("./models-config.js");
      const { resolveWildvineAgentDir } = await import("./agent-paths.js");
