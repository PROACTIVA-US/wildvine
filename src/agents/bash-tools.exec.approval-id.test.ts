...
function getTestConfigPath() {
  return path.join(process.env.HOME ?? "", ".wildvine", "wildvine.json");
}

async function writeWildvineConfig(config: Record<string, unknown>, pretty = false) {
...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".wildvine", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-test-"));
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-test-bin-"));
  ...
  it("auto-continues the same Discord session after approval resolves without a second user turn", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-exec-followup-discord-"));
  ...
  it("executes approved commands and emits a session-only followup in webchat-only mode", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-exec-followup-sidefx-"));
    const markerPath = path.join(tempDir, "marker.txt");
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".wildvine", "skills", "gog");
  ...
  it("denies gateway obfuscated command when approval request times out", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-test-obf-"));
