...
function mockContextDeps(params: {
  ...
  const ensureWildvineModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveWildvineAgentDir: () => "/tmp/wildvine-agent",
  }));
  ...
  return { ensureWildvineModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real wildvine startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "wildvine", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "wildvine", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "wildvine", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureWildvineModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureWildvineModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
