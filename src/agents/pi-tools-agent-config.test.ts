...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: WildvineConfig) {
    return createWildvineCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<WildvineConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<WildvineConfig["agents"]>["list"]>[number]["tools"];
  }): WildvineConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createWildvineCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createWildvineCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createWildvineCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createWildvineCodingTools({
      config: cfg,
    ...
    const bobTools = createWildvineCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createWildvineCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createWildvineCodingTools({
      config: cfg,
