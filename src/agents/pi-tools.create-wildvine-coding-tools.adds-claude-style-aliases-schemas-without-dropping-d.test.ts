...
describe("createWildvineCodingTools", () => {
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-read-"));
    try {
      ...
      const contents = "Hello from wildvine read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "wildvine-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "wildvine-workspace"),
    ...
    const tools = createWildvineCodingTools({ sandbox });
