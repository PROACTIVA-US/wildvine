...
type CoreToolDefinition = {
  ...
  includeInWildvineGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const wildvineTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInWildvineGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:wildvine": wildvineTools,
