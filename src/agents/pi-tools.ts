...
function resolveExecConfig(params: { cfg?: WildvineConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: WildvineConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createWildvineCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createWildvineReadTool(freshReadTool);
