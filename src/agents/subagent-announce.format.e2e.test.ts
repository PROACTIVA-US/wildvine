...
let configOverride: WildvineConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: WildvineConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/PROACTIVA-US/wildvine/issues/31298
    previousFastTestEnv = process.env.WILDVINE_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.WILDVINE_TEST_FAST;
      return;
    }
    process.env.WILDVINE_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // WILDVINE_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Wildvine runtime context (internal):");
