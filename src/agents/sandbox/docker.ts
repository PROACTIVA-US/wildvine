...
import { markWildvineExecEnv } from "../../infra/wildvine-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "wildvine.sandbox=1");
  args.push("--label", `wildvine.sessionKey=${params.scopeKey}`);
  args.push("--label", `wildvine.createdAtMs=${createdAtMs}`);
  args.push("--label", `wildvine.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `wildvine.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markWildvineExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "wildvine.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`wildvine sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`wildvine sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("wildvine sandbox recreate --all");
}
