...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Wildvine handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Wildvine docs: ${docsPath}`,
    "Mirror: https://docs.wildvine.com",
    "Source: https://github.com/PROACTIVA-US/wildvine",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For Wildvine behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `wildvine status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Wildvine process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Wildvine agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Wildvine agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Wildvine.";
  }

  const lines = [
    "You are a personal assistant running inside Wildvine.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Wildvine's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## Wildvine CLI Quick Reference",
    "Wildvine is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- wildvine gateway status",
    "- wildvine gateway start",
    "- wildvine gateway stop",
    "- wildvine gateway restart",
    "If unsure, ask the user to run `wildvine help` (or `wildvine gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Wildvine Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Wildvine pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Wildvine and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Wildvine treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
