...
let configuredConfig: WildvineConfig | undefined;
...
function isLikelyWildvineCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "wildvine" ||
    entryBasename === "wildvine.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Wildvine CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureWildvineModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyWildvineCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): WildvineConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureWildvineModelsJson(cfg);
    } catch {
