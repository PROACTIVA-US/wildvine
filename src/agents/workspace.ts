...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.WILDVINE_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".wildvine", `workspace-${profile}`);
  }
  return path.join(home, ".wildvine", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".wildvine";
