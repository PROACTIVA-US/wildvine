...
/**
 * Security: Canonicalize and validate a gateway WebSocket URL.
 * Prevents SSRF by rejecting malformed or suspicious URLs.
 */
...
/**
 * Security: Validate that a gatewayUrl override from agent tools is allowed.
 * Only loopback addresses and configured remote URLs are permitted.
 * This prevents SSRF attacks where a malicious tool/agent tries to connect
 * to internal services (e.g., AWS metadata at 169.254.169.254).
 */
...
export function resolveGatewayOptions(opts?: GatewayCallOptions) {
  // Security: Validate URL overrides to prevent SSRF attacks.
  const url =
