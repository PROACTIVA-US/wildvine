...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "wildvine" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "Wildvine", url: "https://wildvine.com/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest wildvine release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://wildvine.com/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://wildvine.com/docs"]);
