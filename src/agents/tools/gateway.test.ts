...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    wildvine: process.env.WILDVINE_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.wildvine === undefined) {
      delete process.env.WILDVINE_GATEWAY_TOKEN;
    } else {
      process.env.WILDVINE_GATEWAY_TOKEN = envSnapshot.wildvine;
    }
  });
  ...
  it("uses WILDVINE_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.WILDVINE_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
