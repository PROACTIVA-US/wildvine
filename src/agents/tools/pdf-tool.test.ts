...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-pdf-"));
  try {
...
function withDefaultModel(primary: string): WildvineConfig {
  return {
    agents: { defaults: { model: { primary } } },
  } as WildvineConfig;
}

function withPdfModel(primary: string): WildvineConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureWildvineModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("ANTHROPIC_API_KEY", "anthropic-test");
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-pdf-out-"));
      try {
        const cfg = withDefaultModel(ANTHROPIC_PDF_MODEL);
