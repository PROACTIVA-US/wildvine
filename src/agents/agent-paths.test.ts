...
describe("resolveWildvineAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("honors PI_CODING_AGENT_DIR when WILDVINE_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers WILDVINE_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          WILDVINE_STATE_DIR: undefined,
          WILDVINE_AGENT_DIR: primaryOverride,
