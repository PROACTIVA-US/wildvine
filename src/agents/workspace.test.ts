...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses WILDVINE_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      WILDVINE_HOME: "/srv/wildvine-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/wildvine-home"), ".wildvine", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".wildvine", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("wildvine-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("wildvine-workspace-");
    await fs.mkdir(path.join(tempDir, ".wildvine"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-workspace-hardlink-"));
    try {
