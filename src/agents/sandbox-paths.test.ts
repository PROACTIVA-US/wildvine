...
async function withOutsideHardlinkInWildvineTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside wildvine tmp root",
      media: path.join(os.tmpdir(), "outside-wildvine", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Wildvine tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Wildvine tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInWildvineTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Wildvine tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInWildvineTmp(
