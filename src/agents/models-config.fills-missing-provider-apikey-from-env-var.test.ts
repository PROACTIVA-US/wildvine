...
async function runCustomProviderMergeTest(params: {
  ...
  await ensureWildvineModelsJson({
    models: {
      mode: "merge",
      providers: {
...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): WildvineConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureWildvineModelsJson(validated.config);
  ...
  it("fills anthropic-vertex apiKey with the ADC sentinel when models exist", async () => {
    await withTempHome(async () => {
      const adcDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-adc-"));
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureWildvineModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
