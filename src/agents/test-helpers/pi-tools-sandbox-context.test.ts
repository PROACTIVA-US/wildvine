...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("wildvine-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("wildvine-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("wildvine-sbx-");
