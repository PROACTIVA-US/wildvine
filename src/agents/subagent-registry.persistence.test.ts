...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["WILDVINE_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-subagent-"));
  ...
  it("uses isolated temp state when WILDVINE_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "wildvine-test-state"));
  });
});
