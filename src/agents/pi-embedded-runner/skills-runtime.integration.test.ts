...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("wildvine-bundled-");
  const workspaceDir = await createTempDir("wildvine-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: WildvineConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.WILDVINE_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
