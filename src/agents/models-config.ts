...
import { planWildvineModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  ...
  sourceConfigForSecrets: WildvineConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: WildvineConfig): {
  config: WildvineConfig;
  sourceConfigForSecrets: WildvineConfig;
} {
...
export async function ensureWildvineModelsJson(
  config?: WildvineConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveWildvineAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planWildvineModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
