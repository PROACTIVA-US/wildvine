...
let createWildvineTools: typeof import("./wildvine-tools.js").createWildvineTools;

async function loadFreshWildvineToolsModuleForTest() {
  vi.resetModules();
  vi.doMock("./tools/gateway.js", () => createGatewayToolModuleMocks());
  ({ createWildvineTools } = await import("./wildvine-tools.js"));
}
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshWildvineToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-test-"));

    try {
      await withEnvAsync(
        { WILDVINE_STATE_DIR: stateDir, WILDVINE_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: wildvine --profile isolated doctor --non-interactive",
          );
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/wildvine" } } }',
      }),
    ).rejects.toThrow(
