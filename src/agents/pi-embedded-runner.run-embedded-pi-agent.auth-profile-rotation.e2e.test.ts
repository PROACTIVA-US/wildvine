...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async (importOriginal) => {
    ...
    return {
      ...mod,
      ensureWildvineModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): WildvineConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): WildvineConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `wildvine-auth-rotation-${Date.now()}.log`),
    });
