...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: WildvineConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.WILDVINE_GATEWAY_TOKEN;
    const prevPassword = process.env.WILDVINE_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.WILDVINE_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.WILDVINE_GATEWAY_PASSWORD;
      } else {
        process.env.WILDVINE_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-win-"));
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "wildvine.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: WildvineConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.WILDVINE_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-discord-"));
    process.env.WILDVINE_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.WILDVINE_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "wildvine-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.WILDVINE_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.WILDVINE_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-discord-open-"));
    process.env.WILDVINE_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: WildvineConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.WILDVINE_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-slack-"));
    process.env.WILDVINE_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: WildvineConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.WILDVINE_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-slack-open-"));
    process.env.WILDVINE_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: WildvineConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.WILDVINE_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-telegram-"));
    process.env.WILDVINE_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: WildvineConfig = {
  ...
  it("warns when Telegram allowFrom entries are non-numeric (legacy @username configs)", async () => {
    const prevStateDir = process.env.WILDVINE_STATE_DIR;
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "wildvine-security-audit-telegram-invalid-allowfrom-"),
    );
    process.env.WILDVINE_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.WILDVINE_GATEWAY_TOKEN;
    process.env.WILDVINE_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: WildvineConfig = {
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-"));
    const stateDir = path.join(tmp, "state");
  ...
  it("flags enabled extensions when tool policy can expose plugin tools", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-plugins-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(path.join(stateDir, "extensions", "some-plugin"), {
      recursive: true,
      mode: 0o700,
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-audit-scanner-"));
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.WILDVINE_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.WILDVINE_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.WILDVINE_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.WILDVINE_GATEWAY_PASSWORD;
      } else {
        process.env.WILDVINE_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.WILDVINE_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: WildvineConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.WILDVINE_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: WildvineConfig = {
        gateway: {
          mode: "remote",
