...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: WildvineConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: WildvineConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: WildvineConfig) => string[];
  isConfigured?: (account: unknown, cfg: WildvineConfig) => boolean;
  isEnabled?: (account: unknown, cfg: WildvineConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
async function runConfigAuditCases<T extends { cfg: WildvineConfig }>(
  cases: readonly T[],
...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "wildvine.json"),
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "wildvine.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ WILDVINE_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const runSharedExtensionsAudit = async (config: WildvineConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "wildvine.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        wildvine: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-security-audit-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { WILDVINE_HOME: isolatedHome });
  ...
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    const cases = [
          ...
          withEnvAsync(
            {
              ...
              WILDVINE_GATEWAY_PASSWORD: undefined,
            },
            async () =>
              audit({
                gateway: {
      ...
      },
      {
        ...
        run: async () => {
          const sourceConfig: WildvineConfig = {
            gateway: {
              bind: "lan",
              auth: {
                token: {
          ...
          const resolvedConfig: WildvineConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/wildvine-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "wildvine-sbx-browser-old\nwildvine-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "wildvine-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "wildvine-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("wildvine-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("wildvine-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "wildvine-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "wildvine-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-wildvine.json");
      ...
      cases
        ...
        .map((testCase) => ({
          run: async () => {
            ...
            const configPath = path.join(fixture.stateDir, "wildvine.json");
            await fs.writeFile(configPath, "{}\n", "utf-8");
            if (!isWindows) {
              await fs.chmod(configPath, 0o600);
            }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): WildvineConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    const cases = [
      ...
      },
      {
        ...
        plugin: (sourceConfig: WildvineConfig) =>
          stubChannelPlugin({
            id: "slack",
            label: "Slack",
            inspectAccount: (cfg) => {
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.wildvine/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: WildvineConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<WildvineConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        env: {
          WILDVINE_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        },
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies WildvineConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies WildvineConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies WildvineConfig,
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.wildvine",
      configPath: "/Users/test/Dropbox/.wildvine/wildvine.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: WildvineConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
          ...
          runInstallMetadataAudit(
            {
              plugins: {
                installs: {
                  "voice-call": {
                    source: "npm",
                    spec: "@wildvine/voice-call@1.2.3",
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@wildvine/test-hooks@1.2.3",
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@wildvine/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@wildvine/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            ...
            JSON.stringify({
              name: "escape-plugin",
              wildvine: { extensions: ["../outside.js"] },
            }),
          );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.WILDVINE_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.WILDVINE_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
