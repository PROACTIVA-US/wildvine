...
import type { ConfigFileSnapshot, WildvineConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: WildvineConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Wildvine state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Wildvine state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.WILDVINE_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.WILDVINE_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("wildvine security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.WILDVINE_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.WILDVINE_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: WildvineConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: WildvineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: WildvineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: WildvineConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("wildvine status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("wildvine security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set WILDVINE_GATEWAY_TOKEN/WILDVINE_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("wildvine security audit --deep")}".`,
    });
  }
