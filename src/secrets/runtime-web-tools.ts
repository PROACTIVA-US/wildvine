...
type FetchConfig = NonNullable<WildvineConfig["tools"]>["web"] extends infer Web
...
function hasCustomWebSearchPluginRisk(config: WildvineConfig): boolean {
...
function setResolvedXSearchApiKey(params: { resolvedConfig: WildvineConfig; value: string }): void {
  const tools = ensureObject(params.resolvedConfig as Record<string, unknown>, "tools");
  const web = ensureObject(tools, "web");
