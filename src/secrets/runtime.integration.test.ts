...
function asConfig(value: unknown): WildvineConfig {
...
function createOpenAIFileRuntimeConfig(secretFile: string): WildvineConfig {
  return asConfig({
    secrets: {
      providers: {
        default: {
...
describe("secrets runtime snapshot integration", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "WILDVINE_DISABLE_PLUGIN_DISCOVERY_CACHE",
      "WILDVINE_VERSION",
    ]);
    delete process.env.WILDVINE_BUNDLED_PLUGINS_DIR;
    process.env.WILDVINE_DISABLE_PLUGIN_DISCOVERY_CACHE = "1";
    delete process.env.WILDVINE_VERSION;
  });
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    await withEnvAsync(
      {
        WILDVINE_BUNDLED_PLUGINS_DIR: undefined,
        WILDVINE_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
        WILDVINE_VERSION: undefined,
      },
      async () => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          ...
          agentDirs: ["/tmp/wildvine-agent-main"],
          loadAuthStore: () =>
        ...
        expect(
          ensureAuthProfileStore("/tmp/wildvine-agent-main").profiles["openai:default"],
        ).toMatchObject({
          type: "api_key",
          key: "sk-runtime",
        });
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("wildvine-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("wildvine-secrets-runtime-refresh-fail-", async (home) => {
      const { secretFile, agentDir } = await createOpenAIFileRuntimeFixture(home);
  ...
  it(
    ...
    async () => {
      await withTempHome("wildvine-secrets-runtime-gateway-auth-reload-lkg-", async (home) => {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".wildvine", "wildvine.json"), "utf8"),
        ) as WildvineConfig;
  ...
  it(
    ...
    async () => {
      await withTempHome("wildvine-secrets-runtime-web-reload-lkg-", async (home) => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          config: asConfig({
            tools: {
              web: {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("wildvine-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".wildvine", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".wildvine", "agents", "ops", "agent");
