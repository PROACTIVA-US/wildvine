...
const WILDVINE_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of WILDVINE_COMPILED_SECRET_TARGETS) {
...
const WILDVINE_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: WILDVINE_COMPILED_SECRET_TARGETS,
    entriesById: WILDVINE_TARGETS_BY_ID,
  });
