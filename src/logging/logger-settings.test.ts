...
let originalWildvineLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.WILDVINE_TEST_FILE_LOG;
  originalWildvineLogLevel = process.env.WILDVINE_LOG_LEVEL;
  delete process.env.WILDVINE_TEST_FILE_LOG;
  delete process.env.WILDVINE_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.WILDVINE_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalWildvineLogLevel === undefined) {
    delete process.env.WILDVINE_LOG_LEVEL;
  } else {
    process.env.WILDVINE_LOG_LEVEL = originalWildvineLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
