...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.WILDVINE_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function onDiagnosticEvent(listener) {
  ...
  if (!diagnosticEvents || typeof diagnosticEvents.onDiagnosticEvent !== "function") {
    throw new Error("wildvine/plugin-sdk root alias could not resolve onDiagnosticEvent");
  }
...
function buildPluginSdkAliasMap(useDist) {
  ...
  const aliasMap = {
    "wildvine/plugin-sdk": __filename,
  };
...
function getJiti(tryNative) {
  ...
  const jitiLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled Wildvine core graph.
    tryNative,
