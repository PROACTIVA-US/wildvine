...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`wildvine pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`wildvine pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends WildvineConfig = WildvineConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends WildvineConfig = WildvineConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends WildvineConfig = WildvineConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: WildvineConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends WildvineConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends WildvineConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends WildvineConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
