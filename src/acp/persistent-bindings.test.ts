...
type ConfiguredBinding = NonNullable<WildvineConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<WildvineConfig>,
): WildvineConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: WildvineConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/wildvine" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
