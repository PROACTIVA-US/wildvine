...
const createTempDir = () => tempDirs.make("wildvine-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets WILDVINE_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "wildvine",
    });

    expect(env.WILDVINE_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("wildvine");
  });

  it("overrides pre-existing WILDVINE_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      WILDVINE_SHELL: "wrong",
    });
    expect(env.WILDVINE_SHELL).toBe("acp-client");
  });
  ...
  it("preserves WILDVINE_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        WILDVINE_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["WILDVINE_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Wildvine bridge", () => {
    ...
    expect(env.WILDVINE_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.WILDVINE_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "wildvine",
        serverArgs: ["acp"],
        defaultServerCommand: "wildvine",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("WILDVINE_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "wildvine", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "wildvine", "dist", "entry.js");
    const shimPath = path.join(dir, "wildvine.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\wildvine\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/wildvine-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/wildvine-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/wildvine-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
