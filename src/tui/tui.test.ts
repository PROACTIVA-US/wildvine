...
describe("resolveInitialTuiAgentId", () => {
  const cfg: WildvineConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/wildvine" },
        { id: "ops", workspace: "/tmp/wildvine/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/wildvine/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run wildvine devices list");
    expect(state.pairingHint).toContain("wildvine devices list");
  });
