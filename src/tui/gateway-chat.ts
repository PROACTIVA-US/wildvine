...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set WILDVINE_GATEWAY_TOKEN/WILDVINE_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "wildvine-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.WILDVINE_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.WILDVINE_GATEWAY_PASSWORD);
