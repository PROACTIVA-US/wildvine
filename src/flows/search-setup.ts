...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<WildvineConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<WildvineConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: WildvineConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: WildvineConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: WildvineConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: WildvineConfig, result: WildvineConfig): WildvineConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: WildvineConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as WildvineConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with wildvine configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Wildvine will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.wildvine.com/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Wildvine will store a reference instead of the API key.",
