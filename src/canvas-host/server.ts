...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.wildvineCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.wildvineCanvasA2UIAction &&
        typeof window.wildvineCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.wildvineSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("wildvine:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Wildvine node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.wildvineSendUserAction === "function"
        ? window.wildvineSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.WILDVINE_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.WILDVINE_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Wildvine Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
