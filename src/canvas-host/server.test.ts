...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-canvas-fixtures-"));
  });
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("wildvineCanvasA2UIAction");
    expect(out).toContain("wildvineSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(html).toContain("wildvineSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.wildvineA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      ...
      const res = await realFetch(`http://127.0.0.1:${server.port}/__wildvine__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("wildvine-a2ui-host");
      expect(html).toContain("wildvineCanvasA2UIAction");

      const bundleRes = await realFetch(
        `http://127.0.0.1:${server.port}/__wildvine__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("wildvineA2UI");
