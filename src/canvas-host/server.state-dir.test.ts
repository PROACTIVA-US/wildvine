...
describe("canvas host state dir defaults", () => {
  ...
  it("uses WILDVINE_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Wildvine Canvas");
    } finally {
      await handler.close();
