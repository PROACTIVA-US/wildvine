...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: WildvineConfig) => Array<string | number> | undefined;
}): WildvineConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: WildvineConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: WildvineConfig) => Array<string | number> | undefined;
}): (cfg: WildvineConfig, dmPolicy: DmPolicy) => WildvineConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: WildvineConfig, allowFrom: string[]) => WildvineConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: WildvineConfig, groupPolicy: "open" | "allowlist" | "disabled") => WildvineConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: WildvineConfig) => WildvineConfig | Promise<WildvineConfig>;
  applySet?: (
    cfg: WildvineConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => WildvineConfig | Promise<WildvineConfig>;
}): Promise<{
  cfg: WildvineConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in Wildvine config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "WILDVINE_SECRET",
      envVarFormatError:
        'Use an env var name like "WILDVINE_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends WildvineConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export function createPromptParsedAllowFromForAccount<TConfig extends WildvineConfig>(params: {
...
export function createTopLevelChannelParsedAllowFromPrompt(params: {
  ...
  placeholder: string;
  parseEntries: (raw: string) => ParsedAllowFromResult;
  getExistingAllowFrom?: (cfg: WildvineConfig) => Array<string | number>;
  mergeEntries?: (params: { existing: Array<string | number>; parsed: string[] }) => string[];
}): NonNullable<ChannelSetupDmPolicy["promptAllowFrom"]> {
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: WildvineConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: WildvineConfig) => Array<string | number>;
