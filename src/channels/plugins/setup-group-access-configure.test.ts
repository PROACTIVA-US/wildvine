...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: WildvineConfig, policy: ChannelAccessPolicy) => WildvineConfig;
  resolveAllowlist: (params: { cfg: WildvineConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: WildvineConfig; resolved: TResolved }) => WildvineConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: WildvineConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: WildvineConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: WildvineConfig, policy: ChannelAccessPolicy): WildvineConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: WildvineConfig, policy: ChannelAccessPolicy): WildvineConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: WildvineConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: WildvineConfig; resolved: string[] }) => {
      calls.push("apply");
