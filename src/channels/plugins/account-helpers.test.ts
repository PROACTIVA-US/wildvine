...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): WildvineConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as WildvineConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: WildvineConfig) => string[];
  input: WildvineConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: WildvineConfig, expected: string) {
