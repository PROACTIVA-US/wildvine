  ...
  it.each([
    {
      ...
      setup: () => {
        const dir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-catalog-"));
        const catalogPath = path.join(dir, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@wildvine/demo-channel",
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-channel-catalog-state-"));
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-catalog-state-"));
  ...
  it.each([
    {
      ...
      setup: () => {
        const home = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-catalog-home-"));
        const catalogPath = path.join(home, "catalog.json");
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@wildvine/env-demo-channel",
        ...
        return {
          env: {
            ...process.env,
            WILDVINE_PLUGIN_CATALOG_PATHS: "~/catalog.json",
            WILDVINE_HOME: home,
            HOME: home,
          },
    ...
    },
    {
      ...
      setup: () => {
        ...
        writeCatalogFile(
          catalogPath,
          createCatalogEntry({
            packageName: "@wildvine/default-env-demo",
