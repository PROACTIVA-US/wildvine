...
type SetupContractEntry = {
  ...
  cases: Array<{
    ...
    beforeTest?: () => void;
    assertPatchedConfig?: (cfg: WildvineConfig) => void;
    assertResolvedAccount?: (account: unknown, cfg: WildvineConfig) => void;
  }>;
};
...
setBundledChannelRuntime("line", {
  channel: {
    line: {
      listLineAccountIds,
      resolveDefaultLineAccountId,
      resolveLineAccount: ({ cfg, accountId }: { cfg: WildvineConfig; accountId?: string }) =>
...
const matrixSessionBindingStateDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "wildvine-matrix-session-binding-contract-"),
);
