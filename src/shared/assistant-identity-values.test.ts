...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  Wildvine  ", 20)).toBe("Wildvine");
    expect(coerceIdentityValue("  Wildvine  ", 8)).toBe("Wildvine");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  Wildvine Assistant  ", 8)).toBe("Wildvine");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  Wildvine  ", 0)).toBe("");
    expect(coerceIdentityValue("  Wildvine  ", -1)).toBe("OpenCla");
  });
});
