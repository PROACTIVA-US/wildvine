...
type HomeEnvSnapshot = {
  ...
  WILDVINE_STATE_DIR: string | undefined;
  WILDVINE_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    WILDVINE_AGENT_DIR: process.env.WILDVINE_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".wildvine", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.WILDVINE_STATE_DIR = path.join(home, ".wildvine");
    process.env.WILDVINE_AGENT_DIR = path.join(home, ".wildvine", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".wildvine", "agent");
