...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: WildvineConfig) => AcpRuntimeError | null>(() => null),
  resolveAcpAgentPolicyError: vi.fn<(cfg: WildvineConfig, agent: string) => AcpRuntimeError | null>(
    () => null,
  ),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: WildvineConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
function createAcpConfigWithVisibleToolTags(): WildvineConfig {
...
describe("tryDispatchAcpReply", () => {
  beforeEach(async () => {
    ...
    vi.doMock("../../acp/policy.js", () => ({
      resolveAcpDispatchPolicyError: (cfg: WildvineConfig) =>
        policyMocks.resolveAcpDispatchPolicyError(cfg),
      resolveAcpAgentPolicyError: (cfg: WildvineConfig, agent: string) =>
    ...
    vi.doMock("../../acp/runtime/session-meta.js", () => ({
      readAcpSessionEntry: (params: { sessionKey: string; cfg?: WildvineConfig }) =>
  ...
  it("uses canonical session keys for bound-session identity notices", async () => {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: WildvineConfig }) =>
        params.sessionKey === canonicalSessionKey
          ? {
              cfg: params.cfg ?? createAcpTestConfig(),
              storePath: "/tmp/wildvine-session-store.json",
              sessionKey: canonicalSessionKey,
