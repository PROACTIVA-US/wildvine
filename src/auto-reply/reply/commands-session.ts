...
import { scheduleGatewaySigusr1Restart, triggerWildvineRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Wildvine in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerWildvineRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Wildvine via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
