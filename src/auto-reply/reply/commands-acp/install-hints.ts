...
export function resolveConfiguredAcpBackendId(cfg: WildvineConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: WildvineConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (workspaceLocalPath && existsSync(workspaceLocalPath)) {
      return `wildvine plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^wildvine plugins install /u, "");
    ...
    return "wildvine plugins install acpx";
  }
