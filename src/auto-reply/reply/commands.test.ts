...
function getDiscordExecApprovalApproversForTests(params: { cfg: WildvineConfig }): string[] {
...
function listConfiguredTelegramAccountIds(cfg: WildvineConfig): string[] {
...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-commands-"));
...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-commands-config-"));
  const configPath = path.join(dir, "wildvine.json");
  const previous = process.env.WILDVINE_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.WILDVINE_CONFIG_PATH = previous;
    }
    await fs.rm(dir, {
      recursive: true,
      force: true,
...
function buildParams(commandBody: string, cfg: WildvineConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/wildvine-session-store.json" },
    } as WildvineConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/wildvine-agent-compact";
      ...
      },
      {
        ...
        run: async () => {
          await withTempConfigPath({ messages: { ackReaction: ":)" } }, async (configPath) => {
            ...
            const written = await readJsonFile<WildvineConfig>(configPath);
