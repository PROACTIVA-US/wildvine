...
function snapshotStateEnv(): StateEnvSnapshot {
  return { WILDVINE_STATE_DIR: process.env.WILDVINE_STATE_DIR };
}

function restoreStateEnv(snapshot: StateEnvSnapshot) {
  if (snapshot.WILDVINE_STATE_DIR === undefined) {
    delete process.env.WILDVINE_STATE_DIR;
  } else {
    process.env.WILDVINE_STATE_DIR = snapshot.WILDVINE_STATE_DIR;
  }
}
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "wildvine-typing-heartbeat-"));
  });
