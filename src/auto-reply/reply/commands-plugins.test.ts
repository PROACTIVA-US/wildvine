...
const workspaceHarness = createCommandWorkspaceHarness("wildvine-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".wildvine", "extensions", params.pluginId);
...
function buildCfg(): WildvineConfig {
  return {
    plugins: {
      enabled: true,
    },
