...
const createWildvineToolsMock = vi.fn();
...
async function loadFreshInlineActionsModuleForTest() {
  ...
  vi.doMock("../../agents/wildvine-tools.runtime.js", () => ({
    createWildvineTools: (...args: unknown[]) => createWildvineToolsMock(...args),
  }));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createWildvineToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
