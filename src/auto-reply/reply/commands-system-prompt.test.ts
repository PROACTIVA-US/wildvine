...
const { createWildvineCodingToolsMock } = vi.hoisted(() => ({
  createWildvineCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(async () => {
    vi.restoreAllMocks();
    vi.resetModules();
    createWildvineCodingToolsMock.mockClear();
    createWildvineCodingToolsMock.mockReturnValue([]);
    const piTools = await import("../../agents/pi-tools.js");
    vi.spyOn(piTools, "createWildvineCodingTools").mockImplementation(
      createWildvineCodingToolsMock,
    );
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createWildvineCodingToolsMock).toHaveBeenCalledWith(
