...
const piEmbeddedMocks = getSharedMocks("wildvine.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("wildvine.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("wildvine.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("wildvine.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    wildvineHome: process.env.WILDVINE_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("WILDVINE_HOME", snapshot.wildvineHome);
  restoreKey("WILDVINE_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.WILDVINE_STATE_DIR = join(home, ".wildvine");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "wildvine-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".wildvine", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): WildvineConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "wildvine"),
