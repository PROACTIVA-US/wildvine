...
const ORIGINAL_STATE_DIR = process.env.WILDVINE_STATE_DIR;
...
async function withTaskRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withTempDir({ prefix: "wildvine-task-registry-" }, async (root) => {
...
describe("task-registry", () => {
  afterEach(() => {
    ...
    if (ORIGINAL_STATE_DIR === undefined) {
      ...
      process.env.WILDVINE_STATE_DIR = ORIGINAL_STATE_DIR;
    }
  ...
  it("cancels ACP-backed tasks through the ACP session manager", async () => {
    await withTempDir({ prefix: "wildvine-task-registry-" }, async (root) => {
