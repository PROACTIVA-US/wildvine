...
export function resolveMissingBrowserCommandMessage(config?: WildvineConfig): string | null {
  const allow =
  ...
  if (allow.length > 0 && !allow.includes("browser")) {
    return (
      'The `wildvine browser` command is unavailable because `plugins.allow` excludes "browser". ' +
      'Add "browser" to `plugins.allow` if you want the bundled browser CLI and tool.'
    );
  }
  if (config?.plugins?.entries?.browser?.enabled === false) {
    return (
      "The `wildvine browser` command is unavailable because `plugins.entries.browser.enabled=false`. " +
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? process.env.WILDVINE_CONTAINER?.trim() ?? null;
  ...
  try {
    ...
    process.on("uncaughtException", (error) => {
      console.error("[wildvine] Uncaught exception:", formatUncaughtError(error));
      process.exit(1);
    });
