...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Wildvine and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["wildvine update", "Update a source checkout (git)"],
        ["wildvine update --channel beta", "Switch to beta channel (git + npm)"],
        ["wildvine update --channel dev", "Switch to dev channel (git + npm)"],
        ["wildvine update --tag beta", "One-off update to a dist-tag or version"],
        ["wildvine update --tag main", "One-off package install from GitHub main"],
        ["wildvine update --dry-run", "Preview actions without changing anything"],
        ["wildvine update --no-restart", "Update without restarting the service"],
        ["wildvine update --json", "Output result as JSON"],
        ["wildvine update --yes", "Non-interactive (accept downgrade prompts)"],
        ["wildvine update wizard", "Interactive update wizard"],
        ["wildvine --update", "Shorthand for wildvine update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run wildvine update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.wildvine.com/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["wildvine update status", "Show channel + version status."],
          ["wildvine update status --json", "JSON output."],
          ["wildvine update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.wildvine.com/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
