...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.WILDVINE_DISABLE_ROUTE_FIRST;
    delete process.env.WILDVINE_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.WILDVINE_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });

  it("skips config guard for routed status --json commands", async () => {
    await expect(tryRouteCli(["node", "wildvine", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "wildvine", "status"])).resolves.toBe(true);
  ...
  it("routes logs to stderr during plugin loading in --json mode and restores after", async () => {
    ...
    await tryRouteCli(["node", "wildvine", "agents", "--json"]);
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "wildvine", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "wildvine", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );
