...
function enableInternalHookEntries(config: WildvineConfig, hookNames: string[]): WildvineConfig {
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.wildvine.bot/cli/hooks")}\n`,
    );
