...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-bundled-skills-test-"));
    process.env.WILDVINE_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
