    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "wildvine node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["wildvine node status", "Check node host service status."],
          ["wildvine node install", "Install the node host service."],
          ["wildvine node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.wildvine.com/cli/node")}\n`,
    );
