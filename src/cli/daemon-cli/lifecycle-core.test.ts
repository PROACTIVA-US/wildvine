...
const loadConfig = vi.fn<() => WildvineConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { WILDVINE_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("WILDVINE_CONTAINER_HINT", "wildvine-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for wildvine-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { WILDVINE_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("WILDVINE_GATEWAY_TOKEN", "env-token");
  ...
  it("resolves config token SecretRefs using service command env before drift checks", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: {
        WILDVINE_GATEWAY_TOKEN: "service-token",
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["wildvine gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints).toEqual(expect.arrayContaining(["wildvine gateway install"]));
    expect(payload.hintItems).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "install",
          text: "wildvine gateway install",
        }),
      ]),
    );
    expect(service.restart).not.toHaveBeenCalled();
