...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    WILDVINE_STATE_DIR: "/tmp/wildvine-daemon",
    WILDVINE_CONFIG_PATH: "/tmp/wildvine-daemon/wildvine.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.WILDVINE_STATE_DIR ?? "/tmp/wildvine-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.WILDVINE_CONFIG_PATH ?? `${stateDir}/wildvine.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/wildvine-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.WILDVINE_STATE_DIR = "/tmp/wildvine-cli";
    process.env.WILDVINE_CONFIG_PATH = "/tmp/wildvine-cli/wildvine.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/wildvine-daemon/wildvine.json",
      }),
    );
  });
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.WILDVINE_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.WILDVINE_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.WILDVINE_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "wildvine-gateway" }],
        hints: [],
      },
      healthy: false,
