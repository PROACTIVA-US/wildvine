...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("wildvine-daemon-install-int-");
    configPath = path.join(tempHome, "wildvine.json");
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.WILDVINE_GATEWAY_TOKEN).toBeUndefined();
  });
});
