...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-update-tests-"));
  });
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveWildvinePackageRoot).mockReset();
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await createCaseDir("wildvine-update");

    vi.mocked(resolveWildvinePackageRoot).mockResolvedValue(tempDir);
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: tempDir,
      installKind: "package",
      packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("wildvine-update-wizard");
    const previousGitDir = process.env.WILDVINE_GIT_DIR;
    try {
      setTty(true);
      process.env.WILDVINE_GIT_DIR = tempDir;
      ...
      process.env.WILDVINE_GIT_DIR = previousGitDir;
    }
  });
});
