...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ...
    WILDVINE_CONFIG_PATH: process.env.WILDVINE_CONFIG_PATH,
    WILDVINE_GATEWAY_PORT: process.env.WILDVINE_GATEWAY_PORT,
    WILDVINE_PROFILE: process.env.WILDVINE_PROFILE,
  };

  beforeEach(() => {
    process.env.WILDVINE_STATE_DIR = "/tmp/wildvine-cli-state";
    process.env.WILDVINE_CONFIG_PATH = "/tmp/wildvine-cli-state/wildvine.json";
  ...
  afterEach(() => {
    if (originalEnv.WILDVINE_STATE_DIR !== undefined) {
      process.env.WILDVINE_STATE_DIR = originalEnv.WILDVINE_STATE_DIR;
    } else {
      delete process.env.WILDVINE_STATE_DIR;
    }

    if (originalEnv.WILDVINE_CONFIG_PATH !== undefined) {
      process.env.WILDVINE_CONFIG_PATH = originalEnv.WILDVINE_CONFIG_PATH;
    } else {
      delete process.env.WILDVINE_CONFIG_PATH;
    }

    if (originalEnv.WILDVINE_GATEWAY_PORT !== undefined) {
      process.env.WILDVINE_GATEWAY_PORT = originalEnv.WILDVINE_GATEWAY_PORT;
    } else {
      delete process.env.WILDVINE_GATEWAY_PORT;
    }

    if (originalEnv.WILDVINE_PROFILE !== undefined) {
      process.env.WILDVINE_PROFILE = originalEnv.WILDVINE_PROFILE;
    } else {
      delete process.env.WILDVINE_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        WILDVINE_STATE_DIR: "/tmp/wildvine-daemon-state",
        WILDVINE_CONFIG_PATH: "/tmp/wildvine-daemon-state/wildvine.json",
