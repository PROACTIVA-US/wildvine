...
import type { ConfigFileSnapshot } from "../config/types.wildvine.js";
import { loadConfigForInstall } from "./plugins-install-command.js";

const hoisted = vi.hoisted(() => ({
  loadConfigMock: vi.fn<() => WildvineConfig>(),
...
vi.mock("../commands/doctor/providers/matrix.js", () => ({
  cleanStaleMatrixPluginConfig: (cfg: WildvineConfig) => cleanStaleMatrixPluginConfigMock(cfg),
}));
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as WildvineConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as WildvineConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const matrixNpmRequest = {
    rawSpec: "@wildvine/matrix",
    normalizedSpec: "@wildvine/matrix",
  };
  ...
  beforeEach(() => {
    ...
    cleanStaleMatrixPluginConfigMock.mockImplementation((cfg: WildvineConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("returns the config directly when loadConfig succeeds", async () => {
    const cfg = { plugins: { entries: { matrix: { enabled: true } } } } as WildvineConfig;
  ...
  it("does not run stale Matrix cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as WildvineConfig;
    loadConfigMock.mockReturnValue(cfg);
  ...
  it("allows explicit repo-checkout Matrix reinstall recovery", async () => {
    ...
    const snapshotCfg = { plugins: {} } as WildvineConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Matrix install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `wildvine doctor --fix` before installing plugins.");
    expect(readConfigFileSnapshotMock).not.toHaveBeenCalled();
  });
  ...
  it("throws when loadConfig fails with INVALID_CONFIG and snapshot parsed is empty", async () => {
    ...
    await expect(loadConfigForInstall(matrixNpmRequest)).rejects.toThrow(
      "Config file could not be parsed; run `wildvine doctor` to repair it.",
    );
  });
