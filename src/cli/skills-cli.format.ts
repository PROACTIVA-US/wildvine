...
function appendWildvineHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ...
      : "No skills found.";
    return appendWildvineHubHint(message, opts.json);
  }
  ...
  return appendWildvineHubHint(lines.join("\n"), opts.json);
}
