...
export const loadConfig = vi.fn<() => WildvineConfig>(() => ({}) as WildvineConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: WildvineConfig) => Promise<void>>(
  async () => undefined,
);
export const replaceConfigFile = vi.fn(
  async (params: { nextConfig: WildvineConfig }) => await writeConfigFile(params.nextConfig),
);
export const resolveStateDir = vi.fn(() => "/tmp/wildvine-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: WildvineConfig) => writeConfigFile(config),
  replaceConfigFile: (params: { nextConfig: WildvineConfig }) => replaceConfigFile(params),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as WildvineConfig);
  readConfigFileSnapshot.mockImplementation(async () => {
    const config = loadConfig();
    return {
      path: "/tmp/wildvine-config.json5",
      exists: true,
      raw: "{}",
      parsed: config,
      resolved: config,
  ...
  resolveStateDir.mockReturnValue("/tmp/wildvine-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: WildvineConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: WildvineConfig) => cfg);
  buildPluginStatusReport.mockReturnValue({
    plugins: [],
    diagnostics: [],
  });
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: WildvineConfig }) => ({
    config,
    warnings: [],
  }));
  uninstallPlugin.mockResolvedValue({
    ok: true,
    config: {} as WildvineConfig,
    warnings: [],
    actions: {
      entry: false,
      install: false,
  ...
  recordHookInstall.mockImplementation((cfg: WildvineConfig) => cfg);
}
