...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["wildvine security audit", "Run a local security audit."],
          ["wildvine security audit --deep", "Include best-effort live Gateway probe checks."],
          ["wildvine security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "wildvine security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["wildvine security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["wildvine security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.wildvine.com/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Wildvine security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("wildvine security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("wildvine security audit --fix")}`));
        if (!fixResult) {
