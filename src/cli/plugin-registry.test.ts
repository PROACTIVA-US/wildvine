...
vi.mock("../plugins/loader.js", () => ({
  loadWildvinePlugins: mocks.loadWildvinePlugins,
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("reloads when escalating from configured-channels to channels", async () => {
    ...
    expect(mocks.loadWildvinePlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadWildvinePlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not treat a pre-seeded partial registry as all scope", async () => {
    ...
    expect(mocks.loadWildvinePlugins).toHaveBeenCalledTimes(1);
    expect(mocks.loadWildvinePlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config,
        throwOnLoadError: true,
