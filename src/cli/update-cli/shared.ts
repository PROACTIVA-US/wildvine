...
const WILDVINE_REPO_URL = "https://github.com/PROACTIVA-US/wildvine.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "wildvine";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["wildvine", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.WILDVINE_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "wildvine");
  }
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveWildvinePackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", WILDVINE_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `WILDVINE_GIT_DIR points at a non-git directory: ${params.dir}. Set WILDVINE_GIT_DIR to an empty folder or an wildvine checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`WILDVINE_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "wildvine.mjs");
