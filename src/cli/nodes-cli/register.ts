    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["wildvine nodes status", "List known nodes with live status."],
          ["wildvine nodes pairing pending", "Show pending node pairing requests."],
          [
            'wildvine nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["wildvine nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.wildvine.com/cli/nodes")}\n`,
    );
