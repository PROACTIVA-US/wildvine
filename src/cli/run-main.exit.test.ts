...
vi.mock("../infra/path-env.js", () => ({
  ensureWildvineCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "wildvine", "status"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    await runCli(["node", "wildvine", "--help"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "wildvine", "--help"]);
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "wildvine", "--container", "demo", "status"]);
