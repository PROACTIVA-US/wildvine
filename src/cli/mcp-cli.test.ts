...
const serveWildvineChannelMcp = mocks.serveWildvineChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveWildvineChannelMcp: mocks.serveWildvineChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("wildvine-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("wildvine-cli-mcp-home-", async () => {
      ...
      expect(serveWildvineChannelMcp).toHaveBeenCalledWith({
