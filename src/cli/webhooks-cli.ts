...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.wildvine.bot/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Wildvine hooks")
    ...
    .option("--hook-url <url>", "Wildvine hook URL")
    .option("--hook-token <token>", "Wildvine hook token")
