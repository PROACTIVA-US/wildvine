...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.WILDVINE_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.WILDVINE_DISABLE_LAZY_SUBCOMMANDS);
};

export const loadValidatedConfigForPluginRegistration =
  async (): Promise<WildvineConfig | null> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Wildvine docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  },
  {
    ...
    hasSubcommands: true,
    register: async (program) => {
      const mod = await import("../vinebot-cli.js");
