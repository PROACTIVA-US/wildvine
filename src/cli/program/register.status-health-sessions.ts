...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["wildvine status", "Show channel health + session summary."],
          ["wildvine status --all", "Full diagnosis (read-only)."],
          ["wildvine status --json", "Machine-readable output."],
          ["wildvine status --usage", "Show model provider usage/quota snapshots."],
          [
            "wildvine status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["wildvine status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.wildvine.com/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.wildvine.com/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["wildvine sessions", "List all sessions."],
          ["wildvine sessions --agent work", "List sessions for one agent."],
          ["wildvine sessions --all-agents", "Aggregate sessions across agents."],
          ["wildvine sessions --active 120", "Only last 2 hours."],
          ["wildvine sessions --json", "Machine-readable output."],
          ["wildvine sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.wildvine.com/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["wildvine sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "wildvine sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["wildvine sessions cleanup --enforce", "Apply maintenance now."],
          ["wildvine sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["wildvine sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "wildvine sessions cleanup --enforce --store ./tmp/sessions.json",
