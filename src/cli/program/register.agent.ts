...
${formatHelpExamples([
  ['wildvine agent --to +15555550123 --message "status update"', "Start a new session."],
  ['wildvine agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'wildvine agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'wildvine agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['wildvine agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'wildvine agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.wildvine.com/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.wildvine.com/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['wildvine agents set-identity --agent main --name "Wildvine" --emoji "🦞"', "Set name + emoji."],
  ["wildvine agents set-identity --agent main --avatar avatars/wildvine.png", "Set avatar path."],
  [
    "wildvine agents set-identity --workspace ~/.wildvine/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "wildvine agents set-identity --identity-file ~/.wildvine/workspace/IDENTITY.md --agent main",
