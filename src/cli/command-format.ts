...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+wildvine\b|^wildvine\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+wildvine\b.*(?:^|\s)update(?:\s|$)|^wildvine\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const container = env.WILDVINE_CONTAINER_HINT?.trim();
  const profile = normalizeProfileName(env.WILDVINE_PROFILE);
