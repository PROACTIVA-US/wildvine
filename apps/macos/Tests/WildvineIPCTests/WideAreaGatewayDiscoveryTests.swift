...
import Foundation
import Testing
@testable import WildvineDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("WILDVINE_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("WILDVINE_WIDE_AREA_DOMAIN", "wildvine.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("WILDVINE_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("WILDVINE_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._wildvine-gw._tcp.wildvine.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.wildvine.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Wildvine)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/wildvine/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Wildvine)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/wildvine/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._wildvine-gw._tcp.wildvine.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
