...
@Suite(.serialized)
struct WildvineConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("wildvine-config-\(UUID().uuidString)")
            .appendingPathComponent("wildvine.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["WILDVINE_CONFIG_PATH": override]) {
            #expect(WildvineConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["WILDVINE_CONFIG_PATH": override]) {
            ...
            #expect(WildvineConfigFile.remoteGatewayPort() == 19999)
            #expect(WildvineConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(WildvineConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(WildvineConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["WILDVINE_CONFIG_PATH": override]) {
            ...
            WildvineConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = WildvineConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["WILDVINE_CONFIG_PATH": override]) {
            ...
            WildvineConfigFile.clearRemoteGatewayUrl()
            let root = WildvineConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("wildvine-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "WILDVINE_CONFIG_PATH": nil,
            "WILDVINE_STATE_DIR": dir,
        ]) {
            #expect(WildvineConfigFile.stateDirURL().path == dir)
            #expect(WildvineConfigFile.url().path == "\(dir)/wildvine.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("wildvine-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("wildvine.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "WILDVINE_STATE_DIR": stateDir.path,
            "WILDVINE_CONFIG_PATH": configPath.path,
        ]) {
            WildvineConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-wildvine-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = WildvineConfigFile.loadDict()
