...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["wildvine"]
        let text = "hey wildvine"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey wildvine do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["wildvine"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "wildvine", start: 0.2, duration: 0.1, range: transcript.range(of: "wildvine")),
