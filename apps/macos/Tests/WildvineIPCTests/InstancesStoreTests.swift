...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: WildvineProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: WildvineProtocol.AnyCodable] = [
            "presence": .init([WildvineProtocol.AnyCodable(entry)]),
        ]
        let payload = WildvineProtocol.AnyCodable(payloadMap)
