...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["WILDVINE_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/wildvine")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("wildvine-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("wildvine-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/wildvine/ (rolling or stdout), or nil if none exist.
