...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: WildvineOAuthStore.AnthropicOAuthStatus = WildvineOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: WildvineOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = WildvineOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = WildvineOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try WildvineOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Wildvine can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: WildvineOAuthStore.AnthropicOAuthStatus,
