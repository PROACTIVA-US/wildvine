...
enum WildvineLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = WildvineOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = WildvineFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        WildvineLogging.bootstrapIfNeeded()
        let label = WildvineLogging.makeLabel(subsystem: subsystem, category: category)
...
struct WildvineOSLogHandler: AppLogLevelBackedHandler {
...
struct WildvineFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = WildvineLogging.parseLabel(self.label)
