...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "com.wildvine", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> WildvineSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: WildvineSessionPreviewEntry,
    ...
    private static func cache(payload: WildvineSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: WildvineChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [WildvineKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [WildvineKit.AnyCodable]) -> [WildvineChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(WildvineChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: WildvineChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: WildvineChatMessage) -> Bool {
    ...
    private static func toolNames(for message: WildvineChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: WildvineChatMessage) -> String? {
