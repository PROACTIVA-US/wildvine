...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: WildvineProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "com.wildvine", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: WildvineKit.AnyCodable]()) {
                $0[$1.key] = WildvineKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: WildvineProtocol.AnyCodable?) -> [String: WildvineProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: WildvineProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: WildvineKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: WildvineProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("wildvine.control.heartbeat")
    static let controlAgentEvent = Notification.Name("wildvine.control.agent")
}
