...
enum ExecApprovalsPromptPresenter {
    private static let logger = Logger(subsystem: "com.wildvine", category: "exec-approvals.prompt")

    /// Commands that modify system settings and require Touch ID confirmation
    private static let sensitiveCommands: Set<String> = [
        "tccutil",           // Privacy/TCC permissions
        "defaults",          // System/app preferences
        "systemsetup",       // System configuration
        "spctl",             // Security policy (Gatekeeper)
        "csrutil",           // System Integrity Protection
        "nvram",             // NVRAM settings
        "pmset",             // Power management
        "networksetup",      // Network configuration
        "dscl",              // Directory services (users/groups)
    ...
    static func prompt(_ request: ExecApprovalPromptRequest) -> ExecApprovalDecision {
        ...
        let isSensitive = self.isSensitiveCommand(request.command)

        let alert = NSAlert()
        alert.alertStyle = isSensitive ? .critical : .warning
        alert.messageText = isSensitive ? "⚠️ System Settings Command" : "Allow this command?"
        alert.informativeText = isSensitive
            ? "This command can modify system settings. Touch ID required to approve."
        ...
        let requestedDecision: ExecApprovalDecision? = switch alertResult {
        case .alertFirstButtonReturn: .allowOnce
        case .alertSecondButtonReturn: .allowAlways
        default: nil
        }

        guard let decision = requestedDecision else {
            return .deny
        }

        // Only require Touch ID for sensitive system commands
        if isSensitive {
            if self.authenticateWithBiometrics(reason: "Approve system settings change") {
                return decision
            } else {
                self.logger.warning("biometric auth failed for sensitive command")
                return .deny
            }
        }
    ...
    /// Check if command involves sensitive system modifications
    private static func isSensitiveCommand(_ command: String) -> Bool {
        let trimmed = command.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        // Check if any sensitive command appears at the start or after common prefixes
        for cmd in self.sensitiveCommands {
            // Direct command: "tccutil reset ..."
            if trimmed.hasPrefix(cmd + " ") || trimmed == cmd { return true }
            // With path: "/usr/bin/tccutil ..."
            if trimmed.contains("/\(cmd) ") || trimmed.hasSuffix("/\(cmd)") { return true }
            // After sudo: "sudo tccutil ..."
            if trimmed.contains("sudo \(cmd)") || trimmed.contains("sudo /") && trimmed.contains("/\(cmd)") { return true }
        }
        return false
    }

    /// Authenticate using Touch ID or password fallback.
    ...
    private static func authenticateWithBiometrics(reason: String) -> Bool {
        ...
        guard context.canEvaluatePolicy(.deviceOwnerAuthentication, error: &error) else {
            self.logger.warning("biometric not available: \(error?.localizedDescription ?? "unknown", privacy: .public)")
            // Fall back to allowing if biometrics unavailable (e.g., no Touch ID hardware)
            return true
        }
        ...
        context.evaluatePolicy(.deviceOwnerAuthentication, localizedReason: reason) { result, authError in
            if result {
                ...
                self.logger.warning("biometric auth error: \(authError.localizedDescription, privacy: .public)")
            }
            semaphore.signal()
        }
...
private final class ExecApprovalsSocketServer: @unchecked Sendable {
    private let logger = Logger(subsystem: "com.wildvine", category: "exec-approvals.socket")
