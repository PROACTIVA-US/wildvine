...
enum DebugActions {
    private static let verboseDefaultsKey = "wildvine.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "wildvine.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = WildvinePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Wildvine", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = WildvinePaths.configURL
        guard
