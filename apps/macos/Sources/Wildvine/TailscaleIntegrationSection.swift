...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.wildvine/wildvine.json. Prefer WILDVINE_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.wildvine/wildvine.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.wildvine/wildvine.json. Restart the gateway to apply."
        }
