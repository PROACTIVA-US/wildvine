...
private let webChatSwiftLogger = Logger(subsystem: "ai.wildvine", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "wildvine.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: WildvineChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> WildvineChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(WildvineChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            WildvineChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return WildvineChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    private static func mapModelChoice(_ model: WildvineProtocol.ModelChoice) -> WildvineChatModelChoice {
        WildvineChatModelChoice(
            modelID: model.id,
            name: model.name,
