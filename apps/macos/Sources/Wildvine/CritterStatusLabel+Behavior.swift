...
extension CritterStatusLabel {
    ...
    private func tick(_ now: Date) {
        ...
        // More frequent animations for gentle vine motion
        if now >= self.nextBlink {
            self.blink()
            self.nextBlink = now.addingTimeInterval(Double.random(in: 2.0...5.0))
        }

        if now >= self.nextWiggle {
            self.wiggle()
            self.nextWiggle = now.addingTimeInterval(Double.random(in: 3.0...7.0))
        }

        if now >= self.nextLegWiggle {
            self.wiggleLegs()
            self.nextLegWiggle = now.addingTimeInterval(Double.random(in: 2.5...6.0))
        }

        if now >= self.nextEarWiggle {
            self.wiggleEars()
            self.nextEarWiggle = now.addingTimeInterval(Double.random(in: 2.0...5.0))
        }
    ...
    private var iconImage: Image {
        let badge: VineIconRenderer.Badge? = if let prominence = self.iconState.badgeProminence, !self.isPaused {
            VineIconRenderer.Badge(
        ...
        if self.isPaused {
            return Image(nsImage: VineIconRenderer.makeIcon(isIdle: true, badge: nil))
        }
        ...
        // Map critter animations to vine animations:
        // - earWiggle -> sway (leaf movement)
        // - legWiggle -> growth pulse when working (triggers color)
        // - blinkAmount + earWiggle -> wind effect (stem curve)
        // When working: growth > 0.1 triggers colorful mode
        let workingAmount = self.isWorkingNow ? max(0.5, self.legWiggle) : self.legWiggle
        return Image(nsImage: VineIconRenderer.makeIcon(
            sway: self.earWiggle + self.blinkAmount * 0.3,
            growth: workingAmount,
            windEffect: self.earWiggle * 0.5 + self.blinkAmount * 0.3,
            isIdle: false,
            badge: badge))
    }
    ...
    private func scheduleRandomTimers(from date: Date) {
        // More frequent animations for the vine - gentle constant motion
        self.nextBlink = date.addingTimeInterval(Double.random(in: 2.0...5.0))
        self.nextWiggle = date.addingTimeInterval(Double.random(in: 3.0...7.0))
        self.nextLegWiggle = date.addingTimeInterval(Double.random(in: 2.5...6.0))
        self.nextEarWiggle = date.addingTimeInterval(Double.random(in: 2.0...5.0))
    }
