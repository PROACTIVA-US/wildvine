...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Wildvine token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: WildvineOAuthStore.AnthropicOAuthStatus = WildvineOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "com.wildvine", category: "anthropic-oauth")
...
enum WildvineOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let wildvineOAuthDirEnv = "WILDVINE_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Wildvine OAuth token file not found"
            case .unreadableFile: "Wildvine OAuth token file not readable"
            case .invalidJSON: "Wildvine OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Wildvine OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Wildvine OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.wildvineOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".wildvine", isDirectory: true)
