...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`com.wildvine.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["WILDVINE_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "wildvine.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "wildvine.nixMode") { return true }
