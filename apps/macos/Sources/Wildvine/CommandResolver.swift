...
enum CommandResolver {
    private static let projectRootDefaultsKey = "wildvine.gatewayProjectRootPath"
    private static let helperName = "wildvine"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let wildvineEntry = root.appendingPathComponent("wildvine.mjs").path
        if FileManager().isReadableFile(atPath: wildvineEntry) { return wildvineEntry }
        let binEntry = root.appendingPathComponent("bin/wildvine.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/wildvine")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let wildvinePaths = self.wildvineManagedPaths(home: home)
        if !wildvinePaths.isEmpty {
            extras.insert(contentsOf: wildvinePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + wildvinePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func wildvineManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".wildvine"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func wildvineExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectWildvineExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("wildvine.mjs").path,
            root.appendingPathComponent("bin/wildvine.js").path,
        ]
    ...
    static func hasAnyWildvineInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.wildvineExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func wildvineNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let wildvinePath = self.projectWildvineExecutable(projectRoot: root) {
            return [wildvinePath, subcommand] + extraArgs
        }
        if let wildvinePath = self.wildvineExecutable(searchPaths: searchPaths) {
            return [wildvinePath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "wildvine", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            wildvine entrypoint missing (looked for dist/index.js or wildvine.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real wildvine CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/wildvine"
        ...
        \(cliSection)
        \(projectSection)
        if command -v wildvine >/dev/null 2>&1; then
          CLI="$(command -v wildvine)"
          wildvine \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/wildvine.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/wildvine.mjs"
            node "$PRJ/wildvine.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/wildvine.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/wildvine.js"
            node "$PRJ/bin/wildvine.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent wildvine"
          pnpm --silent wildvine \(quotedArgs);
        else
          echo "wildvine CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? WildvineConfigFile.loadDict()
