...
final class MotionService: MotionServicing {
    func activities(params: WildvineMotionActivityParams) async throws -> WildvineMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[WildvineMotionActivityEntry], Error>) in
        ...
        return WildvineMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: WildvinePedometerParams) async throws -> WildvinePedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<WildvinePedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = WildvinePedometerPayload(
