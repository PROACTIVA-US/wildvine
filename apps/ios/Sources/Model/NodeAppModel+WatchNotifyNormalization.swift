...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: WildvineWatchNotifyParams) -> WildvineWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [WildvineWatchAction]?,
        kind: String?,
        promptId: String?) -> [WildvineWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> WildvineWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return WildvineWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                WildvineWatchAction(id: "approve", label: "Approve"),
                WildvineWatchAction(id: "decline", label: "Decline", style: "destructive"),
                WildvineWatchAction(id: "open_phone", label: "Open iPhone"),
                WildvineWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            WildvineWatchAction(id: "done", label: "Done"),
            WildvineWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            WildvineWatchAction(id: "open_phone", label: "Open iPhone"),
            WildvineWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: WildvineWatchRisk?,
        priority: WildvineNotificationPriority?) -> WildvineWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: WildvineNotificationPriority?,
        risk: WildvineWatchRisk?) -> WildvineNotificationPriority?
    {
