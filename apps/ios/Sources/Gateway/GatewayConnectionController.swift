...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [WildvineCapability.canvas.rawValue, WildvineCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(WildvineCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(WildvineCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = WildvineLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(WildvineCapability.location.rawValue) }

        caps.append(WildvineCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(WildvineCapability.watch.rawValue)
        }
        caps.append(WildvineCapability.photos.rawValue)
        caps.append(WildvineCapability.contacts.rawValue)
        caps.append(WildvineCapability.calendar.rawValue)
        caps.append(WildvineCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(WildvineCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            WildvineScreenCommand.record.rawValue,
            WildvineSystemCommand.notify.rawValue,
            WildvineChatCommand.push.rawValue,
        ...
        if caps.contains(WildvineCapability.camera.rawValue) {
            commands.append(WildvineCameraCommand.list.rawValue)
            commands.append(WildvineCameraCommand.snap.rawValue)
            commands.append(WildvineCameraCommand.clip.rawValue)
        }
        if caps.contains(WildvineCapability.location.rawValue) {
            commands.append(WildvineLocationCommand.get.rawValue)
        }
        if caps.contains(WildvineCapability.device.rawValue) {
            commands.append(WildvineDeviceCommand.status.rawValue)
            commands.append(WildvineDeviceCommand.info.rawValue)
        }
        if caps.contains(WildvineCapability.watch.rawValue) {
            commands.append(WildvineWatchCommand.status.rawValue)
            commands.append(WildvineWatchCommand.notify.rawValue)
        }
        if caps.contains(WildvineCapability.photos.rawValue) {
            commands.append(WildvinePhotosCommand.latest.rawValue)
        }
        if caps.contains(WildvineCapability.contacts.rawValue) {
            commands.append(WildvineContactsCommand.search.rawValue)
            commands.append(WildvineContactsCommand.add.rawValue)
        }
        if caps.contains(WildvineCapability.calendar.rawValue) {
            commands.append(WildvineCalendarCommand.events.rawValue)
            commands.append(WildvineCalendarCommand.add.rawValue)
        }
        if caps.contains(WildvineCapability.reminders.rawValue) {
            commands.append(WildvineRemindersCommand.list.rawValue)
            commands.append(WildvineRemindersCommand.add.rawValue)
        }
        if caps.contains(WildvineCapability.motion.rawValue) {
            commands.append(WildvineMotionCommand.activity.rawValue)
            commands.append(WildvineMotionCommand.pedometer.rawValue)
        }
