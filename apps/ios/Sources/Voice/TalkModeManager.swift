...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__WILDVINE_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<WildvineTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.wildvine", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = WildvineTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> WildvineTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return WildvineTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> WildvineTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> WildvineTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return WildvineTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> WildvineTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = WildvineTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: WildvineTalkPTTStopPayload) {
