...
final class ContactsService: ContactsServicing {
    ...
    func search(params: WildvineContactsSearchParams) async throws -> WildvineContactsSearchPayload {
        ...
        return WildvineContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: WildvineContactsAddParams) async throws -> WildvineContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return WildvineContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return WildvineContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> WildvineContactPayload {
        WildvineContactPayload(
