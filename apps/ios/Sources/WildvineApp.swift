...
@MainActor
final class WildvineAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.wildvine.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.wildvine.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.wildvine.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "wildvine.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "wildvine.watch.promptId"
    static let sessionKeyKey = "wildvine.watch.sessionKey"
    static let actionPrimaryIDKey = "wildvine.watch.action.primary.id"
    static let actionPrimaryLabelKey = "wildvine.watch.action.primary.label"
    static let actionSecondaryIDKey = "wildvine.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "wildvine.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "wildvine.watch.action.primary"
    static let actionSecondaryIdentifier = "wildvine.watch.action.secondary"
    static let actionIdentifierPrefix = "wildvine.watch.action."
    static let actionIDKeyPrefix = "wildvine.watch.action.id."
    static let actionLabelKeyPrefix = "wildvine.watch.action.label."
    static let categoryPrefix = "wildvine.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: WildvineWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> WildvineWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return WildvineWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Wildvine" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [WildvineWatchAction]) -> [UNNotificationAction] {
...
@main
struct WildvineApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(WildvineAppDelegate.self) private var appDelegate
...
extension WildvineApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Wildvine: installing uncaught exception handler")
