...
final class RemindersService: RemindersServicing {
    func list(params: WildvineRemindersListParams) async throws -> WildvineRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[WildvineReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return WildvineReminderPayload(
        ...
        return WildvineRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: WildvineRemindersAddParams) async throws -> WildvineRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = WildvineReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return WildvineRemindersAddPayload(reminder: payload)
    }
