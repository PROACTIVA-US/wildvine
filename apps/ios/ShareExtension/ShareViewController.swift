...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.wildvine.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Wildvine", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Wildvine gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Wildvine.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "WildvineShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Wildvine is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Wildvine Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("wildvine-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
