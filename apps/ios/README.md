...
This is an **alpha** iOS app that connects to an Wildvine Gateway as a `role: node`.
...
3. On iOS: Wildvine → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Wildvine.xcodeproj -scheme Wildvine -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/WildvineKit` contains the shared transport/types used by the iOS app.
