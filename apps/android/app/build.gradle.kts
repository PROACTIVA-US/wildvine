...
val androidStoreFile = providers.gradleProperty("WILDVINE_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("WILDVINE_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("WILDVINE_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("WILDVINE_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set WILDVINE_ANDROID_STORE_FILE, " +
            "WILDVINE_ANDROID_STORE_PASSWORD, WILDVINE_ANDROID_KEY_ALIAS, and " +
            "WILDVINE_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    namespace = "com.wildvine.android"
    compileSdk = 36
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/WildvineKit/Sources/WildvineKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "com.wildvine.android"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "WILDVINE_ENABLE_SMS", "false")
            buildConfigField("boolean", "WILDVINE_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "WILDVINE_ENABLE_SMS", "true")
            buildConfigField("boolean", "WILDVINE_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "wildvine-$versionName-$buildType.apk"
                    } else {
                        "wildvine-$versionName-$flavorName-$buildType.apk"
                    }
