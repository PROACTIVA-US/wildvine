...
private const val NOTIFICATION_CHANNEL_BASE_ID = "wildvine.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Wildvine Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Wildvine Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Wildvine Active")
      }
