...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Wildvine Android gateway connection failure.

    Please:
    - pick one route only: same machine, same LAN, Tailscale, or public URL
    - classify this as pairing/auth, TLS trust, wrong advertised route, wrong address/port, or gateway down
    - quote the exact app status/error below
    - tell me whether `wildvine devices list` should show a pending pairing request
    - if more signal is needed, ask for `wildvine qr --json`, `wildvine devices list`, and `wildvine nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Wildvine gateway diagnostics", report))
