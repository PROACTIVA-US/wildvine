...
import com.wildvine.android.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      WildvineCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      WildvineCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      WildvineCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      WildvineCanvasA2UICommand.Push.rawValue, WildvineCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      WildvineCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      WildvineCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      WildvineCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      WildvineLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      WildvineDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      WildvineDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      WildvineDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      WildvineDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      WildvineNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      WildvineNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      WildvineSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      com.wildvine.android.protocol.WildvinePhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      WildvineContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      WildvineContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      WildvineCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      WildvineCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      WildvineMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      WildvineMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      WildvineSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      WildvineSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      WildvineCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
