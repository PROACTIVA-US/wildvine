...
@MainActor
public struct WildvineChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: WildvineChatViewModel
    ...
    public init(
        viewModel: WildvineChatViewModel,
    ...
    private var visibleMessages: [WildvineChatMessage] {
        let base: [WildvineChatMessage]
    ...
    private func mergeToolResults(in messages: [WildvineChatMessage]) -> [WildvineChatMessage] {
        ...
        for message in messages {
            ...
            let merged = WildvineChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: WildvineChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: WildvineChatMessage) -> Bool {
    ...
    private func primaryText(in message: WildvineChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: WildvineChatMessage) -> Bool {
    ...
    private func toolCalls(in message: WildvineChatMessage) -> [WildvineChatMessageContent] {
    ...
    private func inlineToolResults(in message: WildvineChatMessage) -> [WildvineChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: WildvineChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: WildvineChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(WildvineChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
