...
private let chatUILogger = Logger(subsystem: "ai.wildvine", category: "WildvineChatUI")
...
public final class WildvineChatViewModel {
    ...
    public private(set) var modelChoices: [WildvineChatModelChoice] = []
    ...
    private var sessionDefaults: WildvineChatSessionsDefaults?
    ...
    public init(
        sessionKey: String,
        transport: any WildvineChatTransport,
    ...
    private static func stripInboundMetadata(from message: WildvineChatMessage) -> WildvineChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> WildvineChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return WildvineChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: WildvineChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: WildvineChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: WildvineChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [WildvineChatMessage],
        incoming: [WildvineChatMessage]) -> [WildvineChatMessage]
    {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = WildvineChatSessionEntry(
                key: current.key,
                kind: current.kind,
