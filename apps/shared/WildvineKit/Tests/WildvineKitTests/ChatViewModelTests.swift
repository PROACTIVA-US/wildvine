...
import Foundation
import Testing
@testable import WildvineChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> WildvineChatHistoryPayload
{
    WildvineChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> WildvineChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> WildvineChatSessionEntry
{
    WildvineChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> WildvineChatModelChoice {
    WildvineChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [WildvineChatHistoryPayload],
    sessionsResponses: [WildvineChatSessionsListResponse] = [],
    modelResponses: [[WildvineChatModelChoice]] = [],
    ...
    -> (TestChatTransport, WildvineChatViewModel)
{
...
private func sendUserMessage(_ vm: WildvineChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, WildvineChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [WildvineChatHistoryPayload]
    private let sessionsResponses: [WildvineChatSessionsListResponse]
    private let modelResponses: [[WildvineChatModelChoice]]
    ...
    private let stream: AsyncStream<WildvineChatTransportEvent>
    private let continuation: AsyncStream<WildvineChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<WildvineChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> WildvineChatHistoryPayload {
        ...
        return self.historyResponses.last ?? WildvineChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [WildvineChatAttachmentPayload]) async throws -> WildvineChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return WildvineChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> WildvineChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? WildvineChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: WildvineChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = WildvineChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = WildvineChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: WildvineChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = WildvineChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: WildvineChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(WildvineChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == WildvineChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = WildvineChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: WildvineChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = WildvineChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = WildvineChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = WildvineChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { WildvineChatViewModel(sessionKey: "main", transport: transport) }
