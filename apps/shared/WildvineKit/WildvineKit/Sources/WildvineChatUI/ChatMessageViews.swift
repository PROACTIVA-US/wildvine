...
struct ChatMessageBubble: View {
    let message: WildvineChatMessage
    let style: WildvineChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? WildvineChatTheme.userText : WildvineChatTheme.assistantText
    ...
    private var inlineAttachments: [WildvineChatMessageContent] {
    ...
    private var toolCalls: [WildvineChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [WildvineChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? WildvineChatTheme.userBubble
        }
        if self.style == .onboarding {
            return WildvineChatTheme.onboardingAssistantBubble
        }
        return WildvineChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return WildvineChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: WildvineChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? WildvineChatTheme.userText : WildvineChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: WildvineChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(WildvineChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [WildvineChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: WildvineChatTheme.assistantText)
            }
        }
    }
}
