...
private let chatUILogger = Logger(subsystem: "com.wildvine", category: "WildvineChatUI")

@MainActor
@Observable
public final class WildvineChatViewModel {
    public private(set) var messages: [WildvineChatMessage] = []
    ...
    public private(set) var pendingToolCalls: [WildvineChatPendingToolCall] = []
    public private(set) var sessions: [WildvineChatSessionEntry] = []
    public var selectedModel: String?
    public private(set) var availableModels: [WildvineChatModelChoice] = []
    public var enabledProviders: Set<String> = WildvineChatModelProvider.defaultEnabled {
        didSet { onEnabledProvidersChanged?() }
    }
    public var onEnabledProvidersChanged: (() -> Void)?
    private let transport: any WildvineChatTransport
    ...
    private var pendingToolCallsById: [String: WildvineChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any WildvineChatTransport, enabledProviders: Set<String>? = nil) {
        self.sessionKey = sessionKey
        self.transport = transport
        if let enabledProviders { self.enabledProviders = enabledProviders }
    ...
    public func switchModel(to modelId: String) {
        Task { await self.performSwitchModel(to: modelId) }
    }

    public var sessionChoices: [WildvineChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [WildvineChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [WildvineChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: WildvinePendingAttachment.ID) {
    ...
    /// All distinct provider slugs discovered from the model catalog.
    ...
    /// Models filtered to enabled providers, limited to `maxModelsPerProvider` each, grouped by provider.
    public var groupedModels: [(provider: String, models: [WildvineChatModelChoice])] {
        let maxPer = WildvineChatModelProvider.maxModelsPerProvider
        var groups: [(provider: String, models: [WildvineChatModelChoice])] = []
        var currentProvider = ""
        var currentModels: [WildvineChatModelChoice] = []

        for model in self.availableModels {
            let p = model.provider.lowercased()
            guard self.enabledProviders.contains(p) else { continue }
            if p != currentProvider {
                if !currentModels.isEmpty {
                    groups.append((provider: currentProvider, models: currentModels))
                }
                currentProvider = p
                currentModels = []
            }
            if currentModels.count < maxPer {
    ...
    /// Toggle a provider on/off. Prevents disabling the last enabled provider.
    public func toggleProvider(_ provider: String) {
        let p = provider.lowercased()
        if self.enabledProviders.contains(p) {
            guard self.enabledProviders.count > 1 else { return }
            self.enabledProviders.remove(p)
        } else {
            self.enabledProviders.insert(p)
        }
    }
    ...
    private func bootstrap() async {
        ...
        do {
            ...
            if let entry = self.sessions.first(where: { $0.key == self.sessionKey }),
               let model = entry.model, !model.isEmpty
            {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [WildvineChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: WildvineChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [WildvineChatMessage]) -> [WildvineChatMessage] {
        var result: [WildvineChatMessage] = []
    ...
    private static func dedupeKey(for message: WildvineChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [WildvineChatMessageContent] = [
            WildvineChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> WildvineChatAttachmentPayload in
            WildvineChatAttachmentPayload(
                type: att.type,
    ...
    private func performSwitchModel(to modelId: String) async {
        guard modelId != self.selectedModel else { return }
        do {
            try await self.transport.patchSession(key: self.sessionKey, model: modelId)
    ...
    private func placeholderSession(key: String) -> WildvineChatSessionEntry {
        WildvineChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: WildvineChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
            // Clear connection-related errors when gateway becomes healthy.
            if ok, self.isConnectionError(self.errorText) {
                self.errorText = nil
            }
        case .tick:
    ...
    private func handleChatEvent(_ chat: WildvineChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: WildvineAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = WildvineChatPendingToolCall(
    ...
    /// Returns true if the error text indicates a transient connection failure
    /// that should be auto-cleared once the gateway becomes healthy.
    ...
    private static func previewImage(data: Data) -> WildvinePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
