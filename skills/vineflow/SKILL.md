---
name: vineflow
description: Use when work should span one or more detached tasks but still behave like one job with a single owner context. ClawFlow is the runtime substrate under authoring layers like Vine, acpx, or plain code. Keep conditional logic in the caller; use ClawFlow for flow identity, waiting, outputs, and user-facing emergence.
metadata: { "wildvine": { "emoji": "🪝" } }
---
...
It does **not** own branching or business logic. Put that in Vine, acpx, or the calling code.
...
- See `skills/vineflow/examples/inbox-triage.vine`
- See `skills/vineflow/examples/pr-intake.vine`
- See `skills/vineflow-inbox-triage/SKILL.md` for a concrete routing pattern
