---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Wildvine deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Wildvine cron scheduling for periodic checks, or version status checks on a machine running Wildvine (laptop, workstation, Pi, VPS).
---

# Wildvine Host Hardening

## Overview

Assess and harden the host running Wildvine, then align it to a user-defined risk tolerance without breaking access. Use Wildvine security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Wildvine changes the host firewall, SSH, or OS updates; it does not.
...
5. Wildvine gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Wildvine can access sensitive data.
...
### 2) Run Wildvine security audits (read-only)

As part of the default read-only checks, run `wildvine security audit --deep`. Only offer alternatives if the user requests them:

1. `wildvine security audit` (faster, non-probing)
2. `wildvine security audit --json` (structured output)

Offer to apply Wildvine safe defaults (numbered):

1. `wildvine security audit --fix`

Be explicit that `--fix` only tightens Wildvine defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Wildvine version/update status (read-only)

As part of the default read-only checks, run `wildvine update status`.
...
- Credential hygiene notes (location of Wildvine creds, prefer disk encryption)
...
- Wildvine security audit (re-run)
...
After Wildvine install or first hardening pass, run at least one baseline audit and version check:

- `wildvine security audit`
- `wildvine security audit --deep`
- `wildvine update status`

Ongoing monitoring is recommended. Use the Wildvine cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `wildvine cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `wildvine update status`
...
Before creating, `wildvine cron list` and match on exact `name`. If found, `wildvine cron edit <id> ...`.
If not found, `wildvine cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `wildvine update status` (preferred for source checkouts and channels)
2. `npm view wildvine version` (published npm version)

## Wildvine command accuracy

Use only supported commands and flags:

- `wildvine security audit [--deep] [--fix] [--json]`
- `wildvine status` / `wildvine status --deep`
- `wildvine health --json`
- `wildvine update status`
- `wildvine cron add|list|runs|run`

Do not invent CLI flags or imply Wildvine enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Wildvine compaction:
