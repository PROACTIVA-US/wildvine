...
metadata:
  { "wildvine": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${WILDVINE_TMUX_SOCKET_DIR:-${WILDVINE_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/wildvine-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/wildvine.sock"
SESSION=wildvine-python
...
- Use `WILDVINE_TMUX_SOCKET_DIR` (legacy `WILDVINE_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$WILDVINE_TMUX_SOCKET_DIR/wildvine.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `WILDVINE_TMUX_SOCKET_DIR`).
