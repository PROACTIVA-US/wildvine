...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by Wildvine. Pass it as a Bearer token in all API calls:
...
If still empty, check `/data/.clawdbot/wildvine.json`:

```
cat /data/.clawdbot/wildvine.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the Wildvine dashboard or in the active Wildvine config path (`$WILDVINE_CONFIG_PATH`, default `~/.wildvine/wildvine.json`) under `skills.entries.gh-issues`."
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., wildvine-wildvine)
  ```
...
CONFIG_PATH="${WILDVINE_CONFIG_PATH:-${WILDVINE_STATE_DIR:-$HOME/.wildvine}/wildvine.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/wildvine.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/wildvine.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${WILDVINE_CONFIG_PATH:-${WILDVINE_STATE_DIR:-$HOME/.wildvine}/wildvine.json}"
