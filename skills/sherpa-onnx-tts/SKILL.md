...
1. Download the runtime for your OS (extracts into `$WILDVINE_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.wildvine/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$WILDVINE_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.wildvine/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Wildvine config file (`$WILDVINE_CONFIG_PATH`, default `~/.wildvine/wildvine.json`):
