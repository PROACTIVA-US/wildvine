...
**⚠️ CRITICAL: Never review PRs in Wildvine's own project folder!**
...
8. **NEVER start Codex inside your Wildvine state directory** (`$WILDVINE_STATE_DIR`, default `~/.wildvine`) - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/wildvine/** - that's the LIVE Wildvine instance!
...
For long-running background tasks, append a wake trigger to your prompt so Wildvine gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
wildvine system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: wildvine system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
...
- **Sass works:** Codex responds well to playful prompts. Asked it to write a haiku about being second fiddle to a space vine, got: _"Second chair, I code / Space vine sets the tempo / Keys glow, I follow"_ 🦞
