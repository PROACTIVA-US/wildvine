...
Goal: find the one real route from node -> gateway, verify Wildvine is advertising that route, then fix pairing/auth.
...
- whether `wildvine devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `wildvine qr --json`. It uses the same setup-code payload Android scans.

```bash
wildvine config get gateway.mode
wildvine config get gateway.bind
wildvine config get gateway.tailscale.mode
wildvine config get gateway.remote.url
wildvine config get gateway.auth.mode
wildvine config get gateway.auth.allowTailscale
wildvine config get plugins.entries.device-pair.config.publicUrl
wildvine qr --json
wildvine devices list
wildvine nodes status
```

If this Wildvine instance is pointed at a remote gateway, also run:

```bash
wildvine qr --remote --json
```
...
`wildvine qr --json` success means:
...
If `wildvine qr --json` says `Gateway is only bound to loopback`:
...
```bash
wildvine devices list
wildvine devices approve --latest
```
...
- `wildvine devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run wildvine qr again, rescan, then approve the pending device pairing.`
