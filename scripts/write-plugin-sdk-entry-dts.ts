...
function rewriteFacadeTypeMapSpecifier(specifier: string): string {
  if (!specifier.startsWith("@wildvine/")) {
    return specifier;
  }
  return `${GENERATED_FACADE_TYPE_MAP_DIST_PREFIX}${specifier.slice("@wildvine/".length)}`;
}
...
function rewriteGeneratedFacadeTypeMapDts(): void {
  ...
  const rewritten = source.replace(/@wildvine\/([a-z0-9-]+\/[^")\s]+)/g, (_match, suffix: string) =>
    rewriteFacadeTypeMapSpecifier(`@wildvine/${suffix}`),
  );
  if (rewritten !== source) {
