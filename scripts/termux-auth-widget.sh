...
# Wildvine Auth Widget for Termux
...
SERVER="${WILDVINE_SERVER:-${WILDVINE_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Wildvine auth..."

STATUS=$(ssh "$SERVER" '$HOME/wildvine/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/wildvine/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|WILDVINE_EXPIRING|WILDVINE_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/wildvine/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|WILDVINE_EXPIRED|WILDVINE_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Wildvine won't work until re-auth"
                ;;
        esac
        ;;
