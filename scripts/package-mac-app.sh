...
# Build and bundle Wildvine into a minimal .app we can open.
# Outputs to dist/Wildvine.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Wildvine.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Wildvine"
BUNDLE_ID="${BUNDLE_ID:-com.wildvine.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/wildvine/wildvine/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Wildvine/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :WildvineBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :WildvineGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Wildvine"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Wildvine"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Wildvine"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Wildvine" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Wildvine/Resources/Wildvine.icns" "$APP_ROOT/Contents/Resources/Wildvine.icns"

echo "🖼  Copying menubar icon"
MENUBAR_ICON_SRC="$ROOT_DIR/apps/macos/Sources/Wildvine/Resources/menubar-icon.png"
if [ -f "$MENUBAR_ICON_SRC" ]; then
  cp "$MENUBAR_ICON_SRC" "$APP_ROOT/Contents/Resources/menubar-icon.png"
else
  echo "WARN: menubar-icon.png not found at $MENUBAR_ICON_SRC (continuing)" >&2
fi

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Wildvine/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "⏹  Stopping any running Wildvine"
killall -q Wildvine 2>/dev/null || true
