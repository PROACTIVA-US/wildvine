...
run_protocol_ci_mirror() {
  local targets=(
    ...
    "apps/macos/Sources/WildvineProtocol/GatewayModels.swift"
    "apps/shared/WildvineKit/Sources/WildvineProtocol/GatewayModels.swift"
  )
  local before after
...
has_native_swift_changes() {
  if git rev-parse --verify --quiet origin/main >/dev/null; then
    if git diff --name-only --relative origin/main...HEAD -- apps/macos apps/ios apps/shared/WildvineKit | rg -q .; then
      return 0
    fi
  fi

  if git rev-parse --verify --quiet HEAD^ >/dev/null; then
    git diff --name-only --relative HEAD^..HEAD -- apps/macos apps/ios apps/shared/WildvineKit | rg -q .
    return $?
  fi

  git show --name-only --relative --pretty='' HEAD -- apps/macos apps/ios apps/shared/WildvineKit | rg -q .
}
...
run_linux_ci_mirror() {
  ...
  log_step "WILDVINE_TEST_WORKERS=${WILDVINE_TEST_WORKERS:-1} WILDVINE_TEST_MAX_OLD_SPACE_SIZE_MB=${WILDVINE_TEST_MAX_OLD_SPACE_SIZE_MB:-6144} pnpm test"
  WILDVINE_TEST_WORKERS="${WILDVINE_TEST_WORKERS:-1}" \
  WILDVINE_TEST_MAX_OLD_SPACE_SIZE_MB="${WILDVINE_TEST_MAX_OLD_SPACE_SIZE_MB:-6144}" \
    pnpm test
}

run_macos_ci_mirror() {
  if [[ "${WILDVINE_PREPUSH_SKIP_MACOS:-0}" == "1" ]]; then
    log_step "Skipping macOS mirror because WILDVINE_PREPUSH_SKIP_MACOS=1"
    return 0
  fi
