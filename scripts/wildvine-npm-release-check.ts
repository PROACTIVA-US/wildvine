...
const EXPECTED_REPOSITORY_URL = "https://github.com/PROACTIVA-US/wildvine";
const MAX_CALVER_DISTANCE_DAYS = 2;
const REQUIRED_PACKED_PATHS = ["dist/control-ui/index.html"];
const CONTROL_UI_ASSET_PREFIX = "dist/control-ui/assets/";
const NPM_PACK_MAX_BUFFER_BYTES = 64 * 1024 * 1024;
const skipPackValidationEnv = "WILDVINE_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "wildvine") {
    errors.push(`package.json name must be "wildvine"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.wildvine !== "wildvine.mjs") {
    errors.push(
      `package.json bin.wildvine must be "wildvine.mjs"; found "${pkg.bin?.wildvine ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`wildvine-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `wildvine-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
