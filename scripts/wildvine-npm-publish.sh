...
if [[ "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/wildvine-npm-publish.sh --publish [package.tgz]" >&2
  exit 2
fi
...
current_beta_version="$(npm view wildvine dist-tags.beta 2>/dev/null || true)"
...
if [[ -n "${mirror_dist_tags_csv}" ]]; then
  ...
  for dist_tag in "${mirror_dist_tags[@]}"; do
    [[ -n "${dist_tag}" ]] || continue
    echo "Mirroring wildvine@${package_version} onto dist-tag ${dist_tag}"
    NPM_CONFIG_USERCONFIG="${mirror_userconfig}" \
      npm dist-tag add "wildvine@${package_version}" "${dist_tag}"
  done
fi
