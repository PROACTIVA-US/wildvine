...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "wildvine.plugin.json");
    ...
    if (packageJson.wildvine && "extensions" in packageJson.wildvine) {
      packageJson.wildvine = {
        ...packageJson.wildvine,
        extensions: rewritePackageExtensions(packageJson.wildvine.extensions),
        ...(typeof packageJson.wildvine.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.wildvine.setupEntry) }
          : {}),
      };
    }
