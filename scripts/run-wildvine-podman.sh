...
#   wildvine --container wildvine dashboard --no-open
#   wildvine --container wildvine channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      WILDVINE_GATEWAY_TOKEN|WILDVINE_PODMAN_CONTAINER|WILDVINE_PODMAN_IMAGE|WILDVINE_IMAGE|WILDVINE_PODMAN_PULL|WILDVINE_PODMAN_GATEWAY_HOST_PORT|WILDVINE_GATEWAY_PORT|WILDVINE_PODMAN_BRIDGE_HOST_PORT|WILDVINE_BRIDGE_PORT|WILDVINE_GATEWAY_BIND|WILDVINE_PODMAN_USERNS|WILDVINE_BIND_MOUNT_OPTIONS|WILDVINE_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-wildvine-podman.sh as your normal user so Podman stays rootless."
fi
...
PUBLISH_HOST="${WILDVINE_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$WILDVINE_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  WILDVINE_CONTAINER="" WILDVINE_CONFIG_DIR="$config_dir" \
    wildvine config get gateway.bind 2>/dev/null || true
}
...
# WILDVINE_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${WILDVINE_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v wildvine >/dev/null 2>&1; then
    echo "Warning: wildvine not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    WILDVINE_CONTAINER="" WILDVINE_CONFIG_DIR="$config_dir" \
      wildvine config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    WILDVINE_CONTAINER="" WILDVINE_CONFIG_DIR="$config_dir" \
      wildvine config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  WILDVINE_CONTAINER="" WILDVINE_CONFIG_DIR="$config_dir" \
    wildvine config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
SELINUX_MOUNT_OPTS=""
if [[ -z "${WILDVINE_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${WILDVINE_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$WILDVINE_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.wildvine/.npm \
    -e WILDVINE_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.wildvine:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.wildvine/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$WILDVINE_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME wildvine dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME wildvine devices approve --latest  # if pairing required"
