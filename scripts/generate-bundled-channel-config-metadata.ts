...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const wildvineMeta =
    ...
    "wildvine" in source.packageJson
      ? (source.packageJson.wildvine as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    wildvineMeta &&
    typeof wildvineMeta.channel === "object" &&
    wildvineMeta.channel &&
    !Array.isArray(wildvineMeta.channel)
      ? (wildvineMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
