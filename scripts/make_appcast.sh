...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Wildvine-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/wildvine/wildvine/releases/download/v${VERSION}/"}
