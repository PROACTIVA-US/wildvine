...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.wildvine) ? packageJson.wildvine : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.wildvine?.channel?.id) || left.name;
    const rightId = trimString(right.wildvine?.channel?.id) || right.name;
