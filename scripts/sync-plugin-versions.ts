...
const WILDVINE_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncWildvineDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.wildvine;
  if (!current || current === "workspace:*" || !WILDVINE_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncMinHostVersion(pkg: PackageJson, targetVersion: string): boolean {
  const installConfig = pkg.wildvine?.install;
  const current = installConfig?.minHostVersion;
  if (!current || !WILDVINE_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core Wildvine release numbers.\n\n`;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncWildvineDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncWildvineDependencyRange(pkg.peerDependencies, targetVersion);
