...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-wildvine-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://wildvine.com/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/wildvine-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-wildvine-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://wildvine.com/install.ps1
...
\$wildvine = Join-Path \$env:APPDATA 'npm\wildvine.cmd'
\$args = $args_literal
if ('${env_name_q}' -ne '') {
  Set-Item -Path ('Env:' + '${env_name_q}') -Value '${env_value_q}'
}
# wildvine.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$wildvine @args 2>&1
if (\$null -ne \$output) {
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "wildvine-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\Wildvine\\deps\" mkdir \"%LOCALAPPDATA%\\Wildvine\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/wildvine-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\wildvine.cmd') --version
EOF
)"
...
install_main_tgz() {
  ...
  run_windows_retry "main tgz install" 2 \
    guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Wildvine\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\wildvine.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_wildvine "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/wildvine-onboard-$PROVIDER.ps1"
...
try {
  \$wildvine = Join-Path \$env:APPDATA 'npm\wildvine.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice ${AUTH_CHOICE} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$wildvine, \$LogPath)
...
run_ref_onboard() {
  ...
  runner_name="wildvine-onboard-$RANDOM-$RANDOM.ps1"
  log_name="wildvine-onboard-$RANDOM-$RANDOM.log"
  done_name="wildvine-onboard-$RANDOM-$RANDOM.done"
...
verify_gateway() {
  guest_run_wildvine "" "" gateway status --deep --require-rpc
}
...
run_gateway_daemon_action() {
  ...
  local runner_name log_name done_name done_status launcher_state
  local poll_rc state_rc log_rc start_seconds poll_deadline startup_checked
  runner_name="wildvine-gateway-$action-$RANDOM-$RANDOM.ps1"
  log_name="wildvine-gateway-$action-$RANDOM-$RANDOM.log"
  done_name="wildvine-gateway-$action-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
try {
  \$wildvine = Join-Path \$env:APPDATA 'npm\wildvine.cmd'
  & \$wildvine gateway $action *>&1 | Tee-Object -FilePath \$log -Append | Out-Null
...
show_gateway_status_compat() {
  if guest_run_wildvine "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_wildvine "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_wildvine "" "" gateway status --deep
}

verify_turn() {
  guest_run_wildvine "" "" models set "$MODEL_ID"
  guest_run_wildvine "$API_KEY_ENV" "$API_KEY_VALUE" \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "wildvine-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "wildvine-main-upgrade.tgz" || return $?
