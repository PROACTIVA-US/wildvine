...
async function main() {
  const stateDir = process.env.WILDVINE_STATE_DIR?.trim() || path.join(os.homedir(), ".wildvine");
  const configPath =
    process.env.WILDVINE_CONFIG_PATH?.trim() || path.join(stateDir, "wildvine.json");
