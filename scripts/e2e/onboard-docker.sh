...
IMAGE_NAME="wildvine-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/wildvine-bin:$PATH"
  mkdir -p /tmp/wildvine-bin
  cat > /tmp/wildvine-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/wildvine-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$WILDVINE_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_wildvine_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/wildvine-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/wildvine-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$WILDVINE_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/wildvine-e2e-$1.XXXXXX"
  }

  set_isolated_wildvine_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export WILDVINE_HOME="$home_dir"
    export WILDVINE_STATE_DIR="$home_dir/.wildvine"
    export WILDVINE_CONFIG_PATH="$WILDVINE_STATE_DIR/wildvine.json"
    mkdir -p "$WILDVINE_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$WILDVINE_STATE_DIR/workspace"
    config_path="$WILDVINE_CONFIG_PATH"
    sessions_dir="$WILDVINE_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_wildvine_env "$home_dir"
	    ...
	    node "$WILDVINE_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$WILDVINE_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$WILDVINE_ENTRY\" configure --section skills" send_skills_flow
