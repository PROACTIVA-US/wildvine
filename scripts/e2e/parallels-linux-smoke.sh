...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/wildvine-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/wildvine-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/wildvine-install.sh
  guest_exec /usr/bin/env WILDVINE_NO_ONBOARD=1 bash /tmp/wildvine-install.sh "${version_args[@]}" --no-onboard
  guest_exec wildvine --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec wildvine --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" wildvine onboard \
    --non-interactive \
    --mode local \
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "wildvine gateway run" >/dev/null 2>&1 || true
rm -f /tmp/wildvine-parallels-linux-gateway.log
setsid sh -lc 'exec env WILDVINE_HOME=/root WILDVINE_STATE_DIR=/root/.wildvine WILDVINE_CONFIG_PATH=/root/.wildvine/wildvine.json ${API_KEY_ENV}=${api_key_value_q} wildvine gateway run --bind loopback --port 18789 --force >/tmp/wildvine-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec wildvine gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec wildvine gateway status --deep --require-rpc
    return
  fi
  guest_exec wildvine gateway status --deep
}

verify_local_turn() {
  guest_exec wildvine models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" wildvine agent \
    --local \
    --agent main \
    --message ping \
    --json
