...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/wildvine-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: wildvine@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/wildvine-main-$CURRENT_HEAD_SHORT.tgz"
  cp "$MAIN_TGZ_DIR/$pkg" "$MAIN_TGZ_PATH"
}

write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/wildvine-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/wildvine warnings do not
...
try {
  $env:PATH = "$env:LOCALAPPDATA\Wildvine\deps\portable-git\cmd;$env:LOCALAPPDATA\Wildvine\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\Wildvine\deps\portable-git\usr\bin;$env:PATH"
  $tgz = Join-Path $env:TEMP 'wildvine-main-update.tgz'
  ...
  $version = Invoke-CaptureLogged 'wildvine --version' { & $wildvine --version }
  if ($version -notmatch [regex]::Escape($HeadShort)) {
    throw "version mismatch: expected substring $HeadShort"
  }
  Invoke-Logged 'wildvine models set' { & $wildvine models set $ModelId }
  ...
  Invoke-Logged 'wildvine gateway restart' { & $wildvine gateway restart }
  Start-Sleep -Seconds 5
  Invoke-Logged 'wildvine gateway status' { & $wildvine gateway status --deep --require-rpc }
  Invoke-CaptureLogged 'wildvine agent' { & $wildvine agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/wildvine-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
matches = [match for match in matches if re.search(r"Wildvine \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="wildvine-update-$RANDOM-$RANDOM.ps1"
  log_name="wildvine-update-$RANDOM-$RANDOM.log"
  done_name="wildvine-update-$RANDOM-$RANDOM.done"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/wildvine-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/wildvine-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/wildvine-main-update.tgz
version="\$(/opt/homebrew/bin/wildvine --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/wildvine models set "$MODEL_ID"
/opt/homebrew/bin/wildvine gateway status --deep --require-rpc
/usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /opt/homebrew/bin/wildvine agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/wildvine-main-update.sh
}
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/wildvine-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/wildvine-main-update.tgz
npm install -g /tmp/wildvine-main-update.tgz --no-fund --no-audit
version="\$(wildvine --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
wildvine models set "$MODEL_ID"
wildvine agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/wildvine-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="wildvine@$LATEST_VERSION"
fi
