...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://wildvine.com/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_WILDVINE_BIN="/opt/homebrew/bin/wildvine"
GUEST_WILDVINE_ENTRY="/opt/homebrew/lib/node_modules/wildvine/wildvine.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/wildvine-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/wildvine-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://wildvine.com/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: wildvine@2026.3.13-beta.1
...
spawn {*}$cmd
send -- "printf '__WILDVINE_READY__\\n'\r"
expect "__WILDVINE_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/wildvine-prl.sh <<'__WILDVINE_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__WILDVINE_SCRIPT__\r"
send -- "/bin/bash /tmp/wildvine-prl.sh; rc=\$?; rm -f /tmp/wildvine-prl.sh; printf '__WILDVINE_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__WILDVINE_RC__:(-?[0-9]+)} {
...
resolve_latest_version() {
  ...
  npm view wildvine version --userconfig "$(mktemp)"
}
...
export WILDVINE_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/wildvine-install.sh
bash /tmp/wildvine-install.sh${version_arg_q}
$GUEST_WILDVINE_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_WILDVINE_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/wildvine-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/wildvine-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/wildvine"; check_path "\$root/wildvine/extensions"; if [ -d "\$root/wildvine/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/wildvine/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
verify_gateway() {
  guest_current_user_cli "$GUEST_WILDVINE_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_WILDVINE_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_WILDVINE_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_WILDVINE_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli "$GUEST_WILDVINE_BIN" models set "$MODEL_ID"
  guest_current_user_cli \
    /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" \
    "$GUEST_WILDVINE_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_WILDVINE_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from wildvine dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/wildvine-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Wildvine Control</title>' /tmp/wildvine-dashboard-smoke.html >/dev/null; then
      if grep -F '<wildvine-app></wildvine-app>' /tmp/wildvine-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>Wildvine Control</title>' /tmp/wildvine-dashboard-smoke.html >/dev/null
grep -F '<wildvine-app></wildvine-app>' /tmp/wildvine-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/wildvine-discord-token <<'__WILDVINE_TOKEN__'
$DISCORD_TOKEN_VALUE
__WILDVINE_TOKEN__
cat >/tmp/wildvine-discord-guilds.json <<'__WILDVINE_GUILDS__'
$guilds_json
__WILDVINE_GUILDS__
token="\$(tr -d '\n' </tmp/wildvine-discord-token)"
guilds_json="\$(cat /tmp/wildvine-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_WILDVINE_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_WILDVINE_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_WILDVINE_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_WILDVINE_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_WILDVINE_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_WILDVINE_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_WILDVINE_ENTRY channels status --probe --json
rm -f /tmp/wildvine-discord-token /tmp/wildvine-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "wildvine-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "wildvine-main-upgrade.tgz"
