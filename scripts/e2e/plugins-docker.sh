...
IMAGE_NAME="wildvine-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  WILDVINE_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  WILDVINE_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export WILDVINE_ENTRY

home_dir=$(mktemp -d "/tmp/wildvine-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
WILDVINE_PLUGIN_HOME="$HOME/.wildvine/$BUNDLED_PLUGIN_ROOT_DIR"
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$WILDVINE_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$WILDVINE_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$WILDVINE_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/wildvine.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/wildvine.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$WILDVINE_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/wildvine.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$WILDVINE_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/wildvine-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@wildvine/demo-plugin-tgz",
  "version": "0.0.1",
  "wildvine": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/wildvine.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$WILDVINE_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$WILDVINE_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/wildvine-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@wildvine/demo-plugin-dir",
  "version": "0.0.1",
  "wildvine": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/wildvine.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$WILDVINE_ENTRY" plugins install "$dir_plugin"
node "$WILDVINE_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/wildvine-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@wildvine/demo-plugin-file",
  "version": "0.0.1",
  "wildvine": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/wildvine.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$WILDVINE_ENTRY" plugins install "file:$file_pack_dir/package"
node "$WILDVINE_ENTRY" plugins list --json > /tmp/plugins4.json
...
bundle_root="$WILDVINE_PLUGIN_HOME/$bundle_plugin_id"
...
const configPath = path.join(process.env.HOME, ".wildvine", "wildvine.json");
...
gateway_log="/tmp/wildvine-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/wildvine-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@wildvine/slash-install-plugin",
  "version": "0.0.1",
  "wildvine": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/wildvine.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "wildvine/fixture-marketplace"
    }
  }
}
JSON

node "$WILDVINE_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$WILDVINE_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$WILDVINE_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$WILDVINE_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$WILDVINE_ENTRY" plugins update marketplace-shortcut --dry-run
node "$WILDVINE_ENTRY" plugins update marketplace-shortcut
node "$WILDVINE_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
