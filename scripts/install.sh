...
# Wildvine Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Wildvine."
...
GUM_VERSION="${WILDVINE_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Wildvine Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://wildvine.com/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$WILDVINE_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.wildvine.com/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_wildvine_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.wildvine-* "$npm_root"/wildvine 2>/dev/null || true
}

extract_wildvine_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_wildvine_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/wildvine" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/wildvine"|"/usr/local/bin/wildvine")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/wildvine/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale wildvine symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing wildvine binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Wildvine package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_wildvine_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Wildvine npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*wildvine" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_wildvine_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_wildvine_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Wildvine npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an wildvine binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("I'll butter your workflow like a vine roll: messy, delicious, effective.")
...
pick_tagline() {
    ...
    if [[ -n "${WILDVINE_TAGLINE_INDEX:-}" ]]; then
        if [[ "${WILDVINE_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((WILDVINE_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${WILDVINE_NO_ONBOARD:-0}
NO_PROMPT=${WILDVINE_NO_PROMPT:-0}
DRY_RUN=${WILDVINE_DRY_RUN:-0}
INSTALL_METHOD=${WILDVINE_INSTALL_METHOD:-}
WILDVINE_VERSION=${WILDVINE_VERSION:-latest}
USE_BETA=${WILDVINE_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/wildvine"
GIT_DIR=${WILDVINE_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${WILDVINE_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${WILDVINE_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${WILDVINE_VERBOSE:-0}"
VERIFY_INSTALL="${WILDVINE_VERIFY_INSTALL:-0}"
WILDVINE_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Wildvine installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/wildvine)
...
Environment variables:
  WILDVINE_INSTALL_METHOD=git|npm
  WILDVINE_VERSION=latest|next|main|<semver>|<spec>
  WILDVINE_BETA=0|1
  WILDVINE_GIT_DIR=...
  WILDVINE_GIT_UPDATE=0|1
  WILDVINE_NO_PROMPT=1
  WILDVINE_VERIFY_INSTALL=1
  WILDVINE_DRY_RUN=1
  WILDVINE_NO_ONBOARD=1
  WILDVINE_VERBOSE=1
  WILDVINE_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Wildvine checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Wildvine source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_wildvine_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"wildvine"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://wildvine.com/install.sh | bash"
}
...
ensure_wildvine_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/wildvine" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/wildvine" ]]; then
        ln -sf "$npm_root/wildvine/dist/entry.js" "${npm_bin}/wildvine"
        ui_info "Created wildvine bin link at ${npm_bin}/wildvine"
    fi
    return 0
}

# Check for existing Wildvine installation
check_existing_wildvine() {
    if [[ -n "$(type -P wildvine 2>/dev/null || true)" ]]; then
        ui_info "Existing Wildvine installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make wildvine show as \"command not found\" in new terminals."
...
warn_wildvine_not_found() {
    ui_warn "Installed, but wildvine is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t wildvine 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named wildvine; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_wildvine_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P wildvine 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/wildvine" ]]; then
        echo "${npm_bin}/wildvine"
        return 0
    fi
...
install_wildvine_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/PROACTIVA-US/wildvine.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Wildvine from git checkout: ${repo_dir}"
    else
        ui_info "Installing Wildvine from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Wildvine" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Wildvine" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/wildvine" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/wildvine"
    ui_success "Wildvine wrapper installed to \$HOME/.local/bin/wildvine"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view wildvine dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:wildvine/wildvine#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_wildvine() {
    local package_name="wildvine"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            WILDVINE_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="wildvine"
        else
            WILDVINE_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${WILDVINE_VERSION}" ]]; then
        WILDVINE_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${WILDVINE_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${WILDVINE_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Wildvine v${resolved_version}"
    else
        ui_info "Installing Wildvine (${WILDVINE_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${WILDVINE_VERSION}")"

    if ! install_wildvine_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_wildvine_paths
        install_wildvine_npm "${install_spec}"
    fi

    if [[ "${WILDVINE_VERSION}" == "latest" && "${package_name}" == "wildvine" ]]; then
        if ! resolve_wildvine_bin &> /dev/null; then
            ui_warn "npm install wildvine@latest failed; retrying wildvine@next"
            cleanup_npm_wildvine_paths
            install_wildvine_npm "wildvine@next"
        fi
    fi

    ensure_wildvine_bin_link || true

    ui_success "Wildvine installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${WILDVINE_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_wildvine_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (wildvine not on PATH yet)"
        warn_wildvine_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${WILDVINE_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.wildvine/workspace-${profile}"
    else
        echo "${HOME}/.wildvine/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${WILDVINE_CONFIG_PATH:-$HOME/.wildvine/wildvine.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run wildvine onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but wildvine not on PATH; skipping onboarding"
        warn_wildvine_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run wildvine onboard to retry"
        return
    }
}
...
resolve_wildvine_version() {
    local version=""
    local raw_version_output=""
    local claw="${WILDVINE_BIN:-}"
    if [[ -z "$claw" ]] && command -v wildvine &> /dev/null; then
        claw="$(command -v wildvine)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_wildvine_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/wildvine/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/wildvine/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: wildvine not on PATH yet"
        warn_wildvine_not_found
        return 1
    fi

    run_quiet_step "Checking Wildvine version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: wildvine gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_wildvine_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Wildvine checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set WILDVINE_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_wildvine; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Wildvine"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g wildvine &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g wildvine 2>/dev/null || true
        ...
        install_wildvine_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/wildvine" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/wildvine"
    ...
    WILDVINE_BIN="$(resolve_wildvine_bin || true)"
    ...
    installed_version=$(resolve_wildvine_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Wildvine installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Wildvine installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            "Leveled up! New skills unlocked. You're welcome."
            "Fresh code, same vine. Miss me?"
            ...
            "The vine has molted. Harder shell, sharper claws."
        ...
        local completion_messages=(
            ...
            "The vine has landed. Your terminal will never be the same."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/wildvine"
        ui_kv "Update command" "wildvine update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://wildvine.com/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running wildvine doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                WILDVINE_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                WILDVINE_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                WILDVINE_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run wildvine doctor and wildvine plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run wildvine onboard later"
        else
            local config_path="${WILDVINE_CONFIG_PATH:-$HOME/.wildvine/wildvine.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (wildvine not on PATH yet)"
                    warn_wildvine_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run wildvine onboard to finish setup"
            return 0
        fi
    fi

    if command -v wildvine &> /dev/null; then
        local claw="${WILDVINE_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_wildvine_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (wildvine daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if WILDVINE_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: wildvine daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${WILDVINE_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
