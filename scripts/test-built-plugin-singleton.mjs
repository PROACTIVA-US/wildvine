...
const warningFilterKey = Symbol.for("wildvine.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadWildvinePlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadWildvinePlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@wildvine/build-smoke-plugin",
      type: "module",
      wildvine: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "wildvine.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'wildvine/plugin-sdk';",
...
const registry = loadWildvinePlugins({
  ...
  env: {
    ...process.env,
    WILDVINE_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
