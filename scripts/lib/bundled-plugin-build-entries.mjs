...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@wildvine/${params.dirName}`) {
    return false;
  }
...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.wildvine?.extensions)
    ? packageJson.wildvine.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.wildvine?.setupEntry === "string" &&
    packageJson.wildvine.setupEntry.trim().length > 0
      ? packageJson.wildvine.setupEntry
      : undefined;
  if (setupEntry) {
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "wildvine.plugin.json");
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "wildvine.plugin.json"));
    }
    if (hasPackageJson) {
