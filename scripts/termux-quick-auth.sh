...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/wildvine/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Wildvine Re-Auth" -c "After getting key, run: ssh $SERVER '~/wildvine/scripts/mobile-reauth.sh'" --id wildvine-auth
        ;;
    *)
