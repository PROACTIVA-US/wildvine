...
SMOKE_PREVIOUS_VERSION="${WILDVINE_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${WILDVINE_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="wildvine"
PACKAGE_NAME="${WILDVINE_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (WILDVINE_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${WILDVINE_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${WILDVINE_INSTALL_LATEST_OUT:-}"
fi
