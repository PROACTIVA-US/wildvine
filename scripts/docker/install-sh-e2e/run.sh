...
INSTALL_URL="${WILDVINE_INSTALL_URL:-https://wildvine.bot/install.sh}"
MODELS_MODE="${WILDVINE_E2E_MODELS:-both}" # both|openai|anthropic
INSTALL_TAG="${WILDVINE_INSTALL_TAG:-latest}"
E2E_PREVIOUS_VERSION="${WILDVINE_INSTALL_E2E_PREVIOUS:-}"
SKIP_PREVIOUS="${WILDVINE_INSTALL_E2E_SKIP_PREVIOUS:-0}"
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  WILDVINE_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  WILDVINE_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
...
INSTALLED_VERSION="$(extract_wildvine_semver "$INSTALLED_VERSION")"
