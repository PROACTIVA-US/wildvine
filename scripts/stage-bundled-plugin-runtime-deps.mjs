...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.wildvine?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.wildvine) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.wildvine;
  ...
  if (packageJson.peerDependenciesMeta?.wildvine) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.wildvine;
  ...
  if (packageJson.devDependencies?.wildvine) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.wildvine;
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".wildvine-runtime-deps-stamp.json");
}
...
function stageInstalledRootRuntimeDeps(params) {
  ...
  const stagedNodeModulesDir = path.join(
    makeTempDir(
      os.tmpdir(),
      `wildvine-runtime-deps-${sanitizeTempPrefixSegment(path.basename(pluginDir))}-`,
    ),
    "node_modules",
  );
...
function installPluginRuntimeDeps(params) {
  ...
  const tempInstallDir = makeTempDir(
    os.tmpdir(),
    `wildvine-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}-`,
  );
