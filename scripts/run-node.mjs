...
const extensionRestartMetadataFiles = new Set(["wildvine.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.WILDVINE_FORCE_BUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by WILDVINE_FORCE_BUILD",
...
const logRunner = (message, deps) => {
  if (deps.env.WILDVINE_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[wildvine] ${message}\n`);
};

const runWildvine = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["wildvine.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
