#!/bin/bash
# Setup Wildvine Auth Management System
...
echo "=== Wildvine Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'wildvine-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Wildvine message: Send warning via Wildvine itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/wildvine-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/wildvine-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/wildvine-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now wildvine-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/WildvineAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/WildvineAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Wildvine*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (WildvineAuth) shows status and opens auth URL if needed."
echo "The full widget (WildvineAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status wildvine-auth-monitor.timer"
echo ""
echo "Quick commands:"
