...
#
# Suggested cron: */30 * * * * /home/admin/wildvine/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Wildvine notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., wildvine-alerts)
...
STATE_FILE="$HOME/.wildvine/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Wildvine if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use wildvine
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Wildvine to $NOTIFY_PHONE..."
            wildvine send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Wildvine is down. Run: ssh l36 '~/wildvine/scripts/mobile-reauth.sh'" "urgent"
    exit 1
