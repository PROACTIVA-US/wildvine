...
- regenerates apps/ios/Wildvine.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${WILDVINE_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid WILDVINE_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid WILDVINE_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid WILDVINE_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid WILDVINE_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing WILDVINE_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(WILDVINE_URL_SLASH)$(WILDVINE_URL_SLASH)#g'
)"
...
WILDVINE_CODE_SIGN_STYLE = Automatic
WILDVINE_DEVELOPMENT_TEAM = ${TEAM_ID}
WILDVINE_IOS_SELECTED_TEAM = ${TEAM_ID}
WILDVINE_APP_BUNDLE_ID = ai.wildvine.client
WILDVINE_SHARE_BUNDLE_ID = ai.wildvine.client.share
WILDVINE_ACTIVITY_WIDGET_BUNDLE_ID = ai.wildvine.client.activitywidget
WILDVINE_WATCH_APP_BUNDLE_ID = ai.wildvine.client.watchkitapp
WILDVINE_WATCH_EXTENSION_BUNDLE_ID = ai.wildvine.client.watchkitapp.extension
WILDVINE_APP_PROFILE =
WILDVINE_SHARE_PROFILE =
WILDVINE_PUSH_TRANSPORT = relay
WILDVINE_PUSH_DISTRIBUTION = official
WILDVINE_URL_SLASH = /
WILDVINE_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
WILDVINE_PUSH_APNS_ENVIRONMENT = production
EOF
