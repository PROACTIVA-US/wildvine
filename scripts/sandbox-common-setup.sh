...
BASE_IMAGE="${BASE_IMAGE:-wildvine-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-wildvine-sandbox-common:bookworm-slim}"
...
WILDVINE_DOCKER_BUILD_USE_BUILDX="${WILDVINE_DOCKER_BUILD_USE_BUILDX:-0}"
WILDVINE_DOCKER_BUILD_CACHE_FROM="${WILDVINE_DOCKER_BUILD_CACHE_FROM:-}"
WILDVINE_DOCKER_BUILD_CACHE_TO="${WILDVINE_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${WILDVINE_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${WILDVINE_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${WILDVINE_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${WILDVINE_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${WILDVINE_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=wildvine.sandbox=1)
NOTE
