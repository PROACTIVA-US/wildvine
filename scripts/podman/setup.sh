#!/usr/bin/env bash
# One-time host setup for rootless Wildvine in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.wildvine by default, and uses the repo-local
# launch script at ./scripts/run-wildvine-podman.sh.
#
...
#   systemctl --user start wildvine.service
set -euo pipefail
...
WILDVINE_USER="$(id -un)"
WILDVINE_HOME="${HOME:-}"
WILDVINE_CONFIG_DIR="${WILDVINE_CONFIG_DIR:-}"
WILDVINE_WORKSPACE_DIR="${WILDVINE_WORKSPACE_DIR:-}"
WILDVINE_IMAGE="${WILDVINE_PODMAN_IMAGE:-${WILDVINE_IMAGE:-wildvine:local}}"
WILDVINE_CONTAINER_NAME="${WILDVINE_PODMAN_CONTAINER:-wildvine}"
...
if [[ "$WILDVINE_IMAGE" == "wildvine:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
...
if [[ -z "$WILDVINE_HOME" ]]; then
  WILDVINE_HOME="$(resolve_user_home "$WILDVINE_USER")"
fi
if [[ -z "$WILDVINE_HOME" ]]; then
  echo "Unable to resolve HOME for user $WILDVINE_USER." >&2
  exit 1
fi
if [[ -z "$WILDVINE_CONFIG_DIR" ]]; then
  WILDVINE_CONFIG_DIR="$WILDVINE_HOME/.wildvine"
fi
if [[ -z "$WILDVINE_WORKSPACE_DIR" ]]; then
  WILDVINE_WORKSPACE_DIR="$WILDVINE_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$WILDVINE_HOME"
validate_mount_source_path "config directory" "$WILDVINE_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$WILDVINE_WORKSPACE_DIR"
validate_container_name "$WILDVINE_CONTAINER_NAME"
validate_image_name "$WILDVINE_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$WILDVINE_CONFIG_DIR" "$WILDVINE_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$WILDVINE_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$WILDVINE_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${WILDVINE_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "WILDVINE_DOCKER_APT_PACKAGES=${WILDVINE_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${WILDVINE_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "WILDVINE_EXTENSIONS=${WILDVINE_EXTENSIONS}")
fi

if [[ "$WILDVINE_IMAGE" == "wildvine:local" ]]; then
  echo "Building image $WILDVINE_IMAGE ..."
  podman build -t "$WILDVINE_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$WILDVINE_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $WILDVINE_IMAGE"
  else
    echo "Pulling image $WILDVINE_IMAGE ..."
    podman pull "$WILDVINE_IMAGE"
  fi
fi

ENV_FILE="$WILDVINE_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated WILDVINE_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "WILDVINE_PODMAN_CONTAINER" "$WILDVINE_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "WILDVINE_PODMAN_IMAGE" "$WILDVINE_IMAGE"

CONFIG_JSON="$WILDVINE_CONFIG_DIR/wildvine.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  ...
  QUADLET_DST="$QUADLET_DIR/wildvine.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  WILDVINE_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$WILDVINE_HOME")"
  WILDVINE_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$WILDVINE_CONFIG_DIR")"
  WILDVINE_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$WILDVINE_WORKSPACE_DIR")"
  WILDVINE_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$WILDVINE_IMAGE")"
  WILDVINE_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$WILDVINE_CONTAINER_NAME")"
  sed \
    -e "s|{{WILDVINE_HOME}}|$WILDVINE_HOME_ESCAPED|g" \
    -e "s|{{WILDVINE_CONFIG_DIR}}|$WILDVINE_CONFIG_ESCAPED|g" \
    -e "s|{{WILDVINE_WORKSPACE_DIR}}|$WILDVINE_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$WILDVINE_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$WILDVINE_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start wildvine.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start wildvine.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-wildvine-podman.sh launch"
echo "  ./scripts/run-wildvine-podman.sh launch setup"
echo "  wildvine --container $WILDVINE_CONTAINER_NAME dashboard --no-open"
