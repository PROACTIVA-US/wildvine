 ...
 * `wildvine update`. Use it to verify the completion installation flow
...
\`wildvine update\`, \`wildvine doctor\`, and \`wildvine onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Wildvine Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`wildvine completion --install\` later to enable.`));
    return;
  }
