# Wildvine Installer for Windows (PowerShell)
# Usage: iwr -useb https://wildvine.com/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://wildvine.com/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\wildvine",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 Wildvine Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Wildvine.$NC" -Level info
    Write-Host ""
}
...
function Install-WildvineGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Wildvine from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/PROACTIVA-US/wildvine.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\wildvine\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\wildvine.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Wildvine from git to $GitDir" -Level info
        } else {
            Install-WildvineGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Wildvine via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-WildvineNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'wildvine onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 Wildvine installed successfully!" -Level success
}
