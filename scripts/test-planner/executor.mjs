...
export function createExecutionArtifacts(env = process.env) {
  ...
  const ensureTempArtifactDir = () => {
    if (tempArtifactDir === null) {
      tempArtifactDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-test-parallel-"));
    }
  ...
  const cleanupTempArtifacts = () => {
    ...
    if (env.WILDVINE_TEST_KEEP_TEMP_ARTIFACTS === "1") {
...
const isFsModuleCacheEnabled = (env, platform = process.platform) => {
  if (isWindowsEnv(env, platform)) {
    return isEnvFlagEnabled(env.WILDVINE_VITEST_FS_MODULE_CACHE);
  }
  return !isEnvFlagDisabled(env.WILDVINE_VITEST_FS_MODULE_CACHE);
};
...
export const resolveVitestFsModuleCachePath = ({
  ...
  const explicitPath = env.WILDVINE_VITEST_FS_MODULE_CACHE_PATH?.trim();
...
export async function executePlan(plan, options = {}) {
  ...
  const windowsCiArgs = plan.runtimeCapabilities.isWindowsCi
    ...
    : [];
  const silentArgs = env.WILDVINE_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
  const rawMemoryTrace = env.WILDVINE_TEST_MEMORY_TRACE?.trim().toLowerCase();
  ...
  const memoryTracePollMs = Math.max(
    250,
    parseEnvNumber("WILDVINE_TEST_MEMORY_TRACE_POLL_MS", 1000),
  );
  const memoryTraceTopCount = Math.max(
    1,
    parseEnvNumber("WILDVINE_TEST_MEMORY_TRACE_TOP_COUNT", 6),
  );
  const requestedHeapSnapshotIntervalMs = Math.max(
    0,
    parseEnvNumber("WILDVINE_TEST_HEAPSNAPSHOT_INTERVAL_MS", 0),
  );
  ...
  const heapSnapshotSignal = env.WILDVINE_TEST_HEAPSNAPSHOT_SIGNAL?.trim() || "SIGUSR2";
  const closeGraceMs = Math.max(100, parseEnvNumber("WILDVINE_TEST_CLOSE_GRACE_MS", 2000));
  const heapSnapshotBaseDir = heapSnapshotEnabled
    ? path.resolve(
        env.WILDVINE_TEST_HEAPSNAPSHOT_DIR?.trim() ||
          path.join(os.tmpdir(), `wildvine-heapsnapshots-${Date.now()}`),
      )
    : null;
  const maxOldSpaceSizeMb = (() => {
    const raw = env.WILDVINE_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
    ...
    new Promise((resolve) => {
      ...
      try {
        ...
        if (vitestFsModuleCachePath) {
          childEnv.WILDVINE_VITEST_FS_MODULE_CACHE_PATH = vitestFsModuleCachePath;
