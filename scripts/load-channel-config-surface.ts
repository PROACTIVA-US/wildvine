...
function copyModuleImportGraphWithoutNodeModules(params: {
  ...
  const tempParent = path.join(params.repoRoot, ".wildvine-config-doc-cache");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.WILDVINE_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing WILDVINE_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        WILDVINE_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
