...
type DriverMode = "token" | "webhook" | "wildvine";
...
function resolveStateDir(): string {
  const override = process.env.WILDVINE_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.WILDVINE_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".wildvine");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver wildvine] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Wildvine spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|wildvine> Driver transport mode (default: token)\n" +
    ...
    "  --wildvine-bin <path>        Wildvine CLI binary for driver=wildvine (default: wildvine)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  WILDVINE_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  WILDVINE_DISCORD_SMOKE_DRIVER\n" +
    "  WILDVINE_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  WILDVINE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  WILDVINE_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  WILDVINE_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  WILDVINE_DISCORD_SMOKE_AGENT\n" +
    "  WILDVINE_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  WILDVINE_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  WILDVINE_DISCORD_SMOKE_POLL_MS\n" +
    "  WILDVINE_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  WILDVINE_DISCORD_SMOKE_WILDVINE_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.WILDVINE_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.WILDVINE_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "wildvine"
        ? "wildvine"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.WILDVINE_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.WILDVINE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.WILDVINE_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.WILDVINE_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.WILDVINE_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.WILDVINE_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.WILDVINE_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.WILDVINE_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.WILDVINE_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.WILDVINE_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const wildvineBin =
    resolveArg("--wildvine-bin") || process.env.WILDVINE_DISCORD_SMOKE_WILDVINE_BIN || "wildvine";
...
async function wildvineCliJson<T>(params: { wildvineBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.wildvineBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`wildvine ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await wildvineCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    wildvineBin: params.wildvineBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "wildvine") {
    return await readMessagesWithOpenclaw({
      wildvineBin: params.args.wildvineBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `wildvine-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await wildvineCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("wildvine message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Wildvine.`,
        diagnostics: {
          bindingCandidates: [
            {
