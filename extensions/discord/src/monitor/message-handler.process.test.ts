import { DEFAULT_EMOJIS } from "wildvine/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/wildvine-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("wildvine/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("wildvine/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("wildvine/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/wildvine-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/wildvine-discord-process-test-sessions.json" },
      },
    });
