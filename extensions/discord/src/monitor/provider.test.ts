...
import { AcpRuntimeError } from "wildvine/plugin-sdk/acp-runtime";
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): WildvineConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("wildvine/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/plugin-runtime")>(
