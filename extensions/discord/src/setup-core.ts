import { DEFAULT_ACCOUNT_ID } from "wildvine/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "wildvine/plugin-sdk/config-runtime";
import type { WildvineConfig } from "wildvine/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "wildvine/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "wildvine/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "wildvine/plugin-sdk/setup-runtime";
import { formatDocsLink } from "wildvine/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: WildvineConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: WildvineConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: WildvineConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: WildvineConfig; accountId: string }) =>
    ...
    disable: (cfg: WildvineConfig) => setSetupChannelEnabled(cfg, channel, false),
