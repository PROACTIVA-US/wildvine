...
import type { WildvineConfig } from "../../../../src/plugin-sdk/discord.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: WildvineConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): WildvineConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("wildvine/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/acp-runtime")>(
    "wildvine/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("wildvine/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/command-auth")>(
...
vi.mock("wildvine/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/reply-runtime")>(
    "wildvine/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("wildvine/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/config-runtime")>(
...
vi.mock("wildvine/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/runtime-env")>(
    "wildvine/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("wildvine/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/infra-runtime")>(
