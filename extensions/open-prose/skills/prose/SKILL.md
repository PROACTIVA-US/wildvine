...
metadata: { "wildvine": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Wildvine `sessions_spawn`
- **File I/O** == Wildvine `read`/`write`
- **Remote fetch** == Wildvine `web_fetch` (or `exec` with curl when POST is required)
