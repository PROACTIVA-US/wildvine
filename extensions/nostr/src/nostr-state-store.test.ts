import type { PluginRuntime } from "wildvine/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.WILDVINE_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-nostr-"));
  process.env.WILDVINE_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".wildvine");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.WILDVINE_STATE_DIR = previous;
    }
