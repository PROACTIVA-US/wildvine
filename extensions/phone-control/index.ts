import type { WildvinePluginApi, WildvinePluginService } from "wildvine/plugin-sdk";
...
function normalizeDenyList(cfg: WildvinePluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: WildvinePluginApi["config"]): string[] {
...
export default function register(api: WildvinePluginApi) {
  ...
  const timerService: WildvinePluginService = {
