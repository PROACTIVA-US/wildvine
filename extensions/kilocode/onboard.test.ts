...
} from "wildvine/plugin-sdk/kilocode";
import {
  resolveApiKeyForProvider,
  resolveEnvApiKey,
} from "wildvine/plugin-sdk/provider-auth-runtime";
import { resolveAgentModelPrimaryValue } from "wildvine/plugin-sdk/provider-onboard";
...
describe("Kilo Gateway provider config", () => {
  ...
  describe("env var resolution", () => {
    ...
    it("resolves the kilocode api key via resolveApiKeyForProvider", async () => {
      const agentDir = mkdtempSync(join(tmpdir(), "wildvine-test-"));
      const envSnapshot = captureEnv(["KILOCODE_API_KEY"]);
