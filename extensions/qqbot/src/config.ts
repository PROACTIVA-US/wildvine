...
export function listQQBotAccountIds(cfg: WildvineConfig): string[] {
...
export function resolveDefaultQQBotAccountId(cfg: WildvineConfig): string {
  const qqbot = cfg.channels?.qqbot as QQBotChannelConfig | undefined;
  if (qqbot?.appId || process.env.QQBOT_APP_ID) {
...
/** Apply account config updates back into the Wildvine config object. */
