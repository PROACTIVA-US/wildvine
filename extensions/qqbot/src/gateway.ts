...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  } else if (isGlobalTTSAvailable(cfg as WildvineConfig)) {
    const globalProvider = (cfg as WildvineConfig).messages?.tts?.provider ?? "auto";
    log?.info(
    ...
    try {
      ...
      const handleMessage = async (event: {
        ...
        const hasTTS =
          ...
          isGlobalTTSAvailable(cfg as WildvineConfig);
