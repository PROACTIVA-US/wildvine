...
describe("qqbot local media path remapping", () => {
  ...
  it("remaps missing workspace media paths to the real media directory", () => {
    ...
    const wildvineDir = path.join(actualHome, ".wildvine");
    fs.mkdirSync(wildvineDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(wildvineDir, "qqbot-platform-test-"));
