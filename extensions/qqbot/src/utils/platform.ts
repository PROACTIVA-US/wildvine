...
/**
 * Return a path under `~/.wildvine/qqbot`, creating it on demand.
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".wildvine", "qqbot", ...subPaths);
...
/**
 * Return a path under `~/.wildvine/media/qqbot`, creating it on demand.
 *
 * Unlike `getQQBotDataDir`, this lives under Wildvine's core media allowlist so
...
export function getQQBotMediaDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".wildvine", "media", "qqbot", ...subPaths);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  return dir;
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".wildvine", "workspace", "qqbot");
