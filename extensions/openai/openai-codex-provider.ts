...
import { buildOauthProviderAuthResult } from "wildvine/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "wildvine/plugin-sdk/provider-auth-login";
import {
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `wildvine models auth login --provider openai-codex` or `wildvine configure`.";
}
