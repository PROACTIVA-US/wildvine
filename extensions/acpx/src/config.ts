...
import { buildPluginConfigSchema } from "wildvine/plugin-sdk/core";
import { z } from "wildvine/plugin-sdk/zod";
import type { WildvinePluginConfigSchema } from "../runtime-api.js";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "wildvine-plugin-tools";
const ACPX_BIN_NAME = process.platform === "win32" ? "acpx.cmd" : "acpx";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "wildvine.plugin.json")) &&
...
function resolveWildvineRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const openClawRoot = resolveWildvineRoot(pluginRoot);
...
export function createAcpxPluginConfigSchema(): WildvinePluginConfigSchema {
