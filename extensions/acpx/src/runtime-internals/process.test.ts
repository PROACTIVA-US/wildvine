...
import { createWindowsCmdShimFixture } from "wildvine/plugin-sdk/testing";
...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "wildvine-acpx-process-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("spawnAndCollect", () => {
  ...
  async function collectSpawnedEnvSnapshot(options?: {
    ...
    const result = await spawnAndCollect({
      ...
      args: [
        "-e",
        `process.stdout.write(JSON.stringify({openai:process.env.${openAiEnvKey},github:process.env.${githubEnvKey},hf:process.env.${hfEnvKey},wildvine:process.env.WILDVINE_API_KEY,shell:process.env.WILDVINE_SHELL}), () => process.exit(0))`,
      ],
      cwd: process.cwd(),
  ...
  it("strips shared provider auth env vars from spawned acpx children", async () => {
    ...
    expect(parsed.wildvine).toBe("keep-me");
