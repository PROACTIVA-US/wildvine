...
Wildvine extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
wildvine message send --channel zalouser --target <threadId> --message "Hello from Wildvine!"
```
...
```bash
wildvine channels login --channel zalouser              # Login via QR
wildvine channels login --channel zalouser --account work
wildvine channels status --probe
wildvine channels logout --channel zalouser
```
...
wildvine directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
wildvine message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
wildvine message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `wildvine channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
wildvine channels login --channel zalouser --account work
wildvine message send --channel zalouser --account work --target <id> --message "Hello"
