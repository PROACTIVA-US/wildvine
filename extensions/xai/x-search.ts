...
import { getRuntimeConfigSnapshot } from "wildvine/plugin-sdk/config-runtime";
import type { WildvineConfig } from "wildvine/plugin-sdk/plugin-entry";
import {
  jsonResult,
  readCache,
  ...
  writeCache,
} from "wildvine/plugin-sdk/provider-web-search";
import {
...
type XSearchConfig = NonNullable<WildvineConfig["tools"]>["web"] extends infer Web
...
const X_SEARCH_CACHE_KEY = Symbol.for("wildvine.xai.x-search.cache");
...
function readLegacyGrokApiKey(cfg?: WildvineConfig): string | undefined {
...
function readPluginXaiWebSearchApiKey(cfg?: WildvineConfig): string | undefined {
  return readConfiguredSecretString(
...
function resolveFallbackXaiApiKey(cfg?: WildvineConfig): string | undefined {
  return readPluginXaiWebSearchApiKey(cfg) ?? readLegacyGrokApiKey(cfg);
}

function resolveXSearchConfig(cfg?: WildvineConfig): XSearchConfig {
...
export function createXSearchTool(options?: {
  ...
  runtimeConfig?: WildvineConfig | null;
}) {
