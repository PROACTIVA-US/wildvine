import type { ImageGenerationProvider } from "wildvine/plugin-sdk/image-generation";
...
import { createProviderApiKeyAuthMethod } from "wildvine/plugin-sdk/provider-auth-api-key";
import type { ProviderPlugin } from "wildvine/plugin-sdk/provider-model-shared";
import { createGoogleThinkingPayloadWrapper } from "wildvine/plugin-sdk/provider-stream";
import {
...
const GOOGLE_GEMINI_CLI_ENV_VARS = [
  "WILDVINE_GEMINI_OAUTH_CLIENT_ID",
  "WILDVINE_GEMINI_OAUTH_CLIENT_SECRET",
...
async function loadGoogleGeminiCliProvider(): Promise<ProviderPlugin> {
  if (!googleGeminiCliProviderPromise) {
    googleGeminiCliProviderPromise = import("./gemini-cli-provider.js").then((mod) => {
      ...
      } as Pick<WildvinePluginApi, "registerProvider"> as WildvinePluginApi);
      if (!provider) {
