...
const { stopWildvineChromeMock } = vi.hoisted(() => ({
  stopWildvineChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopWildvineChrome: stopWildvineChromeMock,
}));
...
beforeEach(() => {
  ...
  stopWildvineChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["wildvine", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      wildvine: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.wildvine).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopWildvineChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("wildvine browser stop failed: Error: oops");
  });
});
