...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => WildvineConfig>(),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<WildvineConfig["gateway"]>["auth"]> | undefined;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: WildvineConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
