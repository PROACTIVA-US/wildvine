...
export function resolveWildvineUserDataDir(profileName = DEFAULT_WILDVINE_BROWSER_PROFILE_NAME) {
...
export function buildWildvineChromeLaunchArgs(params: {
...
export async function launchWildvineChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveWildvineUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_WILDVINE_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildWildvineChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateWildvineProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 wildvine browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`wildvine browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`wildvine browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 wildvine browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopWildvineChrome(
