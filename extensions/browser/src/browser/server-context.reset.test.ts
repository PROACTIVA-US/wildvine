...
function localWildvineProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "wildvine",
...
function createLocalWildvineResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localWildvineProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveWildvineUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-reset-"));
    const profileDir = path.join(tempRoot, "wildvine");
    ...
    const ops = createLocalWildvineResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveWildvineUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-reset-no-own-"));
    const profileDir = path.join(tempRoot, "wildvine");
    fs.mkdirSync(profileDir, { recursive: true });
