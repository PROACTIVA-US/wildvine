...
function resolveBrowserFetchOperatorHint(url: string): string {
  ...
  return isLocal
    ? `Restart the Wildvine gateway (Wildvine.app menubar, or \`${formatCliCommand("wildvine gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      appendBrowserToolModelHint(
        `Can't reach the Wildvine browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
      ),
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the Wildvine browser control service. ${operatorHint} (${msg})`,
    ),
  );
}
