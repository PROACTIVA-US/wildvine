...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".wildvine-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + vine-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateWildvineProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_WILDVINE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_WILDVINE_BROWSER_COLOR).toUpperCase();
