...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredWildvineTmpDir: vi.fn(() => "/tmp/wildvine"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-wildvine-dir.js");
...
describe("pw-tools-core", () => {
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-wildvine-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredWildvineTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredWildvineTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredWildvineTmpDir.mockReturnValue("/tmp/wildvine");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".wildvine-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredWildvineTmpDir.mockReturnValue("/tmp/wildvine-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "wildvine-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "wildvine-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredWildvineTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "wildvine-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "wildvine-preferred", "downloads")}${path.sep}`),
    );
  });
