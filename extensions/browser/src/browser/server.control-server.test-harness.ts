...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.WILDVINE_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "wildvine",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/wildvine" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    wildvine: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchWildvineChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "wildvine";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.WILDVINE_GATEWAY_PORT;
  process.env.WILDVINE_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.WILDVINE_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.WILDVINE_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.WILDVINE_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.WILDVINE_GATEWAY_PASSWORD;
  } else {
    process.env.WILDVINE_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
