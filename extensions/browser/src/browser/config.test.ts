...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and vine-orange color", () => {
    ...
    expect(profile?.name).toBe("wildvine");
    expect(profile?.driver).toBe("wildvine");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const wildvine = resolveProfile(resolved, "wildvine");
    expect(wildvine?.driver).toBe("wildvine");
    expect(wildvine?.cdpPort).toBe(18800);
    expect(wildvine?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from WILDVINE_GATEWAY_PORT when unset", () => {
    withEnv({ WILDVINE_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const wildvine = resolveProfile(resolved, "wildvine");
      expect(wildvine?.cdpPort).toBe(19012);
      expect(wildvine?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ WILDVINE_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const wildvine = resolveProfile(resolved, "wildvine");
      expect(wildvine?.cdpPort).toBe(19022);
      expect(wildvine?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(wildvine?.cdpPort).toBe(19000);
    expect(wildvine?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "wildvine");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "wildvine")!;
  ...
  describe("default profile preference", () => {
    it("defaults to wildvine profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("wildvine");
    });

    it("keeps wildvine default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("wildvine");
    ...
    it("keeps wildvine default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("wildvine");
    ...
    it("keeps wildvine default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
