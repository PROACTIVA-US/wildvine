...
import { createSandboxTestContext } from "wildvine/plugin-sdk/testing";
...
describe("openshell plugin config", () => {
  ...
  it("keeps the runtime json schema in sync with the manifest config schema", () => {
    const manifest = JSON.parse(
      fsSync.readFileSync(new URL("../wildvine.plugin.json", import.meta.url), "utf8"),
...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from Wildvine config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "wildvine-session-1234",
        backendId: "openshell",
        runtimeLabel: "wildvine-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "wildvine-session-1234",
        config: expect.objectContaining({
          from: "custom-source",
        }),
      }),
      args: ["sandbox", "get", "wildvine-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "wildvine-session-5678",
        backendId: "openshell",
        runtimeLabel: "wildvine-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "wildvine",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "wildvine-session-5678",
        config: expect.objectContaining({
          command: "/usr/local/bin/openshell",
          gateway: "lab",
        }),
      }),
      args: ["sandbox", "delete", "wildvine-session-5678"],
    });
  });
});
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("wildvine-openshell-fs-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("wildvine-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("wildvine-openshell-agent-");
  ...
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("wildvine-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("wildvine-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("wildvine-openshell-remote-agent-");
