...
const WILDVINE_OPENSHELL_E2E = process.env.WILDVINE_E2E_OPENSHELL === "1";
const WILDVINE_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const WILDVINE_OPENSHELL_COMMAND =
  process.env.WILDVINE_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "wildvine-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && WILDVINE_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: WILDVINE_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(WILDVINE_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-openshell-e2e-"));
      ...
      const gatewayName = `wildvine-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `wildvine-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/wildvine-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: WILDVINE_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("wildvine-openshell-e2e");
