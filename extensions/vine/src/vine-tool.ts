...
import type { WildvinePluginApi } from "../runtime-api.js";
...
async function runVineSubprocessOnce(params: {
  execPath: string;
  argv: string[];
  cwd: string;
  timeoutMs: number;
  ...
  const env = { ...process.env, VINE_MODE: "tool" } as Record<string, string | undefined>;
  ...
  const spawnTarget =
    process.platform === "win32"
      ? resolveWindowsVineSpawn(execPath, argv, env)
  ...
  return await new Promise<{ stdout: string }>((resolve, reject) => {
    ...
    child.stdout?.on("data", (chunk) => {
      ...
      if (stdoutBytes > maxStdoutBytes) {
        failAndTerminate("vine output exceeded maxStdoutBytes");
        return;
      }
      stdout += str;
    });
    ...
    const timer = setTimeout(() => {
      failAndTerminate("vine subprocess timed out");
    }, timeoutMs);
    ...
    child.once("exit", (code) => {
      if (code !== 0) {
        settle({
          ok: false,
          error: new Error(`vine failed (${code ?? "?"}): ${stderr.trim() || stdout.trim()}`),
        });
        return;
      }
...
function parseEnvelope(stdout: string): VineEnvelope {
  ...
  if (parsed === undefined) {
    throw new Error("vine returned invalid JSON");
  }

  if (!parsed || typeof parsed !== "object") {
    throw new Error("vine returned invalid JSON envelope");
  }

  const ok = (parsed as { ok?: unknown }).ok;
  if (ok === true || ok === false) {
    return parsed as VineEnvelope;
  }

  throw new Error("vine returned invalid JSON envelope");
}

function buildVineArgv(action: string, params: Record<string, unknown>): string[] {
  if (action === "run") {
...
export function createVineTool(api: WildvinePluginApi) {
  return {
    name: "vine",
    label: "Vine Workflow",
    description:
      "Run Vine pipelines as a local-first workflow runtime (typed JSON envelope + resumable approvals).",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      const argv = buildVineArgv(action, params);

      if (api.runtime?.version && api.logger?.debug) {
        api.logger.debug(`vine plugin runtime=${api.runtime.version}`);
      }

      const { stdout } = await runVineSubprocessOnce({
        execPath,
        argv,
        cwd,
        timeoutMs,
