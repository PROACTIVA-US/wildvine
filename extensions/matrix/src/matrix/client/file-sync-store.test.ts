...
import * as jsonStore from "wildvine/plugin-sdk/json-store";
...
describe("FileBackedMatrixSyncStore", () => {
  ...
  function createStoragePath(): string {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-matrix-sync-store-"));
