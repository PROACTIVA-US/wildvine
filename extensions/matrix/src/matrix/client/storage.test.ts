...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-wildvine-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildvine-matrix-storage-"));
    const stateDir = path.join(homeDir, ".wildvine");
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      WILDVINE_HOME: path.dirname(stateDir),
      WILDVINE_STATE_DIR: stateDir,
      WILDVINE_TEST_FAST: "1",
