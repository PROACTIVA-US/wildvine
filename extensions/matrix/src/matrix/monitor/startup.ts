...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleWildvineDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Wildvine devices detected for ${params.auth.userId}: ${deviceHealth.staleWildvineDevices.map((device) => device.deviceId).join(", ")}. Run 'wildvine matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
