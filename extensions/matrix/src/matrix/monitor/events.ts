...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Wildvine runtime is using the same account. " +
    `Check 'wildvine matrix verify status --verbose --account ${accountId}' and 'wildvine matrix devices list --account ${accountId}'.`
  );
}
