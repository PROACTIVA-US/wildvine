...
const WILDVINE_PLUGIN_SDK_PREFIX = ["wildvine", "plugin-sdk"].join("/");
...
function findWildvinePackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "wildvine" && pkg.exports?.["./plugin-sdk"]) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findWildvinePackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    aliasMap[WILDVINE_PLUGIN_SDK_PREFIX] = rootAlias;
  }
  ...
  for (const exportKey of Object.keys(packageJson.exports ?? {})) {
    ...
    if (resolvedPath) {
      aliasMap[`${WILDVINE_PLUGIN_SDK_PREFIX}/${subpath}`] = resolvedPath;
    }
  }
  ...
  if (extensionApi) {
    aliasMap["wildvine/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findWildvinePackageRoot(moduleDir);
  if (location) {
