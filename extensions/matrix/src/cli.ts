...
import { isWildvineManagedMatrixDevice } from "./matrix/device-health.js";
import {
...
function formatMatrixCliCommand(command: string, accountId?: string): string {
  ...
  return `wildvine matrix ${command}${suffix}`;
}
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleWildvineDeviceIds: addedDevices
        .filter((device) => !device.current && isWildvineManagedMatrixDevice(device.displayName))
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.wildvine.com/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleWildvineDeviceIds.length > 0) {
              console.log(
                `Matrix device hygiene warning: stale Wildvine devices detected (${result.deviceHealth.staleWildvineDeviceIds.join(", ")}). Run 'wildvine matrix devices prune-stale --account ${result.accountId}'.`,
              );
            }
            ...
            const bindHint = `wildvine agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale Wildvine-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
      ...
      await runMatrixCliCommand({
        ...
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale Wildvine devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.join(", ") : "none"}`,
          );
