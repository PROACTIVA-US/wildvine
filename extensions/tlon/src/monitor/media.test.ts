import { MAX_IMAGE_BYTES } from "wildvine/plugin-sdk/media-runtime";
...
describe("tlon monitor media", () => {
  async function loadMediaModule() {
    const mediaRuntime = await import("wildvine/plugin-sdk/media-runtime");
  ...
  it("stores fetched media through the shared inbound media store with the image cap", async () => {
    ...
    saveMediaBuffer.mockResolvedValue({
      id: "photo---uuid.png",
      path: "/tmp/wildvine/media/inbound/photo---uuid.png",
    ...
    expect(result).toEqual({
      localPath: "/tmp/wildvine/media/inbound/photo---uuid.png",
      contentType: "image/png",
