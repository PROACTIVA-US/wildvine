...
import type { ChannelSetupWizard } from "wildvine/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "wildvine/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: WildvineConfig,
  patch: Partial<NonNullable<NonNullable<WildvineConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: WildvineConfig, dmPolicy: DmPolicy): WildvineConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: WildvineConfig, allowFrom?: string[]): WildvineConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: WildvineConfig, selfChatMode: boolean): WildvineConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: WildvineConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Wildvine can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Wildvine" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("wildvine channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
