...
import { generateSecureUuid } from "wildvine/plugin-sdk/core";
...
import { getChildLogger } from "wildvine/plugin-sdk/text-runtime";
import { redactIdentifier } from "wildvine/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "wildvine/plugin-sdk/text-runtime";
import { markdownToWhatsApp } from "wildvine/plugin-sdk/text-runtime";
import { toWhatsappJid } from "wildvine/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: WildvineConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
