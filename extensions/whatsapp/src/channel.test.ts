...
import { createWhatsAppPollFixture, expectWhatsAppPollSent } from "wildvine/plugin-sdk/testing";
...
import type { WildvineConfig } from "./runtime-api.js";
...
const hoisted = vi.hoisted(() => ({
  ...
  listWhatsAppAccountIds: vi.fn((cfg: WildvineConfig) => {
  ...
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/wildvine-whatsapp-test",
  })),
}));
...
vi.mock("wildvine/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/setup")>(
    "wildvine/plugin-sdk/setup",
  );
  return {
    ...actual,
...
describe("whatsapp setup wizard", () => {
  ...
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/wildvine-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("wildvine channels login"),
      "WhatsApp",
    );
  });
