...
function createWhatsAppDirectStreamingArgs(params?: {
  ...
  return makeProcessMessageArgs({
    ...
    } as unknown as ReturnType<typeof import("wildvine/plugin-sdk/config-runtime").loadConfig>,
    msg: {
      id: "msg1",
      from: "+1555",
      to: "+2000",
...
describe("web processMessage inbound context", () => {
  ...
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-process-message-"));
        ...
        list: [
          {
            id: "main",
            default: true,
            identity: { name: "Mainbot", emoji: "🦞", theme: "space vine" },
          },
        ],
      },
      messages: {},
  ...
  it("does not force an [wildvine] response prefix in self-chats when identity is unset", async () => {
    await processSelfDirectMessage({
      messages: {},
      session: { store: sessionStorePath },
    } as unknown as ReturnType<typeof import("wildvine/plugin-sdk/config-runtime").loadConfig>);
