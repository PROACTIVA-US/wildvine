...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-group-gating-"));
...
function runGroupGating(params: {
  cfg: ReturnType<typeof import("wildvine/plugin-sdk/config-runtime").loadConfig>;
...
function makeInboundCfg(messagePrefix = "") {
  return {
    agents: { defaults: { workspace: "/tmp/wildvine" } },
