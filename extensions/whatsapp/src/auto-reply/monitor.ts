import { resolveInboundDebounceMs } from "wildvine/plugin-sdk/channel-inbound";
import { enqueueSystemEvent } from "wildvine/plugin-sdk/channel-runtime";
import { formatCliCommand } from "wildvine/plugin-sdk/cli-runtime";
import { waitForever } from "wildvine/plugin-sdk/cli-runtime";
import { hasControlCommand } from "wildvine/plugin-sdk/command-detection";
import { loadConfig } from "wildvine/plugin-sdk/config-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "wildvine/plugin-sdk/reply-history";
import { getReplyFromConfig } from "wildvine/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "wildvine/plugin-sdk/routing";
import { logVerbose } from "wildvine/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "wildvine/plugin-sdk/runtime-env";
import { getChildLogger } from "wildvine/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "wildvine/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("wildvine channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("wildvine channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
