import { redactIdentifier } from "wildvine/plugin-sdk/logging-core";
import type { getReplyFromConfig } from "wildvine/plugin-sdk/reply-runtime";
import { HEARTBEAT_TOKEN } from "wildvine/plugin-sdk/reply-runtime";
...
vi.mock("wildvine/plugin-sdk/agent-runtime", () => ({
...
vi.mock("wildvine/plugin-sdk/routing", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/routing")>();
  return {
    ...actual,
...
vi.mock("wildvine/plugin-sdk/channel-runtime", async (importOriginal) => {
...
vi.mock("wildvine/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/runtime-env")>();
  const logger = {
    child: () => logger,
...
vi.mock("wildvine/plugin-sdk/text-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/text-runtime")>();
  return {
    ...actual,
    redactIdentifier,
  };
...
vi.mock("wildvine/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.mock("../auth-store.js", () => ({
  WA_WEB_AUTH_DIR: "/tmp/wildvine-oauth/whatsapp/default",
  resolveDefaultWebAuthDir: () => "/tmp/wildvine-oauth/whatsapp/default",
