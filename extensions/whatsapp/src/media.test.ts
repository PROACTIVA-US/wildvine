...
import { optimizeImageToPng } from "wildvine/plugin-sdk/media-runtime";
import { resolveStateDir } from "wildvine/plugin-sdk/state-paths";
import { resolvePreferredWildvineTmpDir } from "wildvine/plugin-sdk/temp-path";
import { mockPinnedHostnameResolution } from "wildvine/plugin-sdk/testing";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredWildvineTmpDir(), "wildvine-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub WILDVINE_STATE_DIR.
    // Also keep it outside the Wildvine temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["WILDVINE_STATE_DIR"]);
    process.env.WILDVINE_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "wildvine-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredWildvineTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Wildvine state workspace and sandbox roots", async () => {
  ...
  it("rejects default Wildvine state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
