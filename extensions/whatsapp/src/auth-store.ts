...
import { info, success } from "wildvine/plugin-sdk/runtime-env";
import { getChildLogger } from "wildvine/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "wildvine/plugin-sdk/runtime-env";
import { resolveOAuthDir } from "wildvine/plugin-sdk/state-paths";
import type { WebChannel } from "wildvine/plugin-sdk/text-runtime";
import { resolveUserPath } from "wildvine/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("wildvine channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
