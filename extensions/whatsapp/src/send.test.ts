...
let resetLogger: typeof import("wildvine/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("wildvine/plugin-sdk/runtime-env").setLoggerOverride;
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("wildvine/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `wildvine-outbound-${crypto.randomUUID()}.log`);
