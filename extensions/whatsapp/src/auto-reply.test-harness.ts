...
import { createPinnedLookup } from "wildvine/plugin-sdk/fetch-runtime";
import { resetInboundDedupe } from "wildvine/plugin-sdk/reply-runtime";
import { resetLogger, setLoggerOverride } from "wildvine/plugin-sdk/runtime-env";
import * as ssrf from "wildvine/plugin-sdk/ssrf-runtime";
...
vi.mock("wildvine/plugin-sdk/agent-runtime", async (importOriginal) => {
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-session-"));
