...
const CONFIG_KEY = Symbol.for("wildvine:testConfigMock");
...
vi.mock("wildvine/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/media-runtime")>();
  const mockModule = Object.create(null) as Record<string, unknown>;
  Object.defineProperties(mockModule, Object.getOwnPropertyDescriptors(actual));
...
vi.mock("wildvine/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/wildvine-oauth",
  };
});
...
vi.mock("@whiskeysockets/baileys", async (importOriginal) => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("wildvine:lastSocket")] =
    created.lastSocket;
  return {
    ...actual,
    ...created.mod,
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("wildvine:lastSocket")];
  if (typeof getter === "function") {
