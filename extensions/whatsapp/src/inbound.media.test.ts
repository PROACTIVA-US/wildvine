...
vi.mock("wildvine/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("wildvine/plugin-sdk/media-runtime")>(
    "wildvine/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `wildvine-inbound-media-${crypto.randomUUID()}`);
