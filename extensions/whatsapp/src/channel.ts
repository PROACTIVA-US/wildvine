...
import { resolveReactionMessageId } from "wildvine/plugin-sdk/channel-actions";
import { createChatChannelPlugin } from "wildvine/plugin-sdk/core";
import { chunkText } from "wildvine/plugin-sdk/reply-runtime";
import {
...
function areWhatsAppAgentReactionsEnabled(params: { cfg: WildvineConfig; accountId?: string }) {
...
function hasAnyWhatsAppAccountWithAgentReactionsEnabled(cfg: WildvineConfig) {
...
function resolveWhatsAppAgentReactionGuidance(params: { cfg: WildvineConfig; accountId?: string }) {
  if (!params.cfg.channels?.whatsapp) {
    return undefined;
  }
  const gate = createActionGate(params.cfg.channels.whatsapp.actions);
