...
import { VERSION } from "wildvine/plugin-sdk/cli-runtime";
import { danger, success } from "wildvine/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "wildvine/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "wildvine/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("wildvine channels login")}`,
              ),
            );
          }
        }
