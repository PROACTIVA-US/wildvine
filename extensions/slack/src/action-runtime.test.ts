...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): WildvineConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as WildvineConfig;
  ...
  async function resolveReadToken(cfg: WildvineConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: WildvineConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
