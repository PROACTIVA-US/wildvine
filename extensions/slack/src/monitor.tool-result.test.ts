...
let resetInboundDedupe: typeof import("wildvine/plugin-sdk/reply-runtime").resetInboundDedupe;
...
beforeEach(async () => {
  vi.resetModules();
  ({ resetInboundDedupe } = await import("wildvine/plugin-sdk/reply-runtime"));
...
describe("monitorSlackProvider tool results", () => {
        ...
        list: [
          {
            id: "main",
            default: true,
            identity: { name: "Mainbot", theme: "space vine", emoji: "🦞" },
          },
          {
            id: "rich",
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bwildvine\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("wildvine: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("wildvine: hello <@U2>");
  });
