...
// Prefix for Wildvine-generated action IDs to scope our handler
const WILDVINE_ACTION_PREFIX = "wildvine:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${WILDVINE_ACTION_PREFIX}`);

  // Handle Wildvine modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
