...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "wildvine_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "wildvine_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("wildvine_cmdarg")).toBe(true);
    expect(testHarness.options.has("wildvine_cmdarg")).toBe(true);
  ...
  it("falls back to static menus when app.options() throws during registration", async () => {
    ...
    expect(actions.has("wildvine_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("wildvine_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("wildvine_cmdarg");
    expect(blockId).toContain("wildvine_cmdarg_ext:");
    const token = (blockId ?? "").slice("wildvine_cmdarg_ext:".length);
