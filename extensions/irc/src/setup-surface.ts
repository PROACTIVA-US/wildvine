...
import type { ChannelSetupDmPolicy } from "wildvine/plugin-sdk/setup";
import type { ChannelSetupWizard } from "wildvine/plugin-sdk/setup";
import { formatDocsLink } from "wildvine/plugin-sdk/setup";
import type { WizardPrompter } from "wildvine/plugin-sdk/setup";
...
export const ircSetupWizard: ChannelSetupWizard = {
  ...
  textInputs: [
    ...
    },
    {
      ...
      initialValue: ({ cfg, accountId }) =>
        resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.realname || "Wildvine",
      validate: ({ value }) => (String(value ?? "").trim() ? undefined : "Required"),
      normalizeValue: ({ value }) => String(value).trim(),
      applySet: async ({ cfg, accountId, value }) =>
        updateIrcAccountConfig(cfg as CoreConfig, accountId, {
  ...
  completionNote: {
    ...
    lines: [
      ...
      "Command: wildvine channels status --probe",
      `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
    ],
  },
