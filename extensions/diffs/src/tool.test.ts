...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("wildvine-diffs-tool-"));
  });
...
function createApi(): WildvinePluginApi {
  return createTestPluginApi({
    ...
    runtime: {} as WildvinePluginApi["runtime"],
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: WildvinePluginToolContext | undefined = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
