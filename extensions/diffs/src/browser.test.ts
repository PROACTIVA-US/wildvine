...
import type { WildvinePluginApi, WildvinePluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("wildvine-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<WildvinePluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: WildvinePluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<WildvinePluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as WildvinePluginApi);
