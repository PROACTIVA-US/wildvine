...
Read-only diff viewer plugin for **Wildvine** agents.
...
Set plugin-wide defaults in `~/.wildvine/wildvine.json`:
...
Before:
Wildvine supports plugins.

After:
Wildvine supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/wildvine-diffs`).
