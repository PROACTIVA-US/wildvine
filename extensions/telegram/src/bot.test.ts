...
import type { PluginInteractiveTelegramHandlerContext } from "wildvine/plugin-sdk/core";
import {
  clearPluginInteractiveHandlers,
  registerPluginInteractiveHandler,
} from "wildvine/plugin-sdk/plugin-runtime";
import { expectChannelInboundContextContract as expectInboundContextContract } from "wildvine/plugin-sdk/testing";
...
let normalizeTelegramCommandName: typeof import("wildvine/plugin-sdk/config-runtime").normalizeTelegramCommandName;
...
describe("createTelegramBot", () => {
  beforeAll(async () => {
    ...
    ({ normalizeTelegramCommandName } = await import("wildvine/plugin-sdk/config-runtime"));
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/wildvine-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/wildvine-telegram-model-compact-${process.pid}-${Date.now()}.json`;
    const config = {
      agents: {
        defaults: {
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/wildvine-telegram-model-default-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Wildvine" },
        },
      },
      me: { id: 999, username: "wildvine_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
