...
import {
  ...
  type WildvineConfig,
} from "wildvine/plugin-sdk/account-core";
import {
  listBoundAccountIds,
  resolveDefaultAgentBoundAccountId,
} from "wildvine/plugin-sdk/routing";
import { formatSetExplicitDefaultInstruction } from "wildvine/plugin-sdk/routing";
import { createSubsystemLogger, isTruthyEnvValue } from "wildvine/plugin-sdk/runtime-env";
import type {
  TelegramAccountConfig,
  TelegramActionConfig,
} from "wildvine/plugin-sdk/telegram-core";
...
const debugAccounts = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.WILDVINE_DEBUG_TELEGRAM_ACCOUNTS)) {
...
function listConfiguredAccountIds(cfg: WildvineConfig): string[] {
...
export function listTelegramAccountIds(cfg: WildvineConfig): string[] {
...
export function resolveDefaultTelegramAccountId(cfg: WildvineConfig): string {
...
export function mergeTelegramAccountConfig(
  ...
  // See: https://github.com/PROACTIVA-US/wildvine/issues/30673
...
export function listEnabledTelegramAccounts(cfg: WildvineConfig): ResolvedTelegramAccount[] {
