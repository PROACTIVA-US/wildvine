import { resolveDefaultModelForAgent } from "wildvine/plugin-sdk/agent-runtime";
import type { WildvineConfig } from "wildvine/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "wildvine/plugin-sdk/reply-runtime";
import type { MsgContext } from "wildvine/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "wildvine/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "wildvine/plugin-sdk/reply-runtime";
import type { MockFn } from "wildvine/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("wildvine/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("wildvine/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("wildvine/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("wildvine/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/wildvine-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("wildvine/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("wildvine/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("wildvine/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("wildvine/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("wildvine/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: WildvineConfig): {
...
vi.doMock("wildvine/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("wildvine/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("wildvine/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("wildvine/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("wildvine/plugin-sdk/channel-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/channel-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: WildvineConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: WildvineConfig) => {
