...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): WildvineConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as WildvineConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): WildvineConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as WildvineConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: WildvineConfig, accountId = "media") {
    await handleTelegramAction(
