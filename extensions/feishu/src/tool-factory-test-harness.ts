import type { AnyAgentTool, WildvinePluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: WildvinePluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<WildvinePluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
