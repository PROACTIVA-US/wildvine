...
vi.mock("wildvine/plugin-sdk/provider-web-search", () => ({
...
vi.mock("wildvine/plugin-sdk/security-runtime", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: wildvine", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "wildvine" });
  });

  it("runTavilyExtract sends X-Client-Source: wildvine", async () => {
