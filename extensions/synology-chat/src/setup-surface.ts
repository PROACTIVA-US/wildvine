...
function getChannelConfig(cfg: WildvineConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: WildvineConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: WildvineConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: WildvineConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Wildvine uses to send replies back to Chat.",
      ],
