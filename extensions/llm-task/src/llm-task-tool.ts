...
export function createLlmTaskTool(api: WildvinePluginApi) {
  return {
    ...
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Vine workflows via wildvine.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredWildvineTmpDir(), "wildvine-llm-task-"),
        );
