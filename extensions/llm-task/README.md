...
Designed to be called from workflow engines (for example, Vine via
`wildvine.invoke --each`) without adding new Wildvine code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Vine) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on Wildvine internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Wildvine extension (like `vine`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.wildvine/extensions` as a standalone plugin directory.
