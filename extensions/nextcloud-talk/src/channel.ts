import { describeWebhookAccountSnapshot } from "wildvine/plugin-sdk/account-helpers";
...
} from "wildvine/plugin-sdk/channel-pairing";
import { createAllowlistProviderRouteAllowlistWarningCollector } from "wildvine/plugin-sdk/channel-policy";
import { createChatChannelPlugin } from "wildvine/plugin-sdk/core";
import { runStoppablePassiveMonitor } from "wildvine/plugin-sdk/extension-shared";
import {
    ...
    base: {
      ...
      gateway: {
        ...
        logoutAccount: async ({ accountId, cfg }) => {
          const nextCfg = { ...cfg } as WildvineConfig;
          ...
          if (changed) {
            if (nextSection && Object.keys(nextSection).length > 0) {
              ...
              if (Object.keys(nextChannels).length > 0) {
                nextCfg.channels = nextChannels as WildvineConfig["channels"];
              } else {
