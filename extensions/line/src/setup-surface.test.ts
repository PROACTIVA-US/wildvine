...
import type { WildvineConfig, PluginRuntime, ResolvedLineAccount } from "../api.js";
...
describe("line runtime api", () => {
  it("loads through Jiti without duplicate export errors", () => {
    ...
    expect(
      loadRuntimeApiExportTypesViaJiti({
        ...
        realPluginSdkSpecifiers: ["wildvine/plugin-sdk/line-runtime"],
      }),
    ).toEqual({
  ...
  it("keeps the LINE runtime barrel self-contained", () => {
    ...
    expect(runtimeApiSource).not.toContain("wildvine/plugin-sdk/line-runtime");
    expect(collectRuntimeApiPreExports(runtimeApiPath)).toEqual([]);
  });
});
