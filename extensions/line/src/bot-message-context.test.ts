...
import { __testing as sessionBindingTesting } from "wildvine/plugin-sdk/conversation-runtime";
...
type AgentBinding = NonNullable<WildvineConfig["bindings"]>[number];
...
describe("buildLineMessageContext", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-line-context-"));
  ...
  it("group peer binding matches raw groupId without prefix (#21907)", async () => {
    ...
    const bindingCfg: WildvineConfig = {
