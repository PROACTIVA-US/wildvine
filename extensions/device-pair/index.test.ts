...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredWildvineTmpDir: vi.fn(() => path.join(os.tmpdir(), "wildvine-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredWildvineTmpDir: pluginApiMocks.resolvePreferredWildvineTmpDir,
...
function createApi(params?: {
  runtime?: WildvinePluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as WildvinePluginApi["runtime"],
    registerCommand: params?.registerCommand,
  }) as WildvinePluginApi;
}
...
function registerPairCommand(params?: {
  ...
  let command: WildvinePluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as WildvinePluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredWildvineTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredWildvineTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Wildvine iOS app:");
    expect(text).toContain("![Wildvine pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Wildvine iOS app:");
