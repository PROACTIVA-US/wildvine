...
import { approveDevicePairing, listDevicePairing } from "wildvine/plugin-sdk";
...
function resolveGatewayPort(cfg: WildvinePluginApi["config"]): number {
  const envRaw =
    process.env.WILDVINE_GATEWAY_PORT?.trim() || process.env.WILDVINE_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: WildvinePluginApi): Promise<string | null> {
...
function resolveAuth(cfg: WildvinePluginApi["config"]): ResolveAuthResult {
