...
import { normalizeChannelId, type ChannelId } from "wildvine/plugin-sdk/channel-runtime";
import type {
  WildvineConfig,
  TtsAutoMode,
  TtsConfig,
...
import { redactSensitiveText } from "wildvine/plugin-sdk/logging-core";
...
import { isVerbose, logVerbose } from "wildvine/plugin-sdk/runtime-env";
import { resolvePreferredWildvineTmpDir } from "wildvine/plugin-sdk/sandbox";
import { CONFIG_DIR, resolveUserPath, stripMarkdown } from "wildvine/plugin-sdk/text-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.WILDVINE_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: WildvineConfig) {
...
function resolveRegistryDefaultSpeechProviderId(cfg?: WildvineConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: WildvineConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: WildvineConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: WildvineConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: WildvineConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredWildvineTmpDir();
