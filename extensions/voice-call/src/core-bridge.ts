...
function resolveWildvineRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.WILDVINE_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set WILDVINE_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveWildvineRoot(), "dist", "extensionAPI.js");
