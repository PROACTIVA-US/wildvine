...
import {
  ...
  resolveStateDir,
  type WildvineConfig,
} from "wildvine/plugin-sdk/memory-core-host-runtime-core";
import {
  listMemoryFiles,
  normalizeExtraMemoryPaths,
} from "wildvine/plugin-sdk/memory-core-host-runtime-files";
...
function resolveAgent(cfg: WildvineConfig, agent?: string) {
...
function resolveAgentIds(cfg: WildvineConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
