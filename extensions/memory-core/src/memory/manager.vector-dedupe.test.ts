...
type MemoryStorageModule = typeof import("wildvine/plugin-sdk/memory-core-host-engine-storage");
...
describe("memory vector dedupe", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ buildFileEntry } = await import("wildvine/plugin-sdk/memory-core-host-engine-storage"));
    ({ createMemoryManagerOrThrow } = await import("./test-manager.js"));
    ({ closeAllMemorySearchManagers } = await import("./index.js"));
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-mem-"));
