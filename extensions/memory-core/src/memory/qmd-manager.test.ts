...
const MCPORTER_STATE_KEY = Symbol.for("wildvine.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("wildvine.qmdEmbedQueueTail");
...
vi.mock("wildvine/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("wildvine/plugin-sdk/memory-core-host-engine-foundation")
  >("wildvine/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("wildvine/plugin-sdk/file-lock", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/file-lock")>();
  return {
    ...actual,
...
import type { WildvineConfig } from "wildvine/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
  resolveMemoryBackendConfig,
} from "wildvine/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: WildvineConfig }) {
