...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "wildvine-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): WildvineConfig {
    const defaults: NonNullable<NonNullable<WildvineConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: WildvineConfig) {
