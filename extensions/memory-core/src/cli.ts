...
import {
  ...
  formatHelpExamples,
  theme,
} from "wildvine/plugin-sdk/memory-core-host-runtime-cli";
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["wildvine memory status", "Show index and provider status."],
          ["wildvine memory status --deep", "Probe embedding provider readiness."],
          ["wildvine memory index --force", "Force a full reindex."],
          ['wildvine memory search "meeting notes"', "Quick search using positional query."],
          [
            'wildvine memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["wildvine memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.wildvine.com/cli/memory")}\n`,
    );
