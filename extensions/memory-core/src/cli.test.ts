...
import {
  ...
  spyRuntimeJson,
  spyRuntimeLogs,
} from "wildvine/plugin-sdk/testing";
...
vi.mock("wildvine/plugin-sdk/memory-core-host-runtime-core", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("wildvine/plugin-sdk/memory-core-host-runtime-core")>();
  return {
    ...actual,
    loadConfig,
...
vi.mock("wildvine/plugin-sdk/memory-core-host-runtime-cli", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("wildvine/plugin-sdk/memory-core-host-runtime-cli")>();
  return {
    ...actual,
...
let defaultRuntime: typeof import("wildvine/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("wildvine/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("wildvine/plugin-sdk/memory-core-host-runtime-cli").setVerbose;

beforeAll(async () => {
  ({ registerMemoryCli } = await import("./cli.js"));
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("wildvine/plugin-sdk/memory-core-host-runtime-cli"));
});
...
describe("memory cli", () => {
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("wildvine memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain('wildvine memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('wildvine memory search --query "deployment" --max-results 20');
