...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["wildvinePollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["wildvine", "pollId"]) ??
    readNestedString(value, ["wildvine", "poll", "id"]) ??
    readNestedString(value, ["data", "wildvinePollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "wildvine", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          wildvinePollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "wildvine poll vote",
          displayText: "Vote recorded",
          value: { wildvinePollId: pollId, pollId },
        },
      },
    ],
  };
