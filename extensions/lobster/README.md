...
- This plugin integrates Lobster with Wildvine _without core changes_.
...
## Using `wildvine.invoke` (Lobster → Wildvine tools)

Some Lobster pipelines may include a `wildvine.invoke` step to call back into Wildvine tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).
