...
function createSignalAccountOverrideCfg(): WildvineConfig {
  return {
    channels: {
      signal: {
...
describe("signalMessageActions", () => {
  ...
  it("lists actions based on configured accounts and reaction gates", () => {
    expect(
      signalMessageActions.describeMessageTool?.({ cfg: {} as WildvineConfig })?.actions ?? [],
    ).toEqual([]);
  ...
  it("maps reaction targets into sendReactionSignal calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as WildvineConfig,
        params: {
