...
vi.mock("wildvine/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("wildvine/plugin-sdk/core", async (importOriginal) => {
  const actual = await importOriginal<typeof import("wildvine/plugin-sdk/core")>();
  return {
    ...actual,
...
vi.mock("wildvine/plugin-sdk/text-runtime", () => ({
