...
export const signalDmPolicy = createTopLevelChannelDmPolicy({
  ...
  getCurrent: (cfg: WildvineConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "Wildvine"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("wildvine gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
