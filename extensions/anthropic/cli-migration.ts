import type { WildvineConfig, ProviderAuthResult } from "wildvine/plugin-sdk/provider-auth";
import { readClaudeCliCredentialsCached } from "wildvine/plugin-sdk/provider-auth";

const DEFAULT_CLAUDE_CLI_MODEL = "claude-cli/claude-sonnet-4-6";
type AgentDefaultsModel = NonNullable<NonNullable<WildvineConfig["agents"]>["defaults"]>["model"];
type AgentDefaultsModels = NonNullable<NonNullable<WildvineConfig["agents"]>["defaults"]>["models"];
...
export function buildAnthropicCliMigrationResult(config: WildvineConfig): ProviderAuthResult {
