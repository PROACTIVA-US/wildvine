...
import { parseDurationMs } from "wildvine/plugin-sdk/cli-runtime";
import {
  definePluginEntry,
...
import {
  ...
  validateApiKeyInput,
} from "wildvine/plugin-sdk/provider-auth";
import { createProviderApiKeyAuthMethod } from "wildvine/plugin-sdk/provider-auth-api-key";
import { cloneFirstTemplateModel } from "wildvine/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "wildvine/plugin-sdk/provider-usage";
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("wildvine doctor --yes")}"`,
  ].join("\n");
}
