...
import type { WildvineApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as WildvineApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as WildvineApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as WildvineApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as WildvineApp);
  }
  if (host.tab === "agents") {
    ...
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as WildvineApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as WildvineApp, agentId);
      if (host.agentsPanel === "files") {
        void loadAgentFiles(host as unknown as WildvineApp, agentId);
      }
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as WildvineApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as WildvineApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as WildvineApp);
    await loadDevices(host as unknown as WildvineApp);
    await loadConfig(host as unknown as WildvineApp);
    await loadExecApprovals(host as unknown as WildvineApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "debug") {
    await loadDebug(host as unknown as WildvineApp);
...
export function inferBasePath() {
  ...
  const configured = window.__WILDVINE_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as WildvineApp;
...
async function loadOverviewLogs(host: WildvineApp) {
...
function buildAttentionItems(host: WildvineApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.wildvine.com/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as WildvineApp, true),
    loadConfigSchema(host as unknown as WildvineApp),
    loadConfig(host as unknown as WildvineApp),
  ]);
}
