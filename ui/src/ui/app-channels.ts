...
export async function handleWhatsAppStart(host: WildvineApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: WildvineApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: WildvineApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: WildvineApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: WildvineApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: WildvineApp): string {
...
function resolveGatewayHttpAuthHeader(host: WildvineApp): string | null {
...
function buildGatewayHttpHeaders(host: WildvineApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: WildvineApp) {
...
export function handleNostrProfileToggleAdvanced(host: WildvineApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: WildvineApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: WildvineApp) {
  const state = host.nostrProfileFormState;
